...
const RETAIL_BROKERS = new Set(['XC', 'PD', 'YP', 'NI', 'XL']);
...
function calculateBandarmologiScore(brokerData, options) {
  ...
  const evalDateStr = options.evaluationDate || options.currentDate || options.date || new Date().toISOString().slice(0, 10);
  const evalTime = new Date(evalDateStr).getTime();
  ...
  for (const ins of insiderRoster) {
    ...
    const isBuy = change > 0 || ins.is_recent_buyer || ins.action_type === 'BUY' || ins.action === 'BUY';

    // Validasi jendela waktu transaksi: wajib <= 30 hari kalender
    let isWithin30Days = false;
    const txDateStr = ins.last_date || ins.date || ins.transaction_date;
    if (txDateStr) {
      const txTime = new Date(txDateStr).getTime();
      if (!isNaN(txTime) && !isNaN(evalTime)) {
        const diffDays = Math.round((evalTime - txTime) / (1000 * 60 * 60 * 24));
        if (diffDays >= 0 && diffDays <= 30) {
          isWithin30Days = true;
        }
      }
    } else if (ins.is_recent_buyer) {
      isWithin30Days = true;
    }

    if (isBuy && isWithin30Days) {
...
function enrichCandidateWithBandarmologi(candidate, options) {
  ...
  const scoringResult = calculateBandarmologiScore(brokerData, {
    ...
    evaluationDate: options.evaluationDate || candidate.price_date || candidate.date
  });
