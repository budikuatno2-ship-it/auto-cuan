...
const KNOWN_OUTCOMES = [
  'WAITING', 'RUNNING', 'ENTRY_HIT', 'TP1_HIT', 'TP2_HIT',
  'SL_HIT', 'EXPIRED', 'NEVER_ENTERED'
];
...
function buildTrackRecordData(rows = [], options = {}) {
  ...
  const signals = [];
  // All-time largest profit % across every signal considered here (not a
  // rolling window — scoped only by whatever `rows` the caller already
  // passed in, same as every other summary stat in this function).
  let bestGain = null;
  ...
  for (const row of rows) {
    ...
    // BUG-F7-013: beberapa sumber sudah menyimpan outcome final secara eksplisit
    // (mis. NEVER_ENTERED / EXPIRED dari monitor). Nilai eksplisit itu adalah
    // sumber kebenaran; classifyOutcome() hanya dipakai sebagai fallback saat
    // kolomnya tidak ada.
    const explicitOutcome = String(row.outcome || raw.outcome || '').toUpperCase();
    const outcome = KNOWN_OUTCOMES.indexOf(explicitOutcome) !== -1
      ? explicitOutcome
      : reportHelpers.classifyOutcome(row);
    ...
    // BUG-F7-012: baris anomali (gain > 500%) dibuang sebelum penghitungan, sehingga
    // total_signals dan seluruh bucket selalu konsisten dengan isi signals[].
    const gainPct = calculateGainPct(row, outcome);
    if (gainPct != null && gainPct > 500) {
      continue;
    }
    ...
    const signalDate = row.date || (startTime ? startTime.slice(0, 10) : '—');
    const signalTicker = String(row.ticker).toUpperCase();

    // MFE and best_gain calculation
    const entryPrice = normalizeNumber(row.entry1) || normalizeNumber(row.entry2) || normalizeNumber(row.price_at_signal);
    const highPrice = normalizeNumber(row.high) || normalizeNumber(row.highest_price) || normalizeNumber(row.hit_price);
    let mfe = row.mfe != null ? normalizeNumber(row.mfe) : null;
    if (mfe == null && entryPrice && entryPrice > 0 && highPrice && highPrice >= entryPrice) {
      mfe = ((highPrice - entryPrice) / entryPrice) * 100;
    }
    const signalMfe = mfe != null ? normalizeNumber(mfe) : gainPct;
    const best_gain = signalMfe != null ? Math.max(0, signalMfe) : 0;

    if (best_gain > 500) {
      continue;
    }

    // BUG-F7-013: gain hanya bermakna untuk sinyal yang benar-benar dipegang.
    // Sinyal yang tidak pernah masuk area entry atau sudah kedaluwarsa tidak boleh
    // mengklaim best_gain.
    const gainEligible = !isNeverEntered && !isExpired;
    if (gainEligible && best_gain > 0 && best_gain <= 500 && (!bestGain || best_gain > bestGain.gain_pct)) {
      bestGain = { gain_pct: Math.round(best_gain * 10) / 10, ticker: signalTicker, date: signalDate };
    }
