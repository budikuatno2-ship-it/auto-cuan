...
function normalizeTradingPlanLevels(candidate) {
  ...
  if (entry1 != null) { out.entry1 = entry1; out.entry_1 = entry1; out.entry_low = entry1; }
  if (entry2 != null) { out.entry2 = entry2; out.entry_2 = entry2; out.entry_high = entry2; }
  ...
  var mainEntry = entry1 || entry2;
  var valid = true;
  if (entry1 != null && !isValidIdxPriceLevel(entry1)) valid = false;
  if (entry2 != null && !isValidIdxPriceLevel(entry2)) valid = false;
  if (mainEntry != null && sl != null && sl >= mainEntry) valid = false;
  if (mainEntry != null && tp1 != null && tp1 <= mainEntry) valid = false;
  if (mainEntry != null && tp2 != null && tp2 <= mainEntry) valid = false;
  ...
  if (mainEntry != null && sl != null && tp1 != null && sl < mainEntry && tp1 > mainEntry) {
    out.risk_reward = round2((tp1 - mainEntry) / (mainEntry - sl));
    out.tp1_upside = round2((tp1 - mainEntry) / mainEntry * 100);
    if (tp2 != null) out.tp2_upside = round2((tp2 - mainEntry) / mainEntry * 100);
    out.sl_risk = round2((sl - mainEntry) / mainEntry * 100);
  }
