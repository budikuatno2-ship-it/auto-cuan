...
function normalizeLevelsToIdxTicks(levels, opts) {
  ...
  if (stop_loss != null && lowerEntry != null && stop_loss >= lowerEntry) {
    stop_loss = lowerEntry - getIdxTickSize(lowerEntry);
    if (stop_loss <= 0) stop_loss = getIdxTickSize(lowerEntry);
    notes.push('SL adjusted below entry');
  }

  var upperEntry = entry_high || entry_low;
  if (tp1 != null && upperEntry != null && tp1 <= upperEntry) {
    tp1 = upperEntry + getIdxTickSize(upperEntry);
  ...
  var risk_reward = (risk > 0 && reward > 0) ? round2(reward / risk) : 0;
  ...
  return {
    ...
    tick_normalized: true, tick_notes: notes.length ? notes.join('; ') : null
  };
}
