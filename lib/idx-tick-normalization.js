...
function deriveIdxAutoRejectLevels(input) {
  input = input || {};
  var ref = firstNum(input, ['reference_price', 'previous_close', 'prev_close', 'prior_close', 'close_prev']);
  if (ref == null) ref = firstNum(input, ['last_price', 'current_price', 'close']);
  ...
  if (!band) return {
    ...
    ara_band_label: 'insufficient_reference', ara_arb_note: 'ARA/ARB belum cukup data reference price.',
    near_ara: false, near_arb: false, tp1_beyond_ara: false, tp2_beyond_ara: false, sl_below_arb: false,
    intraday_realistic_cap: null, tp1_intraday_realistic: null, tp2_intraday_realistic: null, tp_realism_note: null
  };
...
function deriveCandlePotentialRange(input) {
  ...
  if (tp1 && potHigh && tp1 > potHigh) realism = 'TP1 butuh breakout lanjutan; potensi candle hari ini belum mendukung.';
