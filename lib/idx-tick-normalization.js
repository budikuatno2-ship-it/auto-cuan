...
var KNOWN_FCA_TICKERS = new Set(['LUCK', 'MAHA', 'RATU', 'ZBRA', 'GHON', 'CLAY', 'KOTA', 'BOSS']);
...
function isAkselerasiOrFca(board, isFca, ticker) {
  ...
  var b = String(board || '').toUpperCase().trim();
  if (b === 'AKSELERASI' || b === 'PEMANTAUAN_KHUSUS' || b.indexOf('AKSELERASI') >= 0 || b.indexOf('PEMANTAUAN') >= 0) return true;
  var t = String(ticker || '').toUpperCase().trim();
  if (t && KNOWN_FCA_TICKERS.has(t)) return true;
  if (typeof window !== 'undefined' && window.FCA_STOCKS && window.FCA_STOCKS[t]) return true;
  return false;
}

function getIdxTickSize(price, board, isFca, ticker) {
  price = toNum(price, null);
  if (price == null || !isFinite(price) || price <= 0) return null;
  if (isAkselerasiOrFca(board, isFca, ticker)) return 1;
...
function roundToIdxTick(price, mode, board, isFca, ticker) {
  price = toNum(price, null);
  if (price == null || !isFinite(price) || price <= 0) return null;
  mode = mode || 'nearest';
...
function normalizeIdxPriceLevel(price, mode, board, isFca, ticker) {
  return roundToIdxTick(price, mode || 'nearest', board, isFca, ticker);
}
...
function validateTradingPlanSanity(candidate) {
  ...
  var board = candidate && (candidate.board || candidate.papan);
  var isFca = candidate && (candidate.is_fca != null ? candidate.is_fca : candidate.isFca);
  var ticker = candidate && candidate.ticker;
...
function deriveBreakoutConfirmation(candidate) {
  ...
  if (close != null && resistance != null) {
    if (close > resistance) {
      // Batch 11: a live/intraday tick above resistance is a wick, not a confirmed close.
      // Hold the breakout at NEEDS_CLOSE_CONFIRMATION until the bar actually closes.
      if (!screenerConfig.isCandleCloseConfirmed(candidate)) {
        status = 'NEEDS_CLOSE_CONFIRMATION';
        label = 'Needs Close Confirmation';
        note = 'Harga (' + close + ') sempat di atas resistance ' + resistance + ', tetapi candle belum close terkonfirmasi (masih bar berjalan). Tunggu penutupan bar.';
      } else {
        var volRatio = firstNum(candidate, ['volume_ratio', 'volume_ratio_20d', 'volume_ratio_avg20', 'volumeRatio', 'vr', 'vol_ratio']);
        var minVR = candidate.min_breakout_volume_ratio != null ? toNum(candidate.min_breakout_volume_ratio, 1.0) : 1.0;
        if (volRatio != null && volRatio < minVR) {
          status = 'VOLUME_CONFIRMATION_NEEDED';
          label = 'Needs Volume Confirmation';
          note = 'Close di atas resistance ' + resistance + ', tetapi volume ratio (' + round2(volRatio) + 'x) belum memenuhi konfirmasi volume breakout (min ' + minVR + 'x).';
        } else {
...
function normalizeSetupSource(candidate, opts) {
  candidate = candidate || {};
  opts = opts || {};
  var raw = candidate.raw_payload || {};
  var source = opts.monitor_source || opts.source || candidate.monitor_source || candidate.source || candidate.category || raw.monitor_source || raw.source || raw.category || '';
  var s = String(source || '').toLowerCase();
  if (s.indexOf('daytrade') >= 0 || s.indexOf('day trade') >= 0) return 'daytrade';
  if (s.indexOf('swing') >= 0 || s === 'top5' || s.indexOf('top 5') >= 0) return 'swing';
  return 'daytrade';
}
...
function countWeekdaysBetween(start, end) {
  ...
  var d = new Date(Date.UTC(startDate.getUTCFullYear(), startDate.getUTCMonth(), startDate.getUTCDate()));
  var e = new Date(Date.UTC(endDate.getUTCFullYear(), endDate.getUTCMonth(), endDate.getUTCDate()));
  var days = 0;
  while (d <= e) {
    var day = d.getUTCDay();
    if (day >= 1 && day <= 5) days++;
...
function deriveSetupFreshness(candidate, opts) {
  candidate = candidate || {};
  opts = opts || {};
  var sourceType = normalizeSetupSource(candidate, opts);
  var isSwingMonitor = sourceType === 'swing';
  var swingMaxTradingDays = opts.swing_max_trading_days != null ? Number(opts.swing_max_trading_days) : 5;
  if (!(swingMaxTradingDays >= 1)) swingMaxTradingDays = 5;
  // Setup age must be anchored to an immutable origin. Generic updated_at and
  // last_checked_at are deliberately excluded because lifecycle monitoring updates
  // them repeatedly and must never make an old recommendation look new again.
  var ts = candidate.setup_origin_at || candidate.freshness_timestamp || candidate.calculated_at || candidate.run_at || candidate.published_at || candidate.registered_at || candidate.created_at || candidate.first_sent_at || candidate.run_date || candidate.date || opts.setup_origin_at || opts.freshness_timestamp || opts.calculated_at || opts.run_at || opts.published_at || opts.registered_at || opts.created_at || opts.first_sent_at || opts.run_date || opts.date || null;
  var price = firstNum(candidate, ['current_price', 'last_price', 'lastn', 'close']);
  ...
  if (ts) {
    ...
    if (!isNaN(d.getTime())) {
      ...
      ageTradingDays = countWeekdaysBetween(d, new Date(nowMs));
      ...
      } else if (isSwingMonitor) {
        if (ageTradingDays != null && ageTradingDays > swingMaxTradingDays) {
          status = 'EXPIRED'; label = 'Expired'; note = 'Setup melewati ' + swingMaxTradingDays + ' hari bursa masa pantau swing; perlu revalidasi.';
        } else if (ageTradingDays != null && ageTradingDays >= Math.max(3, swingMaxTradingDays - 1)) {
          status = 'AGING'; label = 'Aging'; note = 'Setup swing mulai menua tetapi masih dalam masa pantau; cek ulang harga/volume.';
        } else {
          status = 'FRESH'; label = 'Fresh'; note = 'Setup swing masih dalam masa pantau dan belum invalid.';
        }
  ...
  return {
    ...
    setup_age_trading_days: ageTradingDays,
    setup_origin_at: ts || null,
    setup_freshness_source_type: sourceType,
...
function normalizeTradingPlanLevels(candidate) {
  ...
  var board = out.board || out.papan;
  var isFca = out.is_fca != null ? out.is_fca : out.isFca;
  var ticker = out.ticker;

  var entry1 = normalizeIdxPriceLevel(firstNum(out, ['entry_1', 'entry1', 'entry_low']), 'nearest', board, isFca, ticker);
  var entry2 = normalizeIdxPriceLevel(firstNum(out, ['entry_2', 'entry2', 'entry_high']), 'nearest', board, isFca, ticker);
  var sl = normalizeIdxPriceLevel(firstNum(out, ['stop_loss', 'sl']), 'floor', board, isFca, ticker);
  var tp1 = normalizeIdxPriceLevel(firstNum(out, ['target_1', 'tp1n', 'tp1', 'target1']), 'ceil', board, isFca, ticker);
  var tp2 = normalizeIdxPriceLevel(firstNum(out, ['target_2', 'tp2n', 'tp2', 'target2']), 'ceil', board, isFca, ticker);
  var support = normalizeIdxPriceLevel(firstNum(out, ['support']), 'floor', board, isFca, ticker);
  var resistance = normalizeIdxPriceLevel(firstNum(out, ['resistance']), 'ceil', board, isFca, ticker);
  var trigger = normalizeIdxPriceLevel(firstNum(out, ['breakout_trigger', 'trigger_price', 'reclaim_level']), 'ceil', board, isFca, ticker);
  ...
  var valid = true;
  if (entry1 != null && !isValidIdxPriceLevel(entry1, board, isFca, ticker)) valid = false;
  if (entry2 != null && !isValidIdxPriceLevel(entry2, board, isFca, ticker)) valid = false;
...
function normalizeLevelsToIdxTicks(levels, opts) {
  ...
  opts = opts || {};
  var board = levels.board || opts.board || levels.papan || opts.papan;
  var isFca = levels.is_fca != null ? levels.is_fca : (opts.is_fca != null ? opts.is_fca : (levels.isFca || opts.isFca));
  var ticker = levels.ticker || opts.ticker;
...
function deriveSignalVerdict(candidate) {
  ...
  // Observation notes describe factual candle/market conditions.
  // Advice/timing fields (entry_timing, time_plan, telegram_verdict) contain instructions
  // to the trader (e.g. 'jangan chase dekat ARA') and must NOT be read as market observations.
  var observationNotes = [r.notes, r.status_reason, r.candle_note, r.respect_zone_notes, r.entry_status_note, r.plan_quality_note, r.liquidity_notes, r.stale_notes, r.chase_risk_label].filter(Boolean).join(' ').toLowerCase();
  ...
  function buildPlan(action, reason) {
    ...
    else if (entry === 'CHASE_RISK' || entry === 'EXTENDED' || hasAny(cleanObservation, ['pullback', 'menjauh dari entry', 'chase'])) label = 'Tunggu pullback ke Entry 1/Entry 2.';
  ...
  var chaseExtended = entry === 'CHASE_RISK' || entry === 'EXTENDED' || hasAny(cleanObservation, ['chase', 'extended', 'long candle']);
...
function deriveIdxAutoRejectLevels(input) {
  ...
  if (!band) return {
    ...
    near_ara: false, near_arb: false, ara_hit: false, arb_hit: false, entry_near_ara: false, trigger_near_ara: false,
    execution_reality_status: 'UNKNOWN_LIMITS', execution_reality_label: 'Butuh harga realistis',
    execution_reality_note: 'ARA/ARB belum tersedia; eksekusi harus dianggap belum aman sampai reference price valid.',
    buy_execution_realistic: false, sell_risk_near_arb: false,
  ...
  // FLOOR UPDATE 2026-09: ARB -15% dengan floor Rp1 => Math.max(1, Math.floor(prev_close*0.85))
  var arbRaw = Math.max(1, Math.floor(ref * band.arb_multiplier));
  var arb = roundToIdxTick(arbRaw, 'ceil');
  if (arb != null && arb < 1) arb = 1;
  ...
  var rawEntryLow = firstNum(input, ['entry_low', 'entry2', 'entry2n', 'entry']);
  var rawEntryHigh = firstNum(input, ['entry_high', 'entry1', 'entry1n', 'entry']);
  var entryHigh = rawEntryHigh;
  if (rawEntryLow != null && rawEntryHigh != null) {
    entryHigh = Math.max(rawEntryLow, rawEntryHigh);
  } else if (entryHigh == null) {
    entryHigh = rawEntryLow;
  }
  var trigger = firstNum(input, ['breakout_trigger', 'trigger_price', 'reclaim_level', 'resistance']);
  var araRoom = last && ara ? round2((ara - last) / last * 100) : null;
  var arbRoom = last && arb ? round2((arb - last) / last * 100) : null;
  var entryAraRoom = entryHigh && ara ? round2((ara - entryHigh) / entryHigh * 100) : null;
  var triggerAraRoom = trigger && ara ? round2((ara - trigger) / trigger * 100) : null;
  var araHit = !!(last && ara && last >= ara);
  var arbHit = !!(last && arb && last <= arb);
  var nearAra = araRoom != null ? araRoom <= 3 : false;
  var nearArb = arbRoom != null ? arbRoom >= -3 : false;
  var entryNearAra = entryAraRoom != null ? entryAraRoom <= 3 : false;
  var triggerNearAra = triggerAraRoom != null ? triggerAraRoom <= 3 : false;
  var out = {
    ...
    ara_hit: araHit,
    arb_hit: arbHit,
    entry_near_ara: entryNearAra,
    trigger_near_ara: triggerNearAra,
    tp1_beyond_ara: !!(tp1 && ara && tp1 > ara),
    tp2_beyond_ara: !!(tp2 && ara && tp2 > ara),
    sl_below_arb: !!(sl && arb && sl < arb),
    intraday_realistic_cap: ara,
    execution_reality_status: 'EXECUTABLE',
    execution_reality_label: 'Eksekusi realistis',
    execution_reality_note: 'Harga masih punya ruang ARA/ARB yang memadai.',
    buy_execution_realistic: true,
    sell_risk_near_arb: false,
    tp1_intraday_realistic: tp1 && ara ? tp1 <= ara : null,
    tp2_intraday_realistic: tp2 && ara ? tp2 <= ara : null,
    tp_realism_note: null
  };
  if (out.ara_hit) { out.execution_reality_status = 'ARA_HIT'; out.execution_reality_label = 'Eksekusi: rawan ARA'; out.execution_reality_note = 'Harga sudah menyentuh/di atas ARA; buy tidak realistis, pantau saja.'; }
  else if (out.near_ara || out.entry_near_ara || out.trigger_near_ara) { out.execution_reality_status = 'NEAR_ARA'; out.execution_reality_label = 'Eksekusi: rawan ARA'; out.execution_reality_note = 'Harga/entry/trigger terlalu dekat ARA; pantau, bukan entry langsung.'; }
  else if (out.arb_hit) { out.execution_reality_status = 'ARB_HIT'; out.execution_reality_label = 'Eksekusi: rawan ARB'; out.execution_reality_note = 'Harga sudah menyentuh/di bawah ARB; plan invalid/risk tinggi.'; }
  else if (out.near_arb || out.sl_below_arb) { out.execution_reality_status = 'NEAR_ARB'; out.execution_reality_label = 'Eksekusi: rawan ARB'; out.execution_reality_note = 'Harga/SL terlalu dekat batas ARB; plan berisiko tinggi.'; }
  if (out.execution_reality_status !== 'EXECUTABLE') out.buy_execution_realistic = false;
  out.sell_risk_near_arb = !!(out.near_arb || out.arb_hit || out.sl_below_arb);
  ...
  if (out.near_ara || out.entry_near_ara || out.trigger_near_ara || out.ara_hit) out.ara_arb_note += ' Harga dekat/mentok ARA; jangan chase agresif.';
  if (out.near_arb || out.arb_hit || out.sl_below_arb) out.ara_arb_note += ' Harga dekat/mentok ARB; plan berisiko tinggi.';
  return out;
}
...
module.exports = {
  ...
  isValidIdxPriceLevel: isValidIdxPriceLevel,
