...
function deriveSignalVerdict(candidate) {
  var r = candidate || {};
  var entry = String(r.entry_status || '').toUpperCase();
  var plan = String(r.plan_quality_status || '').toUpperCase();
  ...
  var liq = String(r.liquidity_label || '').toLowerCase();
  ...
  function out(action, label, verdict, reason, priority, maxGrade) {
    return {
  ...
  if (entry === 'INVALID_BELOW_SL' || plan === 'INVALID' || sl === 'sl invalid' || tp === 'tp invalid') {
    ...
    return out('INVALID', 'Invalid', 'Invalid — trading plan perlu validasi ulang.', 'Entry/SL/TP atau plan quality invalid.', 100, 'C');
  }
  if (risk === 'very high risk') {
    addBadge('Very High Risk');
    return out('AVOID', 'Hindari', 'Hindari dulu — risiko terlalu tinggi.', 'Risk Label v2 Very High Risk.', 90, 'C');
  }
  if (entry === 'TP1_NEAR' || entry === 'TP1_HIT' || entry === 'TP2_HIT') {
    addBadge('Dekat target');
    return out('TAKE_PROFIT_ZONE', 'Dekat Target', 'Dekat target — entry baru kurang menarik.', 'Harga sudah dekat/menyentuh target.', 80, entry === 'TP1_NEAR' ? 'B' : 'C');
  }
  if (entry === 'CHASE_RISK' || entry === 'EXTENDED') {
    addBadge('Anti-chase');
    return out('WAIT_PULLBACK', 'Tunggu Pullback', 'Tunggu pullback — harga sudah menjauh dari entry.', 'Entry status menunjukkan chase/extended.', 70, entry === 'CHASE_RISK' ? 'B' : 'C');
  }
  if (rr === 'rr kurang menarik' || plan === 'POOR_RR') {
    addBadge('Poor RR');
    return out('WAIT_CONFIRMATION', 'Tunggu Konfirmasi', 'Tunggu — RR kurang menarik.', 'Risk/reward belum memenuhi minimum kategori.', 60, 'C');
  }
  if ((entry === 'IN_ENTRY_AREA' || entry === 'NEAR_ENTRY') && plan === 'OK' && (risk === 'low risk' || risk === 'medium risk')) {
    addBadge(risk === 'low risk' ? 'Low Risk' : 'Medium Risk');
    return out('ENTRY_AREA', 'Area Entry', 'Area entry — tetap tunggu konfirmasi.', 'Harga berada/dekat area entry dengan plan OK dan risiko terkendali.', 50, null);
  }
  if (entry === 'BELOW_ENTRY') {
    addBadge('Below entry');
    return out('WAIT_CONFIRMATION', 'Tunggu Konfirmasi', 'Di bawah area entry — tunggu reclaim/konfirmasi.', 'Harga masih di bawah area entry.', 40, null);
  }
  var dataLimited = entry === 'NO_DATA' || plan === 'LIMITED' || sl.indexOf('data terbatas') >= 0 || tp.indexOf('data terbatas') >= 0 || liq.indexOf('data terbatas') >= 0;
  var weak = dataLimited || vol.indexOf('weak') >= 0 || vol.indexOf('lemah') >= 0 || vol.indexOf('belum') >= 0 || vol.indexOf('unconfirm') >= 0 || trend.indexOf('weak') >= 0 || trend.indexOf('lemah') >= 0 || trend.indexOf('bearish') >= 0 || stale.indexOf('stale') >= 0 || stale.indexOf('terbatas') >= 0 || r.data_stale;
  if (weak) {
    if (dataLimited) addBadge('Data terbatas');
    if (vol) addBadge('Volume belum lengkap');
    if (trend.indexOf('bearish') >= 0 || trend.indexOf('weak') >= 0 || trend.indexOf('lemah') >= 0) addBadge('Trend lemah');
    if (stale || r.data_stale) addBadge('Data stale');
    return out(dataLimited ? 'DATA_LIMITED' : 'WATCHLIST', dataLimited ? 'Data Terbatas' : 'Watchlist', 'Pantau dulu — konfirmasi belum lengkap.', 'Volume/trend/stale/data belum cukup kuat.', 30, null);
  }
  return out('WATCHLIST', 'Watchlist', 'Watchlist — tunggu konfirmasi.', 'Fallback konservatif.', 10, null);
}
