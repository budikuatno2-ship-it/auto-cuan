...
/**
 * Derive multi-timeframe candle context from daily candles.
 * NOTE: 5D and 20D are APPROXIMATIONS from daily candles, NOT true weekly/monthly OHLC.
 * @param {Array} candles - daily OHLCV array (oldest first)
 * @returns {object} All TF contexts + bias
 */
function deriveMultiTimeframeContext(candles) {
  ...
  // Compute each window
  var tf1d = classifyCandle(latest, len >= 2 ? candles[len - 2] : null);
  ...
  if (tf1d.bias === 'bullish' && tf5d.bias === 'bullish') {
    bias = 'bullish';
    mtfNotes.push('1D+5D bullish aligned');
  } else if (tf1d.bias === 'bullish' && tf5d.bias === 'bearish') {
    bias = 'mixed_caution';
    mtfNotes.push('1D bullish tapi 5D bearish — konfirmasi lemah');
  } else if (tf1d.bias === 'bearish' && tf5d.bias === 'bearish') {
    bias = 'bearish';
    mtfNotes.push('1D+5D bearish aligned — tekanan jual dominan');
  } else if (tf1d.bias === 'bearish' && tf5d.bias === 'bullish') {
    bias = 'pullback';
    mtfNotes.push('1D bearish tapi 5D bullish — potensi pullback sehat');
  }
