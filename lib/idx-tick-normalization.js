...
function deriveSetupFreshness(candidate, opts) {
  candidate = candidate || {};
  opts = opts || {};
  var ts = candidate.first_sent_at || candidate.calculated_at || candidate.created_at || candidate.updated_at || candidate.last_checked_at || candidate.run_at || opts.calculated_at || opts.created_at || null;
  var price = firstNum(candidate, ['current_price', 'last_price', 'lastn', 'close']);
...
function deriveSignalVerdict(candidate) {
  ...
  if ((entry === 'IN_ENTRY_AREA' || entry === 'NEAR_ENTRY') && plan === 'OK' && (risk === 'low risk' || risk === 'medium risk') && rr !== 'rr kurang menarik' && !chaseExtended && !staleData && (respect === '' || respect === 'valid respect' || respect === 'strong respect')) {
...
function deriveIdxAutoRejectLevels(input) {
  ...
  if (!band) return {
    ...
    tp_realism_note: 'ARA/ARB belum tersedia; TP realism hanya berbasis candle potential.'
  };
  ...
  if (out.tp1_beyond_ara) out.tp_realism_note = 'TP1 melewati ARA hari ini; target realistis intraday terbatas.';
  else if (out.tp2_beyond_ara) out.tp_realism_note = 'TP1 realistis · TP2 multi-day / perlu lanjutan.';
...
function deriveCandlePotentialRange(input) {
  ...
  if (guard.ara_arb_source === 'missing_reference') realism = 'ARA/ARB belum tersedia; TP realism hanya berbasis candle potential.';
  if (tp1 && potHigh && tp1 > potHigh) realism = guard.ara_arb_source === 'missing_reference' ? 'ARA/ARB belum tersedia; TP realism hanya berbasis candle potential. TP1 butuh breakout lanjutan; potensi candle hari ini belum mendukung.' : 'TP1 butuh breakout lanjutan; potensi candle hari ini belum mendukung.';
  else if (guard.tp2_beyond_ara) realism = 'TP1 realistis, TP2 butuh lanjutan / multi-day.';
  else if (tp1Real) realism = tp2Real ? 'TP1 realistis, TP2 realistis intraday jika momentum lanjut.' : 'TP1 realistis.';
