...
function detectTrailingZoneEvents(p) {
  ...
  for (const z of zones) {
    ...
    if (pierced) {
      ...
      // Fase 3 / BUG-F3-14: reclaim hanya sah bila TIDAK ada penutupan kembali
      // di bawah level SETELAH reclaim terakhir. Flag reclaim lama tidak boleh
      // menutupi breakdown permanen di ujung data.
      if (firstPierceIdx >= 0) {
    ...
    // Structural failure: the final close is below the zone and there is no
    // later close back above the level (the last close is the decisive one).
    const structureFailed = closedBelow;
...
function resolveSoftHardExit(p) {
  ...
  // Fase 3 / BUG-F3-15: emergency stop yang sudah tertembus adalah keputusan
  // mutlak. Tidak boleh ada state machine yang menunda exit ("DELAYED") atau
  // meminta konfirmasi struktur saat hard stop darurat sudah jebol.
  if (emergencyHit) {
    return {
      soft_exit_state: SOFT_EXIT_STATE.TRIGGERED,
      hard_exit_state: HARD_EXIT_STATE.HIT,
