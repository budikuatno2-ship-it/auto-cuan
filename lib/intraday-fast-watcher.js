...
// BUG-F9-03: the original `typeof value === 'number'` guard rejected every
// numeric STRING. JSON/JSONL payloads, PostgREST rows and replay fixtures all
// deliver prices as strings ('1500'), so a perfectly valid observation was
// classified INVALID_DATA / invalid_current_price and the anti-chase baseline
// was never seeded. Coerce like the sibling momentum module does, while still
// refusing booleans, empty strings and non-numeric text.
function finite(value) {
if (value === null || value === undefined || value === '' || typeof value === 'boolean') return null;
const number = Number(value);
return Number.isFinite(number) ? number : null;
}
// BUG-F9-01b: boolean-like payload flags ('true'/'false' strings, 1/0) must be
// honoured the same way as real booleans, without letting the literal string
// 'false' read as a positive flag.
...
if (productionEligible === true) return { value: true, source: 'production_eligible' };
if (productionEligible === false) return { value: false, source: 'production_eligible' };
const readyFlag = explicitBoolean(obs.ready);
if (readyFlag === true) return { value: true, source: 'ready' };
if (readyFlag === false) return { value: false, source: 'ready' };
...
stale: explicitTrueFlag(obs.freshness && obs.freshness.is_stale) ||
explicitTrueFlag(obs.is_stale) ||
explicitTrueFlag(obs.data_stale)
};
}
