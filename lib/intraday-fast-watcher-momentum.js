...
const OPENING_VELOCITY_WINDOW_START_MINUTE = 556; // 09:16 WIB (9 * 60 + 16)
const OPENING_VELOCITY_WINDOW_END_MINUTE = 570;   // 09:30 WIB (9 * 60 + 30)
const VELOCITY_THRESHOLD_MAIN_DEVELOPMENT = 250000000; // Rp 250.000.000 (250M)
const VELOCITY_THRESHOLD_ACCELERATION = 100000000;     // Rp 100.000.000 (100M)

function toWibMinutes(input) {
...
function isOpeningRangeVelocityWindow(time) {
  const min = toWibMinutes(time);
  return min != null && min >= OPENING_VELOCITY_WINDOW_START_MINUTE && min <= OPENING_VELOCITY_WINDOW_END_MINUTE;
}

function isAccelerationBoard(board, obs) {
  const b = String(board || (obs && (obs.board || obs.market_board || obs.board_type)) || '').trim().toUpperCase();
  if (b.includes('AKSELERASI') || b.includes('ACCELERAT') || b.includes('SMALL')) return true;
  if (obs && (obs.is_acceleration_board === true || obs.is_small_cap === true)) return true;
  const capCategory = String((obs && obs.market_cap_category) || '').trim().toUpperCase();
  if (capCategory === 'SMALL' || capCategory === 'MICRO') return true;
  return false;
}

function resolveDeltaTurnover(obs, tracker, metrics) {
  obs = obs || {};
  tracker = tracker || {};
  metrics = metrics || {};

  // 1. Explicit delta in obs or metrics
  const explicitDelta = finite(
    obs.delta_turnover_5m ??
    obs.turnover_delta_5m ??
    obs.delta_turnover ??
    obs.turnover_delta ??
    obs.delta_value_5m ??
    obs.value_delta_5m ??
    obs.delta_value ??
    metrics.delta_turnover_5m ??
    metrics.turnover_delta_5m ??
    metrics.delta_turnover
  );
  if (explicitDelta != null) return explicitDelta;

  // 2. Explicit current turnover vs turnover_5m_ago in obs
  const currentTurnover = finite(
    obs.turnover ?? obs.turnover_value ?? obs.value ?? obs.accumulated_turnover ?? obs.total_turnover ?? obs.value_today
  );
  const prevTurnover5m = finite(
    obs.turnover_5m_ago ?? obs.previous_turnover_5m ?? obs.prev_turnover_5m ?? obs.turnover_prev_5m ?? obs.last_turnover_5m ??
    obs.value_5m_ago ?? obs.previous_value_5m
  );
  if (currentTurnover != null && prevTurnover5m != null) {
    return Math.max(0, currentTurnover - prevTurnover5m);
  }

  // 3. Current turnover vs tracker / history
  if (currentTurnover != null) {
    if (Array.isArray(tracker.turnover_history) && tracker.turnover_history.length > 0) {
      const currentMin = finite(tracker.last_observation_minute) ?? toWibMinutes(observationTime(obs));
      const past = tracker.turnover_history.slice().reverse().find(h => currentMin != null && (currentMin - h.minute >= 4));
      const ref = past || tracker.turnover_history[0];
      if (ref && ref.turnover != null && currentTurnover >= ref.turnover) {
        return currentTurnover - ref.turnover;
      }
    }
    if (tracker.last_turnover != null && currentTurnover >= tracker.last_turnover) {
      return currentTurnover - tracker.last_turnover;
    }
  }

  // 4. Proxy: volumeRate * price (or volumeDelta * price)
  const price = finite(
    metrics.current_price ?? obs.current_price ?? obs.last_price ?? obs.price ?? tracker.last_price
  );
  const volumeRate = finite(
    metrics.volume_rate ?? obs.volume_rate ?? tracker.last_volume_rate
  );
  const volumeDelta = finite(
    metrics.volume_delta ?? obs.volume_delta
  );

  if (volumeRate != null && price != null && price > 0) {
    return volumeRate * price;
  }
  if (volumeDelta != null && price != null && price > 0) {
    return volumeDelta * price;
  }

  return null;
}

function evaluateOpeningVelocityGuard(options) {
  const opts = options || {};
  if (opts.bypass === true || (opts.obs && opts.obs.bypass_opening_velocity_guard === true)) {
    return {
      in_window: false,
      is_velocity_window: false,
      passes: true,
      bypassed: true,
      delta_turnover: null,
      threshold: null,
      board: opts.board || (opts.obs && opts.obs.board) || null,
      reason: 'opening_velocity_guard_bypassed'
    };
  }
  const time = opts.time || observationTime(opts.obs) || opts.scheduled_time || opts.scheduledTime;
  const isVelocityWindow = isOpeningRangeVelocityWindow(time);
  ...
  const isAccel = isAccelerationBoard(opts.board, opts.obs);
  const threshold = isAccel ? VELOCITY_THRESHOLD_ACCELERATION : VELOCITY_THRESHOLD_MAIN_DEVELOPMENT;
  const deltaTurnover = resolveDeltaTurnover(opts.obs, opts.tracker, opts.metrics);

  const validDelta = deltaTurnover != null && Number.isFinite(deltaTurnover) && deltaTurnover > 0;
  const passes = validDelta && deltaTurnover >= threshold;

  return {
    in_window: true,
    is_velocity_window: true,
    passes,
    delta_turnover: deltaTurnover,
    threshold,
    board: isAccel ? 'AKSELERASI' : 'UTAMA/PENGEMBANGAN',
    reason: passes
      ? 'opening_range_velocity_passed'
      : (!validDelta ? 'opening_range_velocity_data_invalid' : 'opening_range_velocity_insufficient')
  };
}
...
// BUG-F9-01/F9-02: payloads arriving over JSON (Supabase/PostgREST, cached
// snapshots, replay fixtures) carry booleans as 'true'/'false' strings or 1/0.
// A raw `=== true` comparison silently downgraded a genuinely READY engine
// payload to engine_class 'unknown' AND let a genuinely stale quote through the
// fail-closed freshness gate. Accept only explicit true-like values so the
// literal string 'false' can never be read as a positive flag.
...
function explicitFalseFlag(value) {
...
function scoreObservation(obs, tracker) {
  ...
  const volumeStrong = metrics.relative_volume == null
    ...
    : metrics.relative_volume >= 1.35 || (volumeAcceleration != null && volumeAcceleration >= 0.05);
  ...
  const enginePass = engineClass === 'ready' && publish >= 68;
  const velocityGuard = evaluateOpeningVelocityGuard({
    time: observationTime(obs),
    board: obs.board || (tracker && tracker.board),
    obs,
    tracker,
    metrics
  });
  ...
  return {
    ...
    metrics: Object.assign({}, metrics, {
      ...
      opening_velocity_guard: velocityGuard,
      delta_turnover_5m: velocityGuard.delta_turnover
    })
  };
}
...
function evaluate(obs, tracker) {
  ...
  // BATCH 3 Module 3: floor RVOL — emiten dengan RVOL < 1.2x wajib DROP
  // sebelum masuk evaluasi alert (pakai resolved ratio: pace > efektif > legacy).
  // Ditempatkan setelah guard struktural/chase (INVALID_DATA/STALE/INVALIDATED/
  // BLOCKED_CHASE/SPIKE_RADAR) agar klasifikasi terminal spesifik tidak berubah.
  if (m.relative_volume != null && m.relative_volume < 1.2) return Object.assign(scored, { passes: false, entry_eligible: false, status: 'WATCHING', reasons: ['rvol_below_minimum'] });
  ...
  const velocityGuard = m.opening_velocity_guard || evaluateOpeningVelocityGuard({
    time: observationTime(obs),
    board: obs.board || (tracker && tracker.board),
    obs,
    tracker,
  ...
  if (velocityGuard.is_velocity_window) {
    if (velocityGuard.passes) {
      reasons.push('opening_range_velocity_passed');
    } else {
      reasons.push(velocityGuard.reason || 'opening_range_velocity_insufficient');
      reasons.push('opening_range_velocity_gate_blocked');
      return Object.assign(scored, {
        passes: false,
        entry_eligible: false,
...
// BUG-F9-04: `Number.isFinite(data.volume_ratio_20d)` is false for the numeric
// STRINGS that JSON payloads carry ('3.0'), so both radar detectors silently
// fell back to vr = 0 and never fired. Use the coercing `finite()` helper so
// string and number payloads behave identically.
function isPrespikeRadar(data) {
  if (!data) return false;
  const dp = finite(data.change_pct) ?? 0;
...
module.exports = {
  ...
  OPENING_VELOCITY_WINDOW_START_MINUTE,
  OPENING_VELOCITY_WINDOW_END_MINUTE,
  VELOCITY_THRESHOLD_MAIN_DEVELOPMENT,
  VELOCITY_THRESHOLD_ACCELERATION,
  toWibMinutes,
  isOpeningRangeVelocityWindow,
  isAccelerationBoard,
  resolveDeltaTurnover,
  evaluateOpeningVelocityGuard,
  finite,
  clamp,
  round2,
  safeToFixed,
