...
function metricsFrom(obs) {
  ...
  const plan = obs && obs.selected_trade_plan && typeof obs.selected_trade_plan === 'object' ? obs.selected_trade_plan : {};
  const entryLow = finite(obs && (obs.entry_low ?? obs.buy_low)) ?? finite(plan.entry_zone_low);
  const entryHigh = finite(obs && (obs.entry_high ?? obs.buy_high)) ?? finite(plan.entry_zone_high);
  const tp1 = finite(obs && (obs.tp1 ?? obs.target_1)) ?? finite(plan.tp1);
  const stopLoss = finite(obs && (obs.stop_loss ?? obs.sl)) ?? finite(plan.stop_loss);
  let riskReward = finite(obs && (obs.risk_reward ?? obs.rr)) ?? finite(plan.rr_to_tp1);
  ...
  return {
    ...
    minimum_risk_reward: finite(obs && obs.minimum_risk_reward) ?? finite(plan.minimum_rr_to_tp1) ?? 1,
    plan_lock_id: String(obs && (obs.plan_lock_id || plan.plan_lock_id) || '').trim() || null,
...
function momentumFlowProxy(input) {
  const data = input || {};
  const priceVelocity = data.elapsed && data.price_change_pct != null ? data.price_change_pct / data.elapsed : null;
  const volumeRateVsAverage = data.volume_rate != null && data.average_volume != null && data.average_volume > 0 ? data.volume_rate / (data.average_volume / AVERAGE_TRADING_MINUTES) : null;
  const rangePosition = data.current_price != null && data.high != null && data.low != null && data.high > data.low ? clamp((data.current_price - data.low) / (data.high - data.low), 0, 1) : null;
  const components = {
    price_velocity: round2(interpolate(priceVelocity, [[-0.01,0],[0,0],[0.05,8],[0.15,16],[0.30,24],[0.50,30]])),
    volume_acceleration: round2(interpolate(data.volume_acceleration, [[-0.05,0],[0,0],[0.05,6],[0.20,12],[0.50,20],[1,25]])),
    relative_volume: round2(interpolate(data.relative_volume, [[0.70,0],[1,5],[1.25,10],[1.50,14],[2,18],[3,20]])),
    volume_rate: round2(interpolate(volumeRateVsAverage, [[0.30,0],[0.75,3],[1,5],[2,9],[4,13],[6,15]])),
    range_position: round2(interpolate(rangePosition, [[0.45,0],[0.60,2],[0.70,4],[0.80,7],[0.90,9],[0.97,10]]))
  };
  const available=[priceVelocity,data.volume_acceleration,data.relative_volume,volumeRateVsAverage,rangePosition].filter(value=>value!=null&&Number.isFinite(value)).length;
  ...
  const label=!temporalReady?'DATA_AWAL':score>=75?'KUAT':score>=60?'MENINGKAT':score>=45?'BELUM_MEYAKINKAN':'LEMAH';
  const confidence=available>=4?'HIGH':available===3?'MEDIUM':'LOW';
  const publishBonus=!temporalReady?0:score>=85?6:score>=75?4:score>=60?2:score<35?-4:score<45?-2:0;
  return {version:MOMENTUM_FLOW_VERSION,proxy_only:true,score,label,confidence,available_components:available,publish_bonus:publishBonus,price_velocity_pct_per_min:priceVelocity==null?null:round2(priceVelocity),volume_rate_vs_average:volumeRateVsAverage==null?null:round2(volumeRateVsAverage),range_position:rangePosition==null?null:round2(rangePosition),components};
}
function scoreObservation(obs, tracker) {
  ...
  const flow = momentumFlowProxy({elapsed,price_change_pct:priceChangePct,volume_rate:volumeRate,volume_acceleration:volumeAcceleration,relative_volume:metrics.relative_volume,average_volume:metrics.average_volume,current_price:metrics.current_price,high:metrics.high,low:metrics.low});
  ...
  if (metrics.entry_low != null && metrics.entry_high != null && metrics.entry_low <= metrics.entry_high && metrics.current_price) {
    ...
    else if (metrics.current_price < metrics.entry_low) { const below=((metrics.entry_low-metrics.current_price)/metrics.entry_low)*100; watch += below<=volPct?9:-Math.min(15,below*2); }
    else { const above=((metrics.current_price-metrics.entry_high)/metrics.entry_high)*100; watch += above<=clamp(volPct*0.35,0.25,1.25)?8:-30; }
  }
  if (normalizedImpulse >= 1) watch += 18; else if (normalizedImpulse >= 0.45) watch += 12; else if (normalizedImpulse >= 0.15) watch += 6; else if (normalizedImpulse <= -0.65) watch -= 16; else if (normalizedImpulse < -0.2) watch -= 7;
  if (metrics.relative_volume != null) { if(metrics.relative_volume>=2)watch+=18; else if(metrics.relative_volume>=1.35)watch+=13; else if(metrics.relative_volume>=1.05)watch+=7; else if(metrics.relative_volume<0.7)watch-=8; }
  if (volumeAcceleration != null) { if(volumeAcceleration>=0.35)watch+=12; else if(volumeAcceleration>=0.05)watch+=6; else if(volumeAcceleration<=-0.45)watch-=10; } else if (volumeDelta != null && volumeDelta > 0) watch += 3;
  ...
  let publish=watch+(engineClass==='ready'?8:0)-(engineClass==='near_miss'?4:0)-(tracker.in_latest_shortlist===false?4:0); publish+=flow.publish_bonus; publish=round2(clamp(publish,0,100));
  const volumeStrong=metrics.relative_volume==null?volumeAcceleration!=null&&volumeAcceleration>=0.05:metrics.relative_volume>=1.05||(volumeAcceleration!=null&&volumeAcceleration>=0.05);
  ...
  return {ready,engine_class:engineClass,passes:enginePass||nearMissPass,momentum_strong:momentumStrong,volume_strong:volumeStrong,metrics:Object.assign({},metrics,{volatility_pct:round2(volPct),price_change_pct:priceChangePct==null?null:round2(priceChangePct),advance_pct:advancePct==null?null:round2(advancePct),normalized_impulse:round2(normalizedImpulse),volume_delta:volumeDelta,volume_rate:volumeRate==null?null:round2(volumeRate),volume_acceleration:volumeAcceleration==null?null:round2(volumeAcceleration),momentum_flow_version:flow.version,momentum_flow_proxy:flow.proxy_only,momentum_flow_score:flow.score,momentum_flow_label:flow.label,momentum_flow_confidence:flow.confidence,momentum_flow_available_components:flow.available_components,momentum_flow_publish_bonus:flow.publish_bonus,price_velocity_pct_per_min:flow.price_velocity_pct_per_min,volume_rate_vs_average:flow.volume_rate_vs_average,range_position:flow.range_position,momentum_flow_components:flow.components,watch_score:watch,publish_score:publish})};
}
function evaluate(obs, tracker) {
  const scored=scoreObservation(obs,tracker); const m=scored.metrics;
  ...
  if(m.risk_reward==null)return Object.assign(scored,{passes:false,status:'WATCHING',reasons:['missing_risk_reward']});
  if(m.risk_reward<m.minimum_risk_reward)return Object.assign(scored,{passes:false,status:'WATCHING',reasons:['risk_reward_below_minimum']});
  const highest=finite(tracker&&tracker.highest_price_since_lock)??m.high;
  if(m.tp1==null||m.current_price>=m.tp1||(highest!=null&&highest>=m.tp1))return Object.assign(scored,{passes:false,status:'BLOCKED_CHASE',reasons:[m.tp1==null?'invalid_tp1':'tp1_already_reached']});
  ...
  const reasons=[];
  if(m.current_price>m.entry_high){const above=((m.current_price-m.entry_high)/m.entry_high)*100;if(above>clamp(m.volatility_pct*0.35,0.25,1.25))return Object.assign(scored,{passes:false,status:'BLOCKED_CHASE',reasons:['above_adaptive_entry_tolerance']});reasons.push('adaptive_entry_overshoot_tolerated');}
  ...
  if(m.momentum_flow_score>=75)reasons.push('momentum_flow_proxy_strong'); else if(m.momentum_flow_label!=='DATA_AWAL'&&m.momentum_flow_score<45)reasons.push('momentum_flow_proxy_weak');
...
module.exports={READY_VALUES,NEAR_MISS_VALUES,HARD_REJECT_VALUES,MAX_ADVANCE_PCT,MOMENTUM_FLOW_VERSION,AVERAGE_TRADING_MINUTES,finite,clamp,round2,toMinutes,explicitReady,metricsFrom,volatilityPct,momentumFlowProxy,scoreObservation,evaluate};
