...
const MOMENTUM_FLOW_VERSION = 'MOMENTUM_FLOW_PROXY_V2_VOLUME_PACE';
...
function spikeRadarResult(scored, reason) {
...
function evaluate(obs, tracker) {
  ...
  if (m.advance_pct != null && m.advance_pct > maxAdvance) return spikeRadarResult(scored, 'adaptive_advance_chase');
  const reasons = [];
  if (m.current_price > m.entry_high) {
    const above = ((m.current_price - m.entry_high) / m.entry_high) * 100;
    if (above > clamp(m.volatility_pct * 0.35, 0.25, 1.25)) return spikeRadarResult(scored, 'above_adaptive_entry_tolerance');
