...
function detectPattern(candles, ctx) {
  ...
  if (!c0 || !Number.isFinite(c0.open) || !Number.isFinite(c0.close) || !Number.isFinite(c0.high) || !Number.isFinite(c0.low)) return empty();
 ...
 * Delegates to lib/chart-engine/indicators.js (single source of truth).
 */
const chartIndicators = require('./chart-engine/indicators');

function classifySwingTrend(candles) {
  return chartIndicators.classifySwingTrend(candles);
}
...
module.exports = {
  ...
  classifySwingTrend: classifySwingTrend
};
