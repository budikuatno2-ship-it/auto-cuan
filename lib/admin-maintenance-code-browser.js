...
async function status(req, res, db) {
  ...
  if (!result.available) {
    return res.status(200).json({ success: true, featureAvailable: false, state: 'legacy' });
  }
...
async function consume(req, res, db) {
  ...
  // Security is already complete at this point: the one-time DB grant is
  // consumed and the signed session cookie is set. Telegram notification and
  // message cleanup are best-effort UX only and can never roll back access.
