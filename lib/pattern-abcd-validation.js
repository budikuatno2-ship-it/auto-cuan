...
function walkForwardAbcdValidation(candles, options) {
  ...
  for (var i = 0; i < candles.length; i++) {
    ...
    var result = findPattern(asOf, { ticker: symbol, dataDate: date });
    var reason = boundedReason(result && result.reason);
    if (result && result.candidate) {
      var contract = validate(result.candidate, { ticker: symbol, timeframe: '1D', dataDate: date, candles: asOf });
...
function invalidLevels(event) {
  ...
  if (event.direction === 'bullish' && !(event.invalidation < event.currentPriceAtFirstSeen && event.tp1 < event.tp2) ||
      event.direction === 'bearish' && !(event.invalidation > event.currentPriceAtFirstSeen && event.tp1 > event.tp2) ||
...
function evaluateAbcdOutcome(event, futureCandles, options) {
  ...
  hs.forEach(function(horizon) {
    ...
    var classification = terminal || (after.length < horizon ? 'insufficient_future_data' : (tp1Bar !== null ? 'tp1_before_invalidation' : 'unresolved'));
