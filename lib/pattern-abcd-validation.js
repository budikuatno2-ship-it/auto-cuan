...
function walkForwardAbcdValidation(candles, options) {
  ...
  for (var i = 0; i < candles.length; i++) {
    ...
    if (reason !== 'found') {
      examples.push({ ticker: symbol, dataDate: date, reason: reason });
...
function classifyFirstSeenEligibility(event) {
  var levels = ['currentPriceAtFirstSeen', 'invalidation', 'tp1', 'tp2'];
  var current, risk, rr1, rr2;
  if (!event || !levels.every(function(k) { return finite(event[k]) && event[k] > 0; })) {
    return { valid: false, status: 'invalid_event_levels', invalidReason: 'non_finite_or_non_positive_level' };
  }
  if (event.direction !== 'bullish' && event.direction !== 'bearish') {
    return { valid: false, status: 'invalid_event_levels', invalidReason: 'nonsensical_level_order' };
  }
  current = event.currentPriceAtFirstSeen;
  if (event.direction === 'bullish') {
    if (!(event.invalidation < event.tp1 && event.tp1 < event.tp2)) {
      return { valid: false, status: 'invalid_event_levels', invalidReason: 'nonsensical_level_order' };
    }
    if (current <= event.invalidation) return { valid: true, status: 'invalidation_reached_before_first_seen', invalidReason: null };
    if (current >= event.tp2) return { valid: true, status: 'tp2_reached_before_first_seen', invalidReason: null };
    if (current >= event.tp1) return { valid: true, status: 'tp1_reached_before_first_seen', invalidReason: null };
  } else {
    if (!(event.invalidation > event.tp1 && event.tp1 > event.tp2)) {
      return { valid: false, status: 'invalid_event_levels', invalidReason: 'nonsensical_level_order' };
    }
  ...
  risk = Math.abs(current - event.invalidation);
  rr1 = Math.abs(event.tp1 - current) / risk;
  rr2 = Math.abs(event.tp2 - current) / risk;
  if (!finite(risk) || risk <= 0 || !finite(rr1) || !finite(rr2)) {
    return { valid: false, status: 'invalid_event_levels', invalidReason: 'nonsensical_level_order' };
  }
  return { valid: true, status: 'eligible', invalidReason: null };
}
function evaluateAbcdOutcome(event, futureCandles, options) {
  options = options || {};
  var hs = horizons(options.horizons), eligibility = classifyFirstSeenEligibility(event);
  var after = (futureCandles || []).filter(function(c) { return c.time > event.firstSeenDate; });
  var output = { invalidReason: eligibility.invalidReason, firstSeenEligibility: eligibility.status, horizons: {} };
  hs.forEach(function(horizon) {
    if (!eligibility.valid) {
      output.horizons[String(horizon)] = { classification: 'invalid_event_levels', sameBarConflict: false };
      return;
    }
    if (eligibility.status !== 'eligible') {
      output.horizons[String(horizon)] = { classification: 'ineligible_at_first_seen', firstSeenOutcome: eligibility.status,
        sameBarConflict: false, barsEvaluated: 0 };
      return;
    }
...
function summarizeAbcdValidation(events, options) {
  ...
  (events || []).forEach(function(event) {
    hs.forEach(function(h) {
      var outcome = event.outcomes && event.outcomes[String(h)];
      if (!outcome || outcome.classification === 'invalid_event_levels' || outcome.classification === 'ineligible_at_first_seen') return;
...
module.exports = { walkForwardAbcdValidation: walkForwardAbcdValidation, evaluateAbcdOutcome: evaluateAbcdOutcome,
  summarizeAbcdValidation: summarizeAbcdValidation, classifyFirstSeenEligibility: classifyFirstSeenEligibility,
  validateCandles: validateCandles, DEFAULT_HORIZONS: DEFAULT_HORIZONS };
