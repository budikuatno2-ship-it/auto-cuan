...
const earlyWatchPublisher = require('./intraday-fast-watcher-early-watch-publisher');
...
async function run(options) {
  ...
  try {
    ...
    // Additive hook. Defaults OFF (single authoritative flag:
    // FAST_WATCHER_EARLY_WATCH_ENABLED). Runs strictly after the 2/2
    // confirmation pool state above is finalized and written; never reads
    // back into it, never influences publication below, and never
    // suppresses the existing confirmed Telegram/Supabase path. Telegram
    // sending itself is delegated to
    // intraday-fast-watcher-early-watch-publisher.js (injected here), which
    // independently re-checks FAST_WATCHER_LIVE_ENABLED,
    // FAST_WATCHER_TELEGRAM_ENABLED, and the global TELEGRAM_ENABLED switch
    // BEFORE persisting any send reservation, and always sends through the
    // canonical lib/telegram-notifier.js sender.
    let earlyWatchResult = { enabled: false, informational_only: true, status: 'disabled', version: earlyWatch.VERSION };
    try {
      earlyWatchResult = await (opts.runEarlyWatch || earlyWatch.runEarlyWatch)({
        env,
        sampleDate: opts.sampleDate,
        scheduledTime: opts.scheduledTime,
        now: opts.now,
        ...
        notifyFn: opts.earlyWatchNotifyFn,
        sendNotifications: opts.sendEarlyWatchNotifications || earlyWatchPublisher.sendPendingNotifications
      });
    } catch (error) {
      earlyWatchResult = { enabled: earlyWatch.isEnabled(env), informational_only: true, status: 'error', version: earlyWatch.VERSION, error: error && error.message };
    }
