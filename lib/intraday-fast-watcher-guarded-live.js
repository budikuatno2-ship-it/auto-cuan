...
async function run(options) {
  ...
  try {
    ...
    // Additive hook. Defaults OFF (FAST_WATCHER_EARLY_WATCH_ENABLED, with the
    // legacy FAST_WATCHER_EARLY_WATCH_SHADOW_ENABLED name still honored).
    // Runs strictly after the 2/2 confirmation pool state above is finalized
    // and written; never reads back into it, never influences publication
    // below, and never suppresses the existing confirmed Telegram/Supabase
    // path. Telegram sending itself is delegated to
    // intraday-fast-watcher-early-watch-publisher.js (injected here), which
    // independently re-checks FAST_WATCHER_LIVE_ENABLED/FAST_WATCHER_TELEGRAM_ENABLED
    // and always sends through the canonical lib/telegram-notifier.js sender.
