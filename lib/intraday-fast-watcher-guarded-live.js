...
async function run(options) {
  ...
  try {
    ...
    const mergedPayload = pool.mergePayload(normalized.payload, priorState, opts.scheduledTime, supplemental, opts.maxShortlist || 20);
    const shortlist = watcher.buildShortlist(mergedPayload, opts.maxShortlist || 20);
    if (!shortlist.ok || !shortlist.rows.length) return { status: 'empty_shortlist', shortlist_count: 0 };
    ...
    const collected = await shadowLive.collectLiveSnapshot({
      ...
      maxShortlist: opts.maxShortlist || 20,
