...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!termsCheck.ok) {
      return res.status(400).json({ success:false, error:'Anda wajib menyetujui ketentuan pembayaran dan aktivasi voucher.' });
    }
    ...
    await accountTerms.recordTermsAcceptance(db, auth.account.id, 'voucher');
