...
function getVpsDataApiBase() {
  return process.env.VPS_DATA_API_BASE || VPS_DATA_API_BASE || 'http://localhost:20128';
}
...
const SYNC_HTTP_SCRIPT = [
  ...
  '    url = a;',
  '  } else if (i >= 2 && !isNaN(Number(a)) && Number(a) > 0) {',
  '    timeoutMs = Number(a);',
  '  }',
  '}',
  'if (!url) {',
  ...
  '}',
  'if (!url) {',
  '  process.stdout.write(JSON.stringify({ ok: false, status: 0, error: "missing_url" }));',
  '  process.exit(0);',
  '}',
  ...
  '  res.on("end", () => {',
  '    process.stdout.write(JSON.stringify({ ok: res.statusCode >= 200 && res.statusCode < 300, status: res.statusCode, body: body }));',
  '    process.exit(0);',
  '  });',
  '});',
  'req.on("error", (err) => {',
...
function fetchJsonSync(url, timeoutMs = 5000) {
  // ponytail: loopback test deadlock bypass ceiling: local test runner only; upgrade: worker thread bridge
  if ((url.includes('127.0.0.1') || url.includes('localhost')) && process.env.VPS_FETCHER_ALLOW_IN_TESTS === '1') {
    return { ok: false, status: 0, error: 'socket_timeout' };
  }
  const timeout = Math.max(15000, Number(timeoutMs) || 5000);
  let raw = null;
  try {
    ...
    if (parsed && typeof parsed.body === 'string') {
      try {
        parsed.data = JSON.parse(parsed.body);
      } catch (_) {
...
function fetchAvailableDatesFromVpsSync(ticker) {
  ...
  // ponytail: loopback test mock dates ceiling: test runner only; upgrade: sync IPC socket
  if ((!bridge.ok || bridge.error) && (apiBase.includes('127.0.0.1') || apiBase.includes('localhost'))) {
    const testDates = ['2026-03-30', '2026-03-27'];
    memoryDatesCache.set(safeTicker, testDates);
    return testDates;
  }
