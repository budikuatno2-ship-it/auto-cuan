...
async function fetchAvailableDatesFromVps(ticker) {
  ...
  try {
    ...
    const res = await fetch(`${VPS_DATA_API_BASE}/api/available-dates?ticker=${encodeURIComponent(safeTicker)}`, {
...
function fetchAvailableDatesFromVpsSync(ticker) {
  ...
  try {
    const curlOut = execFileSync('curl.exe', [
      '-s',
      '--max-time', '4',
      `${VPS_DATA_API_BASE}/api/available-dates?ticker=${encodeURIComponent(safeTicker)}`
    ], { encoding: 'utf8', timeout: 5000 });

    if (curlOut && curlOut.trim().startsWith('{')) {
  ...
  for (const targetDate of candidateDates) {
    try {
      const curlOut = execFileSync('curl.exe', [
        '-s',
        '--max-time', '5',
        `${VPS_DATA_API_BASE}/api/broker-summary?ticker=${encodeURIComponent(safeTicker)}&date=${encodeURIComponent(targetDate)}`
 ...
 */
async function fetchBrokerSummaryFromVps(ticker, date = '2026-09-08') {
  if (!ticker) return null;
  const safeTicker = String(ticker).trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
  const safeDate = String(date || '2026-09-08').trim().replace(/[^0-9\-a-zA-Z]/g, '');
  const cacheKey = `${safeTicker}_${safeDate}`;
  ...
  for (const targetDate of candidateDates) {
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 5000);
      const res = await fetch(`${VPS_DATA_API_BASE}/api/broker-summary?ticker=${encodeURIComponent(safeTicker)}&date=${encodeURIComponent(targetDate)}`, {
        signal: controller.signal
      });
      clearTimeout(timeout);
      if (res.ok) {
