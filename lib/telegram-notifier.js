  ...
  body.parse_mode = options.parse_mode;
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          // BATCH 10: honor retry_after — park the whole gate and warn (never throw).
          var backoffMs = applyRateLimitBackoff(retryAfter);
          console.warn('[TELEGRAM_RATE_LIMITED] honoring retry_after=' + (retryAfter == null ? 'unknown' : retryAfter) + 's, backoff=' + Math.round(backoffMs / 1000) + 's, chunks_sent=' + sentCount + '/' + chunks.length);
          return { ok: false, sent: false, skipped: false, reason: 'rate_limited', status: 429, retry_after_seconds: retryAfter, backoff_ms: backoffMs, error_message: errMsg, chunks_total: chunks.length, chunks_sent: sentCount };
        }
