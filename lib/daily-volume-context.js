...
function buildVolumeContext(rows) {
  ...
  return {
    ...
    // Explicit, UI-friendly label for "is today's volume figure a live
    // intraday partial count, or the settled final end-of-day total?" —
    // never leave this ambiguous the way a bare boolean can be misread.
    today_session_status: !today ? 'UNKNOWN' : (isTodayPartial ? 'INTRADAY_PARTIAL' : 'FINAL_EOD'),
