...
function computeCacheKey(params = {}) {
  ...
  // ponytail: handle edge-of-midnight UTC vs Jakarta date discrepancy in cache lookup
  if (!mem && params && params.marketDate && params.marketDate === formatJakartaDate(new Date())) {
    const altKey = computeCacheKey(Object.assign({}, params, { marketDate: new Date().toISOString().slice(0, 10) }));
    mem = memoryCache.get(altKey);
  }
  if (mem) {
