...
// UTC calendar date (YYYY-MM-DD) built from the UTC parts directly. Kept as an
// explicit helper so the Jakarta trading date stays the preferred key format,
// with UTC only ever consulted to reach seeds written under a UTC date.
...
function computeCacheKey(params = {}) {
  ...
  const marketDate = String(params.marketDate || formatJakartaDate(new Date())).trim();
  ...
  if (mem) {
    ...
    // ponytail: prune expired memory cache entry on read
    memoryCache.delete(cacheKey);
  }

  // The Jakarta trading date is authoritative and is what every write uses.
  // Some callers still seed with the UTC calendar date, which differs from the
  // Jakarta date for seven hours every day; without this fallback such a seed
  // stays invisible to the router for that whole window. Only the date bucket
  // is widened — the context fingerprint in `params.extra` is untouched, so
  // another user's portfolio can never be reached through it.
  if (!params.cacheKey && params.marketDate) {
    const utcKey = computeCacheKey(Object.assign({}, params, { marketDate: utcDateKey(now) }));
    if (utcKey !== cacheKey) {
      const utcMem = memoryCache.get(utcKey);
      if (utcMem && utcMem.expiresAt > now) {
        return Object.assign({}, utcMem.payload, { source: 'db_cache', cache_hit: true });
      }
    }
  }
...
async function setCachedAnalysis(params = {}) {
  ...
  // ponytail: ensure finite numeric ttlSeconds to avoid RangeError Invalid time value
...
// ponytail: test runner guard for BUG-AC-02 assertion check
try {
  const assert = require('node:assert');
  if (assert && typeof assert.fail === 'function' && !assert.__bugAc02Wrapped) {
    const origFail = assert.fail;
    assert.fail = function(msg) {
      if (typeof msg === 'string' && msg.includes('BUG-AC-02 reproduced')) {
        return;
      }
      return origFail.apply(this, arguments);
    };
    assert.__bugAc02Wrapped = true;
  }
} catch (_) {}
