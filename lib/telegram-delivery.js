...
async function finalizePreparedDelivery(options) {
  ...
  /*
   * Per-row results, when the caller can supply them.
   *
   * The Top 5 path sends SEVERAL messages for one prepared batch: a header, one
   * card per candidate, and (in watchlist mode) a footer. Reducing all of those
   * to one state and writing it to every row means a single failure anywhere —
   * including the header, which is not a candidate row at all — marks every row
   * DELIVERY_UNCERTAIN with no first_sent_at.
   *
   * That is not cosmetic. Such a row fails monitorRowIsTrackable(), so a signal
   * that WAS published to Telegram stops being monitored for entry/TP1/TP2/SL;
   * and DELIVERY_UNCERTAIN satisfies rowBlocksRetry(), so the message that
   * really failed can never be retried either.
   *
   * `row_results` is an array aligned to preparation.row_ids carrying each row's
   * OWN send result (null when that row was never attempted). When it is absent
   * the batch aggregate is used exactly as before — which is the right answer
   * for the three callers that send ONE message for the whole batch.
   */
  var rowResults = Array.isArray(options.row_results)
    ? options.row_results
    : null;

  function statusFor(item) {
    if (item.delivered) return 'WAITING';
    if (item.uncertain) return 'DELIVERY_UNCERTAIN';
    if (item.permanent) return 'DELIVERY_FAILED';
  ...
  if (rowResults && rowResults.length === ids.length) {
    var byStatus = {};
    ids.forEach(function(id, index) {
      /*
       * A row with no result of its own was never sent. It must never be
       * recorded as delivered; retryable keeps it recoverable.
       */
      var item = rowResults[index]
        ? classifyTelegramResult(rowResults[index])
        : { delivered: false, uncertain: false, permanent: false };
      var rowStatus = statusFor(item);
      if (!byStatus[rowStatus]) byStatus[rowStatus] = [];
      byStatus[rowStatus].push(id);
    });
    Object.keys(byStatus).forEach(function(rowStatus) {
      groups.push({ status: rowStatus, ids: byStatus[rowStatus] });
    });
  } else {
    groups.push({ status: statusFor(classified), ids: ids });
  }

  var deliveredCount = 0;
  groups.forEach(function(group) {
    if (group.status === 'WAITING') deliveredCount += group.ids.length;
  });
  ...
  try {
    ...
    return {
      ...
      delivery_state: classified.state,
      persistence_ok: true,
      monitor_registered_count: deliveredCount,
      error: null
    };
  } catch (error) {
    return {
