...
function monitorRowIsEligible(row) {
  if (!row) {
    return false;
  }

  var raw = getRawPayload(row);
  var source = String(row.monitor_source || raw.monitor_source || row.category || raw.category || '').trim().toLowerCase();

  /*
   * Silent background screener registers ('daytrade') are for internal outcome
   * tracking only and must NEVER trigger public Telegram hit notifications.
   * Public Telegram alerts are strictly reserved for verified signals
   * ('daytrade_signal', 'swing_konglo', 'swing_nk', 'daily_top5', etc.).
   */
  if (source === 'daytrade' || source === 'day_trade') {
    return false;
  }
