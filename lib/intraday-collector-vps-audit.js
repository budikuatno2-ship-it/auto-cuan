 ...
 *
 * Required runner environment (all seven MUST be present, exact values):
 *   TRADE_PLAN_V2_SHADOW_ENABLED=true
 *   TRADE_PLAN_V2_LIQUIDITY_SWEEP_SHADOW_ENABLED=true
 *   TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED=true
 *   TRADE_PLAN_V2_PUBLIC_ENABLED=false
 *   DAYTRADE_INTRADAY_SCORE_ENABLED=false
 *   DAYTRADE_WORKER_ALLOW_MUTATION=false
 *   TELEGRAM_ENABLED=0
 *
 * The intraday-lock flag decouples the internal intraday plan-lock (which may use
 * the validated Trade Plan V2 SL/TP in shadow mode) from the public rollout:
 * public V2 on the website / Telegram stays OFF (TRADE_PLAN_V2_PUBLIC_ENABLED=false).
 */
...
function auditRunnerScript(content, meta) {
  ...
  const intraday_lock_enabled = flag_values.TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED === 'true';
  ...
  if (!intraday_lock_enabled) issues.push('intraday plan lock not enabled (TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED must be true)');
  ...
  return {
    ...
    intraday_lock_enabled: intraday_lock_enabled,
 ...
 * exports the seven required flags, runs the collector under Node 22, logs to the
 ...
 * and keeps public V2 off (TRADE_PLAN_V2_PUBLIC_ENABLED=false) while enabling the
 * decoupled internal intraday plan-lock (TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED=true)
 * so the collector locks the validated V2 SL/TP in shadow mode.
 */
