...
function isEnforcementCandidate(input) {
  ...
  return input.grantRevoked === true || input.blocked === true || input.approved !== true || !input.entitlement ||
    ['revoked', 'expired'].includes(input.entitlement.status) || (input.entitlement.lifetime !== true && new Date(input.entitlement.endsAt) <= (input.now || new Date()));
}
...
function menuFor(input) {
  ...
  if (!access.bot) return ['Akun Saya', 'Paket', 'Beli Paket', 'Gunakan Voucher', 'Hubungi Admin'];
  return ['Akun Saya', 'Paket Aktif', 'Beli / Perpanjang', 'Akses Channel', 'Gunakan Voucher', 'Cara Pakai', 'Hubungi Admin'];
}
function menuKeyboard(input) { return { keyboard: menuFor(input).map(x => [{ text: x }]), resize_keyboard: true }; }

module.exports = { PURCHASE_STATES, TRANSITIONS, MENU, MAX_UPDATE_BYTES, MAX_PROOF_BYTES, normalizeVoucher, voucherHash, generateVoucherCode, maskVoucher, calculatePrice, canTransition, purchaseAllowsProof, purchaseAllowsApproval, voucherReservationAfter, canPurchase, isEnforcementCandidate, accessFor, extendEntitlement, validateProof, isPrivate, validCallback, safeEqual, menuFor, menuKeyboard };
