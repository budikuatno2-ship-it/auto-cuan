...
const MENU = Object.freeze([
  ...
  'Pesanan Saya', 'Akses Channel', 'Cara Pakai', 'Hubungi Admin'
]);
...
function generateVoucherCode(bytes) { return crypto.randomBytes(bytes || 16).toString('base64url').toUpperCase(); }
...
function purchaseAllowsProof(purchase, now) {
  return Boolean(purchase && ['awaiting_payment', 'rejected'].includes(purchase.status) &&
    new Date(purchase.expiresAt) > (now || new Date()));
}
function purchaseAllowsApproval(purchase, now) {
  return Boolean(purchase && purchase.status === 'awaiting_admin_review' &&
    new Date(purchase.expiresAt) > (now || new Date()) && !purchase.approvedAt);
}
function voucherReservationAfter(purchaseStatus) {
  return purchaseStatus === 'approved' ? 'finalized' :
    ['expired', 'cancelled'].includes(purchaseStatus) ? 'released' : 'reserved';
}
function canPurchase(input) {
  return Boolean(input && input.verified === true && input.blocked !== true && input.accountStatus !== 'blocked');
}
function dashboardEligible(input) {
  ...
  return Boolean(input && input.approved === true && input.blocked !== true && input.verified === true && e &&
    e.status === 'active' && e.lifetime === true && !e.revokedAt &&
    (!e.endsAt || new Date(e.endsAt) > (input.now || new Date())));
}
...
function validateProof(file) {
  ...
  return {
    ...
    metadata: {
      fileId: file.file_id,
      uniqueId: String(file.file_unique_id || '').slice(0, 256),
...
function validCallback(data) {
  return typeof data === 'string' &&
    /^(?:menu|package|checkout|novoucher|voucher|channel|nav|cancel|cancelpurchase|current|confirm|proof|notpaid):[a-z0-9_-]{1,48}$/.test(data);
}
...
function menuFor(input) {
  ...
  if (!access.bot) {
    const out = ['Akun Saya', 'Paket', 'Beli Paket'];
    if (input.pendingPurchase === true) out.push('Pesanan Saya');
    out.push('Hubungi Admin');
    return out;
  }
...
function menuKeyboard(input) {
  ...
  for (let i = 0; i < items.length; i += 2) {
    rows.push(items.slice(i, i + 2).map(text => ({ text })));
  }
  return { keyboard: rows, resize_keyboard: true };
}

module.exports = {
  PURCHASE_STATES, TRANSITIONS, MENU, MAX_UPDATE_BYTES, MAX_PROOF_BYTES,
  normalizeVoucher, voucherHash, generateVoucherCode, maskVoucher, calculatePrice,
  canTransition, purchaseAllowsProof, purchaseAllowsApproval, voucherReservationAfter,
  canPurchase, dashboardEligible, isEnforcementCandidate, accessFor, extendEntitlement,
  validateProof, isPrivate, validCallback, safeEqual, menuFor, menuKeyboard
};
