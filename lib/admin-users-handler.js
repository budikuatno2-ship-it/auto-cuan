...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const voucherActions = ['voucher_admin_create', 'voucher_admin_revoke', 'voucher_admin_list'];
    const protectedSubscriptionActions = catalogActions.concat(voucherActions);
    // Keep the original catalog gate explicit; vouchers extend, not weaken, it.
    ...
    if (protectedSubscriptionActions.indexOf(action) >= 0 && !isSubscriptionFeatureEnabled()) {
      return res.status(503).json({ success: false, error: 'Fitur langganan belum tersedia.' });
    }
    if (protectedSubscriptionActions.indexOf(action) >= 0 && String(auth.session.un || '').toLowerCase() !== 'budi') {
    ...
    if (protectedSubscriptionActions.indexOf(action) >= 0) {
    ...
    // This is server-only protection for the separate Telegram admin bot that
    // will be configured later. No voucher controls are rendered in web admin.
    if (voucherActions.indexOf(action) >= 0) {
      if (action === 'voucher_admin_list') {
        const listed=await supabase.from('subscription_vouchers').select('id,code_hint,plan_code,duration_days,max_redemptions,redemption_count,active,expires_at,created_at,revoked_at').order('created_at',{ascending:false}).limit(100);
        if (listed.error) return res.status(503).json({success:false,error:'Voucher tidak tersedia.'});
        return res.status(200).json({success:true,vouchers:listed.data||[]});
      }
      const code=vouchers.normalizeVoucherCode(req.body&&req.body.voucher_code); let hash;
      try { hash=vouchers.voucherCodeHash(code); } catch (_) { return res.status(400).json({success:false,error:'Voucher tidak valid.'}); }
      const result=await supabase.rpc(action==='voucher_admin_create'?'create_subscription_voucher':'revoke_subscription_voucher',{p_voucher_code_hash:hash,p_voucher_code_hint:code.slice(-4),p_plan_code:req.body&&req.body.plan_code,p_duration_days:req.body&&req.body.duration_days,p_max_redemptions:req.body&&req.body.max_redemptions,p_actor_user_id:auth.session.uid});
      if(result.error) return res.status(409).json({success:false,error:'Voucher tidak dapat diproses.'});
    ...
    if (action === 'reset_password') {
      ...
      // The body must carry a CLIENT hash (64 hex). Anything else — a malformed
      // value, or an already-protected `k1...` stored shape — is refused rather
      // than written: a row that can never verify would lock the account out
      // with no error anywhere.
      const clientPasswordHash = passwordCredential.normalizeClientHash(newPasswordHash);
      if (!clientPasswordHash) {
        return res.status(400).json({ success: false, error: 'Password hash tidak valid.' });
      }
      ...
      // Store the PROTECTED credential (random salt + scrypt), exactly like
      // every other write path: api/register-user.js:147, api/login-user.js:428
      // and lib/reset-password-legacy-handler.js:252. Writing the raw client
      // hash left the row in the legacy shape that
      // lib/password-credential.js exists to eliminate — one that can be
      // replayed straight back as a login hash — until the user's next login
      // happened to upgrade it.
      let protectedPasswordHash;
      try {
        protectedPasswordHash = passwordCredential.protectClientHash(clientPasswordHash);
      } catch (e) {
        return res.status(400).json({ success: false, error: 'Password hash tidak valid.' });
      }
      ...
      const { error: updateError } = await supabase
        .from('app_users')
        .update({ password_hash: protectedPasswordHash })
        .eq('id', user.id);
    ...
    if (action === 'remove_device') {
      const targetUser = String(username || auth.session.un || '').trim().toLowerCase();
      const deviceIdToRemove = String(req.body && (req.body.deviceId || req.body.device_id) || '').trim();

      if (!targetUser || !deviceIdToRemove) {
        return res.status(400).json({ success: false, error: 'Username dan Device ID diperlukan.' });
      }

      const { data: user, error: findError } = await supabase
        .from('app_users')
        .select('id, username, devices, device_id')
        .eq('username', targetUser)
        .maybeSingle();
      ...
      const updatedDevices = currentDevices.filter(d => d !== deviceIdToRemove);

      const updatePayload = { devices: updatedDevices };
      if (user.device_id === deviceIdToRemove) {
        updatePayload.device_id = updatedDevices[0] || null;
      }
      ...
      if (updateError) {
        return res.status(500).json({ success: false, error: 'Gagal menghapus perangkat: ' + updateError.message });
      }
      ...
      return res.status(200).json({
        ...
        username: targetUser,
        remaining_devices: updatedDevices
      });
    }

    // === LIST ADMIN DEVICES ===
    if (action === 'list_admin_devices') {
      const { data: user, error: findError } = await supabase
        .from('app_users')
        .select('id, username, devices, device_id, last_login_at, user_agent')
        .eq('username', 'budi')
        .maybeSingle();

      if (findError || !user) {
        return res.status(404).json({ success: false, error: 'Admin budi tidak ditemukan.' });
      }

      return res.status(200).json({
        success: true,
        devices: Array.isArray(user.devices) ? user.devices : [],
        current_device_id: (auth.session && auth.session.dev) || user.device_id || null,
