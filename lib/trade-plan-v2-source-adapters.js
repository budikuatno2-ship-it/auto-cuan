...
function adaptDayTrade(source) {
  ...
  const swingLow = pickNum(gen, ['swing_low', 'latest_swing_low', 'confirmed_swing_low', 'swingLow5']);
  const resistance = pickNum(gen, ['resistance', 'resistance1', 'r1']);
  // Next resistance: the intermediate swing high (`swingHigh10`) when it sits
  // above the primary resistance; otherwise not invented.
  const swingHigh = pickNum(gen, ['next_resistance', 'resistance2', 'r2', 'swingHigh10']);
  const nextResistance = (swingHigh !== null && resistance !== null && swingHigh > resistance) ? swingHigh
    : (swingHigh !== null && resistance === null ? swingHigh : null);
  ...
  structural.available = support !== null || swingLow !== null;
...
function adaptSwingKonglo(source) {
  ...
  const ticker = pickVal(gen, ['ticker']) || null;
  const support = pickNum(gen, ['support', 'support1', 's1']);
  const swingLow = pickNum(gen, ['swing_low', 'latest_swing_low', 'confirmed_swing_low']);
  const resistance = pickNum(gen, ['resistance', 'resistance1', 'r1']);
