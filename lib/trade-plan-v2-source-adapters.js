...
function adaptDayTrade(source) {
  ...
  const explicitLocalSupport = pickNum(gen, ['local_support', 'recent_support', 'nearest_local_support']);
  const todayLow = pickNum(gen, ['today_low', 'low_price']);
  const localSupport = explicitLocalSupport !== null ? explicitLocalSupport : (todayLow !== null ? todayLow : null);
  ...
  let entryLow = pickNum(gen, ['entry_low', 'entry2', 'entry_2']);
  let entryHigh = pickNum(gen, ['entry_high', 'entry1', 'entry_1']);
  if (entryLow !== null && entryHigh !== null && entryLow > entryHigh) {
    const _swp = entryLow; entryLow = entryHigh; entryHigh = _swp;
  }
  ...
  noteField(structural, 'today_low', todayLow);
  ...
  structural.available = support !== null || explicitLocalSupport !== null || majorSupport !== null || swingLow !== null || localResistance !== null || majorResistance !== null;
  ...
  const input = {
    ...
    low_price: todayLow,
    today_low: todayLow,
    open_price: pickNum(gen, ['open_price', 'open']),
    support,
