...
function deriveDayTradeEntryDiscipline(row) {
  ...
  const hasAnyEntry = entryLow != null || entryHigh != null;
  ...
  // Legacy/unit-test rows without any canonical entry fields retain the prior
  // execution-ranking behavior. Once an entry field is present, however, a
  // partial or malformed zone fails closed.
  if (!hasAnyEntry) {
    return {
      entry_reference_price: null,
      entry_chase_pct: null,
      entry_discipline_status: 'ENTRY_NOT_PROVIDED',
      entry_discipline_label: 'Canonical entry tidak disediakan',
      entry_executable_now: true,
      entry_discipline_reason: 'Tidak ada canonical entry zone pada record; entry discipline tidak dapat menilai pay-up.'
    };
  }
