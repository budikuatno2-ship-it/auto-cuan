...
async function resolveAccount(req, res, supabase) {
  ...
  if (!auth.ok) {
    res.status(auth.status || 401).json({ success: false, error: auth.error || 'Sesi tidak valid.' });
    return null;
  }
  ...
  if (account.is_blocked === true) {
    res.status(403).json({ success: false, error: 'Akun sedang diblokir.' });
    return null;
  }
  if (account.is_approved !== true) {
    res.status(403).json({ success: false, error: 'Akun belum di-approve admin.' });
    return null;
  }

  return { id: String(account.id), username: accountUsername };
}
...
async function saveState(req, res, supabase, account) {
  ...
  const result = await supabase
    .from('app_user_portfolio_state')
    .upsert({ user_id: account.id, state, updated_at: updatedAt }, { onConflict: 'user_id' })
    .select('updated_at')
    .single();
  ...
  return res.status(200).json({
    ...
    storage: 'supabase',
    user_id: account.id,
    updated_at: result.data && result.data.updated_at || updatedAt
  });
}
