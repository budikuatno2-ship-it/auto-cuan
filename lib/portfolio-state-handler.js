...
/**
 * BUG-F7-015: predikat ini dipakai untuk memutuskan apakah state perlu ditulis.
 * State yang melewati batas ukuran sebelumnya melempar error 413 dari
 * normalizePortfolioState() dan menjatuhkan pemanggilnya. Sebagai predikat,
 * fungsi ini harus selalu mengembalikan boolean: bila state terlalu besar untuk
 * dinormalisasi, kita periksa struktur mentahnya secara dangkal dan tetap
 * melaporkan true (state jelas berisi data).
 */
function hasPortfolioData(state) {
  let value;
  try {
    value = normalizePortfolioState(state);
  } catch (_) {
    const raw = isPlainObject(state) ? state : {};
    return (Array.isArray(raw.plans) && raw.plans.length > 0)
      || (Array.isArray(raw.journal) && raw.journal.length > 0)
      || (Array.isArray(raw.changes) && raw.changes.length > 0)
      || (isPlainObject(raw.prices) && Object.keys(raw.prices).length > 0)
      || raw.snapshot != null;
  }
