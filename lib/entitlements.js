...
function isActive(row, now) {
  ...
  return row.status === 'active' && Number.isFinite(start) && start <= now &&
    (row.lifetime === true || (Number.isFinite(end) && now < end));
}
...
async function resolveEntitlements(user, account, supabase, nowValue) {
  ...
  if (!account) return noEntitlement();
  if (!supabase || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(account.id || ''))) return getEntitlements(user, account);
