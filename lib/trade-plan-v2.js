...
function buildTradePlanV2(candidate, options) {
  ...
  const stopLoss = tick(structuralLevel - volatilityBuffer, 'floor');
  const structuralInvalidation = tick(structuralLevel, 'floor');
  // Emergency hard-stop: always retained, one extra buffer below the SL.
  const emergencyStop = tick(structuralLevel - volatilityBuffer * 2, 'floor');
  ...
  const stopLossReason = atr !== null
    ? 'structural level (' + structuralLevel + ') minus ' + profile.volatility_buffer_atr + ' ATR volatility buffer (>= ' + MIN_TICK_BUFFER + ' ticks)'
    : 'structural level (' + structuralLevel + ') minus ' + MIN_TICK_BUFFER + '-tick floor buffer (ATR unavailable)';
