...
function isUsableOverheadLevel(level, entryLow, tolerance) {
  return !!(level && level.valid && level.confirmed && level.defended &&
    !level.stale && !level.failed && level.value > entryLow + tolerance);
}
...
function buildTradePlanV2(candidate, options) {
  ...
  let resistanceCandidates = overheadLevels.filter((level) =>
    isUsableOverheadLevel(level, entryLow, resistanceTolerance));
  ...
  const emergencyCandidates = supportLevels.filter((level) =>
    ...
    level.value < stopAnchor && isUsableStructuralLevel(level, entryLow, profile.emergency_max_structural_distance_pct));
