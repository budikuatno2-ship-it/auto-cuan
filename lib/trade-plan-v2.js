...
/** Round a price to a valid IDX tick (defaults to nearest). Returns null when invalid. Board-aware so FCA Rp1 tick is honoured (251 stays 251 instead of 252). */
...
function deriveSupportTestState(p) {
  ...
  if (consecBelow >= 2 || (consecBelow >= 1 && explicitConfirm)) {
    ...
    reason = 'close below support but above buffered support threshold';
...
function computeTrailingStop(p) {
  ...
  if (!reached) {
    return {
      ...
      activation_price: activationPrice !== null ? tick(activationPrice, 'nearest', p.board, p.is_fca != null ? p.is_fca : p.isFca, p.ticker) : null,
      basis: 'not_activated',
  ...
  if (!candidates.length) {
    return {
      active: true,
      trailing_stop: prior !== null ? prior : null,
      activation_price: tick(activationPrice, 'nearest', p.board, p.is_fca != null ? p.is_fca : p.isFca, p.ticker),
  ...
  const trailing = tick(raw, 'floor', p.board, p.is_fca != null ? p.is_fca : p.isFca, p.ticker) ;
  return {
    active: true,
    trailing_stop: trailing,
    // F4-B3-01: this path returned the activation price through a board-blind
    // tick(), so on an FCA/Akselerasi ticker (flat Rp1 fraction) an activation
    // price like 261 was snapped with the REGULAR table and displayed as 262 —
    // a level that does not exist on that board. Every other return of this
    // function already threads the board context.
    activation_price: tick(activationPrice, 'nearest', p.board, p.is_fca != null ? p.is_fca : p.isFca, p.ticker),
    basis,
...
//
// It stays independent of max_structural_distance_pct (12/18/22) but honours the
// profile's own emergency_max_structural_distance_pct (30/35/40). That key was
// declared in all three profiles and read nowhere, which left the emergency stop
// with no distance bound at all — a major_support 85% below entry was accepted
// verbatim. Callers already fall back to the normal support candidate when no
// emergency level qualifies. See test/trade-plan-v2-declared-guards.test.js.
function isUsableEmergencyStructuralLevel(level, entryLow, maxDistancePct) {
  if (!(level && level.valid && level.confirmed && level.defended &&
    !level.stale && !level.failed && level.value < entryLow)) return false;
  if (maxDistancePct == null) return true;
  return structuralDistancePct(entryLow, level.value) <= maxDistancePct;
}
...
// NOTE: `tolerance` is accepted and deliberately not applied as a filter here.
// Callers compute max(tickSize, 0.25*ATR) and pass it, which reads like a dead
// parameter, but filtering on it is wrong: a resistance sitting inside that
// tolerance is exactly the condition the ENTRY_TOO_CLOSE_TO_RESISTANCE warning
// exists to report. Discarding the level instead suppresses that warning and
// the plan goes out silently — see test 26 in test/trade-plan-v2.test.js. The
// parameter should be removed from the signature, not honoured.
function isUsableOverheadLevel(level, entryHigh, tolerance) {
  return !!(level && level.valid && level.confirmed && level.defended &&
    !level.stale && !level.failed && entryHigh !== null && level.value > entryHigh);
}
...
function buildTradePlanV2(candidate, options) {
  ...
  const isFca = (candidate.is_fca != null ? candidate.is_fca : (candidate.isFca != null ? candidate.isFca : (options.is_fca != null ? options.is_fca : options.isFca)));
  ...
  entryLow = entryLow !== null ? tick(entryLow, 'nearest', board, isFca, ticker) : null;
  entryHigh = entryHigh !== null ? tick(entryHigh, 'nearest', board, isFca, ticker) : null;
  const entryRef = entryHigh !== null ? entryHigh : (entryLow !== null ? entryLow : (currentPrice !== null ? tick(currentPrice, 'nearest', board, isFca, ticker) : null));
  const entryFloor = entryLow !== null ? entryLow : entryRef;
  const entryCeil = entryHigh !== null ? entryHigh : entryRef;
  ...
  const entryTriggerLevel = entryTrigger !== null ? tick(entryTrigger, 'ceil', board, isFca, ticker) : entryHigh;
  ...
  if (screenerType === 'DAY_TRADE') {
    collectSupport(candidate.today_low, SUPPORT_ANCHOR_TYPE.LOCAL_SUPPORT, 'today_low');
    collectSupport(candidate.low_price, SUPPORT_ANCHOR_TYPE.LOCAL_SUPPORT, 'low_price');
  }
  ...
  const support = rawSupport !== null ? tick(rawSupport, 'floor', board, isFca, ticker) : null;
  ...
  const validNearbySupports = supportLevels.filter((level) =>
    [SUPPORT_ANCHOR_TYPE.LOCAL_SUPPORT, SUPPORT_ANCHOR_TYPE.CONFIRMED_SWING_LOW, SUPPORT_ANCHOR_TYPE.DEMAND_GAP_INVALIDATION]
      .includes(level.type) && isUsableStructuralLevel(level, entryFloor, profile.max_structural_distance_pct));
  ...
  let resistanceCandidates = overheadLevels.filter((level) =>
    isUsableOverheadLevel(level, entryCeil, resistanceTolerance));
  let nearestResistanceCandidate = chooseNearestOverhead(resistanceCandidates);
  let resistance = nearestResistanceCandidate ? tick(nearestResistanceCandidate.value, 'ceil', board, isFca, ticker) : null;
  ...
  let majorResistance = majorResistanceCandidate ? tick(majorResistanceCandidate.value, 'ceil', board, isFca, ticker) : null;
  ...
  // independent of every distance cap, including the emergency one: on a
  // REJECTED plan this is the only structure the operator gets to see, and
  // hiding a distant major support would remove information rather than remove
  // risk. Tests 35 and 36 in test/trade-plan-v2.test.js pin that behaviour.
  const emergencyStructuralCandidates = supportLevels.filter((level) =>
    ...
    isUsableEmergencyStructuralLevel(level, entryFloor));
  ...
  const emergencyDiagnosticStop = emergencyDiagnosticCandidate
    ? tick(emergencyDiagnosticCandidate.value - emergencyBuffer, 'floor', board, isFca, ticker) : null;
  ...
  if (!normalSupportCandidate) {
    ...
    return base({
      ...
      // F4-B3-02: board-blind tick() snapped the diagnostic anchor of a
      // NO_STRUCTURAL_LEVEL plan with the regular table (803 -> 800 on an FCA
      // ticker), so the only structure shown to the operator was a price that
      // cannot be traded on that board.
      emergency_anchor_price: emergencyDiagnosticCandidate ? tick(emergencyDiagnosticCandidate.value, 'floor', board, isFca, ticker) : null
    });
  }
  ...
  resistance = nearestResistanceCandidate ? tick(nearestResistanceCandidate.value, 'ceil', board, isFca, ticker) : null;
  ...
  nearestLocalResistance = chooseNearestOverhead(resistanceCandidates.filter((level) =>
    level.type === RESISTANCE_ANCHOR_TYPE.LOCAL_RESISTANCE));
  majorResistance = majorResistanceCandidate ? tick(majorResistanceCandidate.value, 'ceil', board, isFca, ticker) : null;
  ...
  const emergencyCandidates = supportLevels.filter((level) =>
    [SUPPORT_ANCHOR_TYPE.MAJOR_SUPPORT, SUPPORT_ANCHOR_TYPE.EMERGENCY_SUPPORT, SUPPORT_ANCHOR_TYPE.DEMAND_GAP_INVALIDATION].includes(level.type) &&
    level.value < stopAnchor && isUsableEmergencyStructuralLevel(level, entryLow, profile.emergency_max_structural_distance_pct));
  ...
  const stopLoss = tick(stopAnchor - volatilityBuffer, 'floor', board, isFca, ticker);
  const structuralInvalidation = tick(structuralLevel, 'floor', board, isFca, ticker);
  const emergencyStop = tick(emergencyAnchorPrice - emergencyBuffer, 'floor', board, isFca, ticker);
  ...
  if (stopLoss === null || (entryFloor !== null && stopLoss >= entryFloor)) {
    return base({
      status: STATUS.REJECTED,
      ...
      // F4-B3-03: both anchors of the STOP_NOT_BELOW_ENTRY rejection were
      // snapped with the regular table, so an FCA ticker reported 995 where its
      // own board's level is 998.
      stop_anchor_type: stopAnchorType, stop_anchor_price: tick(stopAnchor, 'floor', board, isFca, ticker),
      structural_invalidation: structuralInvalidation, emergency_stop: emergencyStop,
      emergency_anchor_type: emergencyAnchorType, emergency_anchor_price: tick(emergencyAnchorPrice, 'floor', board, isFca, ticker),
  ...
  if (supplyGapBlocksEntry) {
    ...
    const baseTp1 = tick(entryRef + tp1TargetR * riskAmount, 'floor', board, isFca, ticker);
    const obstacleBuffer = Math.max(tickSize, atr !== null ? TP1_ATR_BUFFER * atr : 0);
    const cappedTp1 = tick(resistance - obstacleBuffer, 'floor', board, isFca, ticker);
    tp1 = baseTp1;
  ...
  if (breakoutOk) {
    if (nextResistanceCandidate && nextResistanceCandidate.value > (tp1 !== null ? tp1 : entryRef)) {
      tp2 = tick(nextResistanceCandidate.value, 'floor', board, isFca, ticker);
      ...
      tp2 = tick(entryRef + profile.tp2_r_multiple * riskAmount, 'floor', board, isFca, ticker);
  ...
  const trailingActivation = tp1 !== null ? tick(Math.min(oneR, tp1), 'nearest', board, isFca, ticker) : tick(oneR, 'nearest', board, isFca, ticker);
  ...
  const trailingReference = num(options.trailing_reference) != null
    ? tick(num(options.trailing_reference), 'floor', board, isFca, ticker)
    : stopLoss;
  ...
  return base({
    ...
    stop_anchor_price: tick(stopAnchor, 'floor', board, isFca, ticker),
    structural_invalidation: structuralInvalidation,
    emergency_stop: emergencyStop,
    emergency_anchor_type: emergencyAnchorType,
    emergency_anchor_price: tick(emergencyAnchorPrice, 'floor', board, isFca, ticker),
    ...
    nearest_local_resistance: nearestLocalResistance ? tick(nearestLocalResistance.value, 'ceil', board, isFca, ticker) : null,
    tp2,
