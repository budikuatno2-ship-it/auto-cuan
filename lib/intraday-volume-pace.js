...
const MIN_EFFECTIVE_PROGRESS = 0.15;
...
function calculateVolumePace(input) {
  ...
  // Fase 3 / BUG-F3-17: `volume_today` yang disuplai eksplisit dan BERBEDA dari
  // volume candle historis terakhir berarti pemanggil membawa volume sesi berjalan
  // sendiri, sehingga pace harus tetap dihitung walau array candles hanya berisi
  // candle yang sudah closed. Bila nilainya identik dengan candle terakhir, nilai
  // itu semata salinan riwayat (bukan volume sesi ini) — proyeksi tetap dimatikan
  // seperti perilaku lama agar tidak memproyeksikan volume basi sebagai volume hari ini.
  const lastCandle = hasCandleContext ? data.candles[data.candles.length - 1] : null;
  const lastCandleVolume = finite(lastCandle && lastCandle.volume);
  const volumeTodayFromHistory = volumeToday != null && lastCandleVolume != null && volumeToday === lastCandleVolume;
  const explicitLiveVolume = volumeToday != null && volumeToday >= 0 && !volumeTodayFromHistory;
  const currentSessionPresent = explicitCurrentSession || Boolean(sessionCandle) || !hasCandleContext || explicitLiveVolume;
