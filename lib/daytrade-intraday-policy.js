...
function rowIntradayDiagnostics(row) {
  const d = row && (row.intraday_data_diagnostics || row.intraday_diagnostics || row.data_quality_diagnostics) || {};
  return {
    session_date: d.session_date || row && row.session_date || null,
    updated_at: d.updated_at || row && row.updated_at || null,
    candle_count: numberOrZero(d.candle_count ?? (row && row.candle_count)),
    valid_ohlc_candles: numberOrZero(d.valid_ohlc_candles ?? (row && row.valid_ohlc_candles)),
    zero_ohlc_candles: numberOrZero(d.zero_ohlc_candles ?? (row && row.zero_ohlc_candles)),
    positive_volume_candles: numberOrZero(d.positive_volume_candles ?? (row && row.positive_volume_candles)),
    total_volume: numberOrZero(d.total_volume ?? (row && (row.total_volume ?? row.intraday_volume_sum))),
    source: d.source || row && row.source || null,
    interval: d.interval || row && row.interval || null
  };
}


function bundleCandidateTickers(bundle) {
  const rows = getBundleRows(bundle);
  if (rows.length) return normalizeTickerList(rows);
  const count = numberOrZero(bundle && bundle.intraday_candidates_count);
  if (!count) return [];
  return collectTickers(bundle, null);
}
...
function decidePolicy(ticker, bundleData, aggregateData) {
  ...
  const hasIncomplete = dataQuality === 'INCOMPLETE_INTRADAY' || dataQuality === 'STALE_CACHE' || bundleLists.incomplete_intraday.includes(normalizedTicker) || recurring.incomplete_intraday.includes(normalizedTicker);

  if (hasNoData || (hasUnknown && !hasIncomplete)) {
  ...
  if (hasIncomplete) {
    ...
    return {
      ...
      priority_label: priorityLabel || null,
      confirmation_label: confirmationLabel || null,
      intraday_data_diagnostics: rowIntradayDiagnostics(row),
...
function buildPolicyReport(bundle, aggregate, opts) {
  ...
  const bundleCandidateTickersList = bundleCandidateTickers(bundle);
  const bundleCandidateCount = Math.max(bundleCandidateTickersList.length, numberOrZero(bundle && bundle.intraday_candidates_count));
  const missingPolicyTickers = bundleCandidateTickersList.filter((ticker) => !tickers.includes(ticker));
  const coverageStatus = bundleCandidateCount > tickers.length || missingPolicyTickers.length ? 'INCOMPLETE' : 'OK';
  const recurring = recurringListsFromAggregate(aggregate);
  const bundleLists = bundleTickerLists(bundle);
  const policies = [];
  ...
  if (coverageStatus === 'INCOMPLETE') {
    policyStatus = 'BLOCK';
    blockReasons.push(`policy coverage incomplete: evaluated ${tickers.length}/${bundleCandidateCount} candidates`);
  } else if (hasBlockProduction || aggregateStatus === 'BLOCK' || hasRecurringNoData || hasRecurringUnknown) {
    policyStatus = 'BLOCK';
  ...
  return {
    ...
    bundle_candidate_count: bundleCandidateCount,
    policy_evaluated_count: tickers.length,
    coverage_status: coverageStatus,
    missing_policy_ticker_count: missingPolicyTickers.length || Math.max(0, bundleCandidateCount - tickers.length),
    missing_policy_tickers: missingPolicyTickers,
    ...
    incomplete_intraday_diagnostics: policies.filter((p) => p.fallback_action === 'DAILY_SCORE_ONLY' && (p.data_quality === 'INCOMPLETE_INTRADAY' || p.data_quality === 'STALE_CACHE')).map((p) => Object.assign({ ticker: p.ticker }, p.intraday_data_diagnostics || {})),
...
function markdownReport(report) {
  const lines = [
    ...
    `- bundle_candidate_count: ${report.bundle_candidate_count}`,
    `- policy_evaluated_count: ${report.policy_evaluated_count}`,
    `- coverage_status: ${report.coverage_status}`,
    `- missing_policy_ticker_count: ${report.missing_policy_ticker_count}`,
  ...
  lines.push('## Incomplete Intraday Diagnostics', '');
  const diagnostics = report.incomplete_intraday_diagnostics || [];
  if (diagnostics.length) {
    lines.push('| Ticker | Session Date | Updated At | Candles | Valid OHLC | Zero OHLC | Positive Volume | Total Volume | Source | Interval |');
    lines.push('|---|---|---|---:|---:|---:|---:|---:|---|---|');
    for (const d of diagnostics) lines.push(`| ${d.ticker} | ${d.session_date || 'n/a'} | ${d.updated_at || 'n/a'} | ${d.candle_count} | ${d.valid_ohlc_candles} | ${d.zero_ohlc_candles} | ${d.positive_volume_candles} | ${d.total_volume} | ${d.source || 'n/a'} | ${d.interval || 'n/a'} |`);
