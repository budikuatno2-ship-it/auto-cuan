...
// The 2026 exchange holiday list lives in exactly one place
// (lib/idx-holidays-2026-seed-data.js). Backfill/scheduled tools must consume
// this helper instead of keeping their own inline copies, which had drifted
// apart and caused API pulls on real holidays / skipped real trading days.
var seedData = require('./idx-holidays-2026-seed-data');

function getSeedHolidaySet() {
  return new Set(seedData.IDX_HOLIDAYS_2026.map(function(h) { return h.trade_date; }));
}
