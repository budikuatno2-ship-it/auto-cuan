...
function normalizeBrokerSummary(raw, date, ticker) {
  ...
  if (Array.isArray(raw.gross_buyers) && raw.gross_buyers.length > 0 &&
    ...
    const sanitizeArr = (list, isBuyer) => list.map(b => enrichBrokerItem(b, isBuyer, refPrice));
  ...
  if (Array.isArray(raw.brokers)) {
    for (let i = 0; i < raw.brokers.length; i++) {
      ...
      if (b && b.broker_code) {
        ...
        brokersMap.set(b.broker_code, enrichBrokerItem(b, net >= 0, refPrice));
      }
    }
  }
  ...
  if (Array.isArray(raw.broker_levels) && raw.broker_levels.length > 0) {
    for (let i = 0; i < raw.broker_levels.length; i++) {
      ...
      if (lvl.buy && lvl.buy.broker_code) {
        ...
        const avg = normalizeVwapPrice(lvl.buy.bavg || 0, refPrice);
        if (brokersMap.has(code)) {
          const item = brokersMap.get(code);
          const curVwap = item.bvol > 0 ? (item.bval / item.bvol) : 0;
          const curDev = refPrice > 0 && curVwap > 0 ? Math.abs(curVwap - refPrice) / refPrice : 1;
          // If existing bval & bvol already exist and are consistent with refPrice (<= 25%),
          // DO NOT let broker_levels corrupt bvol or bval!
          if (curDev <= 0.25 && item.bval > 0 && item.bvol > 0) {
            if (bfrq > (item.bfrq || 0)) item.bfrq = bfrq;
          } else {
            // Update as atomic pair (never bvol without bval)
            if (bval > 0 && bvol > 0) {
              const newVwap = bval / bvol;
              const newDev = refPrice > 0 ? Math.abs(newVwap - refPrice) / refPrice : 0;
              if (curDev > 0.25 || newDev <= curDev) {
                item.bval = bval;
                item.bvol = bvol;
              }
            }
      ...
      if (lvl.sell && lvl.sell.broker_code) {
        ...
        const avg = normalizeVwapPrice(lvl.sell.savg || 0, refPrice);
        if (brokersMap.has(code)) {
          const item = brokersMap.get(code);
          const curSellVwap = item.svol > 0 ? (item.sval / item.svol) : 0;
          const curSellDev = refPrice > 0 && curSellVwap > 0 ? Math.abs(curSellVwap - refPrice) / refPrice : 1;
          if (curSellDev <= 0.25 && item.sval > 0 && item.svol > 0) {
            if (sfrq > (item.sfrq || 0)) item.sfrq = sfrq;
          } else {
            if (sval > 0 && svol > 0) {
              const newSellVwap = sval / svol;
              const newSellDev = refPrice > 0 ? Math.abs(newSellVwap - refPrice) / refPrice : 0;
              if (curSellDev > 0.25 || newSellDev <= curSellDev) {
                item.sval = sval;
                item.svol = svol;
              }
            }
  ...
  const totalBuyVal = raw.total_buy_val || (allBrokers ? allBrokers.reduce((sum, b) => sum + (b.bval || 0), 0) : grossBuyers.reduce((sum, b) => sum + (b.bval || 0), 0));
  const totalSellVal = raw.total_sell_val || (allBrokers ? allBrokers.reduce((sum, b) => sum + (b.sval || 0), 0) : grossSellers.reduce((sum, s) => sum + (s.sval || 0), 0));
  const totalBuyVol = raw.total_volume || raw.volume || (allBrokers ? allBrokers.reduce((sum, b) => sum + (b.bvol || 0), 0) : grossBuyers.reduce((sum, b) => sum + (b.bvol || 0), 0));
  const totalTurnover = raw.total_turnover || raw.turnover || (totalBuyVal + totalSellVal) || totalBuyVal;

  const flowRatio = totalTurnover > 0 ? (diff / totalTurnover) : 0;
...
function aggregateBrokerSummaries(ticker, dates, requestedDays) {
  ...
  const validDates = filterCalendarWindowDates(dates, reqDays);
...
async function getBandarmologiData(tickerOrQuery, options = {}) {
  ...
  // 3. Fallback to cache offline structure when upstream data is unavailable
  const demo = generateDemoData(ticker, targetDate);
  demo.is_offline = true;
  demo.status = 'CACHE_OFFLINE';
  demo.demo_reason = demoReason || 'cache_offline';
  demo.demo_detail = demoDetail || 'Disk cache broker summary kosong dan koneksi live tidak tersedia';
