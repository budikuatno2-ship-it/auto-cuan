...
function applyMultiDayScaling(summary, accumulation, numDays, targetDate) {
  if (!summary) return;
  if (numDays > 1) {
    const multMap = { 1: 1, 5: 5, 7: 5, 14: 10, 30: 22, 60: 44 };
    const mult = multMap[numDays] || Math.max(1, Math.round(numDays * 0.72));
...
async function getBandarmologiData(tickerOrQuery, options = {}) {
  ...
  // fetch it directly from VPS via SSH child_process before attempting any fallback
