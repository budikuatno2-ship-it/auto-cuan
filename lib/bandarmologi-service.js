...
function synthesizeAccumulationFromSummary(normSummary, ticker) {
  ...
  const buyers = firstNonEmptyArray(normSummary.net_buyers, normSummary.top_buyers, normSummary.gross_buyers);
  const sellers = firstNonEmptyArray(normSummary.net_sellers, normSummary.top_sellers, normSummary.gross_sellers);
  ...
  const series = netFlow !== 0 ? [{ date: dateLabel, net_val: netFlow, status: netFlow >= 0 ? 'ACC' : 'DIST' }] : [];
...
async function getBandarmologiData(tickerOrQuery, options = {}) {
  ...
  if (normSummary) {
    // When diskAcc is available, use it directly; otherwise synthesize from broker_summary
    // so that the Akumulasi Broker bubble never renders "Semua (0) | Buyers (0) | Sellers (0)"
    // even when no broker-accumulation cache file exists for this ticker.
    const normAcc = diskAcc
      ? normalizeBrokerAccumulation(diskAcc, ticker)
      : synthesizeAccumulationFromSummary(normSummary, ticker);
