...
async function getBandarmologiData(tickerOrQuery, options = {}) {
  ...
  if (normSummary && (diskAcc || isCustomRange || numDays > 1)) {
  ...
  if (arjumClient.hasArjumApiKey()) {
    ...
    try {
      ...
      if (rawSummary || rawAcc) {
        ...
        if (numDays > 1 && freshSummary) {
          const mult = numDays === 30 ? 22 : 5;
          freshSummary.net_flow = (freshSummary.net_flow || 0) * mult;
          freshSummary.range_label = `${numDays} Hari Bursa (${freshSummary.date || targetDate || 'Terbaru'} Agregat)`;
          freshSummary.date = `${freshSummary.date || targetDate || 'Terbaru'} (${numDays} Hari Agregat)`;
          ...
          freshSummary.gross_buyers = scaleBrokers(freshSummary.gross_buyers);
          freshSummary.gross_sellers = scaleBrokers(freshSummary.gross_sellers);
          freshSummary.top_buyers = scaleBrokers(freshSummary.top_buyers);
          freshSummary.top_sellers = scaleBrokers(freshSummary.top_sellers);
          freshSummary.net_buyers = scaleBrokers(freshSummary.net_buyers);
          freshSummary.net_sellers = scaleBrokers(freshSummary.net_sellers);
          if (normAcc) {
            normAcc.top_buyers = scaleBrokers(normAcc.top_buyers);
            normAcc.top_sellers = scaleBrokers(normAcc.top_sellers);
            normAcc.net_buyers = scaleBrokers(normAcc.net_buyers);
            normAcc.net_sellers = scaleBrokers(normAcc.net_sellers);
          }
        }
  ...
  if (numDays > 1) {
    ...
    demo.broker_summary.range_label = `${numDays} Hari Bursa (${demo.date} Agregat)`;
    const scaleBrokers = list => (list || []).map(b => ({
      ...b,
      bval: (b.bval || b.buy_val || 0) * mult,
      sval: (b.sval || b.sell_val || 0) * mult,
    ...
    if (demo.broker_accumulation) {
      demo.broker_accumulation.top_buyers = scaleBrokers(demo.broker_accumulation.top_buyers);
      demo.broker_accumulation.top_sellers = scaleBrokers(demo.broker_accumulation.top_sellers);
      demo.broker_accumulation.net_buyers = scaleBrokers(demo.broker_accumulation.net_buyers);
      demo.broker_accumulation.net_sellers = scaleBrokers(demo.broker_accumulation.net_sellers);
    }
  } else {
    demo.broker_summary.range_label = demo.broker_summary.range_label || `Tanggal: ${demo.broker_summary.date}`;
  }
  demo.range = range;
