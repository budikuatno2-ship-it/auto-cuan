...
// latest broker-summary day (2026-09-11) — a close-enough price anchor, not a
// quote. They only need to be within ~35% of the true price for the guard.
...
function applyMultiDayScaling(summary, accumulation, numDays, targetDate) {
  ...
  if (numDays > 1) {
    ...
    const mult = numDays === 60 ? 44 : (numDays === 30 ? 22 : (numDays === 14 ? 10 : (numDays === 7 ? 5 : (numDays === 5 ? 5 : Math.max(1, Math.round(numDays * (5 / 7)))))));
    summary.net_flow = (summary.net_flow || 0) * mult;
    if (summary.total_turnover) summary.total_turnover = Math.round(summary.total_turnover * mult);
    if (summary.total_buy_val) summary.total_buy_val = Math.round(summary.total_buy_val * mult);
    if (summary.total_sell_val) summary.total_sell_val = Math.round(summary.total_sell_val * mult);
    if (summary.total_volume) summary.total_volume = Math.round(summary.total_volume * mult);
    if (summary.total_buy_vol) summary.total_buy_vol = Math.round(summary.total_buy_vol * mult);
    if (summary.total_sell_vol) summary.total_sell_vol = Math.round(summary.total_sell_vol * mult);
  ...
  if (!forceRefresh) {
    if (isCustomRange && availableDates.length > 0) {
      ...
      if (targetDate && targetDate !== 'latest') {
        ...
        if (!diskSummary && availableDates.length > 0) {
          const fallbackDate = availableDates.includes(targetDate) ? targetDate : availableDates[0];
          diskSummary = readDiskCache('broker-summary', ticker, fallbackDate);
        }
        if (diskSummary) {
