...
function normalizeBrokerSummary(raw, date) {
  ...
  // A disk-cache payload can have all four canonical keys while its seller
  // arrays are empty and Arjum keeps the actual rows in `sellers` or
  // `top_sellers`. Do not return such an incomplete shape before the alias
  // fallback below gets a chance to recover the real counter-sellers.
