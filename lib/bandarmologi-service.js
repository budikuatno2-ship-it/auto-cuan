...
function enrichBrokerItem(item, isBuyer) {
  if (!item) return null;
  const bval = Number(item.bval != null ? item.bval : (item.buy_val != null ? item.buy_val : (isBuyer ? (item.val != null ? item.val : (item.value != null ? item.value : (item.net_val != null && item.net_val > 0 ? item.net_val : 0))) : 0))) || 0;
  const sval = Number(item.sval != null ? item.sval : (item.sell_val != null ? item.sell_val : (!isBuyer ? (item.val != null ? item.val : (item.value != null ? item.value : (item.net_val != null ? Math.abs(item.net_val) : (item.nval != null ? Math.abs(item.nval) : 0)))) : 0))) || 0;
  const bvol = Number(item.bvol != null ? item.bvol : (item.buy_vol != null ? item.buy_vol : (isBuyer ? (item.vol != null ? item.vol : (item.volume != null ? item.volume : 0)) : 0))) || 0;
  const svol = Number(item.svol != null ? item.svol : (item.sell_vol != null ? item.sell_vol : (!isBuyer ? (item.vol != null ? item.vol : (item.volume != null ? item.volume : 0)) : 0))) || 0;
  const bfrq = Number(item.bfrq) || 0;
  const sfrq = Number(item.sfrq) || 0;

  let nval;
  if (isBuyer) {
    nval = item.nval != null ? Number(item.nval) : (item.net_val != null ? Number(item.net_val) : (bval - sval));
    if (nval <= 0 && bval > 0) nval = bval;
  } else {
    // Seller MUST have negative net value
    nval = item.nval != null ? -Math.abs(Number(item.nval)) : (item.net_val != null ? -Math.abs(Number(item.net_val)) : (bval - sval));
    if (nval >= 0 && sval > 0) nval = -sval;
  }

  let nvol;
  if (isBuyer) {
    nvol = item.nvol != null ? Number(item.nvol) : (item.net_vol != null ? Number(item.net_vol) : (bvol - svol));
    if (nvol <= 0 && bvol > 0) nvol = bvol;
  } else {
    nvol = item.nvol != null ? -Math.abs(Number(item.nvol)) : (item.net_vol != null ? -Math.abs(Number(item.net_vol)) : (bvol - svol));
    if (nvol >= 0 && svol > 0) nvol = -svol;
  }
...
function normalizeBrokerSummary(raw, date) {
  ...
  // Do not return early if seller lists are empty arrays ([]), as real seller data
  // may reside in alternative keys (sellers, top_sellers, etc.).
  ...
  if (brokersMap.size > 0) {
    ...
    allBrokers = all;
    grossBuyers = all.slice().sort((a, b) => (b.bval || 0) - (a.bval || 0)).slice(0, 20);
    grossSellers = all.slice().sort((a, b) => (b.sval || 0) - (a.sval || 0)).slice(0, 20);
    netBuyers = all.filter(x => (x.nval || 0) > 0).sort((a, b) => b.nval - a.nval).slice(0, 20);
    netSellers = all.filter(x => (x.nval || 0) < 0).sort((a, b) => Math.abs(b.nval) - Math.abs(a.nval)).slice(0, 20);
    ...
    grossBuyers = firstNonEmptyArray(raw.gross_buyers, raw.top_buyers, raw.buyers, raw.net_buyers).map(b => enrichBrokerItem(b, true));
    grossSellers = firstNonEmptyArray(raw.gross_sellers, raw.top_sellers, raw.sellers, raw.net_sellers).map(s => enrichBrokerItem(s, false));

    const rawNetBuyers = firstNonEmptyArray(raw.net_buyers);
    if (rawNetBuyers.length > 0) {
      netBuyers = rawNetBuyers.map(b => enrichBrokerItem(b, true));
    } else {
    ...
    if (rawNetSellers.length > 0) {
      netSellers = rawNetSellers.map(s => enrichBrokerItem(s, false));
    } else {
      netSellers = grossSellers.filter(x => (x.nval || 0) < 0);
      if (netSellers.length === 0) netSellers = grossSellers.slice();
    }

    // Ensure net_sellers strictly retain negative net values and never mirror buyer data
    netSellers = netSellers.map(s => {
      const netVal = s.nval != null ? s.nval : (s.net_val != null ? s.net_val : -(s.sval || s.sell_val || 0));
      const negVal = -Math.abs(Number(netVal) || 0);
      const netVol = s.nvol != null ? s.nvol : (s.net_vol != null ? s.net_vol : -(s.svol || s.sell_vol || 0));
      const negVol = -Math.abs(Number(netVol) || 0);
...
function aggregateBrokerSummaries(ticker, dates) {
  ...
  return {
    ...
    top_buyers: grossBuyers.slice(0, 20),
    top_sellers: grossSellers.slice(0, 20),
...
async function getBandarmologiData(tickerOrQuery, options = {}) {
  ...
  if (normSummary && (diskAcc || isCustomRange)) {
