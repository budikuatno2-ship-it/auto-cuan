...
function normalizeBrokerSummary(raw, date) {
  ...
  if (brokersMap.size > 0) {
    ...
    grossBuyers = firstNonEmptyArray(raw.gross_buyers, raw.top_buyers).map(b => enrichBrokerItem(b, true));
    grossSellers = firstNonEmptyArray(raw.gross_sellers, raw.top_sellers).map(s => enrichBrokerItem(s, false));
