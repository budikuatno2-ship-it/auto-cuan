...
function normalizeBrokerAccumulation(raw, ticker) {
  ...
  if (Array.isArray(raw.series) && raw.series.length > 0 && raw.series[0].date && !raw.series[0].points) {
    const rawBuyers = Array.isArray(raw.net_buyers) && raw.net_buyers.length > 0 ? raw.net_buyers : (Array.isArray(raw.top_buyers) ? raw.top_buyers : []);
    const rawSellers = Array.isArray(raw.net_sellers) && raw.net_sellers.length > 0 ? raw.net_sellers : (Array.isArray(raw.top_sellers) ? raw.top_sellers : []);
    if (rawSellers.length === 0 && rawBuyers.length > 0) {
      const bList = [];
      const sList = [];
  ...
  return {
    ...
    top_buyers: (Array.isArray(raw.top_buyers) && raw.top_buyers.length > 0) ? raw.top_buyers : accBuyers,
    top_sellers: (Array.isArray(raw.top_sellers) && raw.top_sellers.length > 0) ? raw.top_sellers : accSellers,
    net_buyers: accBuyers,
    net_sellers: accSellers
  };
}
