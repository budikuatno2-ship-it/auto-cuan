...
function normalizeBrokerSummary(raw, date) {
  ...
  // Keep an already-normalized payload untouched only when both sides and
  // both modes are present. Arjum can return a partial payload containing
  // gross_buyers/net_buyers plus `sellers`; returning that shape early used
  // to drop the seller alias before it reached the UI.
  if (Array.isArray(raw.gross_buyers) && Array.isArray(raw.gross_sellers) &&
  ...
  if (brokersMap.size > 0) {
    ...
    netSellers = grossSellers.slice();
             ...
             Array.isArray(raw.net_buyers) || Array.isArray(raw.net_sellers)) {
    // Arjum cache/live payloads have used both top_*/gross_* and the newer
    // buyers/sellers aliases. Prefer an available canonical list, but never
    // let a present empty array shadow a populated fallback.
    ...
    netBuyers = firstNonEmptyArray(raw.net_buyers, grossBuyers).filter(x => (x.nval || 0) > 0);
    if (netBuyers.length === 0) netBuyers = grossBuyers.slice();
    netSellers = firstNonEmptyArray(raw.net_sellers, grossSellers).filter(x => (x.nval || 0) < 0);
