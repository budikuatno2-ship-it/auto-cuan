...
function normalizeVwapPrice(raw, refPrice) {
  ...
  // Nilai bval / bvol menghasilkan Rp/Lot (100x harga lembar).
  // Di BEI (1 lot = 100 lembar), nilai per lot untuk saham Rp 1 s/d Rp 50.000+ adalah >= 100.
  // Nilai raw >= 100 harus dibagi 100 agar saham penny (< Rp 500) modal bandarnya tidak membengkak 100x lipat.
  if (raw >= 100) {
...
function enrichBrokerItem(item, isBuyer, refPrice) {
  ...
  if (targetVal > 0 && targetVol > 0) {
    ...
    avgPrice = normalizeVwapPrice(rawVwap, refPrice);
  } else if (item.avg_price != null && item.avg_price > 0) {
    avgPrice = normalizeVwapPrice(item.avg_price, refPrice);
  } else {
    ...
    if (fallbackVal > 0 && fallbackVol > 0) {
      ...
      avgPrice = normalizeVwapPrice(rawFallback, refPrice);
    }
  }
