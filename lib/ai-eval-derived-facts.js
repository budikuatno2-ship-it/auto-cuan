...
function planFacts(plan, simulation) {
  ...
  if (scenario) {
    ...
    // ponytail: calculate total position treating empty/null existing lots as 0
    const totalLots = addLots != null ? (lots || 0) + addLots : null;
    const totalShares = addShares != null ? (shares || 0) + addShares : null;
    const totalCapital = additionalCapital != null ? (capital || 0) + additionalCapital : null;
    ...
    const additionalRisk = assumedEntry != null && stop != null && assumedEntry > stop && addShares != null
      ...
      : null;
    const totalRisk = additionalRisk != null ? (maxLoss || 0) + additionalRisk : null;
...
function stockFacts(context) {
  ...
  // ponytail: support standard screener keys entry1, entry2, and sl
  const entry = recursiveFind(source, new Set(['entry', 'entryprice', 'entry_price', 'entrylow', 'entry_low', 'entrypriceidr', 'entry1', 'entry_1']));
  const entryHigh = recursiveFind(source, new Set(['entryhigh', 'entry_high', 'entry2', 'entry_2']));
  const stop = recursiveFind(source, new Set(['stop', 'stoploss', 'stop_loss', 'stoplossidr', 'sl']));
