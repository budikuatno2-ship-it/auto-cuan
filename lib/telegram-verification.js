...
const adminDeviceApproval = require('./admin-device-approval');
...
// Public BYOK registration form (see public/register.html + api/bot-register.js).
// A member arriving from the group via t.me/<verify bot>?start=verify_<id> is sent
// straight here, so they never see the legacy "copy the code from the website"
// instructions. `user_id` is the Telegram id the form must be bound to.
//
// FASE 2: the origin is no longer a literal. When the Vercel deployment behind
// autocuan.web.id is paused (HTTP 402 DEPLOYMENT_DISABLED) the form is served by
// the VPS Express app instead, and lib/public-web-base.js resolves the live
// public origin (env override -> tunnel URL file -> canonical domain). Keeping
// the path segment here preserves the canonical deep-link contract.
const REGISTER_FORM_PATH = '/register.html';
const publicWebBase = require('./public-web-base');
const REGISTER_FORM_LABEL = '\uD83D\uDCDD Buka Formulir Pendaftaran';
const REGISTER_FORM_MESSAGE =
  'Halo! Silakan lengkapi formulir pendaftaran (Nama Lengkap & Gmail) untuk membuka akses bot grup:';

// Commands that belong to the SIGNAL (group/channel) bot only. Typing one of
// these in the verification bot is answered with a polite redirect instead of
// being parsed as a verification code. Keep this list in sync with
// MARKET_COMMANDS/ADMIN_COMMANDS in lib/telegram-interactive-bot.js.
const SIGNAL_ONLY_COMMANDS = new Set([
  '/analisa', '/a', '/broksum', '/bs', '/bandar', '/bd', '/insider', '/in',
  '/scan', '/tanya', '/foreign', '/ritel', '/opini', '/screener', '/status'
]);
const FOREIGN_COMMAND_MESSAGE =
  '\u26A0\uFE0F Bot ini khusus pendaftaran & akun. Untuk analisa saham silakan gunakan Signal Saham Bot di grup.';
...
// 30-day in-bot rating. Callback data is strictly `rate_<1..5>`; anything else
// is rejected. The rating flow is additive and completely separate from the
// verification/join flow above.
const CALLBACK_RATE_PREFIX = 'rate_';
const REVIEW_MIN_DAYS = 30;
...
// 30-day rating request message (private bot DM). No advertisement, no external
// link; a single inline keyboard of 1..5 star buttons is attached separately.
function buildReviewRequestMessage() {
  return [
    'Sudah 30 hari bersama Auto-Cuan.',
    'Bagaimana pengalaman kamu?'
  ].join('\n');
}

// Inline keyboard for the 30-day rating: five callback buttons rate_1..rate_5.
// One row keeps it compact on mobile.
function reviewRatingButtons() {
  const row = [];
  for (let s = 1; s <= 5; s++) {
    row.push({ text: '\u2B50 ' + s, callback_data: CALLBACK_RATE_PREFIX + s });
  }
  return { inline_keyboard: [row] };
}

// Thank-you text that REPLACES the rating message after a selection. The rating
// buttons are removed by editing with an empty inline keyboard. `score` is shown
// only for a freshly-recorded rating; a duplicate callback shows generic thanks
// so repeated presses are idempotent.
function buildReviewThanksMessage(score) {
  const s = Number(score);
  if (Number.isInteger(s) && s >= 1 && s <= 5) {
    return [
      'Terima kasih atas penilaian kamu.',
      '',
      'Penilaian: ' + '\u2B50'.repeat(s) + ' (' + s + '/5)'
    ].join('\n');
  }
  return [
    'Terima kasih atas penilaian kamu.',
    '',
    'Penilaian kamu sudah tercatat.'
  ].join('\n');
}

// Parse a rating callback payload into an integer score 1..5, or null when it is
// not a valid `rate_<1..5>` value (rate_0, rate_6, rate_x, etc. -> null).
function parseRatingScore(data) {
  if (typeof data !== 'string') return null;
  const m = /^rate_([1-5])$/.exec(data);
...
// Record a 30-day rating (identity resolved by telegram_user_id inside the RPC).
// Returns a coarse outcome only; never throws user-visible detail.
async function recordReviewRating(supabase, telegramUserId, score) {
  const row = firstRow(await rpc(supabase, 'record_review_rating', {
    p_telegram_user_id: telegramUserId,
    p_score: score
  }));
  return {
    outcome: row ? row.outcome : 'not_found',
    userId: row ? row.out_user_id : null
  };
}
...
// ===========================================================================
// Command + account helpers (ACCOUNT commands only — this bot never answers
// market/analysis commands; those belong to the signal bot in the group).
// ===========================================================================

// Lowercase a command token and drop the "@BotName" suffix Telegram appends in
// groups, so /akun@AutoCuanVerificationBot and /akun are the same command.
...
// WIB (UTC+7) calendar day as YYYY-MM-DD, matching the quota bookkeeping used by
// the interactive bot (daily_usage resets at 00:00 WIB).
function wibDayKey(nowMs) {
  const base = typeof nowMs === 'number' && Number.isFinite(nowMs) ? nowMs : Date.now();
  return new Date(base + 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
}

// Read the account row for a Telegram id. Returns null when the account does not
// exist (or the DB is not reachable) — never throws, never leaks an error.
async function readAccountRow(supabase, telegramId) {
  ...
  try {
    const res = await supabase
      .from('bot_users')
      .select('telegram_id, username, email, provider, status, daily_limit, daily_usage, last_usage_date, api_key_encrypted, provider_settings')
      .eq('telegram_id', idNum || idStr)
      .maybeSingle();
    if (res && !res.error && res.data) {
      const row = res.data;
      if (!row.gmail && row.email) row.gmail = row.email;
      return row;
    }
  } catch (e) { /* treated as "not registered" below */ }
  return null;
}

// Human-readable account status. Only coarse states are surfaced.
function describeAccountStatus(row) {
  if (!row) return 'Belum Terdaftar';
  const status = String(row.status || '').toLowerCase();
  if (status === 'active' || status === 'approved') return 'Aktif';
  if (status === 'pending') return 'Pending';
  if (status === 'banned' || status === 'rejected') return 'Tidak Aktif';
...
function maskEmailAddress(email) {
  ...
  return local.slice(0, 2) + new Array(Math.max(1, local.length - 2)).fill('*').join('') + domain;
}

// Build the KONDISI A reply: the registration form button, deep-linked with the
// Telegram id the form must bind to. The legacy "copy the code" text is never
// produced on this path.
//
// `token` is optional: when the caller can mint a one-time registration token
// (see lib/telegram-register-token.js) it is appended so the form can submit
// without a second round-trip. `user_id` is ALWAYS present so the form can bind
// the submission to the right Telegram account.
function registrationFormUrl(targetUserId, token) {
  const id = targetUserId != null && String(targetUserId).trim() ? String(targetUserId).trim() : '';
  const params = [];
  if (id) params.push('user_id=' + encodeURIComponent(id));
  if (token) params.push('token=' + encodeURIComponent(token));
  // Resolved per call (memoised inside the module) so a tunnel hostname change
  // is picked up without restarting the bot.
  const base = publicWebBase.getPublicWebBase();
  return base + REGISTER_FORM_PATH + (params.length ? '?' + params.join('&') : '');
}

function registrationFormButton(targetUserId, token) {
  return { inline_keyboard: [[{ text: REGISTER_FORM_LABEL, url: registrationFormUrl(targetUserId, token) }]] };
}

// Mint a one-time registration token for the target Telegram id. Best effort:
// a failure returns null and the form still works via its `user_id` path.
async function mintRegistrationToken(targetUserId, deps) {
  const store = deps && deps.registerTokenStore;
  if (!store || typeof store.issueToken !== 'function') return null;
  const id = String(targetUserId == null ? '' : targetUserId).trim();
  if (!/^\d+$/.test(id) && id !== 'budi') return null;
  try {
    const token = await store.issueToken(deps.supabase, id, { ttlMs: 2 * 60 * 60 * 1000 });
...
// Mint a one-time magic token for seamless auto-login on Web Screener
async function mintMagicScreenerUrl(targetUserId, username, deps) {
  const store = (deps && deps.magicTokenStore) || require('./telegram-magic-token');
  const base = publicWebBase.getPublicWebBase();
  const id = String(targetUserId == null ? '' : targetUserId).trim();
  const uname = String(username == null ? '' : username).trim().toLowerCase();
  if (!id) return base + '/';
  try {
    const token = await store.issueToken(deps && deps.supabase, id, uname);
    if (token && typeof store.buildMagicLoginUrl === 'function') {
      return store.buildMagicLoginUrl(base, token, id);
    }
  } catch (_) {}
  return base + '/';
}

async function resolveSmartAccountState(supabase, senderId, from) {
  const idStr = String(senderId == null ? '' : senderId).trim();
  const idNum = Number(idStr);
  const username = (from && from.username && String(from.username).trim().toLowerCase()) || '';
  const adminId = String(process.env.ADMIN_TELEGRAM_ID || process.env.TELEGRAM_VERIFY_ADMIN_CHAT_ID || '6396446903').trim();

  // 1. Admin Budi / Donaldtrumpssss (by ID 6396446903, username, or configured adminId)
  const isAdmin = idStr === '6396446903' || idNum === 6396446903 ||
    idStr.toLowerCase() === 'budi' || username === 'budi' || username === 'donaldtrumpssss' ||
    (adminId && (idStr === adminId || (idNum && idNum === Number(adminId))));

  const botRow = await readAccountRow(supabase, senderId);
  const email = (botRow && (botRow.email || botRow.gmail)) || null;
  const hasByok = Boolean(
    (botRow && botRow.api_key_encrypted) ||
    (botRow && botRow.provider_settings && (botRow.provider_settings.byok_active || botRow.provider_settings.api_key || botRow.provider_settings.ai_api_key))
  );
  ...
  if (isAdmin) {
    return {
      status: 'verified',
      condition: 5,
      username: (from && from.username) || (botRow && botRow.username) || 'budi',
      email: email || 'budi@autocuan.com',
      channelAccess: (botRow && botRow.channel_access) || 'Aktif',
      subscription: (botRow && botRow.subscription_status) || 'Lifetime Admin',
      hasByok: true,
      isAdmin: true
    };
  }

  // 2. Check bot_users table
  if (botRow) {
    const st = String(botRow.status || '').toLowerCase();
    if (st === 'active' || st === 'approved') {
      if (!email && st === 'approved' && !botRow.subscription_status && !botRow.channel_access) {
        return {
          status: 'need_email',
          condition: 2,
          username: botRow.username || idStr,
          email: null
        };
      }
      return {
        status: 'verified',
        condition: 5,
        username: botRow.username || botRow.full_name || idStr,
        email: email,
        channelAccess: botRow.channel_access || 'Aktif',
        subscription: botRow.subscription_status || 'Aktif (BYOK)',
        hasByok
      };
    }
    if (st === 'pending') {
      return { status: 'pending', condition: 3, username: botRow.username || idStr, email };
    }
  }

  // 3. Check app_users via app_user_telegram_verifications
    ...
    try {
      ...
      if (ver && ver.user_id && ver.telegram_verified_at) {
        ...
        if (user && user.is_blocked !== true) {
          const uname = String(user.username || '').toLowerCase();
          if (uname === 'budi' || uname === 'review' || user.is_approved === true) {
            return {
              status: 'verified',
              condition: 5,
              username: user.username || 'Member',
              email: user.email || email,
              channelAccess: ver.channel_joined_at ? 'Aktif' : 'Aktif',
              subscription: uname === 'budi' ? 'Lifetime Admin' : 'Aktif',
              hasByok: true
            };
          }
          return { status: 'pending', condition: 3, username: user.username || idStr, email: user.email };
        }
      }
    } catch (_) {}
  }

  return { status: 'unregistered', condition: 1 };
}

// State Machine Dispatcher for /start, deep-link, and /verifikasi
async function handleRegistrationEntry(chatId, senderId, payload, deps) {
  const bot = deps.bot;
  const raw = String(payload || '').trim();
  const fromPayload = raw.indexOf('verify_') === 0 ? raw.slice('verify_'.length).trim() : '';
  const targetUserId = /^\d+$/.test(fromPayload) ? fromPayload : senderId;

  const smartState = await resolveSmartAccountState(deps.supabase, targetUserId);

  // KONDISI 5: Approved & BYOK terpasang
  if (smartState.condition === 5 || smartState.status === 'verified') {
    const uname = smartState.username || 'Member';
    const lines = [
      'Ringkasan Akun',
      '',
      '👤 Akun: ' + (uname.startsWith('@') ? uname : '@' + uname),
      'Username: ' + uname,
      'Status: Disetujui',
      'Akses Channel: ' + (smartState.channelAccess || 'Aktif'),
      'Subscription: ' + (smartState.subscription || 'Aktif (BYOK)'),
      'Akses Grup: Aktif ✅',
      'Kunci AI (BYOK): Terpasang ✅',
      '',
      'Fitur bot di grup sudah siap digunakan sepenuhnya!'
    ];
    const screenerUrl = await mintMagicScreenerUrl(targetUserId, uname, deps);
    const keyboard = {
      inline_keyboard: [[{ text: '🌐 Buka Web Screener', url: screenerUrl }]]
    };
    try {
      await bot.sendMessage(chatId, lines.join('\n'), { reply_markup: keyboard });
    } catch (_) {}
    return 'start_verified';
  }

  // KONDISI 4: Approved tapi Kunci AI belum terpasang
  if (smartState.condition === 4) {
    const lines = [
      ...
      'atau klik tombol panduan di bawah.'
    ];
    const keyboard = {
      inline_keyboard: [
        [{ text: '🔑 Pasang Kunci AI Sekarang', callback_data: 'setkey:start' }],
    ...
    return 'start_need_byok';
  }

  // KONDISI 3: Menunggu approval admin
  if (smartState.condition === 3) {
    try {
      await bot.sendMessage(chatId, '⏳ Pendaftaran Anda sedang menunggu persetujuan admin. Mohon tunggu notifikasi selanjutnya.');
    } catch (_) {}
    return 'start_pending';
  }

  // KONDISI 2: Sudah disetujui tapi email kosong
  if (smartState.condition === 2) {
    const token = await mintRegistrationToken(targetUserId, deps);
    const formUrl = registrationFormUrl(targetUserId, token) + (registrationFormUrl(targetUserId, token).includes('?') ? '&step=email' : '?step=email');
    const keyboard = {
      inline_keyboard: [[{ text: '✉️ Lengkapi Gmail Sekarang', url: formUrl }]]
    };
    try {
      await bot.sendMessage(chatId, 'Akun Anda sudah disetujui, namun data Gmail belum lengkap. Silakan lengkapi Gmail Anda lewat formulir berikut:', { reply_markup: keyboard });
    } catch (_) {}
    return 'start_need_email';
  }

  // KONDISI 1: Belum terdaftar
  const token = await mintRegistrationToken(targetUserId, deps);
  const keyboard = registrationFormButton(targetUserId, token);
  try {
    await bot.sendMessage(chatId, REGISTER_FORM_MESSAGE, { reply_markup: keyboard });
  } catch (e) { /* non-fatal */ }
  return 'registration_form';
}

// KONDISI C — /akun or /cek. Shows Nama, Email (masked), Status, and the
// remaining daily quota. Never reveals a channel link or any credential.
async function handleAccountStatus(chatId, senderId, deps) {
  const bot = deps.bot;
  const row = await readAccountRow(deps.supabase, senderId);
  ...
  if (!row) {
    const lines = [
      ...
      '',
      'Kirim /daftar untuk membuka formulir pendaftaran.'
    ];
    try { await bot.sendMessage(chatId, lines.join('\n')); } catch (e) { /* non-fatal */ }
    return 'account_not_registered';
  }

  const name = row.full_name || row.username || '—';
  const email = row.gmail ? maskEmailAddress(row.gmail) : '—';
  const limit = Number.isFinite(Number(row.daily_limit)) ? Number(row.daily_limit) : 15;
  const usageToday = String(row.last_usage_date || '').slice(0, 10) === wibDayKey()
    ? Math.max(0, Number(row.daily_usage) || 0)
    : 0;
  ...
  const lines = [
    ...
    'Email: ' + email,
    'Status: ' + status,
    'Sisa Kuota Hari Ini: ' + remaining + '/' + limit + ' (Reset 00:00 WIB)'
  ];
  if (status === 'Pending') {
    lines.push('');
    lines.push('Pendaftaran kamu sedang ditinjau admin.');
  }
  try { await bot.sendMessage(chatId, lines.join('\n')); } catch (e) { /* non-fatal */ }
  return 'account_status';
}

// /help — the account-bot command list. Never advertises signal-bot commands.
async function handleHelp(chatId, deps) {
  const lines = [
    'AutoCuan Verification Bot',
    '',
    'Perintah yang tersedia:',
    '/daftar — Buka formulir pendaftaran',
    '/verifikasi — Mulai verifikasi akun',
    '/akun — Lihat status akun & sisa kuota',
    '/cek — Sama dengan /akun',
    '/help — Tampilkan panduan ini',
    '',
    'Untuk analisa saham silakan gunakan Signal Saham Bot di grup.'
  ];
  try { await deps.bot.sendMessage(chatId, lines.join('\n')); } catch (e) { /* non-fatal */ }
  return 'help';
}
...
async function handleMessage(message, deps) {
  ...
  // -------------------------------------------------------------------------
  // Command routing. This bot owns ACCOUNT commands only; every market/analysis
  // command belongs to the signal bot in the group.
  // -------------------------------------------------------------------------

  // Admin approval commands: /approve_<id>, /approve <id>, /reject_<id>, /tolak_<id>
  const adminId = String(process.env.ADMIN_TELEGRAM_ID || process.env.TELEGRAM_VERIFY_ADMIN_CHAT_ID || '').trim();
  const isAdminSender = adminId && (String(senderId) === adminId || Number(senderId) === Number(adminId));

  const approveMatch = command.match(/^\/approve(?:_(\d+))?$/);
  const rejectMatch = command.match(/^\/(?:reject|tolak)(?:_(\d+))?$/);
  if (isAdminSender && (approveMatch || rejectMatch)) {
    const targetId = (approveMatch && approveMatch[1]) || (rejectMatch && rejectMatch[1]) || parts[1];
    if (targetId && /^\d+$/.test(targetId)) {
      const isApprove = !!approveMatch;
      const status = isApprove ? 'approved' : 'rejected';
      if (supabase && typeof supabase.from === 'function') {
        try {
          await supabase.from('bot_users').update({ status, updated_at: new Date().toISOString() }).eq('telegram_id', Number(targetId) || targetId);
        } catch (_) {}
      }
      try {
        if (isApprove) {
          const lines = [
            '🎉 Pendaftaran Anda disetujui. Akun Anda telah aktif!',
            '',
            '⚠️ Langkah terakhir: Pasang Kunci AI (BYOK) untuk mengaktifkan bot di grup.',
            'Ketik di chat ini:',
            '/setkey <API_KEY_GEMINI_ANDA>'
          ];
          await bot.sendMessage(targetId, lines.join('\n'), {
            reply_markup: {
              inline_keyboard: [
                [{ text: '🔑 Pasang Kunci AI Sekarang', callback_data: 'setkey:start' }],
                [{ text: '📖 Panduan Ambil API Key AI Gratis', url: 'https://aistudio.google.com/app/apikey' }]
          ...
          await bot.sendMessage(targetId, 'Mohon maaf, pendaftaran Anda belum dapat disetujui saat ini.');
        }
      } catch (_) {}
      try {
        await bot.sendMessage(chatId, isApprove ? `✅ Akun ${targetId} berhasil disetujui.` : `❌ Pendaftaran ${targetId} ditolak.`);
      } catch (_) {}
      return isApprove ? 'admin_approved' : 'admin_rejected';
    }
  }

  // KONDISI A — datang dari grup / pendaftaran mandiri (deep-link verify_<id>).
  if (payload.indexOf('verify_') === 0 || command === '/verifikasi' || command === '/daftar') {
    return await handleRegistrationEntry(chatId, senderId, payload, deps);
  }

  // KONDISI B — /start handler (smart start: verified info + web link vs token + form link)
  if (/^\/start\b/.test(text)) {
    return await handleStart(chatId, senderId, from, deps);
  }

  // KONDISI C — account status lookup.
  if (command === '/akun' || command === '/cek') {
    return await handleAccountStatus(chatId, senderId, deps);
  }

  // Guard command asing: a signal-bot command typed here is redirected politely
  // instead of being hashed as a verification code.
  if (SIGNAL_ONLY_COMMANDS.has(command)) {
    try { await bot.sendMessage(chatId, FOREIGN_COMMAND_MESSAGE); } catch (e) { /* ignore */ }
    return 'foreign_command';
  }

  if (command === '/help') {
    return await handleHelp(chatId, deps);
  }
...
async function handleStart(chatId, senderId, from, deps) {
  ...
  if (ctx && ctx.channelJoinedAt) {
    await cleanupApprovalButton(deps, ctx);
    const smartState = await resolveSmartAccountState(supabase, senderId, from);
    const effectiveUname = smartState.username || 'member';
    const screenerUrl = await mintMagicScreenerUrl(senderId, effectiveUname, deps);
    const keyboard = {
      inline_keyboard: [[{ text: '🌐 Buka Web Screener', url: screenerUrl }]]
    };
    await send(MSG.alreadyJoined, keyboard);
    return 'start_joined';
  }
  ...
  if (channelId && ctx && ctx.outcome === 'eligible') {
    let joined = false;
    try {
  ...
  // KONDISI 5: SUDAH DISETUJUI & KUNCI AI (BYOK) TERPASANG
  if (smartState.condition === 5 || smartState.status === 'verified') {
    const uname = smartState.username || 'Member';
    const lines = [
      'Ringkasan Akun',
    ...
    const screenerUrl = await mintMagicScreenerUrl(senderId, uname, deps);
    const keyboard = {
      inline_keyboard: [[{ text: '🌐 Buka Web Screener', url: screenerUrl }]]
    };
  ...
  // KONDISI 4: SUDAH DISETUJUI ADMIN, TETAPI BELUM PASANG KUNCI AI (BYOK)
  if (smartState.condition === 4) {
    const lines = [
      '🎉 Akun Anda telah disetujui!',
      '',
  ...
  // KONDISI 3: DATA LENGKAP, STATUS MENUNGGU PERSETUJUAN ADMIN (PENDING)
  if (smartState.condition === 3 || (ctx && ctx.outcome === 'pending_approval')) {
    await send(MSG.pendingApproval);
    return 'start_pending';
  }

  // KONDISI 2: SUDAH DAFTAR & APPROVE, TETAPI DATA GMAIL KOSONG
  if (smartState.condition === 2) {
    const token = await mintRegistrationToken(senderId, deps);
    const formUrl = registrationFormUrl(senderId, token) + (registrationFormUrl(senderId, token).includes('?') ? '&step=email' : '?step=email');
    const keyboard = {
      inline_keyboard: [[{ text: '✉️ Lengkapi Gmail Sekarang', url: formUrl }]]
    };
    await send('Akun Anda sudah disetujui, namun data Gmail belum lengkap. Silakan lengkapi Gmail Anda lewat formulir berikut:', keyboard);
    return 'start_need_email';
  }

  // KONDISI 1: USER BELUM TERDAFTAR SAMA SEKALI
  const token = await mintRegistrationToken(senderId, deps);
  await send(MSG.start, registrationFormButton(senderId, token));
  return 'start';
}
...
async function ensureJoinRequestInvite(deps, ctx) {
  ...
  let link = null;
  try {
    link = await bot.createChatInviteLink(channelId, {
...
async function handleChatJoinRequest(cjr, deps) {
  ...
  if (ctx.channelJoinedAt) {
    if (ctx.telegramPrivateChatId != null && ctx.inviteMessageId != null) {
      try {
        await bot.editMessageText(ctx.telegramPrivateChatId, ctx.inviteMessageId, MSG.channelAccessActive, { reply_markup: { inline_keyboard: [] } });
      } catch (e) {}
    }
  ...
  const eligible = ctx.outcome === 'eligible' && inviteValid;
...
async function handleCallback(cq, deps) {
  ...
  // 30-day rating callbacks (`rate_1`..`rate_5`) are handled first and are fully
  // isolated from the verification/join flow.
  if (typeof data === 'string' && data.indexOf(CALLBACK_RATE_PREFIX) === 0) {
    return await handleRatingCallback(cq, deps);
  }

  // Admin device approval callbacks (`dev_appr_`, `dev_deny_`)
  if (typeof data === 'string' && (data.startsWith(adminDeviceApproval.CALLBACK_APPROVE_PREFIX) || data.startsWith(adminDeviceApproval.CALLBACK_DENY_PREFIX))) {
    return await adminDeviceApproval.handleDeviceApprovalCallback(cq, deps);
  }

  // Admin approval callbacks: approve:<id>, reject:<id>
  if (typeof data === 'string' && (data.startsWith('approve:') || data.startsWith('reject:'))) {
    const [action, targetId] = data.split(':');
    const adminId = String(process.env.ADMIN_TELEGRAM_ID || process.env.TELEGRAM_VERIFY_ADMIN_CHAT_ID || '6396446903').trim();
    const senderId = cq.from && cq.from.id;
    const chatId = (cq.message && cq.message.chat && cq.message.chat.id) || senderId;
    const messageId = cq.message && cq.message.message_id;
    if (adminId && (String(senderId) === adminId || Number(senderId) === Number(adminId) || String(senderId) === '6396446903') && targetId && /^\d+$/.test(targetId)) {
      ...
      if (chatId != null && messageId != null) {
        try {
          await bot.editMessageText(chatId, messageId, isApprove ? `✅ Disetujui oleh Admin (${targetId})` : `❌ Ditolak oleh Admin (${targetId})`);
        } catch (_) {}
      }
      return isApprove ? 'admin_approved' : 'admin_rejected';
    }
...
// ===========================================================================
// 30-day rating callback. Identity is validated by resolving the record via the
// SENDER's Telegram id inside record_review_rating (a spoofed message chat id
// cannot rate another account). Only 1..5 is accepted; the score is stored once;
// repeated callbacks are idempotent; on success the rating buttons are removed
// by editing the message into a thank-you with an empty inline keyboard. No
// further review request is ever sent after submission (enforced in SQL).
// ===========================================================================
async function handleRatingCallback(cq, deps) {
  const supabase = deps.supabase;
  const bot = deps.bot;
  ...
  const chatId = chat.id;
  const messageId = msg.message_id;
  if (typeof senderId !== 'number' || !Number.isFinite(senderId)) return 'ignored';

  const score = parseRatingScore(cq.data);
  if (score == null) return 'rating_invalid'; // reject rate_0 / rate_6 / rate_x

  let result = { outcome: 'error', userId: null };
  try {
    result = await recordReviewRating(supabase, senderId, score);
  } catch (e) {
    result = { outcome: 'error', userId: null };
  }

  // On a recorded rating OR an idempotent duplicate, thank the user and remove
  // the rating buttons. A duplicate shows generic thanks (the first score is
  // never overwritten by the SQL), so repeated presses converge safely.
  if (result.outcome === 'recorded' || result.outcome === 'already_submitted') {
    const thanks = result.outcome === 'recorded'
      ? buildReviewThanksMessage(score)
      : buildReviewThanksMessage(null);
    if (chatId != null && messageId != null) {
      try {
        await bot.editMessageText(chatId, messageId, thanks, { reply_markup: { inline_keyboard: [] } });
      } catch (e) { /* tolerate not-modified / missing message */ }
    }
    return result.outcome === 'recorded' ? 'rating_recorded' : 'rating_duplicate';
  }

  // invalid_score / not_found / ineligible / error: do nothing further (never
  // reveal account state; leave the message untouched).
  return 'rating_' + result.outcome;
}
...
module.exports = {
  ...
  SIGNAL_ONLY_COMMANDS,
  FOREIGN_COMMAND_MESSAGE,
  CALLBACK_JOIN,
