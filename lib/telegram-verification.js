...
const REGISTER_FORM_URL = 'https://autocuan.web.id/register.html';
...
function registrationFormUrl(targetUserId, token) {
  ...
  return REGISTER_FORM_URL + (params.length ? '?' + params.join('&') : '');
}
...
async function mintRegistrationToken(targetUserId, deps) {
  ...
  try {
    const token = await store.issueToken(deps.supabase, id, {});
...
// /daftar). Sends ONLY the registration form CTA.
//
// A Telegram delivery failure is non-fatal: the routing decision still holds, so
// the outcome code reflects it (the same convention the foreign-command guard
// uses). Returning a generic 'error' here would hide the routing from the logs
// and make the outcome code indistinguishable from a real internal failure.
...
async function handleMessage(message, deps) {
  ...
  // The legacy "copy the 15-minute code from the website" instructions must
  // NEVER be shown on this path.
  ...
  // KONDISI B — plain /start without a payload: context-aware recovery. Never
  // leaks account info; only surfaces a fresh join-request link to an approved
  // but not-yet-joined account, otherwise the standard guide.
...
async function handleStart(chatId, senderId, from, deps) {
  ...
  // Unbound / not verified / reserved / lookup error -> the standard legacy
  // web-verification guide, plus a direct registration CTA so a member who never
  // had a website code can still self-register without hunting for the form.
  if (!ctx || ctx.outcome === 'not_found' || ctx.outcome === 'not_verified' ||
      ctx.outcome === 'reserved' || ctx.outcome === 'error') {
    await send(MSG.start, registrationFormButton(senderId));
    return 'start';
  }
