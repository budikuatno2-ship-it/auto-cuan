...
const MSG = {
  ...
  // Sent privately after a chat_join_request is APPROVED by the bot.
  joinRequestApproved: [
...
async function readJoinRequestContext(supabase, telegramUserId) {
  try {
    const { data: ver, error: verErr } = await supabase
      .from('app_user_telegram_verifications')
      .select('user_id, telegram_verified_at, telegram_private_chat_id, channel_joined_at, dynamic_invite_link, invite_expires_at, invite_revoked_at')
      .eq('telegram_user_id', telegramUserId)
      .maybeSingle();
    if (verErr || !ver || !ver.user_id) return { outcome: 'not_found', userId: null, telegramPrivateChatId: null };
    ...
    return { outcome: 'error', userId: null, telegramPrivateChatId: null };
  }
}
...
async function handleChatJoinRequest(cjr, deps) {
  ...
  // Private confirmation to the user (stored private chat id only).
  if (ctx.telegramPrivateChatId != null) {
    try { await bot.sendMessage(ctx.telegramPrivateChatId, MSG.joinRequestApproved); } catch (e) {}
  }
  return confirm.outcome === 'already_joined' ? 'joined_already' : 'joined';
}
...
async function deliverApprovalInvite(deps, userId) {
  ...
  // Tolerate an already-expired or already-revoked link (best-effort).
  const previousLink = await readStoredInviteByUserId(supabase, userId);
  if (previousLink) {
    try { await bot.revokeChatInviteLink(channelId, previousLink); } catch (e) { /* tolerate */ }
  ...
  try {
    await bot.sendMessage(chatId, buildApprovalInviteMessage(), { reply_markup: requestJoinButton(inviteLink) });
  } catch (e) {
