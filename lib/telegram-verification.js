...
function getBotUsername() {
  const configured = runtimeEnv.optional('TELEGRAM_VERIFY_BOT_USERNAME');
  const fallback = runtimeEnv.runtimeEnvironment() === 'production' ? 'AutoCuanVerificationBot' : null;
  const username = (configured || fallback || '').replace(/^@/, '');
  return /^[A-Za-z0-9_]{5,32}$/.test(username) ? username : null;
}
function getBotUrl() { const username = getBotUsername(); return username ? 'https://t.me/' + username : null; }
...
const MSG = {
  ...
  membershipRequired: 'Akun sudah disetujui. Aktifkan paket melalui menu Paket sebelum meminta akses channel.',
...
function getCodeSecret() {
  const s = runtimeEnv.resolve('TELEGRAM_VERIFY_CODE_SECRET');
...
async function readJoinRequestContext(supabase, telegramUserId) {
  try {
    ...
    if (uname === 'review') return Object.assign({ outcome: 'reserved' }, base);
    ...
    const accessResult = await supabase.rpc('membership_account_for_telegram', { p_telegram_user_id: String(telegramUserId) });
    const access = Array.isArray(accessResult && accessResult.data) ? accessResult.data[0] : accessResult && accessResult.data;
    if (accessResult.error || !access || (access.isAdmin !== true && access.botAccess !== true)) return Object.assign({ outcome: 'membership_required' }, base);
...
async function readJoinEligibility(supabase, telegramUserId) {
  try {
    ...
    const accessResult = await supabase.rpc('membership_account_for_telegram', { p_telegram_user_id: String(telegramUserId) });
    const access = Array.isArray(accessResult && accessResult.data) ? accessResult.data[0] : accessResult && accessResult.data;
    if (accessResult.error || !access || (access.isAdmin !== true && access.botAccess !== true)) return { outcome: 'membership_required', userId: ver.user_id };
...
function getAdminChatId() {
  const v = runtimeEnv.resolve('TELEGRAM_VERIFY_ADMIN_CHAT_ID');
  return (typeof v === 'string' && v.trim()) ? v.trim() : null;
}
...
async function handleStart(chatId, senderId, from, deps) {
  ...
  if (ctx.outcome === 'membership_required') {
    await revokeInviteBestEffort(deps, senderId, ctx.userId);
    await send(MSG.membershipRequired);
    return 'start_membership_required';
  }
...
async function handleChatJoinRequest(cjr, deps) {
  ...
  if (!eligible) {
    if (ctx.outcome === 'membership_required' && linkMatches) {
      await revokeInviteBestEffort(deps, requesterId, ctx.userId);
    }
...
function declineOutcomeCode(ctx, providedInviteLink, linkMatches) {
  switch (ctx && ctx.outcome) {
    ...
    case 'membership_required': return 'declined_membership_required';
