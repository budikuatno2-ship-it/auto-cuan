...
async function resolveSmartAccountState(supabase, senderId, from) {
  ...
  if (botRow) {
    ...
    if (st === 'active' || st === 'approved') {
      ...
      const isVerifiedReady = hasByok || Boolean(botRow.subscription_status);
      return {
        status: isVerifiedReady ? 'verified' : 'need_byok',
        condition: isVerifiedReady ? 5 : 4,
        username: botRow.username || idStr,
        email: email,
...
async function handleStart(chatId, senderId, from, deps) {
  ...
  if (smartState.condition === 3) {
    await send('⏳ Pendaftaran Anda sedang menunggu persetujuan admin. Mohon tunggu notifikasi selanjutnya.');
    return 'start_pending';
  }
  ...
  const token = await mintRegistrationToken(senderId, deps);
  await send(REGISTER_FORM_MESSAGE, registrationFormButton(senderId, token));
  return 'start';
  ...
  if (ctx.channelJoinedAt) {
    ...
    const smartState = await resolveSmartAccountState(supabase, senderId);
