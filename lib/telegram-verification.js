...
async function readAccountRow(supabase, telegramId) {
  ...
  try {
    const res = await supabase
      .from('bot_users')
      .select('telegram_id, username, full_name, gmail, status, provider, daily_limit, daily_usage, last_usage_date')
...
async function resolveSmartAccountState(supabase, senderId, from) {
  ...
  // 1. Admin Budi (by ID, username, or literal 'budi')
  if (idStr.toLowerCase() === 'budi' || username === 'budi' || (adminId && (idStr === adminId || (idNum && idNum === Number(adminId))))) {
    return {
      status: 'verified',
      username: 'budi',
      channelAccess: 'Aktif',
      subscription: 'Lifetime Admin'
    };
  }
...
// KONDISI A — member arrives from the group deep-link (or types /verifikasi or
// /daftar). Sends ONLY the registration form CTA for new users, or account info for verified.
async function handleRegistrationEntry(chatId, senderId, payload, deps) {
  ...
  // The deep-link id wins; fall back to the sender's own Telegram id.
  const targetUserId = /^\d+$/.test(fromPayload) ? fromPayload : senderId;

  // Never show registration form to already verified members!
  const smartState = await resolveSmartAccountState(deps.supabase, targetUserId);
  if (smartState.status === 'verified') {
    const lines = [
      'Ringkasan Akun',
      '',
      'Username: ' + smartState.username,
      'Status: Disetujui',
      'Akses Channel: ' + smartState.channelAccess,
      'Subscription: ' + smartState.subscription
    ];
    const screenerUrl = await mintMagicScreenerUrl(targetUserId, smartState.username, deps);
    const keyboard = {
  ...
  } catch (e) { /* delivery failure is non-fatal; the routing decision stands */ }
...
async function handleMessage(message, deps) {
  ...
  if (isAdminSender && (approveMatch || rejectMatch)) {
    ...
    if (targetId && /^\d+$/.test(targetId)) {
      ...
      if (supabase && typeof supabase.from === 'function') {
        try {
          await supabase.from('bot_users').update({ status, updated_at: new Date().toISOString() }).eq('telegram_id', targetId);
        } catch (_) {}
      }
      try {
        await bot.sendMessage(targetId, isApprove
...
async function handleStart(chatId, senderId, from, deps) {
  ...
  const username = (from && from.username && String(from.username).trim().toLowerCase()) || '';
  const adminId = String(process.env.ADMIN_TELEGRAM_ID || process.env.TELEGRAM_VERIFY_ADMIN_CHAT_ID || '').trim();
  const isBudi = idStr.toLowerCase() === 'budi' || username === 'budi' || (adminId && (idStr === adminId || (idNum && idNum === Number(adminId))));
  ...
  if (isBudi) {
    ...
    const screenerUrl = await mintMagicScreenerUrl(senderId, 'budi', deps);
    const keyboard = {
      inline_keyboard: [[{ text: '🌐 Buka Web Screener', url: screenerUrl }]]
    };
  ...
  // Check bot_users if outcome is not_found, not_verified, reserved, or error
    ...
    if (botRow) {
      ...
      if (st === 'active' || st === 'approved') {
        const lines = [
          'Ringkasan Akun',
          '',
          'Username: ' + (botRow.username || botRow.full_name || senderId),
          'Status: Disetujui',
          'Akses Channel: ' + (botRow.channel_access || 'Aktif'),
          'Subscription: ' + (botRow.subscription_status || 'Aktif (BYOK)')
        ];
        const effectiveUname = botRow.username || botRow.full_name || senderId;
...
async function handleCallback(cq, deps) {
  ...
  if (typeof data === 'string' && (data.startsWith('approve:') || data.startsWith('reject:'))) {
    ...
    if (adminId && (String(senderId) === adminId || Number(senderId) === Number(adminId)) && targetId && /^\d+$/.test(targetId)) {
