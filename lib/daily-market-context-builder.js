...
const chartVolume = require('./chart-engine/volume-analyzer');
...
function buildContextFromRows(ticker, historyRows, foreignRows, fundamentalsRow, options) {
  ...
  // Chart-engine trend/volume block for AI grounding. Purely additive: the
  // builder never fails if the module returns an insufficient-data result.
  // historyRows is newest-first, so reverse to the oldest-first order the
  // indicators expect.
  var chartEngine = null;
  try {
    if (historyRows.length >= 50) {
      var candlesOldestFirst = historyRows.slice().reverse().map(function(r) {
        return {
          date: r.trade_date,
          open: Number(r.open),
          high: Number(r.high),
        ...
        return Number.isFinite(c.open) && Number.isFinite(c.high) && Number.isFinite(c.low) &&
          Number.isFinite(c.close) && Number.isFinite(c.volume);
      });
      if (candlesOldestFirst.length >= 50) {
        var trend = chartIndicators.classifySwingTrend(candlesOldestFirst);
        var volAnalyzed = chartVolume.analyze(candlesOldestFirst);
        chartEngine = {
          trend_status: trend.status,
          ema20: trend.ema20 != null ? Math.round(trend.ema20) : null,
          ema50: trend.ema50 != null ? Math.round(trend.ema50) : null,
          higher_high: trend.higher_high,
          higher_low: trend.higher_low,
          volume_rvol: volAnalyzed.rvol != null ? Math.round(volAnalyzed.rvol * 100) / 100 : null,
          volume_label: volAnalyzed.label
        };
      }
    }
  } catch (_) {
  ...
  return {
    ...
    // Chart-engine Swing direction + relative volume (AI grounding only).
