...
// Require a session whose server-signed admin flag is true, or check Bearer/x-admin-key against ADMIN_SECRET/CRON_SECRET.
function requireAdminSession(req) {
  const adminSecret = process.env.ADMIN_SECRET || process.env.CRON_SECRET;
  if (adminSecret && typeof adminSecret === 'string' && adminSecret.length > 0 && req && req.headers) {
    ...
    const keyCandidate = token || adminKey;
    if (keyCandidate && typeof keyCandidate === 'string') {
      const a = Buffer.from(keyCandidate);
      const b = Buffer.from(adminSecret);
      if (a.length === b.length && crypto.timingSafeEqual(a, b)) {
        return { ok: true, session: { un: 'budi', adm: true, isSecretAuth: true } };
      }
    }
  }
...
// Restricted, purpose-bound session for pending-account subscription onboarding.
// It deliberately has a different cookie name, payload shape, and HMAC domain
// from ac_sess, so it can never be accepted by normal application authorization.
const ONBOARDING_COOKIE_NAME = 'ac_onboard';
const ONBOARDING_PURPOSE = 'subscription_onboarding';
const ONBOARDING_MAX_AGE_SECONDS = 15 * 60;
function onboardingSign(input, secret) { return b64urlEncode(crypto.createHmac('sha256', secret).update('ac-onboarding-v1.' + input).digest()); }
function createOnboardingSessionToken(input, opts) {
  const secret = getSessionSecret(); if (!secret) return null;
  const now = Math.floor(Date.now() / 1000), maxAge = (opts && opts.maxAgeSeconds) || ONBOARDING_MAX_AGE_SECONDS;
  const payload = { uid: String(input.userId), un: String(input.username || '').toLowerCase(), typ: ONBOARDING_PURPOSE, iat: now, exp: now + maxAge };
  const encoded = b64urlEncode(JSON.stringify(payload));
  return 'ob1.' + encoded + '.' + onboardingSign('ob1.' + encoded, secret);
}
function verifyOnboardingSessionToken(token) {
  const secret = getSessionSecret(); if (!secret || typeof token !== 'string') return { valid:false };
  const parts=token.split('.'); if(parts.length!==3 || parts[0]!=='ob1') return { valid:false };
  const expected=onboardingSign(parts[0]+'.'+parts[1],secret), a=Buffer.from(parts[2]), b=Buffer.from(expected);
  if(a.length!==b.length || !crypto.timingSafeEqual(a,b)) return { valid:false };
  let p; try { p=JSON.parse(b64urlDecodeToString(parts[1])); } catch (_) { return {valid:false}; }
  const now=Math.floor(Date.now()/1000);
  if(!p || p.typ!==ONBOARDING_PURPOSE || typeof p.uid!=='string' || typeof p.un!=='string' || !Number.isSafeInteger(p.iat) || !Number.isSafeInteger(p.exp) || p.exp<=p.iat || p.exp-p.iat>ONBOARDING_MAX_AGE_SECONDS || p.iat>now+60 || p.exp<=now) return {valid:false};
  return {valid:true,payload:p};
}
function getOnboardingSession(req) { const r=verifyOnboardingSessionToken(parseCookies(req)[ONBOARDING_COOKIE_NAME]); return r.valid?r.payload:null; }
function buildOnboardingCookie(token) { return ONBOARDING_COOKIE_NAME+'='+token+'; '+cookieFlags().join('; ')+'; Max-Age='+ONBOARDING_MAX_AGE_SECONDS; }
function buildClearOnboardingCookie() { return ONBOARDING_COOKIE_NAME+'=; '+cookieFlags().join('; ')+'; Max-Age=0'; }
module.exports.ONBOARDING_COOKIE_NAME=ONBOARDING_COOKIE_NAME;
module.exports.ONBOARDING_PURPOSE=ONBOARDING_PURPOSE;
module.exports.ONBOARDING_MAX_AGE_SECONDS=ONBOARDING_MAX_AGE_SECONDS;
module.exports.createOnboardingSessionToken=createOnboardingSessionToken;
module.exports.verifyOnboardingSessionToken=verifyOnboardingSessionToken;
module.exports.getOnboardingSession=getOnboardingSession;
module.exports.buildOnboardingCookie=buildOnboardingCookie;
module.exports.buildClearOnboardingCookie=buildClearOnboardingCookie;
