...
function normalizeRegimeCategory(regimeOrLabel) {
  if (!regimeOrLabel) return 'sideways';
  var label = typeof regimeOrLabel === 'string'
    ? regimeOrLabel.toUpperCase()
    : String(regimeOrLabel.market_regime_label || regimeOrLabel.label || regimeOrLabel.regime || '').toUpperCase();
  if (label === 'RISK_ON' || label === 'BULL') return 'bull';
  if (label === 'RISK_OFF' || label === 'BEAR') return 'bear';
  return 'sideways';
}

function getRegimeAdaptiveThresholds(regimeOrLabel) {
  var category = normalizeRegimeCategory(regimeOrLabel);
  if (category === 'bull') {
    return {
      category: 'bull',
      regime_label: 'RISK_ON',
      score_offset: 0,
      daytrade_min_score: 65,
      swing_min_score: 75,
      min_rr: 1.2,
      require_a_plus_daytrade: false,
      daytrade_cutoff: 65,
      notes: 'Bull Regime (^JKSE): Standar normal (default cutoff score 65).'
    };
  }
  if (category === 'bear') {
    return {
      ...
      score_offset: 10,
      daytrade_min_score: 75,
      swing_min_score: 85,
      min_rr: 1.5,
      require_a_plus_daytrade: true,
      daytrade_cutoff: 75,
      notes: 'Bear Regime (^JKSE): Mode defensif ketat (+10 poin score, wajib A_PLUS_SETUP, cutoff diperketat).'
    };
  }
  return {
    category: 'sideways',
    ...
    score_offset: 5,
    daytrade_min_score: 70,
    swing_min_score: 80,
    min_rr: 1.5,
    require_a_plus_daytrade: false,
    daytrade_cutoff: 70,
    notes: 'Sideways Regime (^JKSE): Syarat score +5 poin dan filter Risk/Reward minimal 1:1.5.'
  };
}
...
module.exports = {
  ...
  normalizeRegimeCategory: normalizeRegimeCategory,
  getRegimeAdaptiveThresholds: getRegimeAdaptiveThresholds,
