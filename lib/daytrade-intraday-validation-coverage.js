...
function coverageFromObserve(report, requestedLimit) {
  ...
  const nestedCoverage = report && report.coverage || {};
  // Session bundles flatten coverage at the top level, while observe reports
  // retain it under `coverage`. Prefer a complete nested object when present,
  // then use a complete flattened bundle instead of manufacturing zeroes.
  const topLevelCoverage = report && typeof report === 'object' ? report : {};
  const completeCoverage = (value) => Object.prototype.hasOwnProperty.call(value, 'universe_count') && Object.prototype.hasOwnProperty.call(value, 'evaluated_universe_count') && Object.prototype.hasOwnProperty.call(value, 'provider_checked_count');
  const coverage = completeCoverage(nestedCoverage) ? nestedCoverage : (completeCoverage(topLevelCoverage) ? topLevelCoverage : nestedCoverage);
  const hasCoverage = completeCoverage(coverage);
  ...
  return {
    ...
    full_universe_loaded: coverage.full_universe_loaded === true || (coverage.full_universe_loaded !== false && coverage.universe_source === 'full_daytrade_eligible_universe'),
