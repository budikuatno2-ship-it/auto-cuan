...
function compareRankScore(a, b) {
  const as = a.score === null ? Number.NEGATIVE_INFINITY : a.score;
  const bs = b.score === null ? Number.NEGATIVE_INFINITY : b.score;
  return (bs - as) || a.ticker.localeCompare(b.ticker);
}
function rankCandidates(rows) {
  return asArray(rows).map(normalizeCandidate).filter((r) => r.ticker).sort(compareRankScore).map((r, index) => Object.assign({}, r, { rank: index + 1 }));
}
function indexByTicker(rows) { const map = new Map(); asArray(rows).forEach((r) => { if (r.ticker) map.set(r.ticker, r); }); return map; }
...
function compareRuns(normalRun, adjustedRun) {
  const normalRows = rankCandidates(rowsFromRun(normalRun));
  const adjustedRows = rankCandidates(rowsFromRun(adjustedRun));
...
module.exports = { DEFAULT_LOGS_FILE, DEFAULT_OUTPUT_DIR, asArray, normalizeCandidate, rankCandidates, compareRuns, deriveCautions, readJsonFile, readJsonlRuns, findLatestRun, loadInputs, markdownReport, writeReports, buildReport };
