...
const candlePatternEngine = require('./candle-pattern-engine');
...
function verifySwingHighConviction(candidate, options = {}) {
  ...
  const trend = r.swing_trend_structure || r.swingTrendStructure ||
    (Array.isArray(r.candles || r.ohlcv) ? chartIndicators.classifySwingTrend(r.candles || r.ohlcv) : null);
  if (trend && String(trend.status || '').toUpperCase() === 'DOWNTREND') return null;
  ...
  // 1b. HARD GATE (Fase 3 / BUG-F3-03): status AVOID / SPECULATIVE dari modul
  // analisis hulu DILARANG dipromosikan menjadi High Conviction Swing.
  if (status.includes('AVOID') || status.includes('SPECULATIVE')) {
    return null; // Saham berbahaya / tidak likuid tidak boleh disiarkan sebagai High Conviction!
  }
  ...
  // 3b. HARD GATE: Confluence Bandarmologi & Foreign Flow Arjum (Fitur #2 & #7)
  const ticker = r.ticker || r.stock_code || r.symbol;
  let confluenceResult = null;
  if (options.confluenceResult) {
    confluenceResult = options.confluenceResult;
  } else if (ticker) {
    try {
      confluenceResult = bandarmologiService.evaluateConfluenceSignal(ticker, options);
    } catch (_) {
  ...
  if (confluenceResult && confluenceResult.confluence_flag === 'HINDARI') {
    // Breakout teknikal terjadi namun asing dan bandar melakukan net distribution masif
    return null; // Fatal contradiction: Asing & bandar distribusi masif! DOWNGGRADE_GRADE / HINDARI
  }
  ...
  // 5. Hard threshold skor High Conviction (Regime-Adaptive, Fitur #4)
  const regimeInput = options.market_regime || options.marketRegime || options.regime ||
    r.market_regime || r.marketRegime || r.regime || null;
  let minConvictionScore = MIN_SWING_HIGH_CONVICTION_SCORE; // default 75
  if (regimeInput) {
    const adaptive = marketRegimeEngine.getRegimeAdaptiveThresholds(regimeInput);
    if (adaptive && adaptive.swing_min_score) {
      minConvictionScore = adaptive.swing_min_score;
    }
  }

  if (evaluation.score < minConvictionScore) {
    return null;
  }
  ...
  if (confluenceResult) {
    r.confluence_flag = confluenceResult.confluence_flag;
    r.confluence_action = confluenceResult.confluence_action;
    r.foreign_net = confluenceResult.foreign_net;
    r.bandar_net = confluenceResult.bandar_net;
    r.confluence_notes = confluenceResult.notes;
  }
