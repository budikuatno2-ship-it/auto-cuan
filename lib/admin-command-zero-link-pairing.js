...
// admin-command-login.js. This layer replaces laptop pairing/re-pairing:
// an already-open /dashboard maintenance tab registers a hashed browser
...
//
// If browser/site data is cleared, the trusted-device cookie disappears while
// PostgreSQL may still contain the old admin_device_hash. A fresh browser
// pairing request therefore takes precedence over the stale paired-device path;
// approving it atomically replaces the old device binding.
...
function pairButton(candidate, rebind) {
  return {
    text: (rebind ? '🔄 Hubungkan ulang Laptop · ' : '💻 Hubungkan Laptop · ') + candidate.display_tag,
    callback_data: PAIR_CALLBACK_PREFIX + candidate.request_ref
  };
}

async function sendPairMenu(update, options, message, ctx, knownCandidate) {
  const db = options.db;
  ...
  try {
    ...
    const candidate = knownCandidate || await pairCandidate(db, from.id);
    const keyboard = [[hpButton(hpToken)]];
    const rebind = ctx.device_bound === true;
    let laptopText;

    if (candidate && candidate.result_code === 'ok' && REQUEST_REF_RE.test(String(candidate.request_ref || ''))) {
      keyboard.push([pairButton(candidate, rebind)]);
      if (rebind) {
        laptopText = [
          '🔄 Browser baru terdeteksi untuk Laptop utama.',
          'ID perangkat: ' + String(candidate.display_tag || ''),
          'Jika kamu baru menghapus data/cookie Auto-Cuan, tekan Hubungkan ulang Laptop.',
          'Pastikan ID ini sama dengan yang tampil di halaman maintenance laptop.',
          'Persetujuan ini akan mengganti binding Laptop utama lama dengan browser ini.'
        ].join('\n');
      } else {
        laptopText = [
...
async function handleOldLaptopCallback(update, options, callback, ctx) {
  // A live browser pairing request takes precedence even when PostgreSQL still
  // says the old laptop is paired. This is the recovery path after the browser
  // site data/cookies were cleared.
  ...
  // Multiple live browser requests must fail closed; never guess which browser
  // should replace the existing trusted laptop.
  if (candidate && candidate.result_code === 'ambiguous') {
    ...
    const outcome = 'admin_zero_link_pair_ambiguous';
    try {
      await answerCallback(options.bot, callback.id, 'Ada beberapa browser menunggu. Tutup tab lain dan coba /akses lagi.');
      return { handled: true, outcome: outcome };
    } finally {
  ...
  // No fresh browser pairing request exists: preserve the established device
  // grant flow for an intact paired laptop.
  ...
  const outcome = 'admin_zero_link_pair_not_ready';
  try {
    await answerCallback(options.bot, callback.id, 'Buka autocuan.web.id/dashboard di laptop, tunggu beberapa detik, lalu /akses lagi.');
    return { handled: true, outcome: outcome };
  } finally {
    await completeWebhookUpdate(options.db, update.update_id, outcome);
  }
...
async function handleUpdate(update, options) {
  ...
  if (update.message) {
    ...
    if (ctx && ctx.result_code === 'ok' && ctx.device_bound === true) {
      // If a fresh browser request exists, it means this browser does not have
      // the trusted-device cookie (for example after Clear site data). Surface
      // a zero-link rebind instead of silently targeting the stale device hash.
      const candidate = await pairCandidate(db, from.id);
      if (candidate && (candidate.result_code === 'ok' || candidate.result_code === 'ambiguous')) {
        return sendPairMenu(update, options, update.message, ctx, candidate);
      }
      return commandLogin.handleUpdate(update, options);
    }

    return sendPairMenu(update, options, update.message, ctx);
  }
