...
async function showProcessing(botClient, chatId) {
  if (botClient && typeof botClient.sendChatAction === 'function') await botClient.sendChatAction(chatId).catch(function () {});
}
...
async function processUpdate(update, deps) {
  ...
  try {
    await showProcessing(deps.bot, chat.id);
