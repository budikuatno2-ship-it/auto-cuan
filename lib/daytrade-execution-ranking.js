...
const entryDiscipline = require('./daytrade-entry-discipline');
...
function deriveDayTradeExecutionQuality(row) {
  ...
  const discipline = entryDiscipline.deriveDayTradeEntryDiscipline(row);
  ...
  if (!discipline.entry_executable_now) {
    executionQualityStatus = discipline.entry_discipline_status === 'WAIT_PULLBACK' ? 'RADAR_ONLY' : 'BLOCKED';
    executionQualityLabel = discipline.entry_discipline_status === 'WAIT_PULLBACK'
      ? 'Radar only — wait pullback'
      : 'Blocked — entry belum terverifikasi';
    executionRankBucket = discipline.entry_discipline_status === 'WAIT_PULLBACK' ? 3 : 4;
    adjustment = discipline.entry_discipline_status === 'WAIT_PULLBACK' ? -15 : -100;
    reason = discipline.entry_discipline_reason;
  } else if (rr == null) {
  ...
  return {
    ...
    execution_rr: rr,
    entry_reference_price: discipline.entry_reference_price,
    entry_chase_pct: discipline.entry_chase_pct,
    entry_discipline_status: discipline.entry_discipline_status,
    entry_discipline_label: discipline.entry_discipline_label,
    entry_executable_now: discipline.entry_executable_now,
    entry_discipline_reason: discipline.entry_discipline_reason
  };
}
