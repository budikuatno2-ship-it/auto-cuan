...
const registerTokenStore = require('./telegram-register-token');
const { createRateLimiter, clientAddress } = require('./request-rate-limit');

const GENERIC_CREDENTIAL_ERROR = 'Username atau password salah.';
const MAX_WEBHOOK_BODY_BYTES = 32 * 1024;

// Each call re-arms a fresh reset request and pushes a Telegram confirmation
// push to the target account, with no per-account cooldown of its own — an
// unauthenticated caller could otherwise repeat this against any username to
// harass a linked Telegram account or burn the bot's Telegram API budget.
// Keyed on the requester's address, same pattern as api/register-user.js.
const passwordResetRequestLimiter = createRateLimiter({ windowMs: 10 * 60 * 1000, max: 8 });
...
async function handleVerifyWebhook(req, res) {
  ...
  try {
    ...
    const result = await telegramVerification.processWebhookUpdate(update, {
      supabase: db,
      bot: bot,
      // Lets the deep-link registration CTA mint a one-time form token so the
      // member never has to copy a code from the website.
...
async function requestPasswordReset(req, res, db) {
  if (!passwordResetRequestLimiter.check(clientAddress(req))) {
    // Deliberately identical to the generic success branch below: it says
    // nothing about whether any username exists or was actually rate limited.
    return res.status(200).json({
      success: true,
