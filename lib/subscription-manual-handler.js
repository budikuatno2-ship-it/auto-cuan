...
const CANONICAL_HOSTS = ['autocuan.web.id', 'www.autocuan.web.id'];

function allowedHosts() {
  const extra = String(process.env.SUBSCRIPTION_ALLOWED_HOSTS || '')
    .split(',')
    .map(function (entry) { return entry.trim().toLowerCase(); })
    .filter(Boolean);
  return new Set(CANONICAL_HOSTS.concat(extra));
}
...
// The review link is delivered to the admin over Telegram, so it must never be
// derived from headers the requester controls. Host / X-Forwarded-Host are
// attacker-supplied on any non-browser client, which previously let any
// signed-in account point the admin's "Buka & Konfirmasi" button at an
// arbitrary origin. Only an explicitly configured base URL or a known host is
// accepted; anything else falls back to the canonical production origin.
function publicBaseUrl(req) {
  const configured = normalizeBaseUrl(process.env.SUBSCRIPTION_PUBLIC_BASE_URL);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'submit') {
      const termsAccepted = req.body && (req.body.paymentTermsAccepted === true || req.body.termsAccepted === true);
      if (!termsAccepted) {
        return res.status(400).json({ success:false, error:'Anda wajib menyetujui kebijakan pembayaran dan ketentuan subscription.' });
      }
      ...
      if (req.body.paymentTermsAccepted || req.body.termsAccepted) {
        await accountTerms.recordTermsAcceptance(db, auth.account.id, 'payment');
      }
      const row = await paymentRow(db, reference);
      // The SQL function is idempotent: re-submitting an already-submitted
      // payment returns the existing row unchanged. Notifying unconditionally
      // let a caller send the admin an unbounded number of review messages and
      // orphaned every previously recorded admin_telegram_message_id. Notify
      // only while no admin message has been recorded, which still allows a
      // retry after a delivery failure.
