...
function validate(aiOutput, sourceData, options) {
  ...
  if (sourceData.status) {
    ...
    const synonymMap = {
      'WATCHLIST': ['PANTAUAN', 'WATCH LIST', 'EARLY RADAR', 'RADAR'],
      'SWING READY': ['SIAP SWING', 'SWING SIAP', 'WATCHLIST', 'PANTAUAN', 'EARLY RADAR', 'RADAR'],
      'DAY TRADE READY': ['SIAP DAY TRADE', 'DAYTRADE READY', 'WATCHLIST', 'PANTAUAN', 'EARLY RADAR', 'RADAR'],
      'EARLY RADAR': ['WATCHLIST', 'PANTAUAN', 'RADAR', 'SWING READY', 'DAY TRADE READY'],
    ...
    var allVariants = statusVariants.concat(synonyms);

    // For initial signal/watchlist types, allow any watchlist-equivalent status wording
    // as long as ticker and all numeric levels are preserved.
    var nType = (options.notificationType || '').toLowerCase();
    var isInitialSignal = nType === 'new_signal' || nType === 'watchlist';
    if (isInitialSignal) {
      // Add broad set of acceptable initial-signal status wordings
      var initialSignalAccepted = [
        'WATCHLIST', 'PANTAUAN', 'WATCH LIST',
        'SWING READY', 'SIAP SWING', 'SWING SIAP',
        'DAY TRADE READY', 'SIAP DAY TRADE', 'DAYTRADE READY',
        'EARLY RADAR', 'RADAR',
        'SINYAL BARU', 'NEW SIGNAL', 'KANDIDAT',
        'PILIHAN', 'REKOMENDASI', 'TOP PICK'
      ];
      initialSignalAccepted.forEach(function(s) {
        if (allVariants.indexOf(s) < 0) allVariants.push(s);
      });
    }
