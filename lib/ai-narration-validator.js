...
/**
 * Remove explicit HH:MM / HH.MM clock tokens before fabricated-number checks.
 * This preserves real clock references such as 09:45 or 13.59 without creating
 * a blanket numeric exemption that would also let fabricated IDX prices through.
 * Thousand-formatted prices such as 1.200 are not matched because the fractional
 * clock component must contain exactly two digits.
 */
function stripClockReferences(text) {
  // ponytail: strip clock tokens with colon or explicit session/time markers to allow market clocks but retain prices
  return String(text || '')
    .replace(/(?:pukul|jam|sesi|buka|dibuka|tutup|ditutup|sampai|hingga)\s+(?:(?:kedua|ke-\d+|di)\s+)*(?:[01]?\d|2[0-3])\.[0-5]\d(?:\s*wib)?\b/gi, ' ')
    .replace(/\b(?:[01]?\d|2[0-3])\.[0-5]\d\s*wib\b/gi, ' ')
    .replace(/\b([01]?\d|2[0-3]):(\d{2})(?:\s*wib)?\b/gi, function(token, h, m) {
      return Number(m) <= 59 ? ' ' : ' ' + m;
    });
}
...
function validateNote(aiNote, sourceData, options) {
  ...
  if (sourceData && typeof sourceData === 'object') {
    // Strip only syntactically valid clock tokens before numeric extraction. The
    // old 0-2359 exemption was far too broad and allowed fabricated stock prices.
    var aiNumbers = extractNumbers(stripClockReferences(note));
    ...
    collectNumbers(sourceData);

    const MONTH_PATTERN = /\b(?:januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|ags|agu|sep|okt|nov|des|tanggal|tgl)\b/i;
    const hasCalendarContext = MONTH_PATTERN.test(note);

    var fabricatedNumbers = aiNumbers.filter(function(n) {
      if (sourceNumbers.has(n)) return false;
      var num = parseFloat(n);
      // Small ordinals (e.g. target 1, target 2, TP1, TP2)
      if (Number.isInteger(num) && num >= 1 && num <= 3) return false;
      // Day-of-month references only if explicit calendar context is present
      if (Number.isInteger(num) && num >= 1 && num <= 31 && hasCalendarContext) return false;
      // Year references.
