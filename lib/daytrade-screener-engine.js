...
async function runDayTradeBatch(tickers, runMode, options) {
  ...
  for (var i = 0; i < tickers.length; i++) {
    ...
    try {
      ...
      // This is a runtime-only diagnostic field — the persist mapper only writes
      // explicit columns, so it is never inserted. Base scoring is untouched.
      tradePlanV2Integration.attachShadowTradePlanV2(scored, {
        screener_type: 'DAY_TRADE',
        env: (typeof process !== 'undefined' ? process.env : undefined),
        observations: Array.isArray(candles)
          ? candles.slice(-10).map(function(c) { return { open: c.open, high: c.high, low: c.low, close: c.close }; })
          : undefined
      });
