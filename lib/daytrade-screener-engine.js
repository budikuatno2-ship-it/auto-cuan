...
// ============================================================
// PRE-SPIKE DETECTOR (0-25 score) — V2: volume confirmation required
// ============================================================
...
function scorePreSpike(data) {
  ...
  // V2 B2: volume_ratio must be >= 1.2 for meaningful pre-spike signal
...
function classifyStatus(compositeScore, data, levels, liqResult, penaltyResult, board, runMode) {
  ...
  // === CLASSIFICATION (V2: with afternoon, gap, volume guards) ===
  var status, setup, notes;

  // READY_BREAKOUT: score >= 85, NO hard fails, NOT Akselerasi, NOT afternoon new entry
  if (compositeScore >= 85 && hardFails.length === 0 && !isAfternoon) {
    status = 'READY_BREAKOUT';
    setup = determineSetup(data, 'ready');
    // V2 B1: Notes clarify this is conditional/radar, not auto-buy
  ...
  else if (compositeScore >= 85 && hardFails.length === 0 && isAfternoon) {
  ...
  // PRE_SPIKE_WATCH: score >= 70, no hard fails, V2: requires volume >= 1.2
    ...
    // V2 B5: Block PRE_SPIKE if gap > 5%
    if (data.change_pct > 5.0) {
      // V2 B3: Already moving hard — MOMENTUM_CONTINUATION, not pre-spike
  ...
  // V2 B2: PRE_SPIKE blocked due to low volume
  else if (compositeScore >= 70 && hardFails.length === 0 && hasLowVolume && !isAfternoon) {
    status = 'SPECULATIVE';
  ...
  // V2 B4: PRE_SPIKE in afternoon → Watch/Wait
  ...
  // AVOID: only score < 50 reaches here (after severe checks above)
  else {
    status = 'AVOID';
