...
async function fetchForeignUniverseTickers(supabase, knownTickers) {
  // Foreign uploads are an input to discovery, never an implicit buy list.  The
  // actual scan still requires market candles and its normal quality gates.
  var known = knownTickers || {};
  try {
    var res = await supabase.from('foreign_watchlist_daily')
      .select('ticker,trade_date,uploaded_at')
    ...
    (res.data || []).forEach(function(row) {
      ...
      seen[ticker] = true;
      tickers.push({ ticker: ticker, board: null, universe_source: 'foreign_latest', konglo_classification: 'UNVERIFIED_NON_KONGLO', konglo_classification_diagnostic: 'missing_konglo_classification' });
    });
    return { tickers: tickers, diagnostics: { foreign_universe_discovered_count: tickers.length, missing_konglo_classification_count: tickers.length } };
  } catch (e) {
    return { tickers: [], diagnostics: { foreign_universe_error: e.message || String(e) } };
  }
}
...
async function buildDayTradeUniverse(supabase) {
  ...
  var known = {};
  universe.forEach(function(item) { known[item.ticker] = true; });
  var foreign = await fetchForeignUniverseTickers(supabase, known);
  return { tickers: universe.concat(foreign.tickers), error: null, diagnostics: foreign.diagnostics };
}
...
async function buildFastDayTradeUniverse(supabase) {
  ...
  var foreign = await fetchForeignUniverseTickers(supabase, known);
  return { tickers: fastUniverse.concat(foreign.tickers), error: null, diagnostics: foreign.diagnostics };
}
