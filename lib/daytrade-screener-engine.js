...
// ============================================================
// PENALTY CALCULATION (-5 to -40) — V2: enhanced gap/overheat + volume distribution
// ============================================================
...
function calculatePenalty(data) {
  ...
  // V2 B5: Gap-up / Overheat guard (stricter thresholds)
  if (data.change_pct > 8.5) {
    penalty -= 18;
    reasons.push('Gap/kenaikan sangat tinggi (' + data.change_pct.toFixed(1) + '%). Hindari chase.');
    ...
    penalty -= 8;
    reasons.push('Gap/kenaikan sudah tinggi (+' + data.change_pct.toFixed(1) + '%). Waspadai chase.');
  }
  ...
  // V2 B7: Volume distribution guard — price below open with high volume
  if (!data._priceAboveOpen && data.volume_ratio_20d >= 1.5) {
    penalty -= 10;
    reasons.push('Price < open dengan volume tinggi (distribusi intraday)');
  } else if (!data._priceAboveOpen && data.volume_ratio_20d >= 1.2) {
    penalty -= 5;
    reasons.push('Price < open dengan volume meningkat');
  }

  // V2 B7: Upper shadow with volume (rejection candle)
  var dayRange = data.high_price - data.low_price;
  var upperShadow = data.high_price - Math.max(data.open_price, data.last_price);
  if (dayRange > 0 && upperShadow > dayRange * 0.4 && data.volume_ratio_20d >= 1.3) {
    penalty -= 8;
    reasons.push('Upper shadow besar + volume. Potensi distribusi.');
  }
...
function classifyStatus(compositeScore, data, levels, liqResult, penaltyResult, board, runMode) {
  ...
  // READY_BREAKOUT: score >= 82, NO hard fails, NOT Akselerasi, NOT afternoon
  else if (compositeScore >= 82 && hardFails.length === 0 && !isAfternoon) {
  ...
  // V2 B4: Afternoon mode — downgrade READY to MOMENTUM_CONTINUATION
  else if (compositeScore >= 82 && hardFails.length === 0 && isAfternoon) {
  ...
  if (isAkselerasi && (status === 'READY_BREAKOUT' || status === 'PRE_SPIKE_WATCH' || status === 'MOMENTUM_CONTINUATION')) {
    status = 'SPECULATIVE';
...
function generateWaitNotes(hardFails, hasOverheat, hasPoorRR, data) {
  if (hasOverheat) {
    return 'Sudah naik tinggi (+' + data.change_pct.toFixed(1) + '%). Jangan chase, tunggu pullback/retest.';
  }
  if (hasPoorRR) {
    return 'Momentum ada, tapi RR belum layak. Tunggu pullback ke entry lebih dekat.';
  }
  return 'Setup aktif tapi ' + hardFails[0] + '. Tunggu koreksi.';
}

function generateSpeculativeNotes(hardFails, data) {
  if (data.volume_ratio_20d >= 1.5 && data.change_pct > 0) {
    return 'Ada aktivitas volume tapi setup belum lengkap. Monitor saja.';
  }
  if (hardFails.length > 0) {
    return 'Partial setup: ' + hardFails.slice(0, 2).join(', ') + '. Risk tinggi.';
  }
  return 'Setup belum kuat. Hanya untuk watchlist, bukan entry.';
}

function generateAvoidNotes(hardFails, penaltyResult, data) {
  if (penaltyResult.reasons.length > 0) {
    var r = penaltyResult.reasons[0];
    if (r.includes('Overheat')) return 'Sudah terlalu panas. Risiko false breakout tinggi.';
    if (r.includes('Fade')) return 'Sudah fade dari high. Potensi distribusi.';
  }
  if (data.volume_ratio_20d < 0.5) return 'Likuiditas kurang aman untuk day trade.';
  if (data.change_pct < -3) return 'Tekanan jual dominan. Tidak layak entry.';
  if (hardFails.length > 0) return hardFails.slice(0, 2).join(', ') + '. Setup tidak layak.';
...
// ============================================================
// TIME PLAN GENERATION — V2: explicit afternoon warning
// ============================================================
...
function generateTimePlan(status, runMode, data) {
  ...
  if (runMode === 'MORNING_SCOUT') {
    ...
    base = 'Late entry berisiko. Prioritaskan exit sebelum close. Jangan entry agresif kecuali sudah punya posisi dan trailing plan. ';
  } else {
    base = 'Monitor sesi berikutnya. ';
  }

  if (status === 'A_PLUS_SETUP') {
    return base + 'SETUP A+ — prioritas utama. Entry area valid. Wajib konfirmasi manual. Exit sebelum 14:50 jika TP belum hit.';
  }
  if (status === 'READY_BREAKOUT') {
    // V2 B1: Explicit conditional note
    return base + 'READY bersifat kandidat prioritas, bukan instruksi beli otomatis. Entry area valid. Exit sebelum 14:50 jika TP belum hit.';
  }
  if (status === 'EARLY_RADAR') {
    return base + 'Radar awal. Belum entry. Pantau volume + price action. Entry hanya jika konfirmasi breakout terjadi.';
  }
  if (status === 'PRE_SPIKE_WATCH') {
    return base + 'Tunggu volume spike + break resistance. Jangan chase. Konfirmasi manual wajib.';
  }
...
function scoreDayTrade(data, runMode, board) {
  ...
  // 11. V3: Entry timing label
  var entryTiming = 'Hanya pantau';
  if (classification.status === 'A_PLUS_SETUP' || classification.status === 'READY_BREAKOUT') {
    if (data.change_pct <= 3.0 && levels._riskDistPct <= 3.0) entryTiming = 'Masih dekat entry';
    else entryTiming = 'Tunggu breakout';
