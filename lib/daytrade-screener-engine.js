...
var candleFetcher = require('./chart-engine/candle-fetcher');
...
var MIN_RR_RATIO = screenerConfig.MIN_RR_RATIO;
var passesRiskRewardFilter = screenerConfig.passesRiskRewardFilter;
var marketRegimeEngine = require('./market-regime');
var bandarmologiService = require('./bandarmologi-service');
...
// BATCH4-F7-02: quote/derived fields also arrive as formatted strings from some
// feeds ("9.0", "86"). calculatePenalty() and classifyStatus() call `.toFixed()`
// on them, so an un-coerced string threw
// `data.change_pct.toFixed is not a function` on the overheat branch — the exact
// branch a hot, volume-confirmed mover always hits. Coercing here keeps penalty,
// classification, notes and direction on ONE numeric source of truth.
// BUG-F7-03 taught the engine to coerce CANDLES; this closes the same gap for
// the quote fields the scorer reads.
function coerceQuoteNumbers(data) {
  if (!data || typeof data !== 'object') return data;
  var changePct = coerceNumeric(data.change_pct);
  data.change_pct = Number.isFinite(changePct) ? changePct : 0;
  var rsi = coerceNumeric(data.rsi14);
  data.rsi14 = Number.isFinite(rsi) ? rsi : null;
  return data;
}
...
// Jam bursa otoritatif & klasifikasi sesi sekarang berasal dari SATU sumber
// kebenaran: lib/market-hours-guard.js. Sebelumnya helper di file ini
// memakai kalender yang berbeda (sesi 1 berakhir 12:00, sesi 2 tutup 16:00)
// sehingga runner intraday bisa mengklasifikasikan dirinya OUTSIDE_MARKET
// pada jam bursa nyata, dan sebaliknya masih "buka" pada 15:45-16:00 WIB
// saat order book sudah beku. Lihat lib/market-hours-guard.js untuk
// penjelasan dua level keputusan (klasifikasi sesi vs. gembok broadcast).
var marketHoursGuard = require('./market-hours-guard');
...
// ============================================================
// BURSA SCHEDULE (day-of-week aware, WIB) — BATCH 3 Module 1
// ============================================================
// Kalender sesi IDX otoritatif (satu sumber: lib/market-hours-guard.js,
// dipakai juga oleh lib/telegram-notifier.js):
//   Senin-Kamis: sesi 1 09:00-12:00, istirahat 12:00-13:30, sesi 2 13:30-15:45
//   Jumat:       sesi 1 09:00-11:30, istirahat 11:30-14:00, sesi 2 14:00-15:45
//   Sabtu/Minggu: bursa tutup.
// Mengembalikan status eksplisit:
//   { isOpen, session, status, run_mode, reason, wib_date, wib_time,
//     total_minutes, minutes_to_session_end, radar_cutoff_passed, broadcast_allowed }
// `status` = 'LIVE_MARKET' selama salah satu sesi berjalan — ini yang dibaca
// runner intraday untuk membedakan LIVE_MARKET vs OUTSIDE_MARKET.
// Selama BREAK/CLOSED order book beku: dilarang komputasi sinyal / trigger
// alert dari quote basi (degradasi aman: caller wajib skip bersih).

function getMarketSessionStatus(nowValue) {
  return marketHoursGuard.getMarketSessionStatus(nowValue);
}

/**
 * Hard cut-off radar/sinyal pantauan Day Trade: 14:30 WIB (AFTERNOON_EXIT).
 * Setelah cut-off ini tidak ada lagi radar pantauan BARU — sisa sesi terlalu
 * pendek untuk eksekusi, dan radar menjelang closing (mis. 15:37 WIB) hanya
 * menghasilkan noise.
 */
function isDayTradeRadarCutoffPassed(nowValue) {
  return marketHoursGuard.isAfternoonExitCutoff(nowValue);
}

/**
 * Apakah radar/sinyal pantauan baru boleh dipublikasikan sekarang?
 * @returns {{ allowed: boolean, reason: string, session: string, wib_time: string }}
 */
function evaluateDayTradeRadarWindow(nowValue) {
  return marketHoursGuard.evaluateDayTradeRadarWindow(nowValue);
}
...
// Yahoo first (it carries the live intraday bar), then the backfilled
// data/daily-candles cache. The previous bare fetch() had no deadline: one
// unresponsive Yahoo socket hung the whole Day Trade batch, and because meta is
// only persisted between batches, daytrade_screener_meta stayed at
// status='scanning' forever. Top 5 readiness then failed every later run with
// day_trade.not_ready_reason='running_lock_timeout' instead of resuming.
async function fetchDayTradeCandles(ticker) {
  var candles = await candleFetcher.fetchScreenerCandles(ticker, { minCandles: 20, range: '90d' });
  return candles && candles.length >= 20 ? candles : null;
}
...
function analyzeDayTrade(candles, ticker) {
  // BATCH4-F7-01: a provider that answers with an empty array, a null body, or a
  // page of null rows used to crash here (`candles.length` / `last.close` of
  // undefined). Inside runDayTradeBatch the crash was swallowed by the per-ticker
  // try/catch, so the ticker vanished from the scan with a generic "exception:"
  // reason; every caller OUTSIDE that try/catch (backtests, replay tools,
  // diagnostics) crashed outright. Degrade to "no analysis" instead, and drop
  // individual unusable rows so a single provider gap does not discard a whole
  // session series.
  ...
  candles = candles.filter(function(c) {
    return c && typeof c === 'object' && Number.isFinite(c.close) && Number.isFinite(c.open) &&
...
function scoreMomentum(data) {
  // BATCH4-F7-04: coerce before the RSI ladder so a formatted "55" scores the
  // same as the numeric 55 instead of silently falling through every branch.
  coerceQuoteNumbers(data);
  var score = 0;
...
function calculatePenalty(data) {
  // BATCH4-F7-02: `data.change_pct.toFixed(1)` and `data.rsi14.toFixed(0)` below
  // threw a TypeError when a feed delivered formatted strings. Coerce first so
  // the overheat and overbought branches keep working on every feed shape.
  coerceQuoteNumbers(data);
  var penalty = 0;
  var reasons = [];
...
function isPrespikeRadar(data) {
  if (!data) return false;
  var dp = data.change_pct != null ? Number(data.change_pct) : 0;
  var vr = (data.volume_ratio_20d != null && Number.isFinite(data.volume_ratio_20d))
    ? Number(data.volume_ratio_20d)
    : (data.volume_ratio != null && Number.isFinite(data.volume_ratio) ? Number(data.volume_ratio) : 0);
  return dp >= -1.0 && dp <= 1.5 && vr >= 2.5;
}

function isEarlyMomentum(data) {
  if (!data) return false;
  var dp = data.change_pct != null ? Number(data.change_pct) : 0;
  var vr = (data.volume_ratio_20d != null && Number.isFinite(data.volume_ratio_20d))
    ? Number(data.volume_ratio_20d)
    : (data.volume_ratio != null && Number.isFinite(data.volume_ratio) ? Number(data.volume_ratio) : 0);
  return dp >= 2.0 && dp <= 4.5 && vr >= 1.8;
}
...
function classifyStatus(compositeScore, data, levels, liqResult, penaltyResult, board, runMode, candleDowngrade) {
  // BATCH4-F7-02: change_pct/rsi14 are rendered with .toFixed() in the notes and
  // overheat branches; coerce so formatted strings cannot crash classification.
  coerceQuoteNumbers(data);
  ...
  // === HARD R/R GATE (Batch 6) ===
  // Candidates below MIN_RR_RATIO must never reach priority/entry statuses
  // (A_PLUS_SETUP, TRADE_CANDIDATE, READY_BREAKOUT, PRE_SPIKE_WATCH, EARLY_RADAR,
  // MOMENTUM_CONTINUATION, RECLAIM_CANDIDATE). Force them to WAIT_PULLBACK with an
  // explicit warning so low-R/R setups (e.g. SSMS 1.0x) cannot leak into radar/entry.
  if (!passesRiskRewardFilter(levels, MIN_RR_RATIO)) {
    return {
      status: 'WAIT_PULLBACK',
      setup: 'Wait - Poor RR',
      notes: 'Risk:Reward ' + (Number.isFinite(levels.risk_reward) ? levels.risk_reward.toFixed(2) : 'N/A') +
        ':1 di bawah minimum ' + MIN_RR_RATIO + ':1. Tunggu pullback agar entry lebih dekat SL sebelum pertimbangan entry.'
    };
  }
  ...
  // NOTE (Batch 4 review): failing closed on an UNKNOWN ratio here is
  // INTENTIONAL. PRE_SPIKE_WATCH is a PROMOTION that requires volume to be
  // CONFIRMED, so a ratio that was never measured (null) must not promote.
  // This is deliberately different from scoreLiquidity (BUG-F7-02), where the
  // same comparison HARD-FAILED the candidate to AVOID and removed it from the
  // scan entirely — that eliminated valid candidates, this only withholds a
  // promotion. Pinned by daytrade-v7-engine-integrity.test.js.
  //
  // BATCH4-F7-03: `undefined < 1.2` is FALSE (NaN comparison), so an absent
  // field used to PROMOTE while an explicit null did not — the same UNKNOWN
  // state got two different answers depending on how the feed omitted it.
  // Normalise both UNKNOWN forms to the fail-closed branch.
  var gateRvol = (data.volume_ratio_20d == null) ? null : Number(data.volume_ratio_20d);
  if (gateRvol == null || !Number.isFinite(gateRvol) || gateRvol < DT_INITIAL.prespike_volume_ratio) {
    hasLowVolume = true;
  }
  ...
  // Early momentum breakout: change_pct +2.0% s/d +4.5%, volume >= 1.8x
  else if (isEarlyMomentum(data) && hardFails.length === 0 && !isAfternoon && compositeScore >= DT_INITIAL.early_radar_score) {
    status = 'TRADE_CANDIDATE';
    setup = 'Early Momentum Breakout';
    notes = 'Early momentum breakout terdeteksi (+2.0% s/d +4.5%, vol >= 1.8x). Volume mengonfirmasi akumulasi awal.';
  }
  ...
  // PRE_SPIKE_WATCH: score >= 70, no hard fails, V2: requires volume >= 1.2
    ...
    if (data.change_pct > 5.0) {
      ...
      setup = isPrespikeRadar(data) ? 'Pre-Spike Radar' : determineSetup(data, 'prespike');
      notes = isPrespikeRadar(data)
        ? 'Pre-spike radar terdeteksi (-1.0% s/d +1.5%, vol >= 2.5x). Akumulasi volume signifikan sebelum spike.'
        : 'Volume mulai masuk, tunggu breakout area entry. Konfirmasi volume wajib sebelum entry.';
    }
  }
  ...
  // PRE_SPIKE_RADAR: score >= 62, change_pct -1.0% s/d +1.5%, volume >= 2.5x
  else if (isPrespikeRadar(data) && hardFails.length === 0 && !isAfternoon && compositeScore >= DT_INITIAL.early_radar_score) {
    status = 'PRE_SPIKE_WATCH';
    setup = 'Pre-Spike Radar';
    notes = 'Pre-spike radar terdeteksi (-1.0% s/d +1.5%, vol >= 2.5x). Akumulasi volume signifikan sebelum spike.';
  }
...
function scoreDayTrade(data, runMode, board, candleResult, evaluationOptions) {
  ...
  // BATCH4-F7-02: normalise the quote fields ONCE at the entry point so every
  // downstream consumer (levels, liquidity, penalty, classification, notes)
  // reads the same coerced numbers regardless of the feed's string formatting.
  coerceQuoteNumbers(data);
  // 1. Calculate levels
  ...
  if (candleResult && candleResult.pattern) {
    ...
    if (cp.pattern === 'Hanging Man' && (!data.support || data.last_price > data.support * 1.03)) { candleScore -= 4; candleDowngrade = true; }
  ...
  // 8b. Regime-Adaptive Thresholding (Fitur #4)
  var regimeInput = (evaluationOptions && (evaluationOptions.market_regime || evaluationOptions.marketRegime || evaluationOptions.regime)) ||
    data.market_regime || data.marketRegime || data.regime || null;
  var regimeAdaptive = null;
  if (regimeInput) {
    regimeAdaptive = marketRegimeEngine.getRegimeAdaptiveThresholds(regimeInput);
    if (regimeAdaptive) {
      if (regimeAdaptive.category === 'sideways') {
        // Sideways: Syarat score +5 poin (min 70), filter Risk/Reward minimal 1:1.5
        if (levels.risk_reward < 1.5 && (classification.status === 'A_PLUS_SETUP' || classification.status === 'TRADE_CANDIDATE' || classification.status === 'READY_BREAKOUT' || classification.status === 'MOMENTUM_CONTINUATION')) {
          classification.status = 'WAIT_PULLBACK';
          classification.setup = 'Wait - Sideways Poor RR';
          classification.notes = (classification.notes || '') + ' [Sideways: RR ' + (Number.isFinite(levels.risk_reward) ? levels.risk_reward.toFixed(2) : 'N/A') + ' < 1.5].';
        } else if (compositeScore < regimeAdaptive.daytrade_min_score && (classification.status === 'A_PLUS_SETUP' || classification.status === 'TRADE_CANDIDATE' || classification.status === 'READY_BREAKOUT')) {
          classification.status = 'WAIT_PULLBACK';
          classification.setup = 'Wait - Sideways Threshold';
          classification.notes = (classification.notes || '') + ' [Sideways: Skor ' + compositeScore + ' < minimum adaptif ' + regimeAdaptive.daytrade_min_score + '].';
        }
      } else if (regimeAdaptive.category === 'bear') {
        // Bear: Mode defensif ketat, naikkan syarat score (+10 poin, min 75), wajib A_PLUS_SETUP, cutoff diperketat
        if (classification.status !== 'A_PLUS_SETUP' && (classification.status === 'TRADE_CANDIDATE' || classification.status === 'READY_BREAKOUT' || classification.status === 'MOMENTUM_CONTINUATION' || classification.status === 'PRE_SPIKE_WATCH')) {
          classification.status = 'WAIT_PULLBACK';
          classification.setup = 'Wait - Bear Defensive Mode';
          classification.notes = (classification.notes || '') + ' [Bear: Mode defensif ketat, wajib A_PLUS_SETUP untuk entry].';
        }
        if (compositeScore < regimeAdaptive.daytrade_min_score && classification.status === 'A_PLUS_SETUP') {
          classification.status = 'WAIT_PULLBACK';
          classification.setup = 'Wait - Bear Score Cutoff';
          classification.notes = (classification.notes || '') + ' [Bear: Skor ' + compositeScore + ' < minimum adaptif ' + regimeAdaptive.daytrade_min_score + '].';
        }
      }
    }
  }

  // 8c. Confluence Bandarmologi & Foreign Flow Arjum (Fitur #2 & #7)
  var tickerSymbol = data.ticker || data.symbol || (evaluationOptions && evaluationOptions.ticker) || null;
  var confluenceResult = null;
  if (evaluationOptions && evaluationOptions.confluenceResult) {
    confluenceResult = evaluationOptions.confluenceResult;
  } else if (tickerSymbol) {
    try {
      confluenceResult = bandarmologiService.evaluateConfluenceSignal(tickerSymbol);
    } catch (_) {
      confluenceResult = null;
    }
  }

  if (confluenceResult && confluenceResult.confluence_flag === 'HINDARI') {
    if (classification.status === 'A_PLUS_SETUP' || classification.status === 'TRADE_CANDIDATE' || classification.status === 'READY_BREAKOUT' || classification.status === 'MOMENTUM_CONTINUATION') {
      classification.status = 'WAIT_PULLBACK';
      classification.setup = 'Hindari - Massive Distribution';
      classification.notes = (classification.notes || '') + ' [HINDARI / DOWNGGRADE_GRADE: ' + confluenceResult.notes + '].';
    }
    compositeScore = Math.max(0, compositeScore + (confluenceResult.confluence_penalty || -20));
  }
  ...
  var breakoutConfirmation = idxTick.deriveBreakoutConfirmation({
    ...
    // Batch 11: propagate close-confirmation state so a live/intraday wick above
    // resistance is held at NEEDS_CLOSE_CONFIRMATION instead of BREAKOUT_CONFIRMED.
    price_source: data.price_source,
    candle_closed: data.candle_closed,
    bar_closed: data.bar_closed,
    candle_forming: data.candle_forming,
    is_intraday_live: data.is_intraday_live
  });
  ...
  return Object.assign({
    ...
    market_regime: regimeInput ? (typeof regimeInput === 'string' ? regimeInput : (regimeInput.market_regime_label || regimeInput.category || 'UNKNOWN')) : null,
    market_regime_category: regimeAdaptive ? regimeAdaptive.category : null,
    confluence_flag: confluenceResult ? confluenceResult.confluence_flag : 'NEUTRAL',
    confluence_action: confluenceResult ? confluenceResult.confluence_action : 'HOLD',
    foreign_net: confluenceResult ? confluenceResult.foreign_net : 0,
    bandar_net: confluenceResult ? confluenceResult.bandar_net : 0,
    confluence_notes: confluenceResult ? confluenceResult.notes : null
...
function dayTradeEligibilityReason(row, options) {
  ...
  // BATCH 4: JANGAN hard-reject nominal harga (Rp <200 / <100 / <50 TETAP BOLEH
  // LOLOS). Kelayakan murni berbasis LIKUIDITAS RIIL: turnover memadai
  // (value/valuasi/value_today), ketebalan antrean (frequency/freq), lantai
  // RVOL > 1.2x (Batch 3, di momentum.evaluate), dan akumulasi broker summary.
...
async function runDayTradeBatch(tickers, runMode, options) {
  ...
  // Resolve daily market regime if not supplied
  var batchMarketRegime = options.market_regime || options.marketRegime || null;
  if (!batchMarketRegime) {
    try {
      batchMarketRegime = await marketRegimeEngine.getMarketRegime(options);
    } catch (_) {
      batchMarketRegime = null;
    }
  }
  // BATCH 3 Module 1: kunci mati saat BREAK/CLOSED. Order book beku tidak
  // boleh dianggap breakdown/breakout — skip bersih (degradasi aman).
  // Gate hanya aktif bila caller menitipkan waktu tick eksplisit
  // (options.now / options.nowValue, yaitu jalur live terjadwal). Panggilan
  // ad-hoc/API tanpa waktu eksplisit tetap memakai perilaku legacy agar
  // skoring perencanaan di luar jam bursa tidak ikut mati.
  var hasExplicitNow = options.now != null || options.nowValue != null;
  if (hasExplicitNow) {
    var scheduleNow = options.now != null ? options.now : options.nowValue;
    var scheduleStatus = getMarketSessionStatus(scheduleNow);
    if (scheduleStatus.session === 'BREAK' || scheduleStatus.session === 'CLOSED') {
      return { results: results, failed: failed, status: 'paused', skipped: true, reason: 'market_break', session: scheduleStatus.session, schedule_reason: scheduleStatus.reason };
    }
  }
  ...
  for (var i = 0; i < tickers.length; i++) {
    ...
    try {
      ...
      var evalOpts = Object.assign({}, options, {
        captureEvaluationInitial: options.captureEvaluationInitial === true,
        market_regime: batchMarketRegime,
        ticker: item.ticker
      });
      var scored = scoreDayTrade(analysis, runMode, item.board, candleResult, evalOpts);
      ...
      Object.assign(scored, idxTick.deriveBreakoutConfirmation({
        ...
        // Batch 11: propagate close-confirmation state (see scoreDayTrade).
        price_source: analysis.price_source,
        candle_closed: analysis.candle_closed,
        bar_closed: analysis.bar_closed,
        candle_forming: analysis.candle_forming,
        is_intraday_live: analysis.is_intraday_live
      }));
      ...
      if (scored.confluence_flag === 'HINDARI') {
        scored.quality_grade = 'C';
        scored.confidence = 'C';
        scored.grade_reason = (scored.grade_reason || '') + ' [DOWNGGRADE_GRADE: Distribusi masif asing/bandar]';
      }
...
function refineLevelsWithRespectZones(baseLevels, candles, lastPrice, context) {
  ...
  if (supplyLevel && supplyLevel > lastPrice * 1.005) {
    ...
    if (risk > 0 && Number.isFinite((supplyLevel - entryMid) / risk) && (supplyLevel - entryMid) / risk >= 1.2) {
      ...
      // Fase 3 / BUG-F3-05: menaikkan TP1 ke supply zone harus ikut mengangkat TP2,
      // jika tidak, guard urutan level di bawah membatalkan SELURUH refinement dan
      // salah melabelinya "R/R would become too weak" padahal R/R aktual sehat.
      var minTp2 = refined.tp1 + atrProxy * 0.5;
      if (!Number.isFinite(refined.tp2) || refined.tp2 < minTp2) {
        refined.tp2 = Math.round(minTp2);
      }
...
// ============================================================
// FASE 3: MERITOKRASI TRANSAKSI RIIL & BASE SCORE 25
// ============================================================

var BASE_SCORE = 25;
var TRADEABLE_SCORE_THRESHOLD = 65;

function scoreVolumeSurge(data) {
  if (!data) return 0;
  var vr = Number(data.volume_ratio_20d != null ? data.volume_ratio_20d : (data.effective_volume_ratio != null ? data.effective_volume_ratio : (data.volume_ratio != null ? data.volume_ratio : (data.vol_ratio != null ? data.vol_ratio : 0))));
  if (!Number.isFinite(vr) || vr < 1.0) return 0;
  if (vr >= 2.0) return 30;
  if (vr >= 1.5) return 20;
  if (vr >= 1.25) return 10;
  return 0;
}

function scoreOrderFlowVelocity(data) {
  if (!data) return 0;
  var delta15m = Number(data.delta_turnover_15m != null ? data.delta_turnover_15m : (data.turnover_15m != null ? data.turnover_15m : (data.delta_turnover != null ? data.delta_turnover : (data.delta_turnover_5m != null ? data.delta_turnover_5m : 0))));
  var bidDom = data.bid_dominance != null ? data.bid_dominance : (data.bid_ratio != null ? data.bid_ratio : (data.orderbook_dominance != null ? data.orderbook_dominance : null));
  if (bidDom != null) {
    bidDom = Number(bidDom);
    if (bidDom > 1 && bidDom <= 100) bidDom = bidDom / 100;
  }
  if (Number.isFinite(delta15m) && delta15m >= 1000000000) return 15;
  if (bidDom != null && Number.isFinite(bidDom) && bidDom > 0.58) return 10;
  return 0;
}

function calculateDayTradeScore(data, options) {
  data = data || {};
  var base = BASE_SCORE;
  var volSurge = scoreVolumeSurge(data);
  var orderFlow = scoreOrderFlowVelocity(data);
  var vr = Number(data.volume_ratio_20d != null ? data.volume_ratio_20d : (data.effective_volume_ratio != null ? data.effective_volume_ratio : (data.volume_ratio != null ? data.volume_ratio : (data.vol_ratio != null ? data.vol_ratio : 0))));

  var tech = 0;
  if (data.change_pct != null) {
    var cp = Number(data.change_pct);
    if (cp >= 0.5 && cp <= 4.0) tech += 8;
    else if (cp > 4.0 && cp <= 6.0) tech += 5;
    else if (cp > 0) tech += 3;
  }
  if (data.rsi14 != null) {
    var rsi = Number(data.rsi14);
    if (rsi >= 45 && rsi <= 68) tech += 8;
    else if (rsi > 68 && rsi <= 75) tech += 4;
  }
  if (data._priceAboveOpen || data.price_above_open) tech += 4;
  if (data.distance_to_breakout_pct != null && Number(data.distance_to_breakout_pct) <= 2.5) tech += 5;
  if (data.range_position != null && Number(data.range_position) >= 60) tech += 5;

  // Meritokrasi Transaksi Riil: Saham tanpa lonjakan volume (vr < 1.0 atau volSurge === 0)
  // TIDAK BISA menembus skor 65 (tradeable threshold).
  if (vr < 1.0 || volSurge === 0) {
    tech = Math.min(24, tech);
  } else {
    tech = Math.min(30, tech);
  }

  var total = base + volSurge + orderFlow + tech;
...
function safeToFixed(n, d = 2) {
  return Number.isFinite(Number(n)) ? Number(n).toFixed(d) : '0.00';
}

module.exports = {
  BASE_SCORE: BASE_SCORE,
  TRADEABLE_SCORE_THRESHOLD: TRADEABLE_SCORE_THRESHOLD,
  scoreVolumeSurge: scoreVolumeSurge,
  scoreOrderFlowVelocity: scoreOrderFlowVelocity,
  calculateDayTradeScore: calculateDayTradeScore,
  getDayTradeEvaluationConfiguration: getDayTradeEvaluationConfiguration,
  getRunMode: getRunMode,
  getMarketSessionStatus: getMarketSessionStatus,
  isDayTradeRadarCutoffPassed: isDayTradeRadarCutoffPassed,
  evaluateDayTradeRadarWindow: evaluateDayTradeRadarWindow,
  ...
  coerceQuoteNumbers: coerceQuoteNumbers,
  ...
  calcRSI: calcRSI,
  calcMA: calcMA,
  isPrespikeRadar: isPrespikeRadar,
  isEarlyMomentum: isEarlyMomentum,
