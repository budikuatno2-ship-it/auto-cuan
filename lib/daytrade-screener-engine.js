...
function scoreDayTrade(data, runMode, board, candleResult) {
  ...
  return Object.assign({
    ...
    // Observational only: preserve the uncapped calculation-point score.
    daytrade_score_raw: rawScore,
    // Exact classification inputs captured before persistence/shortlisting.
    daytrade_evaluation_gate_inputs: {
      liquidity_pass: liqResult.pass, risk_reward: levels.risk_reward,
      risk_distance_pct: levels._riskDistPct, change_pct: data.change_pct,
      volume_ratio_20d: data.volume_ratio_20d, price_above_open: data._priceAboveOpen,
      distribution: !data._priceAboveOpen && data.volume_ratio_20d >= 1.5,
      overextended_ma20: !!data._overextendedMA20, rsi14: data.rsi14,
      candle_downgrade: !!candleDowngrade, afternoon_mode: runMode === 'AFTERNOON_EXIT'
    },
...
function getRecentAverageRange(candles, endIdx, lookback) {
  ...
  for (var i = start; i < endIdx; i++) {
    var r = (candles[i].high || 0) - (candles[i].low || 0);
    if (r > 0) { sum += r; count++; }
  }
...
function detectHalfCandleDebt(candles, lastPrice) {
  ...
  for (var i = from; i <= lastImpulseIdx; i++) {
    var c = candles[i];
    var range = (c.high || 0) - (c.low || 0);
    var body = (c.close || 0) - (c.open || 0);
    ...
    var rawHalf = c.low + (range * 0.5);
    var half = idxTick.roundToIdxTick(rawHalf, 'nearest');
    if (!half || !idxTick.getIdxTickSize(half)) continue;
    var distPct = half > 0 ? round2(((lastPrice || latest.close || c.close) - half) / half * 100) : 0;
...
function detectMajorRespectZoneContext(candles, lastPrice) {
  ...
  function pctDist(a, b) { return b > 0 ? Math.abs(a - b) / b * 100 : 99; }
...
function detectVolumeProfilePoc(candles, lastPrice) {
  ...
  if (!poc) return null;
  var dist = price > 0 ? round2((price - poc) / price * 100) : null;
...
function classifyRespectVolume(volume, avgVolume) {
  if (!volume || !avgVolume || volume <= 0 || avgVolume <= 0) {
...
function scoreRespectCandleQuality(candles, levels, context) {
  ...
  var latest = candles[len - 1];
  var prev = candles[len - 2];
  var o = latest.open || 0, h = latest.high || 0, l = latest.low || 0, c = latest.close || 0;
  var range = h - l;
  if (range <= 0 || !c) return null;

  var body = Math.abs(c - o);
  var lowerWick = Math.min(o, c) - l;
  var upperWick = h - Math.max(o, c);
  var closePos = (c - l) / range;
  var factors = [];
  var invalid = [];
  var score = 0;
  var keyLevel = null;
  ...
  for (var av = 0; av < lookback.length - 1; av++) {
    if (lookback[av].volume && lookback[av].volume > 0) { avgVol += lookback[av].volume; avgVolCount++; }
  }
  avgVol = avgVolCount > 0 ? avgVol / avgVolCount : null;

  var demand = [];
  var supply = [];
  function pushLevel(list, type, value) {
    value = Number(value);
    if (value > 0) list.push({ type: type, value: value, dist: Math.abs(c - value) / value });
  }
  ...
  for (var bi = Math.max(0, len - 8); bi < len - 1; bi++) {
    var base = candles[bi];
    var br = (base.high || 0) - (base.low || 0);
    if (br <= 0) continue;
    var bBody = (base.close || 0) - (base.open || 0);
    if (bBody > 0 && bBody / br >= 0.55) pushLevel(demand, 'midpoint / half-candle', base.low + br * 0.5);
  ...
  if (levels.resistance && c > 0 && levels.resistance > c && ((levels.resistance - c) / c) <= 0.025) invalid.push('Resistance/supply too close above entry');
...
function getDayTradeEvaluationConfiguration(runMode, options) {
  ...
  return {
    ...
    classification_thresholds: { score_avoid:40, score_early_building:58, score_momentum:60, score_early_radar:62, score_near_breakout:65, score_prespike:70, score_ready:75, score_trade_candidate:78, score_a_plus:88, rr_ready:1.5, risk_distance_max:5, volume_prespike:1.2, volume_a_plus:1.5, overheat_pct:8.5, gap_pct:5, gap_volume:1.5, distribution_volume:1.5, rsi_overbought:80 },
