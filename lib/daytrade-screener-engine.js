...
function classifyStatus(compositeScore, data, levels, liqResult, penaltyResult, board) {
  ...
  if (data.change_pct > 7.0 && data.volume_ratio_20d < 2.0) {
    ...
    hardFails.push('Overheat tinggi');
    hasOverheat = true;
  }
  ...
  // === CLASSIFICATION (softer, watchlist-friendly) ===
  var status, setup, notes;

  // READY_BREAKOUT: score >= 85, NO hard fails, NOT Akselerasi
  ...
  // PRE_SPIKE_WATCH: score >= 70, no hard fails OR near breakout
  else if (compositeScore >= 70 && hardFails.length === 0) {
    status = 'PRE_SPIKE_WATCH';
    setup = determineSetup(data, 'prespike');
    notes = 'Volume mulai masuk, tunggu breakout area entry.';
  }
  else if (compositeScore >= 65 && hardFails.length === 0 && data.distance_to_breakout_pct <= 3.0) {
    status = 'PRE_SPIKE_WATCH';
    setup = determineSetup(data, 'prespike');
    notes = 'Dekat breakout zone. Monitor volume confirmation.';
  }
  // WAIT_PULLBACK: score >= 60 but has overheat/RR/extended issues
  else if (compositeScore >= 60 && (hasOverheat || hasPoorRR || hasRiskFar)) {
    status = 'WAIT_PULLBACK';
    setup = hasOverheat ? 'Wait - Overheat' : (hasPoorRR ? 'Wait - Poor RR' : 'Wait - Risk Far');
    notes = generateWaitNotes(hardFails, hasOverheat, hasPoorRR, data);
  }
  // MOMENTUM_CONTINUATION: score >= 60, active, structurally valid
  else if (compositeScore >= 60 && data._priceAboveOpen && data.volume_ratio_20d >= 1.0) {
