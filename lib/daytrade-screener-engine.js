...
async function runDayTradeBatch(tickers, runMode, options) {
  ...
  for (var i = 0; i < tickers.length; i++) {
    ...
    try {
      ...
      // === V6: MULTI-TIMEFRAME CONTEXT (weekly/monthly as risk filter for Day Trade) ===
      var mtfCtx = idxTick.deriveMultiTimeframeContext(candles);
      scored.daily_candle_context = mtfCtx.daily_candle_context;
      scored.weekly_candle_context = mtfCtx.weekly_candle_context;
      scored.monthly_candle_context = mtfCtx.monthly_candle_context;
