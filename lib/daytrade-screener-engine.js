...
// ============================================================
// Kalender sesi IDX otoritatif (sama seperti tradingSchedule di
// lib/intraday-volume-pace.js):
//   Senin-Kamis: sesi 1 09:00-12:00, istirahat 12:00-13:30, sesi 2 13:30-16:00
//   Jumat:       sesi 1 09:00-11:30, istirahat 11:30-14:00, sesi 2 14:00-16:00
//   Sabtu/Minggu: bursa tutup.
// Mengembalikan status eksplisit:
//   { isOpen: boolean, session: 'PRE'|'SESSION_1'|'BREAK'|'SESSION_2'|'CLOSED', reason: string }
...
function getMarketSessionStatus(nowValue) {
  var now = nowValue == null ? new Date() : new Date(nowValue);
  if (Number.isNaN(now.getTime())) return { isOpen: false, session: 'CLOSED', reason: 'invalid_time' };
  var wibMs = now.getTime() + (7 * 60 * 60 * 1000);
  var wib = new Date(wibMs);
  var day = wib.getUTCDay();
  var totalMin = wib.getUTCHours() * 60 + wib.getUTCMinutes();
  if (day === 0 || day === 6) return { isOpen: false, session: 'CLOSED', reason: 'weekend' };
  var isFriday = (day === 5);
  var session1End = isFriday ? 690 : 720;
  var session2Start = isFriday ? 840 : 810;
  var openMin = 540;
  var closeMin = 960;
  if (totalMin < openMin) return { isOpen: false, session: 'PRE', reason: 'before_open' };
  if (totalMin < session1End) return { isOpen: true, session: 'SESSION_1', reason: isFriday ? 'friday_session_1' : 'session_1' };
  if (totalMin < session2Start) return { isOpen: false, session: 'BREAK', reason: isFriday ? 'friday_break' : 'lunch_break' };
  if (totalMin <= closeMin) return { isOpen: true, session: 'SESSION_2', reason: isFriday ? 'friday_session_2' : 'session_2' };
  return { isOpen: false, session: 'CLOSED', reason: 'after_close' };
}
