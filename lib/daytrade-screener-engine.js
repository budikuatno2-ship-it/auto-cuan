  ...
  for (var i = 0; i < tickers.length; i++) {
    ...
    try {
      ...
      if (scored.entry_low && scored.stop_loss && scored.tp1) {
        ...
        var refined = refineLevelsWithRespectZones(baseLevels, candles, analysis.last_price);
        if (refined) {
...
function refineLevelsWithRespectZones(baseLevels, candles, lastPrice) {
  ...
  if (isNaN(refined.risk_reward) || refined.risk_reward <= 0) valid = false;
  ...
  if (!valid) {
    ...
    return {
      ...
      tp2: baseLevels.tp2 || baseLevels.tp1,
      risk_reward: baseLevels.risk_reward,
      refinement_notes: 'Respect zone refinement skipped due to invalid level ordering.',
