...
function dayTradeEligibilityReason(row, options) {
  ...
  if (listing === 'NEW_LISTING' || listing === 'HISTORY_INSUFFICIENT') return listing.toLowerCase();
...
function filterDayTradeUniverse(rows, options) {
  ...
  (rows || []).forEach(function(row) {
    ...
    if (reason) { excludedByReason[reason] = (excludedByReason[reason] || 0) + 1; excluded.push({ ticker: row.ticker, reason: reason, board: row.board || null }); }
    else eligible.push(row);
  });
  return { tickers: eligible, diagnostics: { raw_universe_count: (rows || []).length, eligible_universe_count: eligible.length, excluded_count: excluded.length, excluded_by_reason: excludedByReason, sample_excluded: excluded.slice(0, 20) } };
}
