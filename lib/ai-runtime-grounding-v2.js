...
function addPerShareFacts(context) {
  ...
  facts.forEach((fact) => {
    ...
    // ponytail: never fall back to plans[index] to prevent cross-ticker metric contamination
    const plan = (factTicker && plansByTicker.get(factTicker)) || {};
    const entry = levelOf(plan, ['entryPriceIdr', 'entry_price', 'entry', 'avgPriceIdr']);
    const stop = levelOf(plan, ['stopLossIdr', 'stop_loss', 'stop', 'sl', 'stopLoss']);
