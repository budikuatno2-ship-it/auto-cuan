...
const { notifyClaim } = require('./subscription-voucher-claim');
const {
...
async function sendPlain(bot, chatId, text, options) {
  if (!bot || typeof bot.sendMessage !== 'function') return null;
  try { return await bot.sendMessage(chatId, text, options); } catch (_) { return null; }
}
...
function purchaseBaseUrl() {
  const configured = String(process.env.SUBSCRIPTION_PUBLIC_BASE_URL || '').trim().replace(/\/$/, '');
  return configured || 'https://autocuan.web.id';
}

function buyKeyboard() {
  const base = purchaseBaseUrl() + '/dashboard?buy=';
  return { inline_keyboard: [
    [{ text:'Premium 1 Bulan', url:base + 'PREMIUM_1_MONTH' }],
    [{ text:'Premium 2 Bulan', url:base + 'PREMIUM_2_MONTHS' }],
    [{ text:'Premium 3 Bulan', url:base + 'PREMIUM_3_MONTHS' }],
    [{ text:'Lifetime', url:base + 'LIFETIME' }]
  ] };
}
...
async function catalogMessage(db) {
  ...
  return [
    ...
    '',
    'Untuk paket berbayar, pilih paket di tombol di bawah. Checkout dilanjutkan di website dengan transfer manual dan akses aktif setelah dikonfirmasi admin.'
  ].join('\n');
}
...
async function handleVoucher(update, db, bot, input, rawCode) {
  ...
  await notifyClaim({ db, userId, voucherHash:hash, redeemed:redeemed.data, source:'telegram' });
...
async function handleUserCommand(update, db, bot, input) {
  ...
  await sendPlain(bot, input.chat.id, await catalogMessage(db), { reply_markup:buyKeyboard() });
  await cleanupOrigin(bot, input);
