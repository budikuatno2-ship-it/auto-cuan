...
// Schedule message deletion after delayMs (e.g. for /akses and OTP messages after expiration)
function scheduleMessageDeletion(chatId, messageId, delayMs) {
  ...
  const timer = setTimeout(async () => {
    try {
      await deleteMessage(Number(chatId), Number(messageId));
    } catch (_) {}
  }, delay);
...
async function createChatInviteLink(chatId, options) {
  ...
  const expireDate = options && options.expire_date != null
    ? options.expire_date
    : Math.floor(Date.now() / 1000) + ttl;
  const createsJoinRequest = options && options.creates_join_request !== undefined
