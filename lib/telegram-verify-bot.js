...
function getVerifyBotToken() {
  const t = runtimeEnv.resolve('TELEGRAM_VERIFY_BOT_TOKEN');
...
async function sendChatAction(chatId) {
  return await callTelegram('sendChatAction', { chat_id: chatId, action: 'typing' });
}
...
async function banChatMember(chatId, userId) { return callTelegram('banChatMember', { chat_id: chatId, user_id: userId }); }
async function unbanChatMember(chatId, userId) { return callTelegram('unbanChatMember', { chat_id: chatId, user_id: userId, only_if_banned: true }); }

// Download a proof only for an already-authorized server endpoint. The tokenized
// Telegram URL is never returned to a browser or logged.
async function downloadFile(fileId, maxBytes) {
  const info = await callTelegram('getFile', { file_id: fileId });
  if (!info || !/^[A-Za-z0-9_./-]{1,512}$/.test(String(info.file_path || ''))) throw botError('telegram_file_invalid');
  const token = getVerifyBotToken();
  let response;
  try { response = await fetch(`https://api.telegram.org/file/bot${token}/${info.file_path}`, { signal: AbortSignal.timeout(DEFAULT_TIMEOUT_MS) }); }
  catch (e) { throw botError('network_error'); }
  if (!response.ok) throw botError('telegram_file_error');
  ...
  if (Number.isFinite(declared) && declared > limit) throw botError('telegram_file_too_large');
  if (!response.body || typeof response.body.getReader !== 'function') throw botError('telegram_file_stream_unavailable');
  const reader = response.body.getReader(); const chunks = []; let total = 0;
  try {
    while (true) {
      const part = await reader.read(); if (part.done) break;
      total += part.value.byteLength;
      if (total > limit) { await reader.cancel(); throw botError('telegram_file_too_large'); }
      chunks.push(Buffer.from(part.value));
    }
  } catch (error) { try { await reader.cancel(); } catch (ignored) {} throw error; }
