...
// Provider-wide outage classification (E3).
//
...
// wz_model_temporarily_unavailable. The model ids are fine; the gateway is not.
//
// That verdict is only allowed on strong evidence, because it is instance-wide:
// the catalog must be healthy, the routes must be in it, there must be enough of
// them, they must span vendors, and every single one must carry the provider's
// OWN explicit temporary-unavailable code. A timeout, a 429, a generic 400 or
// any other status disqualifies the whole request — mixed evidence is not proof
// of a gateway-wide condition.
...
// Decides whether this request proves the gateway itself is temporarily down.
...
function providerWideOutageEvidence(attempts, directoryIds, directoryState) {
  ...
  // Tied to E1: without a healthy catalog we cannot tell a gateway outage from
  // model ids the provider no longer serves, so we refuse to guess.
  ...
  // means the evidence is mixed and this is not a gateway-wide condition.
...
// it must leave provider-wide state alone.
...
module.exports = async function handleContextAI(req, res) {
  ...
  try {
    ...
    // E4. A proven provider-wide outage outranks the generic negative cache: the
    ...
    // gateway-wide outage is never described as "every model refused or is full".
    ...
    if (outageEvidence.qualifies) {
      ...
      console.warn('context-ai provider wide outage', JSON.stringify({
        source, task,
