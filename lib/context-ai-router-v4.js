...
// F-006: the catalog was a hardcoded fallback candidate list, so changing which
// routes the provider serves meant editing code + redeploying. It is the same
// list of last-resort candidates, but now overridable in one place: set
// WEIZEROUTER_CATALOG (comma-separated) to replace the default without a code change.
...
function catalogFromEnv() {
  const env = split(process.env.WEIZEROUTER_CATALOG);
  return env.length ? env : DEFAULT_CATALOG;
}
...
function extractReply(data) {
  ...
  if (message && Array.isArray(message.content)) {
    const joined = message.content.map((p) => p && (p.text || p.content) || '').join('').trim();
...
function priceFreshness(raw, plans, prices) {
  ...
  const rawMeta = input.price_meta && typeof input.price_meta === 'object' ? input.price_meta : {};
  const meta = {};
  Object.keys(rawMeta).forEach((k) => {
    const norm = clean(k, 10).toUpperCase().replace(/\.JK$/i, '');
    if (norm) meta[norm] = rawMeta[k];
    meta[k] = rawMeta[k];
  });
...
function portfolioContext(raw) {
  ...
  const plans = (Array.isArray(input.plans) ? input.plans : []).slice(0, 25).map((p) => {
    ...
    return {
      ...
      // ponytail: alias stop_loss, sl, stop & capital fields; upgrade when portfolio schema v3 lands
...
function stockContext(raw) {
  ...
  return {
    // ponytail: support index symbols like ^JKSE along with 3-5 char tickers; upgrade when full globex supported
    ticker: /^(\^[A-Z]{4}|IHSG|[A-Z]{3,5})$/.test(ticker) ? ticker : '',
