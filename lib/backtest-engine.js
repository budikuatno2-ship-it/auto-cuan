 ...
 * @returns {object|null}
 */
function evaluateSignalAtDay({ ticker, candles, dayIndex, strategy = 'daytrade', board = null, runMode = 'MORNING_SCOUT', marketRegime = null }) {
  ...
  if (strategy === 'daytrade') {
    ...
    const scored = daytradeEngine.scoreDayTrade(analysis, runMode, board, null, { marketRegime, ticker });
    if (!scored) return null;

    // Filter tradeable signals with regime-adaptive calibration
    const tradeableStatuses = ['A_PLUS_SETUP', 'TRADE_CANDIDATE', 'READY_BREAKOUT', 'MOMENTUM_CONTINUATION'];
    const minScore = (scored.market_regime_category === 'bear') ? 75 : ((scored.market_regime_category === 'sideways') ? 70 : 65);
    const isTradeable = (tradeableStatuses.includes(scored.status) && scored.daytrade_score >= minScore) ||
      (scored.daytrade_score >= minScore && scored.status !== 'AVOID' && scored.status !== 'WAIT_PULLBACK' && scored.status !== 'SPECULATIVE');
    ...
    return {
      ...
      market_regime: scored.market_regime_category || marketRegime || 'sideways',
      raw: scored
    };
  }
  ...
  if (strategy === 'swing') {
    ...
    const swingVerified = swingEngine.verifySwingHighConviction(candidate, { marketRegime });
...
function runBacktest(options = {}) {
  ...
  for (const ticker of tickers) {
    ...
    for (let dayIndex = 20; dayIndex < sortedCandles.length - 1; dayIndex++) {
      ...
      // Determine market regime at day T using IHSG candles up to day T (strictly zero look-ahead)
      let regime = 'sideways';
      ...
      // Sinyal evaluation at day T with regime-adaptive calibration
