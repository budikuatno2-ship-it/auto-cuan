...
function getPatternPersonality(patternKey) {
  ...
  if (typeof patternKey === 'object') {
    if (patternKey.key && PATTERN_PERSONALITY_CATALOG[patternKey.key]) {
      return PATTERN_PERSONALITY_CATALOG[patternKey.key];
    }
  ...
  return PATTERN_PERSONALITY_CATALOG[cleanKey] || null;
}
...
function matchTickerPattern(r) {
  ...
  // 6. RSI_OVERBOUGHT_65P (PF 1.73): Momentum RSI >= 65
  if (rsi >= 65) {
