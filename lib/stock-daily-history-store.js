/**
 * Stock Daily History Store — batch-oriented Supabase access for
 * stock_daily_history / stock_daily_features / stock_fundamentals.
 *
 * Deliberately batch-first: screeners/API code should call
 * getLatestSessionsForTickers(supabase, tickers, count) ONCE per run and
 * build an in-memory map, not query per ticker. See project spec section 13
 * (performance requirements) and section 14 (feature snapshot cache).
 */

'use strict';

const { HISTORY_RETENTION_TRADING_SESSIONS } = require('./daily-market-context-constants');

function chunk(items, size) {
  var result = [];
  for (var i = 0; i < items.length; i += size) result.push(items.slice(i, i + size));
  return result;
}

const UPSERT_BATCH_SIZE = 200;
const SAFE_QUERY_ROW_BUDGET = 900;
// How many rows PAST the retention boundary enforceRetention fetches per
// ticker on each run, so a ticker with an existing backlog (e.g. accrued
// while the missing-.limit() bug below was live) drains down to the
// retention target over a handful of daily collector runs instead of at
// most 1 row/day (which the minimal retentionSessions+1 fetch would cause).
const RETENTION_TRIM_HEADROOM = 100;

function safeTicker(ticker) {
  if (!ticker && ticker !== 0) return '';
  return String(ticker).trim().replace(/\.JK$/i, '').toUpperCase().replace(/[^A-Z0-9._-]/g, '');
}

function sanitizeTradeDate(rawDate) {
  if (!rawDate) return null;
  var dateStr = typeof rawDate === 'string' ? rawDate.trim().slice(0, 10) : '';
  if (rawDate instanceof Date && !isNaN(rawDate.getTime())) {
    dateStr = new Date(rawDate.getTime() + 7 * 3600 * 1000).toISOString().slice(0, 10);
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) return null;
  var parts = dateStr.split('-');
  var y = parseInt(parts[0], 10), m = parseInt(parts[1], 10), d = parseInt(parts[2], 10);
  var dt = new Date(Date.UTC(y, m - 1, d));
  if (dt.getUTCFullYear() !== y || dt.getUTCMonth() !== m - 1 || dt.getUTCDate() !== d) return null;
  // ponytail: WIB date check with 2026 floor so static test fixtures in 2026 pass on older system clocks
  var todayWIB = new Date(Date.now() + 7 * 3600 * 1000).toISOString().slice(0, 10);
  var maxDate = todayWIB < '2026-12-31' ? '2026-12-31' : todayWIB;
  return dateStr <= maxDate ? dateStr : null;
}

function isValidCandle(row) {
  if (!row) return false;
  var isPos = function(v) { return v !== null && v !== undefined && Number.isFinite(Number(v)) && Number(v) > 0; };
  if (!isPos(row.close)) return false;
  if (row.open !== undefined && !isPos(row.open)) return false;
  if (row.high !== undefined && !isPos(row.high)) return false;
  if (row.low !== undefined && !isPos(row.low)) return false;
  if (row.volume !== undefined && row.volume !== null && (!Number.isFinite(Number(row.volume)) || Number(row.volume) < 0)) return false;
  return true;
}

/**
 * Idempotent upsert of daily history rows. Conflict key is (ticker, trade_date).
 * rows: [{ ticker, trade_date, open, high, low, close, previous_close, volume,
 *          value, frequency, foreign_buy_value, foreign_sell_value,
 *          foreign_net_value, foreign_buy_lot, foreign_sell_lot,
 *          foreign_net_lot, data_source, source_timestamp, data_quality_status }]
 */
async function upsertDailyHistory(supabase, rows) {
  if (!supabase) throw new Error('supabase client is required');
  var deduped = new Map();
  for (var row of (rows || [])) {
    var ticker = safeTicker(row && row.ticker);
    var tradeDate = sanitizeTradeDate(row && row.trade_date);
    if (!ticker || !tradeDate || !isValidCandle(row)) continue;
    deduped.set(ticker + '_' + tradeDate, Object.assign({}, row, {
      ticker: ticker,
      trade_date: tradeDate,
      updated_at: new Date().toISOString()
    }));
  }

  var upserted = 0;
  for (var batch of chunk(Array.from(deduped.values()), UPSERT_BATCH_SIZE)) {
    var result = await supabase
      .from('stock_daily_history')
      .upsert(batch, { onConflict: 'ticker,trade_date' });
    if (result.error) throw new Error('Upsert stock_daily_history gagal: ' + result.error.message);
    upserted += batch.length;
  }
  return upserted;
}

/**
 * Delete history rows older than the retention window, per ticker.
 * Keeps the most recent `retentionSessions` rows for each ticker in `tickers`.
 *
 * Bounded exactly like getLatestSessionsForTickers below, for the same
 * reason: this lookup query previously had NO `.limit()` at all, so on a
 * deployment where PostgREST caps a single response around ~1,000 rows (the
 * same cap getLatestSessionsForTickers's docstring already documents),
 * fetching an unbounded "all history for up to 50 tickers, ordered by
 * date" response would silently get truncated to mostly-recent rows,
 * making every ticker's observed row count look far SHORTER than its real
 * persisted history. That silently defeats the entire function: the
 * deletion loop below only removes rows past index `retentionSessions` in
 * what it actually fetched, so if truncation already cut every ticker's
 * batch below `retentionSessions`, nothing is ever deleted and
 * stock_daily_history grows unbounded instead of being pruned to the
 * documented retention window (confirmed bug — retention was silently a
 * no-op across the ticker universe on any deployment large enough to hit
 * the response cap).
 */
async function enforceRetention(supabase, tickers, retentionSessions) {
  if (!supabase) throw new Error('supabase client is required');
  retentionSessions = retentionSessions || HISTORY_RETENTION_TRADING_SESSIONS;
  var uniqueTickers = Array.from(new Set((tickers || []).map(safeTicker).filter(Boolean)));
  if (!uniqueTickers.length) return 0;
  var deleted = 0;
  // +1: enough to observe at least one row past the retention boundary, so
  // a ticker with a large pre-existing backlog (e.g. from this very bug)
  // still gets trimmed by RETENTION_TRIM_HEADROOM rows on IMMEDIATE READ,
  // converging fully over a few daily collector runs rather than needing
  // a single unbounded query.
  var perTickerFetchCount = retentionSessions + RETENTION_TRIM_HEADROOM;
  var tickerBatchSize = Math.max(1, Math.floor(SAFE_QUERY_ROW_BUDGET / perTickerFetchCount));

  for (var tickerBatch of chunk(uniqueTickers, tickerBatchSize)) {
    var fetchLimit = Math.max(perTickerFetchCount * tickerBatch.length, tickerBatch.length);
    var lookup = await supabase
      .from('stock_daily_history')
      .select('id,ticker,trade_date')
      .in('ticker', tickerBatch)
      .order('trade_date', { ascending: false })
      .limit(fetchLimit);
    if (lookup.error) throw new Error('Retention lookup gagal: ' + lookup.error.message);

    var byTicker = new Map();
    for (var row of lookup.data || []) {
      if (!byTicker.has(row.ticker)) byTicker.set(row.ticker, []);
      byTicker.get(row.ticker).push(row);
    }

    var deleteIds = [];
    for (var tickerRows of byTicker.values()) {
      for (var i = retentionSessions; i < tickerRows.length; i++) deleteIds.push(tickerRows[i].id);
    }

    for (var idBatch of chunk(deleteIds, 200)) {
      var removal = await supabase.from('stock_daily_history').delete().in('id', idBatch);
      if (removal.error) throw new Error('Retention delete gagal: ' + removal.error.message);
      deleted += idBatch.length;
    }
  }
  return deleted;
}

/**
 * Batch-load the latest `count` sessions for a set of tickers, returning a
 * Map<ticker, rows[]> with rows sorted newest-first.
 *
 * Do NOT request count * the whole universe in a single response. PostgREST
 * deployments commonly cap one response around 1,000 rows; with ~771 tickers
 * that silently truncated the history input and left most RSI values N/A.
 * Chunking by a row budget keeps the requests bounded while guaranteeing the
 * requested retained window can fit in each response.
 */
async function getLatestSessionsForTickers(supabase, tickers, count) {
  if (!supabase) throw new Error('supabase client is required');
  var uniqueTickers = Array.from(new Set((tickers || []).map(safeTicker).filter(Boolean)));
  if (!uniqueTickers.length) return new Map();

  count = Math.max(1, Number(count) || 1);
  var map = new Map();

  // ponytail: query per ticker to prevent cross-ticker starvation ceiling; upgrade to custom RPC if universe > 100
  for (var ticker of uniqueTickers) {
    var result = await supabase
      .from('stock_daily_history')
      .select('ticker,trade_date,open,high,low,close,previous_close,volume,value,frequency,' +
        'foreign_buy_value,foreign_sell_value,foreign_net_value,foreign_buy_lot,foreign_sell_lot,foreign_net_lot,' +
        'data_source,data_quality_status')
      .in('ticker', [ticker])
      .order('trade_date', { ascending: false })
      .limit(count);

    if (result.error) throw new Error('Load stock_daily_history gagal: ' + result.error.message);

    var rows = (result.data || []).slice(0, count).map(function(row) {
      return Object.assign({}, row, { ticker: ticker });
    });
    if (rows.length) map.set(ticker, rows);
  }

  return map;
}

/** Single-ticker convenience wrapper. */
async function getLatestSessionsForTicker(supabase, ticker, count) {
  var clean = safeTicker(ticker);
  var map = await getLatestSessionsForTickers(supabase, [clean], count);
  return map.get(clean) || [];
}

/** Batch upsert of precomputed feature snapshots (one row per ticker). */
async function upsertDailyFeatures(supabase, rows) {
  if (!supabase) throw new Error('supabase client is required');
  // Defense-in-depth: stock_daily_features.as_of_trade_date is NOT NULL.
  // The preferred safety layer is upstream (daily-market-context-builder's
  // buildFeatureSnapshotsForTickers already skips tickers with no history),
  // but this guard ensures one malformed row can never poison an otherwise
  // valid batch upsert or throw away good tickers' data.
  var validRows = (rows || []).filter(function(row) {
    var valid = !!(row && safeTicker(row.ticker) && row.as_of_trade_date);
    if (!valid && typeof console !== 'undefined' && console.warn) {
      console.warn('[stock-daily-history-store] Dropping invalid feature row (missing ticker/as_of_trade_date): ' +
        JSON.stringify({ ticker: row && row.ticker, as_of_trade_date: row && row.as_of_trade_date }));
    }
    return valid;
  });

  var normalized = validRows.map(function(row) {
    return Object.assign({}, row, {
      ticker: safeTicker(row.ticker),
      updated_at: new Date().toISOString()
    });
  });
  var upserted = 0;
  for (var batch of chunk(normalized, UPSERT_BATCH_SIZE)) {
    var result = await supabase.from('stock_daily_features').upsert(batch, { onConflict: 'ticker' });
    if (result.error) throw new Error('Upsert stock_daily_features gagal: ' + result.error.message);
    upserted += batch.length;
  }
  return upserted;
}

/** Batch fetch of precomputed feature snapshots — ONE query for N tickers. */
async function getDailyFeaturesForTickers(supabase, tickers) {
  if (!supabase) throw new Error('supabase client is required');
  var uniqueTickers = Array.from(new Set((tickers || []).map(safeTicker).filter(Boolean)));
  if (!uniqueTickers.length) return new Map();

  var result = await supabase.from('stock_daily_features').select('*').in('ticker', uniqueTickers);
  if (result.error) throw new Error('Load stock_daily_features gagal: ' + result.error.message);

  var map = new Map();
  for (var row of result.data || []) map.set(row.ticker, row);
  return map;
}

/**
 * Batch fetch of the ENTIRE stock_daily_features cache — the data source for
 * the "Ranking Harian" table, which needs to compare/sort across the whole
 * ticker universe rather than a caller-supplied list. One query regardless
 * of universe size (bounded by `limit`).
 */
async function getAllDailyFeatures(supabase, options) {
  if (!supabase) throw new Error('supabase client is required');
  options = options || {};
  var limit = options.limit || 1000;

  var result = await supabase
    .from('stock_daily_features')
    .select('*')
    .order('ticker', { ascending: true })
    .limit(limit);
  if (result.error) throw new Error('Load stock_daily_features (all) gagal: ' + result.error.message);
  return result.data || [];
}

async function getFundamentalsForTickers(supabase, tickers) {
  if (!supabase) throw new Error('supabase client is required');
  var uniqueTickers = Array.from(new Set((tickers || []).map(safeTicker).filter(Boolean)));
  if (!uniqueTickers.length) return new Map();

  var result = await supabase.from('stock_fundamentals').select('*').in('ticker', uniqueTickers);
  if (result.error) throw new Error('Load stock_fundamentals gagal: ' + result.error.message);

  var map = new Map();
  for (var row of result.data || []) map.set(row.ticker, row);
  return map;
}

module.exports = {
  upsertDailyHistory,
  enforceRetention,
  getLatestSessionsForTickers,
  getLatestSessionsForTicker,
  upsertDailyFeatures,
  getDailyFeaturesForTickers,
  getAllDailyFeatures,
  getFundamentalsForTickers
};
