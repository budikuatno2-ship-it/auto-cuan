...
function evaluateGate(bundle, aggregate, policy, dateAlignment, providerCacheQuality) {
  ...
  if (policy) {
    // Provider-bad policy decisions are retained as diagnostics but do not
    // block the validation-only gate once the provider quality report proves
    // every such ticker is explicitly quarantined daily-only.
    if (numberOrZero(policy.decision_counts && policy.decision_counts.BLOCK_PRODUCTION_ENABLE) > 0 && !qualityAllowsQuarantinePass) {
...
// NOTE: recommendationForStatus is defined once, above buildGateReport. The
// duplicate definition that used to live here was removed (Batch 16) — it
// silently shadowed the first one and was a maintenance hazard.
