...
function evaluateGate(bundle, aggregate, policy, dateAlignment) {
  ...
  const qTickers = quarantineTickers(bundle);
  const hasQuarantine = qTickers.size > 0;

  // Provider metrics from bundle (latest sample)
  if (bundle) {
    const nonQProviderMissing = nonQuarantinedCount(bundle, 'non_quarantined_provider_missing_count', bundle.provider_missing_count);
  ...
  if (bundle && bundle.data_quality_counts) {
    ...
    const nonQNoData = nonQuarantinedCount(bundle, 'non_quarantined_provider_missing_count', dq.NO_INTRADAY_DATA);
    const nonQIncomplete = nonQuarantinedCount(bundle, 'non_quarantined_incomplete_intraday_count', dq.INCOMPLETE_INTRADAY);
    if (nonQNoData > 0) {
  ...
  if (bundle && bundle.priority_label_counts) {
    const nonQUnknown = nonQuarantinedCount(bundle, 'non_quarantined_intraday_unknown_count', bundle.priority_label_counts.INTRADAY_UNKNOWN);
    if (nonQUnknown > 0) {
  ...
  if (bundle && bundle.confirmation_label_counts) {
    const nonQUnknown = nonQuarantinedCount(bundle, 'non_quarantined_intraday_unknown_count', bundle.confirmation_label_counts.INTRADAY_UNKNOWN);
    if (nonQUnknown > 0) {
  ...
  if (aggregate) {
    ...
    const recurringIncomplete = aggregate.repeated_incomplete_intraday_tickers || [];
    if (asArray(recurringIncomplete).length > 0) {
      blockReasons.push('recurring incomplete_intraday exists in aggregate');
    }
  }

  if (hasQuarantine) warnReasons.push('PASS_WITH_QUARANTINE for dry-run/reporting only; quarantined tickers remain DAILY_SCORE_ONLY and production flag stays off');
  ...
  if (warnReasons.length > 0) {
    return { blockReasons, warnReasons, gateStatus: hasQuarantine && blockReasons.length === 0 ? 'PASS_WITH_QUARANTINE' : 'WARN' };
  }
...
function buildGateReport(bundle, aggregate, policy, opts) {
  ...
  return {
    ...
    provider_metrics: {
      ...
      non_quarantined_provider_missing_count: bundle ? nonQuarantinedCount(bundle, 'non_quarantined_provider_missing_count', providerMissingCount) : 0
    },
    data_quality_summary: {
      ...
      non_quarantined_incomplete_intraday_count: bundle ? nonQuarantinedCount(bundle, 'non_quarantined_incomplete_intraday_count', dataQualityCounts.INCOMPLETE_INTRADAY) : 0,
      non_quarantined_intraday_unknown_count: bundle ? nonQuarantinedCount(bundle, 'non_quarantined_intraday_unknown_count', numberOrZero(priorityLabelCounts.INTRADAY_UNKNOWN) + numberOrZero(confirmationLabelCounts.INTRADAY_UNKNOWN)) : 0,
      intraday_quarantine_count: bundle ? numberOrZero(bundle.intraday_quarantine_count) : 0,
      intraday_quarantine_tickers: bundle ? asArray(bundle.intraday_quarantine_tickers) : []
    },
    policy_summary: {
