...
async function generateNarration(type, data, options) {
  ...
  // 2. Check API keys (GEMINI_API_KEY_PRIMARY only — no fallback to legacy keys)
  ...
  const validationOptions = Object.assign({}, options || {}, { notificationType: type });
  const validation = narrationValidator.validate(aiText, data, validationOptions);
  if (!validation.valid) {
    return {
      text: null,
      source: 'fallback',
