...
async function generateNarration(type, data, options) {
  ...
  // 2. Check API keys (GEMINI_API_KEY_PRIMARY preferred; falls back to GEMINI_API_KEY)
  const primaryKey = (process.env.GEMINI_API_KEY_PRIMARY || process.env.GEMINI_API_KEY || '').trim();
  if (!primaryKey) {
...
/**
 * Diagnostic: Get AI narration configuration status.
 * Safe for JSON response — never exposes actual key values.
 * @returns {object}
 */
function getNarrationConfigStatus() {
  ...
  const backupKey = (process.env.GEMINI_API_KEY_BACKUP || '').trim();
  const resolvedKey = primaryKey || legacyKey;
  return {
    enabled: isNarrationEnabled(),
    has_primary_key: !!primaryKey,
    has_legacy_key: !!legacyKey,
    resolved_key_source: primaryKey ? 'GEMINI_API_KEY_PRIMARY' : (legacyKey ? 'GEMINI_API_KEY' : 'none'),
    has_backup_key: !!backupKey,
    model: getModel(),
    timeout_ms: getTimeoutMs(),
    cache_ttl_ms: narrationCache.getCacheTtlMs(),
    cache_size: narrationCache.size()
  };
}
