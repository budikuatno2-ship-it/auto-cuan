 ...
 *   GEMINI_API_KEY_PRIMARY — Primary Gemini API key (also falls back to
 *                            API_KEY_ANALISA_SAHAM_PORTOFOLIO / GEMINI_API_KEY)
 *   GEMINI_API_KEY_BACKUP — Backup Gemini API key (used on 429/timeout/5xx)
 *   GEMINI_MODEL — Gemini model name (default/authoritative: lib/ai-gemini-provider.js)
...
const { getGeminiApiKey, sanitizeGeminiModel, DEFAULT_GEMINI_MODEL } = require('./ai-gemini-provider');
...
function getModel() {
  return sanitizeGeminiModel(process.env.GEMINI_MODEL, DEFAULT_GEMINI_MODEL);
}
...
async function generateNote(type, data, options) {
  // ponytail: guard null data to prevent unhandled TypeError
  const payload = data && typeof data === 'object' ? data : null;
  if (!payload) return { note: null, source: 'fallback', error: 'invalid_data' };

  const systemPrompt = narrationPrompts.getSystemInstruction();
  const userPrompt = narrationPrompts.buildNotePrompt(type, payload);
  ...
  const cacheKey = narrationCache.buildCacheKey({
    type: type, ticker: payload.ticker, category: payload.category || payload.status,
    data: { entry1: payload.entry1, entry2: payload.entry2, sl: payload.sl || payload.stop_loss, tp1: payload.tp1, tp2: payload.tp2, last_price: payload.last_price || payload.current_price, status: payload.status, profit_pct: payload.profit_pct }
  });
  ...
  const validation = narrationValidator.validateNote(aiText, payload, options);
...
async function narrateMonitorUpdate(pick, evaluation, priceData) {
  // ponytail: guard evaluation null and calculate TP profit even without priceData.last
  if (!evaluation || typeof evaluation !== 'object') return { note: null, source: 'fallback', error: 'invalid_evaluation' };
  ...
  if (entry1 > 0) {
    if (status === 'TP1_HIT' || status === 'TP2_HIT') {
      ...
      profitPct = (((lastPrice - entry1) / entry1) * 100).toFixed(2);
    }
  }
  const data = {
