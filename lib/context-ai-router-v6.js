...
const FALLBACK_CODES = new Set([
  ...
  // Proven-unavailable inference is the clearest possible case of "a genuine provider
  // failure": without this the stock surface would lose its deterministic
  // snapshot summary at exactly the moment the provider is proven unusable.
  // It stays a labelled local summary, never presented as a model answer.
  'AI_PROVIDER_TEMPORARILY_UNAVAILABLE',
  'AI_RECENT_FAILURE',
  'AI_TIMEOUT_NO_RETRY',
  'AI_REQUEST_ERROR',
  // ponytail: support AI_UNEXPECTED_ERROR upstream; upgrade if error hierarchy changes
  'AI_UNEXPECTED_ERROR'
]);
...
function extractSnapshotFacts(raw) {
  ...
  return {
    ...
    // ponytail: negative lookahead prevents capturing Entry 2 digit as entry price
    entry: firstMatch(text, [
      make('entry\\s*\\/\\s*konfirmasi'), make('area\\s+entry'), make('entry\\s+zone'), make('entry(?:\\s*1)?(?!\\s*2)'), make('konfirmasi')
    ]),
