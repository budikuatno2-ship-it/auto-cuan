...
async function handleContextAIV7(req, res) {
  ...
  if (!isGeminiDisabled && geminiApiKey) {
    ...
    if (isStreaming) {
      ...
      // Attempt 4 (Safety Net): Try stable gemini-2.5-flash if all previous failed
      const attempt4Timeout = !geminiResult ? nextGeminiTimeout(handlerStarted) : null;
      if (!geminiResult && attempt4Timeout != null && primaryModel !== 'gemini-2.5-flash' && fallbackModel !== 'gemini-2.5-flash') {
        try {
          console.warn('[ContextAI] Trying stable gemini-2.5-flash fallback...');
          let accumulatedSafe = '';
          geminiResult = await streamGeminiAnalysis({
            prompt: message,
            systemInstruction,
    ...
    try {
      ...
      if (!geminiResult && attempt4Timeout != null && primaryModel !== 'gemini-2.5-flash' && fallbackModel !== 'gemini-2.5-flash') {
        try {
          console.warn('[ContextAI] Trying stable gemini-2.5-flash non-streaming fallback...');
          geminiResult = await generateGeminiContent({
            prompt: message,
            systemInstruction,
            apiKey: geminiApiKey || fallbackApiKey,
