...
async function handleContextAIV7(req, res) {
  ...
  // If running in a test harness that explicitly configures an OpenAI-compatible/Weize endpoint
  // via PORTFOLIO_AI_BASE_URL (test/ai-provider-failover, test/ai-provider-outage-spillover, etc.),
  // delegate directly to V6 to exercise the multi-model failover contract.
  const isTestHarness = process.env.NODE_ENV === 'test' || process.env.TEST_HARNESS === '1' || process.env.AI_TEST_HARNESS === '1';
  if (isTestHarness && process.env.PORTFOLIO_AI_BASE_URL && process.env.PORTFOLIO_AI_API_KEY && !isGeminiDisabled) {
    recordLocalFallback();
    const handlerStarted = Date.now();
