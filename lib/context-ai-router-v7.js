...
async function handleContextAIV7(req, res) {
  ...
  try {
    ...
    message = clean(body.chatMessage, 2500);
    if (!message) {
      return res.status(400).json({ success: false, code: 'AI_EMPTY_QUESTION', error: 'Pertanyaan belum diisi.' });
    }

    context = source === 'stock_analysis_followup'
      ? Object.assign({ status: body.context && body.context.status }, handleContextAIV4._test.stockContext(Object.assign({}, body.context, body.ticker ? { ticker: body.ticker } : {})))
      : handleContextAIV4._test.portfolioContext(body.context);
    ...
    const ticker = (context && context.ticker) || (body.context && body.context.ticker) || (body.ticker)
      ? String((context && context.ticker) || (body.context && body.context.ticker) || (body.ticker)).toUpperCase()
      : null;
  ...
  if (isStreaming) {
    ...
    console.error('handleContextAIV7 unexpected error:', unexpectedErr);
    if (res.headersSent) {
      try { res.end(); } catch (_) {}
      return;
    }
