...
const { getGeminiApiKey, generateGeminiContent, streamGeminiAnalysis, sanitizeGeminiModel, DEFAULT_GEMINI_MODEL, FALLBACK_GEMINI_MODEL } = require('./ai-gemini-provider');
...
async function handleContextAIV7(req, res) {
  ...
  if (!isGeminiDisabled && geminiApiKey) {
    ...
    try {
      ...
      if (!geminiResult && attempt4Timeout != null && primaryModel !== 'gemini-3.6-flash' && fallbackModel !== 'gemini-3.6-flash') {
        try {
          console.warn('[ContextAI] Trying stable gemini-3.6-flash non-streaming fallback...');
          geminiResult = await generateGeminiContent({
            prompt: message,
            systemInstruction,
            apiKey: geminiApiKey || fallbackApiKey,
