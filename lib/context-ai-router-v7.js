...
const { getGeminiApiKey, generateGeminiContent, streamGeminiAnalysis, sanitizeGeminiModel, DEFAULT_GEMINI_MODEL, FALLBACK_GEMINI_MODEL, SAFETY_NET_GEMINI_MODEL } = require('./ai-gemini-provider');
...
function buildCacheParams(identity, source, context, styleRules) {
  ...
  // The fingerprint is taken from the context EXACTLY as supplied. Normalising
  // here would collapse two portfolios that merely differ in fields this router
  // never reads (cash, holdings) onto one digest, which is the cross-user leak
  // this key exists to prevent. Callers that want the normalised shape must
...
// Entries written before the fingerprint was taken from the RAW portfolio were
// keyed on the NORMALISED shape instead (portfolioContext() drops fields such
// as cash/holdings). This alias is READ-ONLY and only consulted after the
// strict key misses, so those pre-existing entries stay reachable without ever
// being what a new write lands on — a new write always uses the raw digest,
// which is what keeps two distinct portfolios apart.
function legacyCacheParams(identity, source, rawContext, styleRules) {
  if (source !== 'portfolio_chat' || !rawContext || typeof rawContext !== 'object') return null;
  const normalised = handleContextAIV4._test.portfolioContext(rawContext);
  if (!normalised) return null;
  return buildCacheParams(identity, source, normalised, styleRules);
}
...
async function handleContextAIV7(req, res) {
  ...
  // ponytail: resolve market date from context, caller quota wibDate, or current jakarta date
  const marketDate = (context && context.as_of_trade_date) ||
    (context && context.captured_at ? String(context.captured_at).slice(0, 10) : null) ||
    (req && req._aiQuota && req._aiQuota.wibDate) ||
  ...
  const styleRules = clean(body.styleRules, 3000);
  const cacheIdentity = { ticker, analysisType, prompt: message, marketDate };
  // Fingerprint the portfolio EXACTLY as the caller supplied it. Normalising
  // first would collapse two portfolios differing only in fields this router
  // never reads (cash, holdings) onto one digest — the cross-user leak this key
  // exists to prevent. `context` above is already the normalised copy used for
  // the prompt; the raw body payload is the correct basis for cache identity.
  const rawContext = (body.context && typeof body.context === 'object') ? body.context : context;
  const cacheParams = buildCacheParams(cacheIdentity, source, rawContext, styleRules);
  const legacyParams = legacyCacheParams(cacheIdentity, source, rawContext, styleRules);

  // 1. Check Database Response Cache
  let cached = await getCachedAnalysis(Object.assign({}, cacheParams));
  // Only if the strict key misses, try the pre-existing normalised-key entries.
  if (!cached && legacyParams) {
    cached = await getCachedAnalysis(Object.assign({}, legacyParams));
  }
  ...
  if (cached) {
    ...
    // ponytail: overwrite cached user quota with live requester quota or drop it
    const responsePayload = Object.assign({}, cached, {
      success: true,
    ...
    if (req && req._aiQuota) {
      responsePayload.quota = req._aiQuota;
    } else {
      delete responsePayload.quota;
    }
  ...
  if (!isGeminiDisabled && geminiApiKey) {
    ...
    const primaryModel = sanitizeGeminiModel(process.env.GEMINI_MODEL, DEFAULT_GEMINI_MODEL);
    const fallbackModel = sanitizeGeminiModel(process.env.GEMINI_FALLBACK_MODEL, FALLBACK_GEMINI_MODEL);
    ...
    if (isStreaming) {
      ...
      // Attempt 1: Primary key + primary model (authoritative: DEFAULT_GEMINI_MODEL)
      ...
      // Attempt 2: If primary model failed, try fallback model (FALLBACK_GEMINI_MODEL)
      ...
      if (!geminiResult && fallbackModel && fallbackModel !== primaryModel && attempt2Timeout != null) {
        try {
          ...
          if (chunksSent > 0) {
            sendSSEChunk(res, { reset: true, action: 'clear' });
            chunksSent = 0;
          }
          let accumulated = '';
          geminiResult = await streamGeminiAnalysis({
      ...
      if (!geminiResult && attempt4Timeout != null && primaryModel !== SAFETY_NET_GEMINI_MODEL && fallbackModel !== SAFETY_NET_GEMINI_MODEL) {
        try {
          console.warn('[ContextAI] Trying safety-net model ' + SAFETY_NET_GEMINI_MODEL + ' fallback...');
          if (chunksSent > 0) {
            sendSSEChunk(res, { reset: true, action: 'clear' });
            chunksSent = 0;
          }
    ...
    try {
      ...
      if (!geminiResult && attempt4Timeout != null && primaryModel !== SAFETY_NET_GEMINI_MODEL && fallbackModel !== SAFETY_NET_GEMINI_MODEL) {
        try {
          console.warn('[ContextAI] Trying safety-net model ' + SAFETY_NET_GEMINI_MODEL + ' (non-streaming) fallback...');
          geminiResult = await generateGeminiContent({
            prompt: message,
            systemInstruction,
            apiKey: geminiApiKey || fallbackApiKey,
