...
function buildCacheParams(identity, source, context, styleRules) {
  ...
  // ponytail: ensure idempotent portfolio context normalization for cache fingerprint
  const normalizedContext = source === 'portfolio_chat' && context
    ? handleContextAIV4._test.portfolioContext(context)
    : context;
  const extra = contextFingerprint(source, normalizedContext, styleRules);
