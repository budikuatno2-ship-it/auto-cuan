...
async function handleContextAIV7(req, res) {
  ...
  if (!isGeminiDisabled && geminiApiKey) {
    ...
    const fallbackModel = process.env.GEMINI_FALLBACK_MODEL || FALLBACK_GEMINI_MODEL;
    ...
    if (isStreaming) {
      ...
      if (!geminiResult && fallbackModel && fallbackModel !== primaryModel) {
        try {
      ...
      if (!geminiResult && fallbackApiKey && fallbackApiKey !== geminiApiKey) {
        try {
