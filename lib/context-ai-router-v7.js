...
async function handleContextAIV7(req, res) {
  ...
  const geminiApiKey = (body && body.apiKey) || (req.body && req.body.apiKey) || getGeminiApiKey();
  ...
  if (!isGeminiDisabled && geminiApiKey) {
    ...
    if (isStreaming) {
      ...
      try {
        ...
        if (fallbackApiKey && fallbackApiKey !== geminiApiKey) {
          try {
            console.warn('[ContextAI] Primary stream failed, trying fallback key...');
      ...
      if (geminiResult) {
        ...
        recordLocalFallback();
        console.warn('Gemini stream failed, degrading to local deterministic reply.');
    ...
    try {
      ...
      try {
        ...
        if (fallbackApiKey && fallbackApiKey !== geminiApiKey) {
          console.warn('[ContextAI] Primary call failed, trying fallback key...');
          geminiResult = await generateGeminiContent({
            prompt: message,
            systemInstruction,
            apiKey: fallbackApiKey,
