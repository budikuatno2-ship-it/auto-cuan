...
async function showProcessing(chatId) {
  if (typeof bot.sendChatAction === 'function') await bot.sendChatAction(chatId).catch(function () {});
}
function isMembershipUpdate(update) {
  ...
  return ['Verifikasi Akun', 'Akun Saya', 'Paket', 'Paket Aktif', 'Beli Paket', 'Beli / Perpanjang', 'Gunakan Voucher', 'Akses Channel', 'Cara Pakai', 'Hubungi Admin'].includes(text);
}
...
async function processUpdate(update, deps) {
  ...
  if (command === '/unknown') { await respond(chatId, 'Perintah tidak dikenal. Gunakan /menu atau /bantuan untuk melihat pilihan yang aman.'); return 'unknown_command'; }
  ...
  if (file && update.message.caption) {
    ...
    await respond(chatId, 'Bukti diterima dan menunggu tinjauan admin. Pembayaran belum diverifikasi otomatis.');
  ...
  return 'locked';
    ...
    await respond(chatId, 'Maaf, proses belum selesai. Coba kirim perintah sekali lagi.').catch(function () {});
    await db.releaseUpdate(update.update_id).catch(function () {});
    throw error;
  }
}

module.exports = { processUpdate, processChannelJoinRequest, isMembershipUpdate, accountMessage, verificationCodeFromCommand, canonicalCommand, showProcessing };