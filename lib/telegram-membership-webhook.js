...
function cleanPackageName(row) {
  if (!row) return 'Paket';
  if (row.slug === 'channel-30' || row.packageSlug === 'channel-30') return 'Bot 30 Hari';
  if (row.slug === 'channel-90' || row.packageSlug === 'channel-90') return 'Bot 90 Hari';
  if (row.slug === 'lifetime' || row.packageSlug === 'lifetime') return 'Lifetime';
  if (row.slug === 'channel-addon-30' || row.packageSlug === 'channel-addon-30') return 'Perpanjangan Channel 30 Hari';
  return row.name || row.packageName || 'Paket';
}
...
function voucherPromptText(row) {
  return ['🎟️ MASUKKAN KODE VOUCHER','',`Paket: ${cleanPackageName(row)}`,`Harga awal: ${money(row.price_idr || row.priceIdr)}`,'','Balas pesan ini dengan kode voucher. Sesi berlaku 10 menit.'].join('\n');
}
function voucherPromptMarkup(row) {
  const slug=row.slug||row.packageSlug;
  return { inline_keyboard:[
    [{ text:'⬅️ Kembali', callback_data:`checkout:${slug}` },{ text:'❌ Batalkan', callback_data:'cancel:checkout' }]
  ] };
}
...
function isMembershipUpdate(update) {
  ...
  return ['Verifikasi Akun','Akun Saya','Paket','Paket Aktif','Beli Paket','Beli / Perpanjang','Gunakan Voucher','Akses Channel','Cara Pakai','Hubungi Admin','Batalkan','Kembali'].includes(text);
}
...
function accountMessage(account) {
  ...
  return ['👤 AKUN SAYA','','Akun: ' + String(account && (account.maskedUsername || account.username) || 'Tidak tersedia'),'Status: ' + (account && account.blocked ? 'Diblokir — hubungi admin' : account && account.accountStatus === 'pending' ? 'Menunggu aktivasi' : 'Aktif'),'Telegram: ' + (account && account.verified ? 'Terverifikasi' : 'Belum terverifikasi'),'Paket: ' + String(entitlement && entitlement.packageName || 'Tidak aktif'),'Berakhir: ' + (entitlement && entitlement.lifetime ? 'Lifetime' : String(entitlement && entitlement.endsAt || '-')),'Akses channel: ' + (account && account.channelAccess ? 'Aktif' : 'Tidak aktif'),'Akses dashboard: ' + (account && account.dashboardAccess ? 'Aktif' : 'Tidak aktif'),'Pembayaran: ' + (pending ? 'Menunggu tinjauan admin' : 'Tidak ada yang menunggu')].join('\n');
}
...
async function createAndShowPurchase(chatId,id,db,packageRow,voucherCode) {
  const purchase=voucherCode===undefined?await db.createPurchase(id,packageRow.id,null):await db.checkoutPurchase(id,voucherCode);
...
async function processUpdate(update, deps) {
  ...
  try {
    ...
    if(callback){
      ...
      if(action==='nav'&&value==='menu'){await db.cancelCheckout(id).catch(function(){});const acct=await db.account(id);await respond(chatId,'🏠 Menu Keanggotaan Auto-Cuan',core.menuKeyboard(acct));return 'menu';}
      if(action==='nav'&&value==='packages'){await db.cancelCheckout(id).catch(function(){});return showPackages(chatId,db);}
      if(action==='cancel'&&value==='checkout'){await db.cancelCheckout(id).catch(function(){});await respond(chatId,'❌ Proses dibatalkan. Tidak ada pesanan baru yang dibuat.',{inline_keyboard:[[{text:'📦 Lihat Paket',callback_data:'nav:packages'},{text:'🏠 Menu',callback_data:'nav:menu'}]]});return 'checkout_cancelled';}
      ...
      if(action==='package'){const row=await db.packageBySlug(value);await respond(chatId,packageDetailText(row),packageDetailMarkup(row));return 'package_detail';}
      if(action==='checkout'){await db.cancelCheckout(id).catch(function(){});const row=await db.packageBySlug(value);await respond(chatId,checkoutText(row),checkoutMarkup(row));return 'checkout';}
      if(action==='voucher'){
        const row=await db.packageBySlug(value);
        await db.beginCheckout(id,value);
        await respond(chatId,'Gunakan tombol ini untuk kembali atau membatalkan.',voucherPromptMarkup(row));
      ...
      if(action==='novoucher'){await db.cancelCheckout(id).catch(function(){});const row=await db.packageBySlug(value);await createAndShowPurchase(chatId,id,db,row,undefined);return 'purchase_created';}
    ...
    if(session&&isVoucherReply(update)&&!update.message.document&&!update.message.photo){
      ...
      if(!/^[A-Z0-9]{8,64}$/.test(code)){await respond(chatId,'Kode voucher harus terdiri dari 8–64 huruf atau angka. Balas prompt voucher sekali lagi atau batalkan.',voucherPromptMarkup(session));return 'voucher_invalid';}
...
module.exports={processUpdate,processChannelJoinRequest,isMembershipUpdate,accountMessage,verificationCodeFromCommand,canonicalCommand,showProcessing,packageListText,packageDetailText,checkoutText,userError,isVoucherReply};
