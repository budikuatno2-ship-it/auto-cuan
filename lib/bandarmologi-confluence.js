...
function computeBandarmologiConfluence(ticker) {
  ...
  const notes = datesNeeded.length >= 7
    ? ('Bandar 3D/7D/1M/3M dihitung dari broker summary harian (' + perDayNet.size + '/' + datesNeeded.length + ' hari tersedia).')
    : ('Data bandar belum lengkap (' + perDayNet.size + ' hari tersedia).');
