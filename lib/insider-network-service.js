...
function getRosterForTicker(ticker) {
  ...
  return recs.map((r, idx) => {
    // F-079: a missing percentage must stay missing ("—"), not become 0
    // ("0.00%"), which falsely claims the holder owns 0% of the company.
    // Normalized records expose the ownership share as pct_after /
    // current_percentage (a "7.25%" string); read those before giving up.
    const pctSource = r.percentage != null ? r.percentage
      : (r.pct_after != null ? r.pct_after
        : (r.current_percentage != null ? r.current_percentage : r.shares_after_percentage));
    const pctNum = (pctSource == null || pctSource === '')
      ? null
      : (typeof pctSource === 'number' ? pctSource : parseFloat(String(pctSource).replace(/[%\s]/g, '')));
    const pctValid = Number.isFinite(pctNum);
    return {
      no: idx + 1,
      ticker: clean,
      ...
      percentage: pctValid ? pctNum : null,
      percentage_formatted: pctValid ? pctNum.toFixed(2) + '%' : '—',
