...
function canonicalizeName(rawName) {
  ...
  return String(rawName)
    .toLowerCase()
    .replace(/\b(dr|drs|dra|ir|prof|h|hj|sh|s\.h|se|s\.e|si|s\.i|mm|m\.m|msc|m\.sc|mba|phd|ph\.d|llm|akt|cfa|cfp|ak)\b/gi, ' ')
    .replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, ' ')
    .replace(/\b(dr|drs|dra|ir|prof|h|hj|sh|se|si|mm|msc|mba|phd|llm|akt|cfa|cfp|ak)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
const DEDICATED_UNIVERSE_FILE = path.join(__dirname, '..', 'data', 'insider-network', 'universe.json');
...
function loadDedicatedUniverse() {
  try {
    ...
    if (fs.existsSync(DEDICATED_UNIVERSE_FILE)) {
      const data = JSON.parse(fs.readFileSync(DEDICATED_UNIVERSE_FILE, 'utf8'));
      if (Array.isArray(data) && data.length > 0) return data;
    }
  } catch (_) {}
  return null;
...
function aggregateInsiderHoldings(recordsOrUniverse) {
  ...
  return results.sort((a, b) => b.total_emitens - a.total_emitens || b.total_shares - a.total_shares);
}
