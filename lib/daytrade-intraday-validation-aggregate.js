...
function buildAggregateReport(samples, opts) {
  ...
  const noCounts = [], incompleteCounts = [], unknownCounts = [], absDeltas = [];
  ...
  for (const s of samples) {
    ...
    const unknown = (s.priority_label_counts && s.priority_label_counts.INTRADAY_UNKNOWN || 0) || (s.confirmation_label_counts && s.confirmation_label_counts.INTRADAY_UNKNOWN || 0) || asArray(s.intraday_unknown_tickers).length;
    ...
    for (const item of asArray(s.top_score_movers)) { const p = parseMover(item); inc(moverCounts, p.ticker); if (Number.isFinite(p.delta)) absDeltas.push(Math.abs(p.delta)); }
  }
  ...
  const metrics = {
    ...
    avg_absolute_score_delta: avg(absDeltas)
  };
  const blockReasons = [];
...
module.exports = { DEFAULT_REPORTS_DIR, BUNDLE_PREFIX, AGGREGATE_PREFIX, listBundleFiles, loadBundles, buildAggregateReport, markdownReport, writeReports, parseMover };
