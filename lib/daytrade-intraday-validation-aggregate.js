...
async function loadBundles(opts) {
  ...
  for (const entry of files) {
    ...
    const existing = byId.get(enriched._sample_id);
    if (!existing || (existing._source_type === 'daily' && sourceType === 'session_archive')) byId.set(enriched._sample_id, enriched);
  }
  let samples = Array.from(byId.values()).sort((a, b) => String(sampleGeneratedAt(b) || b.date).localeCompare(String(sampleGeneratedAt(a) || a.date)));
