...
function minutesBetween(a, b) { return Math.round((b - a) / 60000 * 100) / 100; }
function sessionTime(sample) {
  const raw = sampleGeneratedAt(sample);
  if (!raw) return null;
  const ms = Date.parse(raw);
  return Number.isFinite(ms) ? ms : null;
}
function buildSessionSpacing(samples, opts) {
  const sampleCount = asArray(samples).length;
  const minGapRequired = Number((opts && opts.minSessionGapMinutes) ?? 15);
  const minSpanRequired = Number((opts && opts.minSessionSpanMinutes) ?? 30);
  const warnReasons = [];
  const blockReasons = [];
  const timed = [];
  for (const s of asArray(samples)) {
    const ms = sessionTime(s);
    if (ms == null) warnReasons.push(`missing generated_at/session_started_at for ${s._sample_id || s.session_id || s._file || s.date || 'sample'}`);
    else timed.push({ ms, sample_id: s._sample_id || s.session_id || s._file || s.date || new Date(ms).toISOString(), generated_at: sampleGeneratedAt(s), source_type: s._source_type || 'daily', date: s.date });
  }
  timed.sort((a, b) => a.ms - b.ms);
  let minGap = null;
  const closePairs = [];
  for (let i = 1; i < timed.length; i++) {
    const gap = minutesBetween(timed[i - 1].ms, timed[i].ms);
    if (minGap == null || gap < minGap) minGap = gap;
    if (gap < minGapRequired) closePairs.push({ previous_sample_id: timed[i - 1].sample_id, previous_generated_at: timed[i - 1].generated_at, next_sample_id: timed[i].sample_id, next_generated_at: timed[i].generated_at, gap_minutes: gap });
  }
  const span = timed.length >= 2 ? minutesBetween(timed[0].ms, timed[timed.length - 1].ms) : null;
  if (sampleCount >= 3 && minGap != null && minGap < minGapRequired) blockReasons.push(`session samples too close: min gap ${minGap} minutes < ${minGapRequired}`);
  if (sampleCount >= 3 && span != null && span < minSpanRequired) blockReasons.push(`session span too short: ${span} minutes < ${minSpanRequired}`);
  const status = blockReasons.length ? 'BLOCK' : warnReasons.length ? 'WARN' : 'OK';
  return { session_spacing_status: status, min_session_gap_minutes: minGap, session_span_minutes: span, close_session_pair_count: closePairs.length, close_session_pairs: closePairs, session_spacing_block_reasons: blockReasons, session_spacing_warn_reasons: warnReasons };
}
...
function buildAggregateReport(samples, opts) {
  ...
  const spacing = buildSessionSpacing(samples, opts);
  const blockReasons = [];
  ...
  blockReasons.push(...spacing.session_spacing_block_reasons);
  const warnReasons = [];
  warnReasons.push(...spacing.session_spacing_warn_reasons);
  ...
  return {
    ...
    sample_count: sampleCount, session_spacing_status: spacing.session_spacing_status, min_session_gap_minutes: spacing.min_session_gap_minutes, session_span_minutes: spacing.session_span_minutes, close_session_pair_count: spacing.close_session_pair_count, close_session_pairs: spacing.close_session_pairs, sample_source_counts: sampleSourceCounts, sample_ids: samples.map((s) => s._sample_id || s.session_id || s._file).filter(Boolean), date_range: dateRange, samples: samples.map((s) => ({ date: s.date, file: s._file, source_type: s._source_type || 'daily', sample_id: s._sample_id || s.session_id, generated_at: sampleGeneratedAt(s), validation_status: s.validation_status })), status_counts: statusCounts,
...
module.exports = { DEFAULT_REPORTS_DIR, BUNDLE_PREFIX, BUNDLE_SESSION_PREFIX, AGGREGATE_PREFIX, listBundleFiles, listSessionArchiveFiles, loadBundles, buildSessionSpacing, buildAggregateReport, markdownReport, writeReports, parseMover, sampleAvgAbsoluteScoreDelta };
