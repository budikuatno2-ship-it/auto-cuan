...
function codeMessage(code, expiresAt) {
  ...
  return [
    ...
    'Kode: ' + code,
    '',
    'Halaman autocuan.web.id yang sedang terbuka akan menampilkan kolom kode otomatis.',
    'Tidak perlu refresh. Verifikasi berjalan otomatis setelah digit ke-6.',
    'Berlaku sampai: ' + expiry,
    'Kode hanya bisa dipakai sekali dan maksimal 5 percobaan.',
    '',
    'Pesan /akses dan pesan kode dibersihkan otomatis.'
  ].join('\n');
}

async function deleteMessageSafely(bot, chatId, messageId) {
  if (!bot || !Number.isSafeInteger(Number(chatId)) || !Number.isSafeInteger(Number(messageId))) return false;
  try {
    ...
    for (const row of result.data || []) {
      ...
      if (Number.isSafeInteger(chatId)) {
        for (const messageId of [row.telegram_code_message_id, row.telegram_success_message_id]) {
          ...
          await deleteMessageSafely(bot, chatId, Number(messageId));
        }
      }
      try {
        await db
...
async function handleUpdate(update, options) {
  ...
  try {
    ...
    if (!maintenance.available || !maintenance.enabled) {
      await deleteMessageSafely(bot, chat.id, message.message_id);
      const text = maintenance.available
        ? 'ℹ️ /akses hanya tersedia saat Auto-Cuan sedang maintenance.'
        : '⚠️ Status maintenance belum dapat diverifikasi. /akses tidak dijalankan.';
      try { await bot.sendMessage(chat.id, text); } catch (_) {}
      outcome = maintenance.available
        ? 'admin_maintenance_code_not_in_maintenance'
        : 'admin_maintenance_code_maintenance_unknown';
    ...
    // Send the new OTP first. Old-message cleanup can involve several Telegram
    // delete calls and must not sit on the user-visible critical path.
    ...
    if (sent && Number.isSafeInteger(Number(sent.message_id))) {
      ...
      // Automatically delete the OTP message after expiration
      if (typeof bot.scheduleMessageDeletion === 'function') {
        bot.scheduleMessageDeletion(chat.id, sent.message_id, CODE_TTL_MS + 3000);
      } else {
        setTimeout(async () => {
          await deleteMessageSafely(bot, chat.id, Number(sent.message_id));
        }, CODE_TTL_MS + 3000).unref?.();
      }
    }

    // Cleanup happens only after the current code is already visible. The
    // current grant is explicitly excluded, so this cannot remove the new OTP.
    await Promise.allSettled([
      cleanupOldMessages(db, bot, from.id, row.grant_id),
      deleteMessageSafely(bot, chat.id, message.message_id)
    ]);
