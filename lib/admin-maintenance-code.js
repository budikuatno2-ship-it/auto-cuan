...
async function handleUpdate(update, options) {
  ...
  try {
    // /akses is maintenance-only. Read the exact same persisted setting that the
    // website uses for its maintenance gate. If state cannot be established,
    // fail closed instead of issuing a code outside the intended window.
    ...
    // Best-effort cleanup of the user's /akses command itself. Deleting this only
    // affects Telegram chat history; the server still keeps the minimal audit
    // state needed to protect the one-time grant from replay.
    await deleteMessageSafely(bot, chat.id, message.message_id);
