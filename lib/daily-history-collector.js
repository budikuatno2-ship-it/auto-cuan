...
function safeTicker(ticker) {
  return String(ticker || '').replace(/\.JK$/i, '').toUpperCase().replace(/[^A-Z0-9_-]/g, '');
}
...
function reconcileMissingCloseFromMeta(args) {
  ...
  if (volume !== metaVolume) {
    if (metaVolume < volume || (metaVolume - volume) > Math.max(volume, metaVolume) * 0.001) return null;
  }
...
async function fetchYahooDailyHistory(ticker, options) {
  ...
  try {
    ...
    var metaDate = meta.regularMarketTime
      ? toDateKey(new Date(meta.regularMarketTime * 1000))
      : null;
    ...
    for (var i = 0; i < timestamps.length; i++) {
      var rowDate = toDateKey(new Date(timestamps[i] * 1000));
...
/**
 * Derive 52-week high/low directly from the full ~1y Yahoo candle fetch
 * (oldest-first), BEFORE it gets trimmed to the stock_daily_history
 * retention window (HISTORY_RETENTION_TRADING_SESSIONS = 120 sessions,
 * roughly 6 months — not enough to answer a true 52-week question). This is
 * the only point in the pipeline where a full year of daily highs/lows is
 * available in memory, so the derived values are computed here and carried
 * through to the feature snapshot layer instead of being recomputed later
 * from the trimmed, insufficient persisted history.
 */
function computeWeek52FromCandles(candles) {
  if (!Array.isArray(candles) || !candles.length) {
    return { week52_high: null, week52_high_date: null, week52_low: null, week52_low_date: null, week52_basis_sessions: 0 };
  }
  var high = null;
  var highDate = null;
  var low = null;
  ...
  for (var i = 0; i < candles.length; i++) {
    ...
    var h = finiteNumberOrNull(candle.high);
    var l = finiteNumberOrNull(candle.low);
    if (h != null && (high == null || h > high)) { high = h; highDate = candle.date; }
    if (l != null && l > 0 && (low == null || l < low)) { low = l; lowDate = candle.date; }
  }
  return {
    week52_high: high,
    week52_high_date: highDate,
    week52_low: low,
    week52_low_date: lowDate,
    week52_basis_sessions: candles.length
  };
}

/**
 * Derive mature Wilder RSI14 directly from the full ~1y Yahoo candle fetch
 * (oldest-first), BEFORE it gets trimmed to the stock_daily_history
 * retention window — the same rationale as computeWeek52FromCandles above.
 *
 * Why this matters: lib/daily-rsi.js implements standard Wilder smoothing
 * correctly, but Wilder's average gain/loss is a RECURSIVE state that only
 * converges to a chart-comparable value after being smoothed forward across
 * many sessions (weight of the initial seed decays by a factor of
 * (period-1)/period per session — for period=14 that is ~0.93 per step, so
 * the seed's influence only drops under 1% after roughly 60+ additional
 * sessions beyond the first 14). Computing RSI from the DB's previous
 * RSI_PERIOD+1 (15) most-recent closes gives ZERO smoothing iterations
 * (closes.length === period + 1 means the Wilder recursion loop in
 * computeRsiSeries never runs) — i.e. just the unsmoothed seed average of
 * the last 14 daily deltas, not the mature, recursively-smoothed value every
 * charting platform (TradingView, Yahoo charts, etc.) shows. This is the
 * confirmed root cause of the BELL/TIRA RSI discrepancy reported against
 * external charts. Using the full fetched candle series here (typically
 * ~250 sessions) gives the smoothing loop 200+ iterations to converge,
 * matching mature chart RSI to within rounding.
 */
function computeRsiFromCandles(candles, options) {
  options = options || {};
  if (!Array.isArray(candles) || !candles.length) {
    return {
      rsi_14: null,
      ...
      history_length: 0,
      required_length: rsi.RSI_PERIOD + 1,
      rsi_basis_sessions: 0
    };
  }
  var closes = candles.map(function(c) { return c.close; });
  var dates = candles.map(function(c) { return c.date; });
  var result = rsi.computeLatestRsi(closes, dates, { period: options.period });
  return Object.assign({}, result, { rsi_basis_sessions: candles.length });
}
...
function candlesToHistoryRows(ticker, candles, options) {
  ...
  // Chain previous_close from the FULL fetched series, not from the trimmed
  // window. Chaining from `trimmed` gave the oldest retained row
  // previous_close: null even though its real prior session was still in
  // `candles`, and re-ran daily it overwrote the correct value written on the
  // previous run as the window rolled forward. Only the true first session of
  // the fetched series has no prior.
  var trimStart = Math.max(0, candles.length - retention);
  var trimmed = candles.slice(trimStart);
  return trimmed.map(function(candle, index) {
    var absoluteIndex = trimStart + index;
    var priorCandle = absoluteIndex > 0 ? candles[absoluteIndex - 1] : null;
...
async function collectDailyHistoryForTickers(supabase, tickers, options) {
  ...
  for (var ticker of uniqueTickers) {
    try {
      ...
      // Both computed from the FULL fetched candle set (up to ~1y), not the
      // retention-trimmed `rows` above — see computeWeek52FromCandles and
      // computeRsiFromCandles.
      week52ByTicker[ticker] = computeWeek52FromCandles(candles);
      rsiByTicker[ticker] = computeRsiFromCandles(candles, { period: options.rsiPeriod });
    } catch (e) {
