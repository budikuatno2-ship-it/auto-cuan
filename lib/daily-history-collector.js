...
/**
 * Compute 52-week high context from the SAME one-year Yahoo candle payload
 * already fetched by this collector. No additional network request is made.
 *
 * For newly listed stocks with less than one year of history, the metric uses
 * every valid session Yahoo returned in the one-year range and reports the
 * available-session count for diagnostics.
 */
function computeRolling52WeekMetrics(candles) {
  ...
  for (var row of rows) {
    var high = finiteNumberOrNull(row && row.high);
    if (high == null) continue;
    if (high52 == null || high > high52) high52 = high;
  }

  var latest = rows[rows.length - 1] || null;
  var lastClose = finiteNumberOrNull(latest && latest.close);

  var distance = null;
  if (high52 != null && high52 > 0 && lastClose != null) {
    distance = Math.round((((lastClose / high52) - 1) * 100) * 100) / 100;
  }

  return {
    high_52w: high52,
    distance_to_high_52w_pct: distance,
    sessions_available: rows.length
  };
}
...
async function collectDailyHistoryForTickers(supabase, tickers, options) {
  ...
  for (var ticker of uniqueTickers) {
    try {
      ...
      tickerMetrics[ticker] = computeRolling52WeekMetrics(candles);
