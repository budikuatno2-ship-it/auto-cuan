...
async function handleAdminAccessUpdate(update, options) {
  ...
  // A valid SESSION_SECRET is required both to HMAC the code and to issue the
  // signed ac_sess. If signing is unavailable, do not steal /akses from the
  // established fallback. The handler itself separately probes for its SQL
  // migration so rollout remains safe before that schema is deployed too.
  if (typeof process.env.SESSION_SECRET === 'string' && process.env.SESSION_SECRET.length > 0 && maintenanceCode.matchesUpdate(update)) {
