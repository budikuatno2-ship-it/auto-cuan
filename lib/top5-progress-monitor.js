...
const SAFETY_TERMS = /\b(avoid|hindari|sell|low_tp|very high risk|stale_level|needs_revalidation|history_insufficient|new_listing)\b/i;
...
function computeTop5Progress(row, latestPrice, options) {
  ...
  const tp1 = positive(row.tp1);
  const tp2 = positive(row.tp2);
  const sl = firstPositive(row, ['sl', 'stop_loss', 'stoploss']);
  const age = priceAgeHours(row, options);
...
function detectTop5ProgressEvents(row, latestPrice, options) {
  ...
  if (progress.block_reason === 'stale_price') add('STALE_PRICE', false);
    ...
    if (progress.sl && progress.latest_price <= progress.sl * 1.02) add('SL_NEAR', true);
    if (progress.gain_pct < 0) add('BELOW_ENTRY', true);
  }
  ...
  events.forEach((event) => { event.notification_enabled = event.actionable && durable; });
...
function deriveTrackingStatus(row, detected, state, options) {
  ...
  const prior = state || {};
  if (prior.status === 'STOP_TRACKING' || prior.status === 'COMPLETE' || prior.status === 'MAX_AGE_EXPIRED') return { status: prior.status, should_track: false, reason: 'terminal_state' };
  ...
  if (progress.tp1_hit) return { status: 'TRACKING_TP1', should_track: true, reason: 'TP1_HIT' };
  ...
  const routineDue = !isSwing || !routineAt || now - routineAt >= 3 * 86400000;
...
module.exports = { computeTop5Progress, detectTop5ProgressEvents, buildTop5ProgressDiagnostics, deriveTrackingStatus, eventKey, resolveLatestPrice, resolveEntry, DEFAULT_MAX_PRICE_AGE_HOURS };
