...
const DEFAULT_PRIMARY_ADMIN_USERNAME = 'budi';
...
function isProductionEnv(env) {
  ...
  return String(source.VERCEL_ENV || '').trim().toLowerCase() === 'production' ||
    String(source.NODE_ENV || '').trim().toLowerCase() === 'production';
}

function getMode(env) {
  const source = env || process.env;
  const safeDefault = isProductionEnv(source) ? 'enforce' : 'off';
  const raw = source.SECURITY_GUARD_MODE;
  if (raw == null || String(raw).trim() === '') return safeDefault;
  const value = String(raw).trim().toLowerCase();
  return ALLOWED_MODES.has(value) ? value : safeDefault;
}

function getPrimaryAdminUsername(env) {
  const source = env || process.env;
  return normalizeUsername(source.PRIMARY_ADMIN_USERNAME || source.ADMIN_USERNAME || DEFAULT_PRIMARY_ADMIN_USERNAME) || DEFAULT_PRIMARY_ADMIN_USERNAME;
}

function shouldFailClosedAdmin(env) {
  const source = env || process.env;
  const value = String(source.SECURITY_GUARD_FAIL_CLOSED_ADMIN == null ? '' : source.SECURITY_GUARD_FAIL_CLOSED_ADMIN).trim();
  if (value === '1') return true;
  if (value === '0') return false;
  return isProductionEnv(source);
}
...
function getPublicStatus(env) {
  ...
  return {
    ...
    admin_fail_closed: shouldFailClosedAdmin(source)
  };
}
...
function buildContext(req, username, env) {
  const source = env || process.env;
  const pepper = getPepper(source);
  const normalizedUsername = normalizeUsername(username);
  const primaryAdminUsername = getPrimaryAdminUsername(source);
  ...
  if (!pepper) {
    return {
      ...
      ip,
      userAgent,
      adminTarget: normalizedUsername === primaryAdminUsername,
      ready: false
    };
  }
...
async function recordBlockedHit(db, context, precheck, mode) {
  ...
  const eventContext = Object.assign({}, context, {
    requestId: context.requestId + '_blocked'
  });
  try {
    await db.rpc('security_login_record_blocked', Object.assign(rpcParams(eventContext), {
...
async function beginLogin(options) {
  ...
  if (!context.ready || !opts.db) {
    const failClosed = mode === 'enforce' && context.adminTarget && shouldFailClosedAdmin(env);
...
function buildSecuritySummary(rows, nowValue) {
  ...
  const suspiciousIps = Array.from(grouped.values()).map(function (item) {
    return {
      ...
      reasons: Array.from(item.reasons.entries()).map(function (entry) {
        return { reason: entry[0], count: entry[1] };
      }).sort(function (a, b) { return b.count - a.count; }),
      routes: Array.from(item.routes).slice(0, 8)
    };
  }).sort(function (a, b) {
    return Number(b.active_block) - Number(a.active_block) ||
  ...
  return {
    ...
    blocked_events_24h: suspicious.filter(function (row) {
      return row.outcome === 'blocked' || row.event_type === 'login_rate_limited';
    }).length,
    admin_target_events_24h: suspicious.filter(function (row) {
      return safeMetadata(row.metadata).admin_target === true;
    }).length,
    unique_ips_24h: new Set(suspicious.map(function (row) {
      return row.ip_address || 'tidak tersedia';
    })).size,
...
async function loadSecurityDashboard(db, options) {
  ...
  try {
    ...
    return {
      available: false,
      reason: isMissingSecuritySchema(error) ? 'schema_missing' : 'query_failed',
      events: [],
      summary: buildSecuritySummary([])
    };
  }
...
module.exports = {
  SECURITY_ROUTE,
  ALLOWED_MODES,
  DEFAULT_PRIMARY_ADMIN_USERNAME,
  isProductionEnv,
  getMode,
  getPrimaryAdminUsername,
  shouldFailClosedAdmin,
  getPepper,
  getPublicStatus,
  normalizeUsername,
  maskUsername,
