...
// Temuan #6 quality gate: Early Watch must not broadcast the engine
// "Wait - Poor RR" path. Blocks ONLY the canonical poor-RR source_status
// (daytrade screener POOR_RR / "RR kurang menarik"). No score floor and no
// derived-RR block: temuan spam had high scores (84/74), pool has no
// canonical score>=70 gate for WATCHING, and derived RR would break the
// frozen TMPO contract (FIX3.7) where informational Early Watch still sends.
function isPoorRrSourceStatus(sourceStatus) {
  const s = String(sourceStatus || '').trim().toUpperCase();
  if (!s) return false;
  if (s === 'POOR_RR') return true;
  if (s.indexOf('POOR') >= 0 && s.indexOf('RR') >= 0) return true;
  if (s.indexOf('RR KURANG') >= 0) return true;
  return false;
}
 ...
 *   - not a poor-quality pre-confirmation candidate (Temuan #6): blocks ONLY
 *     the canonical engine "Wait - Poor RR" source_status (POOR_RR /
 *     "RR kurang menarik"). No derived-RR or score floor: pool has no
 *     canonical score>=70 gate for WATCHING, and derived RR breaks the
 *     frozen TMPO contract (FIX3.7).
 */
function isCurrentlyEarlyWatchEligible(poolItem) {
  ...
  if (isPoorRrSourceStatus(poolItem.source_status)) return false;
  return true;
}
...
async function sendPendingNotifications(state, processedPoolState, context) {
  ...
  for (const ticker of Object.keys(nextState.tickers)) {
    ...
    if (!tracker.early_watch_notification_attempted && tracker.reference_price != null && isCurrentlyEarlyWatchEligible(poolItem)) {
      ...
      try {
        const sendResult = await send(buildEarlyWatchMessage(tracker, ticker, poolItem), { chat_id: chatId, disable_web_page_preview: true, timeout_ms: 5000, ticker: ticker, status: 'WATCHING' });
    ...
    if (!tracker.anti_chase_notification_attempted && earlyWatchAlreadySentBeforeThisRun && isChaseBlocked(poolItem)) {
      ...
      await persist(nextState);
      try {
        const sendResult = await send(buildAntiChaseMessage(tracker, ticker, poolItem), { chat_id: chatId, disable_web_page_preview: true, timeout_ms: 5000, ticker: ticker, status: 'BLOCKED_CHASE' });
        if (sendResult && sendResult.sent) {
