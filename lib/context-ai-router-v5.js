...
function redactDiagnostic(value) {
  return String(value == null ? '' : value)
    ...
    // ponytail: redact Google Gemini API keys (AIza...); upgrade if provider key prefixes expand
    .replace(/\b(?:AIza|AQ)[A-Za-z0-9_\-\.]{15,}\b/g, '[REDACTED_KEY]')
...
// Single source of truth lives in V4, so the inference-unavailable classifier and
// this rejection trace can never disagree about what counts as temporary. V4 is
// required at module scope below; this only runs while serving a request, long
// after that binding is initialised.
function isTemporaryModelUnavailable(value) {
  return handleContextAIV4._test.isTemporaryModelUnavailable(value);
}
...
function allPrimaryFailuresAreTemporary(payload, trace) {
  ...
  // ponytail: require at least 2 distinct models before treating as all-primary failure
  const attempted = Array.from(new Set(Array.isArray(payload.attempted_models) ? payload.attempted_models.filter(Boolean) : []));
...
async function tryOutageSpillover(req, originalPayload, trace, handleContextAIV4, handlerStarted) {
  const idle = { recovered: null, attempts: 0, models: [] };
  // V4 already proved inference is unavailable to us, across vendors, on catalog-valid
  // routes. Six more identical requests cannot change that answer — they would
  // only spend the budget to re-derive a conclusion we already hold.
  if (originalPayload && originalPayload.code === 'AI_PROVIDER_TEMPORARILY_UNAVAILABLE') return idle;
  // Spillover stays available for everything short of that bar: model-specific
  // outages, or temporary failures that did not meet the classification rule.
  if (!allPrimaryFailuresAreTemporary(originalPayload, trace)) return idle;
  ...
  if (!apiKey || !/^https:\/\/weizerouter\.web\.id\/v1$/i.test(baseUrl)) return idle;
  ...
  for (const model of candidates) {
    ...
    emergencyAttempts += 1;
    emergencyModels.push(model);
    // Emergency routes used to be exempt from health bookkeeping entirely, so a
    // dead route was re-probed on every single request forever with no cooldown.
    // They record failures like every other route now.
    if (!result.ok) handleContextAIV4._test.recordFailure(model, !handleContextAIV4._test.isKeyOrBalanceFailure(result.status));
    ...
    if (result.ok) {
      ...
      return { recovered: payload, attempts: emergencyAttempts, models: emergencyModels };
    }
    if (result.status === 401 || result.status === 402 || result.status === 403) break;
  }
  return { recovered: null, attempts: emergencyAttempts, models: emergencyModels };
}
...
async function handleContextAIV5(req, res) {
  ...
  return providerTraceStorage.run(trace, async () => {
    ...
    let spill = { recovered: null, attempts: 0, models: [] };
    try { spill = await tryOutageSpillover(req, payload, trace, handleContextAIV4, handlerStarted) || spill; }
    catch (error) {
      console.warn('context-ai outage spillover exception', JSON.stringify({ message: clean(error && error.message, 160) || 'unknown' }));
    }
    if (spill.recovered) return res.status(200).json(spill.recovered);
    // The failure payload used to be forwarded verbatim, so a request that made
    // three normal calls plus six emergency calls still reported attempted_count
    // 3 — understating real provider spend by three times. The count now covers
    // every chat completion actually issued. The /models probe is not a chat
    // attempt and is still excluded.
    if (spill.attempts > 0 && captured.state.payload && typeof captured.state.payload === 'object') {
      const attempted = Array.isArray(payload.attempted_models) ? payload.attempted_models : [];
      captured.state.payload = Object.assign({}, payload, {
        attempted_count: (Number(payload.attempted_count) || 0) + spill.attempts,
        attempted_models: attempted.concat(spill.models),
        emergency_attempted_count: spill.attempts
      });
    }
