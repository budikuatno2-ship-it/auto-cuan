...
const DEFAULT_OUTAGE_SPILLOVER_ATTEMPTS = 3;
...
function outageCandidatePool(source, task, attemptedModels) {
  ...
  return dedupe(preferred.concat(OUTAGE_FALLBACK_EXTRAS, VERIFIED_BALANCED, VERIFIED_FAST, VERIFIED_EMPATHY))
...
async function tryOutageSpillover(req, originalPayload, trace, handleContextAIV4, handlerStarted) {
  ...
  const configuredExtra = bounded(process.env.PORTFOLIO_AI_OUTAGE_SPILLOVER_ATTEMPTS, DEFAULT_OUTAGE_SPILLOVER_ATTEMPTS, 0, 3);
