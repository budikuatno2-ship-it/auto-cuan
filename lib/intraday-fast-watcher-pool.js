...
const PERSISTENCE_CONFIRMATION_PUBLISH_THRESHOLD = 70;
const PERSISTENCE_CONFIRMATION_MIN_NORMALIZED_IMPULSE = -0.20;
...
function persistenceConfirmationEligible(item, result, context) {
  const ctx = context || {};
  const m = result && result.metrics || {};

  if (!item || !result) return false;
  if (ctx.from !== 'READY_PENDING') return false;
  if (ctx.confirmationSourceChanged === true) return false;
  if (ctx.productionEligible !== true) return false;

  /*
   * Persistence is an alternative second-confirmation path
   * only when normal momentum evaluation did NOT already pass.
   */
  if (result.passes === true) return false;
  if (result.status !== 'WATCHING') return false;
  if (result.engine_class !== 'near_miss') return false;
  if (result.volume_strong !== true) return false;

  const publish =
    momentum.finite(m.publish_score);

  const impulse =
    momentum.finite(m.normalized_impulse);

  const riskReward =
    momentum.finite(m.risk_reward);

  const minimumRiskReward =
    momentum.finite(m.minimum_risk_reward) ?? 1;

  if (
    publish == null ||
    publish < PERSISTENCE_CONFIRMATION_PUBLISH_THRESHOLD
  ) {
    return false;
  }
  ...
  if (
    impulse == null ||
    impulse < PERSISTENCE_CONFIRMATION_MIN_NORMALIZED_IMPULSE
  ) {
    return false;
  }
  ...
  for (const blockedReason of [
    ...
    if (reasons.has(blockedReason)) {
      return false;
    }
  }

  /*
   * Mirror the existing post-observation watch-pool lifecycle.
   * Persistence must not rescue a ticker that would otherwise
   * be dropped for age/expiry/stagnation.
   */
  const watchScore =
    momentum.finite(m.watch_score) ?? 0;

  const priorWatchScore =
    momentum.finite(item.last_watch_score);

  const prospectiveStagnantCount =
    priorWatchScore != null &&
    watchScore <= priorWatchScore + 1
      ? (item.stagnant_count || 0) + 1
      : 0;

  let prospectiveExpiresAt =
    item.expires_at_minute;
  ...
  if (
    minute != null &&
    prospectiveExpiresAt != null &&
    minute >= prospectiveExpiresAt - 2 &&
    item.max_expires_at_minute != null &&
    prospectiveExpiresAt < item.max_expires_at_minute &&
    watchScore >= MIN_EXTENSION_SCORE &&
    prospectiveStagnantCount < 2
  ) {
    prospectiveExpiresAt =
      Math.min(
        prospectiveExpiresAt + WATCH_EXTENSION_MINUTES,
        item.max_expires_at_minute
      );
  }
  ...
  if (
    throughMinute != null &&
    item.max_expires_at_minute != null &&
    throughMinute >= item.max_expires_at_minute
  ) {
    return false;
  }
  ...
  if (
    throughMinute != null &&
    prospectiveExpiresAt != null &&
    throughMinute >= prospectiveExpiresAt
  ) {
    return false;
  }
  ...
  if (
    prospectiveStagnantCount >= 2 &&
    item.in_latest_shortlist === false &&
    watchScore < MIN_EXTENSION_SCORE
  ) {
    return false;
  }
...
function process(options) {
  ...
  for (const row of rows) {
    ...
    const persistenceConfirmation =
      persistenceConfirmationEligible(
        item,
        result,
        {
          from,
          confirmationSourceChanged,
          productionEligible: eligibility.eligible === true,
          minute,
          throughMinute
        }
      );

    if (persistenceConfirmation) {
      result = Object.assign({}, result, {
        passes: true,
        entry_eligible: true,
        status: 'READY_PASS',
        reasons: [...new Set([
          ...(result.reasons || [])
            .filter(reason => reason !== 'engine_not_ready'),
  ...
  return { state, events, confirmed, informational_spikes: informationalSpikes, publishable: rankPublishable(confirmed), active_pool_count: Object.values(state.tickers).filter(item => item.active).length, processed_observation_count: rows.length, shortlist_count: shortlistRows.length };
}

module.exports = { RULE_VERSION, INITIAL_WATCH_MINUTES, WATCH_EXTENSION_MINUTES, MAX_WATCH_MINUTES, REQUIRED_CONFIRMATIONS, CONFIRMATION_WINDOW_SIZE, MIN_FRESH_SHORTLIST_SLOTS, MAX_ACTIVE_POOL, MAX_PUBLISH_COUNT, SUPPLEMENTAL_STATUSES, canonicalSetupMetrics, structuralSetupReason, resetTrackerForReentry, activeRows, mergePayload, process, rankPublishable, setupId, lockSetup, withLockedSetup };
