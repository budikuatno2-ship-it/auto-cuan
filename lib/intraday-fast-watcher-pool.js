...
const productionEligibility = require('./intraday-production-eligibility');

const RULE_VERSION = 'FAST_WATCHER_VOLUME_PACE_V7';
...
const MIN_FRESH_SHORTLIST_SLOTS = 10;
const MAX_ACTIVE_POOL = 30;
const MAX_PUBLISH_COUNT = 3;
const MIN_EXTENSION_SCORE = 42;
// BUG-F9-06: number of consecutive stale reads tolerated before a tracker is
// terminally dropped. Covers short feed hiccups (a few seconds) without letting
// a permanently dead feed keep a stale candidate alive.
...
const SUPPLEMENTAL_STATUSES = new Set([
  ...
  'WAIT_PULLBACK', 'MOMENTUM_CONTINUATION', 'RECLAIM_CANDIDATE', 'WATCHING', 'SPIKE_RADAR'
]);
...
function canonicalSetupMetrics(metrics) {
  const copy = Object.assign({}, metrics || {});
  const entryHigh = momentum.finite(copy.entry_high);
  const stopLoss = momentum.finite(copy.stop_loss);
  const tp1 = momentum.finite(copy.tp1);
  copy.reported_risk_reward = momentum.finite(copy.risk_reward);
  if (entryHigh != null && stopLoss != null && tp1 != null && entryHigh > stopLoss) {
    copy.risk_reward = momentum.round2((tp1 - entryHigh) / (entryHigh - stopLoss));
    copy.risk_reward_source = 'derived_locked_levels';
  } else {
    copy.risk_reward_source = copy.reported_risk_reward == null ? 'missing' : 'reported_fallback';
  }
  return copy;
}
function structuralSetupReason(metrics) {
  const m = metrics || {};
  if (m.entry_low == null || m.entry_high == null || m.entry_low > m.entry_high) return 'invalid_entry_zone';
  if (m.stop_loss == null) return 'missing_stop_loss';
  if (m.stop_loss >= m.entry_low) return 'invalid_stop_loss';
  if (m.tp1 == null || m.tp1 <= m.entry_high) return 'invalid_tp1';
  return null;
}
function setupId(date, ticker, metrics) {
  return crypto.createHash('sha256').update([
    ...
    metrics.tp1,
    metrics.risk_reward,
    metrics.minimum_risk_reward,
    metrics.plan_lock_id || '-',
    RULE_VERSION
  ].join('|')).digest('hex');
}
function trackerFor(row, minute) {
  return {
    ...
    // BUG-F9-05: confirmation windows must count distinct observation MINUTES,
    // never raw tick count, otherwise a burst of same-minute ticks (retry storm
    // / data spike) satisfies REQUIRED_CONFIRMATIONS instantly.
    last_confirmation_minute: null,
    // BUG-F9-06: consecutive stale reads tolerated before the tracker is
    // terminally dropped (feed hiccup grace period).
    stale_grace_count: 0,
    ...
    last_volume: null, last_volume_rate: null, last_turnover: null, turnover_history: [],
    last_watch_score: null, last_publish_score: null,
    stagnant_count: 0, processed_keys: [], last_reasons: [], last_observation: null, last_metrics: null,
    last_source_setup_id: null, last_source_plan_lock_id: null,
...
function supplementalStatusBonus(status) {
  ...
  if (['PRE_SPIKE_WATCH', 'EARLY_RADAR', 'RECLAIM_CANDIDATE', 'SPIKE_RADAR'].includes(status)) return 24;
...
function mergePayload(payload, priorState, scheduledTime, supplemental, maxShortlist) {
  ...
  const requestedLimit = Number.isInteger(maxShortlist) ? maxShortlist : MAX_ACTIVE_POOL;
  const limit = Math.max(1, Math.min(requestedLimit, MAX_ACTIVE_POOL));
...
function process(options) {
  ...
  const shortlistRows = (options.shortlistRows || []).slice(0, MAX_ACTIVE_POOL);
  ...
  const selectedTickers = new Set(shortlistRows.map(row => row.ticker));
  ...
  for (const [ticker, item] of Object.entries(state.tickers)) {
    if (!item.active || selectedTickers.has(ticker)) continue;
    const from = item.status || 'WATCHING';
    item.active = false;
    item.status = 'DROPPED_FROM_WATCH_POOL';
    item.ready_streak = 0;
    item.confirmation_window = [];
    events.push({
      event_id: eventId(date, ticker, options.scheduledTime || item.last_time || '00:00', from, item.status),
      date,
      time: options.scheduledTime || item.last_time || '00:00',
      ticker,
      from_status: from,
      to_status: item.status,
      reasons: ['pool_capacity_evicted'],
  ...
  for (const row of rows) {
    ...
    const obsTurnover = momentum.finite(
      row.obs.turnover ?? row.obs.turnover_value ?? row.obs.value ?? row.obs.accumulated_turnover ?? row.obs.total_turnover ?? row.obs.value_today
    );
    if (obsTurnover != null) {
      item.last_turnover = obsTurnover;
      item.turnover_history = [...(item.turnover_history || []), {
        minute,
        time: row.time,
        turnover: obsTurnover
      }].slice(-20);
    }
    const raw = canonicalSetupMetrics(momentum.metricsFrom(row.obs));
    ...
    const currentPlanLockId = raw.plan_lock_id || null;
    const currentSourceSetupId = setupId(date, row.ticker, raw);
    const hasPriorSourceIdentity = item.last_source_setup_id != null;
    const hasLegacyConfirmation = !hasPriorSourceIdentity && (
      (Array.isArray(item.confirmation_window) && item.confirmation_window.length > 0) ||
      Number(item.ready_streak || 0) > 0
    );
    const priorSourcePlanLockId = hasPriorSourceIdentity
      ? item.last_source_plan_lock_id
      : item.locked_plan_lock_id;
    const priorSourceSetupId = hasPriorSourceIdentity
      ? item.last_source_setup_id
      : item.locked_setup_id;
    const compareSourceIdentity = hasPriorSourceIdentity || hasLegacyConfirmation;
    const planChanged = compareSourceIdentity
      && priorSourcePlanLockId !== currentPlanLockId
      && (priorSourcePlanLockId != null || currentPlanLockId != null);
    const sourceSetupChanged = compareSourceIdentity
      && priorSourceSetupId != null
      && currentSourceSetupId !== priorSourceSetupId;
    const confirmationSourceChanged = planChanged || sourceSetupChanged;
    sourceIdentityDiagnostics.push({
      ticker: row.ticker,
      time: row.time,
      reset: confirmationSourceChanged,
      reason: !compareSourceIdentity
        ? 'SOURCE_IDENTITY_INITIALIZED'
        : (confirmationSourceChanged ? 'SOURCE_IDENTITY_CHANGED' : 'SOURCE_IDENTITY_STABLE'),
      previous_setup_id: compareSourceIdentity ? priorSourceSetupId : null,
      current_setup_id: currentSourceSetupId,
      previous_plan_lock_id: compareSourceIdentity ? priorSourcePlanLockId : null,
      current_plan_lock_id: currentPlanLockId
    });
    item.last_source_plan_lock_id = currentPlanLockId;
    item.last_source_setup_id = currentSourceSetupId;
    const evaluationObs = withLockedSetup(Object.assign({ sample_date: date, scheduled_time: row.time }, row.obs), item);
    let result = momentum.evaluate(evaluationObs, item);
    const structureReason = structuralSetupReason(result.metrics);
    if (structureReason) result = Object.assign({}, result, { passes: false, entry_eligible: false, informational_spike: false, status: 'INVALID_DATA', reasons: [structureReason] });

    const eligibility = productionEligibility.classifyProductionEligibility(evaluationObs);
    if (!eligibility.eligible) {
      result = Object.assign({}, result, {
        passes: false,
        entry_eligible: false,
        status: result.status === 'READY_PASS' ? 'WATCHING' : result.status,
        reasons: [...new Set([
          ...(result.reasons || []),
          'production_eligibility_blocked',
          eligibility.reason
        ])].sort()
      });
    }
    if (planChanged) result.reasons = [...new Set([...(result.reasons || []), 'source_plan_identity_changed_locked_setup_preserved'])].sort();
    ...
    const informationalSpike = result.status === 'SPIKE_RADAR';
    // BUG-F9-06/F9-07: a transient stale read (feed hiccup of a few seconds)
    // must NOT be treated as a terminal failure. Only a sustained stale feed
    // (more than STALE_GRACE_MAX consecutive reads) drops the tracker; the
    // first_price anti-chase baseline is preserved throughout the grace window.
    ...
    // BUG-F9-05: only a NEW observation MINUTE may extend the confirmation
    // window. Repeated ticks inside the same minute (retry storm / burst) must
    // not satisfy REQUIRED_CONFIRMATIONS on their own.
    ...
    item.confirmation_window = terminalFailure || informationalSpike || withinStaleGrace ? [] : confirmationWindow;
    if (withinStaleGrace && confirmationSourceChanged === false) {
      // Preserve whatever confirmation progress already existed; stale data
      // must neither advance nor erase it.
      item.confirmation_window = Array.isArray(item.confirmation_window) && item.confirmation_window.length
        ? item.confirmation_window
        : priorConfirmationWindow;
    }
    ...
    // === OPENING RANGE VELOCITY GUARD (09:16 - 09:30 WIB) ===
    if (to === 'READY_CONFIRMED') {
      const velocityGuard = momentum.evaluateOpeningVelocityGuard({
        time: row.time,
        board: item.board || row.obs.board,
        obs: row.obs,
        tracker: item,
        metrics: result.metrics,
        bypass: options.bypass_opening_velocity_guard || row.obs.bypass_opening_velocity_guard
      });
      result.metrics.opening_velocity_guard = velocityGuard;
      if (velocityGuard.is_velocity_window) {
        if (velocityGuard.passes) {
          result.reasons = [...new Set([...(result.reasons || []), 'opening_range_velocity_passed', 'opening_range_velocity_confirmed'])].sort();
        } else {
          to = 'WAIT_PULLBACK';
          result.passes = false;
          result.entry_eligible = false;
          item.velocity_held = true;
          result.reasons = [...new Set([
            ...(result.reasons || []),
            velocityGuard.reason || 'opening_range_velocity_insufficient',
            'opening_range_velocity_gate_blocked'
          ])].sort();
        }
      }
    }
    ...
    if (result.metrics.opening_velocity_guard) item.last_opening_velocity_guard = result.metrics.opening_velocity_guard;
    if (result.metrics.delta_turnover_5m != null) item.last_delta_turnover_5m = result.metrics.delta_turnover_5m;
    item.last_watch_score = result.metrics.watch_score; item.last_publish_score = result.metrics.publish_score;
    if (to !== from || confirmationSourceChanged) events.push({ event_id: eventId(date, row.ticker, row.time, from, to), date, time: row.time, ticker: row.ticker, from_status: from, to_status: to, ready_streak: item.ready_streak, entry_eligible: result.entry_eligible === true, informational_spike: result.informational_spike === true, reasons: result.reasons, engine_class: result.engine_class, metrics: result.metrics, setup_id: item.locked_setup_id, plan_lock_id: item.locked_plan_lock_id, rule_version: RULE_VERSION, shadow_only: false });
  }
  ...
  const informationalSpikes = Object.entries(state.tickers).filter(([, item]) => item.active && item.status === 'SPIKE_RADAR').map(([ticker, item]) => ({ ticker, time: item.last_time, watch_score: item.last_watch_score, publish_score: item.last_publish_score, reasons: item.last_reasons || [], observation: Object.assign({}, item.last_observation || {}, { metrics: item.last_metrics || {} }), shortlist_rank: item.shortlist_rank }));
  return {
    state,
    events,
    confirmed,
    informational_spikes: informationalSpikes,
    publishable: rankPublishable(confirmed),
    active_pool_count: Object.values(state.tickers).filter(item => item.active).length,
    processed_observation_count: rows.length,
    shortlist_count: shortlistRows.length,
    diagnostics: { source_identity: sourceIdentityDiagnostics }
  };
}

module.exports = {
  RULE_VERSION, INITIAL_WATCH_MINUTES, WATCH_EXTENSION_MINUTES, MAX_WATCH_MINUTES,
  REQUIRED_CONFIRMATIONS, CONFIRMATION_WINDOW_SIZE, MIN_FRESH_SHORTLIST_SLOTS,
  MAX_ACTIVE_POOL, MAX_PUBLISH_COUNT, SUPPLEMENTAL_STATUSES, STALE_GRACE_MAX,
  evaluateOpeningVelocityGuard: momentum.evaluateOpeningVelocityGuard,
  isOpeningRangeVelocityWindow: momentum.isOpeningRangeVelocityWindow,
  canonicalSetupMetrics, structuralSetupReason, resetTrackerForReentry,
  activeRows, mergePayload, process, rankPublishable, setupId, lockSetup, withLockedSetup
};