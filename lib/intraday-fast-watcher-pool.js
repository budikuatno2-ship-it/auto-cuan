...
const RULE_VERSION = 'FAST_WATCHER_ADAPTIVE_MOMENTUM_V4';
...
function process(options) {
  ...
  for (const row of rows) {
    ...
    const confirmationWindow = [...(Array.isArray(item.confirmation_window) ? item.confirmation_window : []), result.passes === true].slice(-CONFIRMATION_WINDOW_SIZE);
