...
function getTrendShort(r) {
  ...
  if (tf1 && tf1 !== '-') {
    var cleanTf1 = tf1.replace(/\s*\(.*?\)\s*/g, '').replace(/^1D\s*/i, '').trim();
    parts.push('1D ' + cleanTf1);
  }
  if (tf5 && tf5 !== '-') {
    var cleanTf5 = tf5.replace(/\s*\(.*?\)\s*/g, '').replace(/^5D\s*/i, '').trim();
    parts.push('5D ' + cleanTf5);
  }
...
function formatSwingKongloSignalMessage(results, options) {
  ...
  results = results || [];
  var lines = [];
  var isWatchlist = options.isWatchlist || (results.length > 0 && results.every(function(r) {
