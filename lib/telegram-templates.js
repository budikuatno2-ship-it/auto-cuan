...
var patternPersonality = require('./pattern-personality');
...
/**
 * BATCH 6 / F10-02: canonical +/- sign rendering shared by numeric values and
 * numeric strings, so '5000000000' can never reach the user as a bare digit run.
 */
...
/**
 * Only unambiguously numeric strings are re-parsed. Grouped values such as
 * '1.500' (id-ID thousands vs en-US decimal) are deliberately left verbatim so
 * a pre-formatted number is never silently rescaled.
 */
function isPlainNumericString(t) {
...
function fmtSignedValue(v) {
  if (typeof v === 'string') {
    ...
    if (lower === 'nan' || lower === 'undefined' || lower === 'null' || lower === '[object object]' || lower === '') return '-';
    // F10-02: numeric strings are re-rendered with the canonical sign format;
    // already-formatted display strings ('+Rp 1 M') still pass through verbatim.
    if (isPlainNumericString(t)) {
      var parsed = Number(t.replace(',', '.'));
      if (!isFinite(parsed) || parsed === 0) return '-';
      return fmtSignedNumber(parsed);
    }
    return v;
  }
  var n = toNum(v);
  if (n == null || n === 0) return '-';
  return fmtSignedNumber(n);
}

/**
 * BATCH 6 / F10-04: HTML entity encoder for every message sent with
 * parse_mode:'HTML'. Telegram rejects the ENTIRE message with
 * `400 Bad Request: can't parse entities` when a dynamic value (ticker, note,
 * broker label, …) interpolated inside a tag contains &, < or >.
 */
...
function safe(value, fallback) {
  ...
  var s = String(value).replace(/[\r\n\t]+/g, ' ').replace(/<\/?(?:[a-zA-Z][a-zA-Z0-9]*)\b[^>]*>/g, '').replace(/\s{2,}/g, ' ').trim();
...
function sanitize1DPrefix(rawText) {
  if (!rawText) return '';
  return String(rawText).replace(/^(1D\s*)+/i, '1D ');
}
...
function getTrendShort(r) {
  ...
  if (tf1 && tf1 !== '-') {
    var cleanTf1 = tf1.replace(/\s*\(.*?\)\s*/g, '').trim();
    cleanTf1 = cleanTf1.replace(/^(1D\s*)+/i, '1D ');
    if (!/^1D\s/i.test(cleanTf1)) cleanTf1 = '1D ' + cleanTf1;
    parts.push(cleanTf1);
  }
  if (tf5 && tf5 !== '-') {
    var cleanTf5 = tf5.replace(/\s*\(.*?\)\s*/g, '').trim();
    cleanTf5 = cleanTf5.replace(/^(5D\s*)+/i, '5D ');
    if (!/^5D\s/i.test(cleanTf5)) cleanTf5 = '5D ' + cleanTf5;
    parts.push(cleanTf5);
  }
...
function getPatternEdgeShort(r) {
  if (!r) return null;
  if (r.pattern_edge_line) {
    if (!/RSI_OVERBOUGHT/i.test(r.pattern_edge_line)) return r.pattern_edge_line;
  }
  var keyOrData = r.pattern_personality || r.patternPersonality || r.matched_pattern || r.pattern_key;
  if (keyOrData && !/RSI_OVERBOUGHT/i.test(String(keyOrData.key || keyOrData))) {
    var line = patternPersonality.formatPatternPersonalityLine(keyOrData);
    if (line) return line;
  }
  try {
    var swingEngine = require('./swing-screener-engine');
    var edgeKey = swingEngine.classifySwingEdge(r);
    if (edgeKey) {
      var edgeLine = patternPersonality.formatPatternPersonalityLine(edgeKey);
      if (edgeLine) return edgeLine;
    }
  } catch (_) {}

  if (typeof patternPersonality.matchTickerPattern === 'function') {
    var matched = patternPersonality.matchTickerPattern(r);
    if (matched && !/RSI_OVERBOUGHT/i.test(matched)) {
      return patternPersonality.formatPatternPersonalityLine(matched);
    }
  }
  return null;
}
...
function formatMonitorHitMessage(pick, ev, px) {
  ...
  var category = safe(pick.category || pick.monitor_source, '');
  var isDaytrade = category.toLowerCase().indexOf('day') >= 0;
  ...
  function getTriggerBasis() {
    if (status === 'SL_HIT') {
      ...
      return 'SL tersentuh';
    } else if (status === 'BEP_CLOSED') {
      if (low != null && sl != null) {
        return 'harga turun menyentuh level BEP (Entry + 1 tick) ' + fmtPrice(sl);
      }
      return 'level Break-Even (BEP) tersentuh';
  ...
  if (status === 'TP1_HIT') {
    ...
    // Kartu aksi tegas: TP1 = amankan 50% profit, lalu geser SL ke BEP.
    catatan = 'AKSI WAJIB \u2014 Amankan profit 50% posisi SEKARANG, lalu geser SL ke BEP (Entry + 1 tick). Target TP1 tercapai! Peringatan: Jual 50-70% posisi & segera geser SL ke BEP (Entry + 1 tick).';
  } else if (status === 'TP2_HIT') {
    emoji = '\uD83D\uDE80';
    statusLabel = 'TP2 HIT';
    triggerLine = 'Trigger: ' + getTriggerBasis();
    ...
    emoji = '\uD83D\uDEE1\uFE0F';
    statusLabel = 'BEP CLOSED — BREAK EVEN';
    triggerLine = 'Trigger: ' + getTriggerBasis();
    profitLine = 'P/L: 0.0% (Break Even)';
    catatan = 'Posisi ditutup di level BEP (Entry + 1 tick) setelah running profit menyentuh +2.0%. Modal aman terproteksi tanpa rugi.';
  } else if (status === 'EARLY_EXIT_DISTRIBUTION') {
    emoji = '🚨';
    statusLabel = 'AUTO-CUAN SWING — BANDAR DISTRIBUTION ALERT / EARLY EXIT';
    var distDetails = [];
    var cr3Val = ev.cr3 != null ? ev.cr3 : (pick.cr3 != null ? pick.cr3 : null);
    var cr5Val = ev.cr5 != null ? ev.cr5 : (pick.cr5 != null ? pick.cr5 : null);
    var netFlowVal = ev.net_flow != null ? ev.net_flow : (pick.net_flow != null ? pick.net_flow : null);
    var retailVal = ev.retail_participation != null ? ev.retail_participation : (pick.retail_participation != null ? pick.retail_participation : null);

    if (cr3Val != null) {
      var cr3Display = typeof cr3Val === 'number' ? (Math.abs(cr3Val) <= 1 ? (cr3Val * 100).toFixed(0) + '%' : cr3Val + '%') : cr3Val;
      distDetails.push('CR3: ' + cr3Display);
    }
    if (cr5Val != null) {
      var cr5Display = typeof cr5Val === 'number' ? (Math.abs(cr5Val) <= 1 ? (cr5Val * 100).toFixed(0) + '%' : cr5Val + '%') : cr5Val;
      distDetails.push('CR5: ' + cr5Display);
    }
    if (netFlowVal != null) {
      distDetails.push('Net Flow: ' + fmtSignedValue(netFlowVal));
    }
    if (retailVal != null) {
      var retDisplay = typeof retailVal === 'number' ? (Math.abs(retailVal) <= 1 ? (retailVal * 100).toFixed(0) + '%' : retailVal + '%') : retailVal;
      distDetails.push('Ritel: ' + retDisplay);
    }

    triggerLine = distDetails.length > 0 ? ('Distribusi: ' + distDetails.join(' · ')) : 'Distribusi: Top broker net sell masif terdeteksi';
    if (entry1 && last && entry1 > 0) {
      var pctDiff = ((last - entry1) / entry1) * 100;
      profitLine = 'P/L saat ini: ' + (pctDiff >= 0 ? '+' : '') + pctDiff.toFixed(1) + '%';
    }
    catatan = 'Peringatan distribusi bandar terdeteksi. Amankan modal atau pertimbangkan exit dini sebelum level SL teknikal.';
    ...
    // Kartu aksi tegas: SL = cut loss proteksi modal, tanpa buyback.
    catatan = 'AKSI WAJIB \u2014 CUT LOSS sekarang untuk proteksi modal. Stop Loss tersentuh! Peringatan: Cut loss disiplin, jangan buyback sebelum ada setup baru (meskipun harga tampak rebound).';
  ...
  if (status === 'EARLY_EXIT_DISTRIBUTION' && category && category !== '-') {
    var catFormatted = category.replace(/_/g, ' ');
    if (catFormatted.toLowerCase().indexOf('swing') >= 0) {
      catFormatted = catFormatted.replace(/\b\w/g, function(l) { return l.toUpperCase(); });
    }
    lines.push('Kategori: ' + catFormatted);
  }
...
function getDayTradeMetrics(r) {
  ...
  // BUG-F9-08: Logical OR collapsed numeric 0 (empty bid queue / ARB / offer-dominated book)
  // to the fallback string 'Bid Dominant', misinforming traders on falling / locked-offer stocks.
  var rawDom = r.bid_offer_dominance != null ? r.bid_offer_dominance : (r.bid_dominance != null ? r.bid_dominance : (r.orderbook_dominance != null ? r.orderbook_dominance : r.dominance));
  var dominance;
  if (rawDom != null && rawDom !== '' && !isNaN(Number(rawDom))) {
    var numDom = Number(rawDom);
    var pctDom = (numDom > 1 && numDom <= 100) ? numDom : (numDom * 100);
    if (pctDom <= 45) {
      dominance = 'Offer Dominant (' + Math.round(100 - pctDom) + '% Offer)';
    } else if (pctDom >= 55) {
      dominance = 'Bid Dominant (' + Math.round(pctDom) + '% Bid)';
    } else {
      dominance = 'Balanced (Bid/Offer Seimbang)';
    }
  } else {
...
function getSwingBandarIntel(r) {
  var ticker = (r && r.ticker ? String(r.ticker).trim().toUpperCase() : '');
  ...
  // Jika CR3 / CR5 atau flow belum tersemat di baris, hubungkan dengan modul Bandarmologi riil
  if ((cr3 == null || cr3 === 0) && ticker) {
    try {
      var intelService = require('./bandarmologi-intel-service');
      if (typeof intelService.computeConcentrationRatios === 'function') {
        var crData = intelService.computeConcentrationRatios(ticker, { range: '7d' });
        if (crData && crData.triggered && crData.cr3 > 0) {
          cr3 = crData.cr3;
          cr5 = crData.cr5;
          if (!bandarStatus || bandarStatus === '-') bandarStatus = crData.label || crData.status;
        }
      }
      if (netFlow == null && typeof intelService.detectSilentForeignAccumulation === 'function') {
        var fData = intelService.detectSilentForeignAccumulation(ticker, { range: '7d' });
        if (fData && fData.triggered && fData.total_foreign_net > 0) {
          netFlow = fData.total_foreign_net;
        }
      }
    } catch (_) {}
  }
  ...
  var items = [];
  if (cr3 != null && Number(cr3) > 0) {
    var cr3Num = Number(cr3);
    var cr3Str = cr3Num <= 1.0 ? (cr3Num * 100).toFixed(1) + '%' : (cr3Num <= 100 ? cr3Num.toFixed(1) + '%' : fmtValue(cr3Num));
    items.push('CR3: ' + cr3Str);
  }
  if (cr5 != null && Number(cr5) > 0) {
    var cr5Num = Number(cr5);
    var cr5Str = cr5Num <= 1.0 ? (cr5Num * 100).toFixed(1) + '%' : (cr5Num <= 100 ? cr5Num.toFixed(1) + '%' : fmtValue(cr5Num));
    items.push('CR5: ' + cr5Str);
  }
  if (netFlow != null && Number(netFlow) !== 0) {
    items.push('Foreign Flow: ' + fmtSignedValue(netFlow));
  }
  if (retail != null && Number(retail) > 0) {
    items.push('Partisipasi Ritel: ' + (typeof retail === 'number' ? retail.toFixed(1) + '%' : retail));
  }
  if (bandarStatus && bandarStatus !== '-' && bandarStatus !== 'NORMAL_DIFFUSE' && bandarStatus !== 'NO_DATA') {
    items.push('Status: ' + bandarStatus);
  }

  if (items.length === 0) {
    return 'Netral / Tersebar';
  }
  return items.join(' · ');
}

// Ambang peringatan R:R tinggi untuk Swing Non-Konglo — memakai konstanta
// kanonik dari lib/swing-nk-rr-warning.js supaya tidak ada ambang kedua.
var SWING_NK_HIGH_RR_THRESHOLD = require('./swing-nk-rr-warning').SWING_NK_HIGH_RR_WARNING_THRESHOLD;

/**
 * SIGNAL CARD RINGKAS — format resmi yang selalu menampilkan enam elemen
 * wajib dalam urutan tetap, supaya pembaca tidak perlu menyisir seluruh blok:
 *   Entry Zone · TP1 (+ pasang BEP) · TP2 · Cut Loss (SL) · R:R · Volume Pace
 *
 * Blok detail di bawahnya tetap dicetak apa adanya (backward compatible),
 * jadi kartu ini bersifat aditif, bukan pengganti.
 *
 * @param {object} ctx - { mode, entryLow, entryHigh, tp1, tp2, sl, rr, volumePace }
 * @returns {string}
 */
function formatSignalCardSummary(ctx) {
  var isDayTrade = ctx.mode === 'daytrade';
  var entryLow = toNum(ctx.entryLow);
  var entryHigh = toNum(ctx.entryHigh);
  var entryZone;
  if (entryLow != null && entryHigh != null && entryLow !== entryHigh) {
    entryZone = fmtPrice(Math.min(entryLow, entryHigh)) + ' - ' + fmtPrice(Math.max(entryLow, entryHigh));
  } else {
    entryZone = fmtPrice(entryHigh != null ? entryHigh : entryLow);
  }
  var refEntry = entryHigh != null && entryHigh > 0 ? entryHigh : entryLow;
  function pctFrom(label, value) {
    var n = toNum(value);
    if (n == null || refEntry == null || !(refEntry > 0)) return null;
    var pct = Math.round(((n / refEntry) - 1) * 1000) / 10;
    return label + ' (' + (pct >= 0 ? '+' : '') + pct + '%)';
  }

  var lines = [];
  lines.push('\uD83D\uDCCB SIGNAL CARD');
  lines.push('Entry Zone: ' + entryZone);
  if (isDayTrade) {
    // Day Trade: TP1 wajib disertai instruksi pasang BEP (Entry + 1 tick).
    var tp1LabelDt = pctFrom('TP1', ctx.tp1);
    lines.push((tp1LabelDt || 'TP1') + ': ' + fmtPrice(ctx.tp1) + ' \u2014 pasang BEP setelah TP1');
    var tp2LabelDt = pctFrom('TP2', ctx.tp2);
    if (ctx.tp2 != null) lines.push((tp2LabelDt || 'TP2') + ': ' + fmtPrice(ctx.tp2));
    lines.push('Cut Loss (SL): ' + fmtPrice(ctx.sl));
  } else {
    var tp1LabelSw = pctFrom('TP1', ctx.tp1);
    lines.push((tp1LabelSw || 'TP1') + ': ' + fmtPrice(ctx.tp1) + ' \u2014 amankan 50% & pasang BEP');
    var tp2LabelSw = pctFrom('TP2', ctx.tp2);
    if (ctx.tp2 != null) lines.push((tp2LabelSw || 'TP2') + ': ' + fmtPrice(ctx.tp2));
    lines.push('Cut Loss (SL): ' + fmtPrice(ctx.sl));
  }
  lines.push('Risk/Reward Ratio: ' + fmtRR(ctx.rr));
  if (isDayTrade) lines.push('Volume Pace: ' + safe(ctx.volumePace, '-'));
  return lines.join('\n');
}
...
function formatSignalCard(r, idx, mode) {
  ...
  // Unified Scoring (Fase 3): prefer the unified number so a template-rendered
  // score matches the web card. Falls back through the legacy fields for rows
  // that predate the unified engine.
  var convScore = r.unified_score || r.conviction_score || r.daytrade_score || r.telegram_conviction_score || r.score || r.publish_score;
  ...
  // SIGNAL CARD RINGKAS — enam elemen wajib (lihat formatSignalCardSummary).
  var _summaryPace = mode === 'daytrade' ? getDayTradeMetrics(r).volPace : null;
  lines.push(formatSignalCardSummary({
    mode: mode,
    entryLow: entryLow,
    entryHigh: entryHigh,
    tp1: tp1,
    ...
    sl: sl,
    rr: rr,
    volumePace: _summaryPace
  }));
  lines.push('');

  // Swing: grup/sektor + setup eksplisit, sesuai standar kartu Swing.
  if (mode !== 'daytrade') {
    var swingGroupLabel = mode === 'swing_non_konglo' ? 'Non-Konglo' : 'Konglo';
    var swingSector = sector && sector !== '-' ? sector : '-';
    lines.push('Grup/Sektor: ' + swingGroupLabel + ' / ' + swingSector);
    var swingSetup = getPatternShort(r);
    lines.push('Setup: ' + (swingSetup || signalLabel));
    // Peringatan R:R tinggi khusus Swing Non-Konglo (ambang kanonik 2.5x).
    if (mode === 'swing_non_konglo' && rr != null && rr > SWING_NK_HIGH_RR_THRESHOLD) {
      lines.push('\u26A0\uFE0F Peringatan R:R tinggi: target ' + fmtRR(rr) + ' > ' + SWING_NK_HIGH_RR_THRESHOLD + 'x. Secara historis sinyal Non-Konglo dengan R:R setinggi ini lebih sering kena SL sebelum TP dalam window pantau 3-7 hari. Pertimbangkan target lebih dekat atau tunggu konfirmasi tambahan.');
    }
    lines.push('');
  }
  ...
  if (_tpv2Public) {
    ...
    var lowV2 = Math.min(_c.entry_zone_low || 0, _c.entry_zone_high || 0);
    var highV2 = Math.max(_c.entry_zone_low || 0, _c.entry_zone_high || 0);
    var areaBeliV2 = (lowV2 > 0 && highV2 > 0 && lowV2 !== highV2) ? fmtPrice(lowV2) + ' - ' + fmtPrice(highV2) : fmtPrice(highV2 || lowV2);
    lines.push('Area Beli (Entry): ' + areaBeliV2);
    ...
    if (mode === 'daytrade') {
      ...
      var tpLineV2 = 'Target Profit 1 (+5% s/d +6% Partial TP 50%): ' + _d.tp1;
      if (_c.tp2 != null && _d.tp2 !== '—' && _d.tp2 !== '-') {
        tpLineV2 += ' / Target Profit 2 (Fib Extension): ' + _d.tp2;
      }
      lines.push(tpLineV2);
    }
    ...
    var lowVal = Math.min(e1, e2);
    var highVal = Math.max(e1, e2);
    var areaBeliText = (lowVal > 0 && highVal > 0 && lowVal !== highVal) ? fmtPrice(lowVal) + ' - ' + fmtPrice(highVal) : fmtPrice(highVal || lowVal);
    lines.push('Area Beli (Entry): ' + areaBeliText);
    if (mode === 'daytrade') {
      var tp1Text = tp1 ? fmtPrice(tp1) : '—';
      var tp2Text = tp2 ? fmtPrice(tp2) : '—';
      var tp1Pct = (tp1 && e1 > 0) ? Math.round(((tp1 / e1) - 1) * 1000) / 10 : null;
      var tp2Pct = (tp2 && e1 > 0) ? Math.round(((tp2 / e1) - 1) * 1000) / 10 : null;
      var tp1Label = tp1Pct != null ? 'Target Profit 1 (' + (tp1Pct >= 0 ? '+' : '') + tp1Pct + '%)' : 'Target Profit 1';
      var tp2Label = tp2Pct != null ? 'Target Profit 2 (' + (tp2Pct >= 0 ? '+' : '') + tp2Pct + '%)' : 'Target Profit 2';
      lines.push(tp1Label + ': ' + tp1Text + (tp2Text !== '—' ? ' / ' + tp2Label + ': ' + tp2Text : ''));
    } else {
      var refEntry = lowVal > 0 ? lowVal : (highVal > 0 ? highVal : last);
      var tp1SwingText = tp1 ? fmtPrice(tp1) : '—';
      var tp2SwingText = tp2 ? fmtPrice(tp2) : (r.fib_extension ? fmtPrice(r.fib_extension) : null);
      var tp1SwingPct = (tp1 && refEntry > 0) ? Math.round(((tp1 / refEntry) - 1) * 1000) / 10 : null;
      var tp1SwingLabel = tp1SwingPct != null ? 'Target Profit 1 (' + (tp1SwingPct >= 0 ? '+' : '') + tp1SwingPct + '% Partial TP 50%)' : 'Target Profit 1 (Partial TP 50%)';
      var tpLine = tp1SwingLabel + ': ' + tp1SwingText;
      if (tp2SwingText) {
        tpLine += ' / Target Profit 2 (Fib Extension): ' + tp2SwingText;
      }
      lines.push(tpLine);
    }
  ...
  if (mode === 'daytrade') {
    ...
    // BATCH 4: ringkasan modal bandar (harga rata-rata beli bandar utama)
    // pada kartu Day Trade, agar sinyal dapat di-validasi terhadap akumulasi
    // broker summary tanpa menunggu tampilan swing.
    var bandarAvgBuy = toNum(r.bandar_avg_buy || r.avg_buy_price || r.broker_cost || r.harga_modal);
    if (bandarAvgBuy != null && bandarAvgBuy > 0) {
      var bandarDiscount = toNum(r.bandar_discount_pct != null ? r.bandar_discount_pct : r.discount_pct);
      lines.push('Modal Bandar: ' + fmtPrice(bandarAvgBuy) + (bandarDiscount != null ? ' (diskon ' + bandarDiscount + '%)' : ''));
    }
  } else {
  ...
  var patternEdgeLine = getPatternEdgeShort(r);
  if (patternEdgeLine) lines.push('Edge: ' + patternEdgeLine);
...
function formatSwingKongloSignalMessage(results, options) {
  ...
  results = results || [];
  var lines = [];
  var isWatchlist = options.isWatchlist || (results.length > 0 && results.some(function(r) {
    var s = safe(r.status || r.final_status, '').toUpperCase();
    var a = safe(r.telegram_action_label || r.action || '', '').toLowerCase();
    return s.indexOf('WAIT') >= 0 || s.indexOf('WATCH') >= 0 || a.indexOf('tunggu') >= 0 || a.indexOf('pantau') >= 0;
  }));
  if (isWatchlist) {
    lines.push('🎯 AUTO-CUAN SWING TRADE — WATCHLIST & PULLBACK');
  } else {
...
function formatOpeningRadarMessage(results, options) {
  ...
  lines.push('📡 Day Trade RADAR/MONITOR — BUKAN REKOMENDASI BELI');
...
function formatRadarDigestMessage(results, title) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    var eLow = Math.min(entryLow || 0, entryHigh || 0);
    var eHigh = Math.max(entryLow || 0, entryHigh || 0);
    var sl = toNum(r.stop_loss || r.sl);
    var tp1 = toNum(r.tp1 || r.tp1n);

    lines.push((i + 1) + '. ' + safe(r.ticker, '-'));
    var entryDisplay = (eLow > 0 && eHigh > 0 && eLow !== eHigh)
      ? fmtPrice(eLow) + ' - ' + fmtPrice(eHigh)
      : fmtPrice(eHigh || eLow);
    lines.push('Entry: ' + entryDisplay + ' | SL: ' + fmtPrice(sl) + ' | TP: ' + fmtPrice(tp1));
...
module.exports = {
  ...
  SWING_NK_HIGH_RR_THRESHOLD: SWING_NK_HIGH_RR_THRESHOLD,
  formatSignalCardSummary: formatSignalCardSummary,
  safe: safe,
  fmtPrice: fmtPrice,
  fmtRR: fmtRR,
  fmtValue: fmtValue,
  fmtSignedValue: fmtSignedValue,
  fmtSignedNumber: fmtSignedNumber,
  ...
  sanitize1DPrefix: sanitize1DPrefix,
  getPatternEdgeShort: getPatternEdgeShort,
