...
/**
 * Classify signal label based on status AND price-vs-entry position.
 *
 * Priority: entry_status > status
 * This ensures accurate labeling based on actual price position, not just setup type.
 *
 * Mapping:
 * - EARLY RADAR: early detection, not near entry yet, no chase signal
 * - WATCHLIST: valid candidate but needs confirmation
 * - TUNGGU PULLBACK / WAIT PULLBACK: price already above/too far from entry, do not chase
 * - ENTRY ZONE: price is near the entry area
 * - ENTRY HIT: price touched valid entry zone
 * - RUNNING: already entered, moving between entry and TP
 * - TP1 HIT: first target hit
 * - TP2 HIT: second target hit
 * - SL HIT / INVALIDATED: stop loss or invalidation hit
 * - AVOID / SKIP: risk too high or quality gate weak
 */
/**
 * Classify signal label based on status AND price-vs-entry position.
 *
 * PRECEDENCE (highest to lowest):
 * 1. SL HIT / INVALIDATED — terminal, never override
 * 2. TP2 HIT — terminal, never override
 * 3. TP1 HIT — terminal, never override
 * 4. ENTRY HIT — monitor active, never override with pullback
 * 5. RUNNING — monitor active, never override with pullback
 * 6. ENTRY ZONE — price in entry area
 * 7. TUNGGU PULLBACK — only applies BEFORE entry (chase risk)
 * 8. READY / EARLY RADAR / WATCHLIST / AVOID — setup statuses
 *
 * IMPORTANT: TUNGGU PULLBACK only applies before entry.
 * If signal is already ENTRY_HIT, RUNNING, TP1_HIT, TP2_HIT, or SL_HIT,
 * do NOT relabel it as TUNGGU PULLBACK just because entry_distance_pct > 3%.
 */
function classifySignalLabel(r) {
  var status = safe(r.status || r.final_status, '').toUpperCase().replace(/_/g, ' ');
  var entryStatus = safe(r.entry_status || r.entry_quality_status, '').toUpperCase().replace(/_/g, ' ');
  var entryDistPct = toNum(r.entry_distance_pct);

  // === PRIORITY 1: TERMINAL STATUSES (never override) ===
  // SL HIT / INVALIDATED
  if (entryStatus.indexOf('SL') >= 0 || entryStatus.indexOf('INVALID') >= 0) return 'SL HIT / INVALIDATED';
  if (status.indexOf('SL HIT') >= 0 || status.indexOf('SL_KENA') >= 0) return 'SL HIT / INVALIDATED';

  // TP2 HIT
  if (entryStatus.indexOf('TP2') >= 0) return 'TP2 HIT';
  if (status.indexOf('TP2 HIT') >= 0 || status.indexOf('TP2_HIT') >= 0) return 'TP2 HIT';

  // TP1 HIT
  if (entryStatus.indexOf('TP1') >= 0) return 'TP1 HIT';
  if (status.indexOf('TP1 HIT') >= 0 || status.indexOf('TP1_HIT') >= 0) return 'TP1 HIT';

  // === PRIORITY 2: MONITOR ACTIVE STATUSES (never override with pullback) ===
  // ENTRY HIT / RUNNING — already entered, monitor active
  if (entryStatus.indexOf('ENTRY HIT') >= 0 || status.indexOf('ENTRY HIT') >= 0 || status.indexOf('ENTRY_HIT') >= 0) return 'ENTRY HIT';
  if (entryStatus.indexOf('RUNNING') >= 0 || status.indexOf('RUNNING') >= 0) return 'RUNNING';

  // === PRIORITY 3: ENTRY ZONE (price in entry area) ===
  if (entryStatus.indexOf('IN ENTRY') >= 0 || entryStatus === 'IN_ENTRY_AREA' || entryStatus === 'IN ENTRY AREA') {
    return 'ENTRY ZONE';
  }

  // === PRIORITY 4: TUNGGU PULLBACK (only BEFORE entry) ===
  // This only applies to signals that have NOT yet entered.
  // If price is extended/chase risk BEFORE entry, show WAIT PULLBACK
  if (entryStatus.indexOf('EXTENDED') >= 0 || entryStatus.indexOf('CHASE') >= 0) {
    return 'TUNGGU PULLBACK';
  }

  // Explicit WAIT_PULLBACK status
  if (status.indexOf('WAIT PULLBACK') >= 0) return 'TUNGGU PULLBACK';

  // Use entry_distance_pct for chase detection (> 3% above entry = pullback)
  // BUT only for signals that haven't entered yet (no ENTRY HIT/RUNNING/TP/SL status)
  if (entryDistPct != null && entryDistPct > 3) {
    return 'TUNGGU PULLBACK';
  }

  // === PRIORITY 5: SETUP STATUSES ===
  // High-conviction setups (A+, Trade Candidate, Ready Breakout)
  if (status.indexOf('A PLUS') >= 0 || status.indexOf('TRADE CANDIDATE') >= 0) return 'READY';
  if (status.indexOf('READY') >= 0 || status.indexOf('SWING READY') >= 0) {
    // But if entry_distance_pct shows chase risk (2-3%), downgrade to pullback
    if (entryDistPct != null && entryDistPct > 2) return 'TUNGGU PULLBACK';
    return 'READY';
  }

  // Early detection / pre-spike
  if (status.indexOf('EARLY RADAR') >= 0 || status.indexOf('PRE SPIKE') >= 0) return 'EARLY RADAR';

  // Speculative / avoid
  if (status.indexOf('AVOID') >= 0) return 'AVOID / SKIP';
  if (status.indexOf('SPECULATIVE') >= 0) return 'WATCHLIST';

  // Watchlist / needs confirmation
  if (status.indexOf('WATCHLIST') >= 0) return 'WATCHLIST';
  if (status.indexOf('REBOUND') >= 0) return 'WATCHLIST';
...
function formatSignalCard(r, idx, mode) {
  ...
  // Header with status prominently displayed
  lines.push(idx + '. \uD83C\uDDEE\uD83C\uDDE9 ' + ticker);
  lines.push('Signal: ' + signalLabel);

  // Add warning note for TUNGGU PULLBACK status
  if (signalLabel === 'TUNGGU PULLBACK') {
    lines.push('');
    lines.push('\u26A0\uFE0F Harga sudah di atas area entry. Hindari chase, tunggu pullback ke area entry.');
  }
  ...
  if (entryDistPct != null && last != null && last > 0) {
    lines.push('Jarak dari entry: ' + (entryDistPct >= 0 ? '+' : '') + entryDistPct.toFixed(1) + '%');
  }
  lines.push('');
