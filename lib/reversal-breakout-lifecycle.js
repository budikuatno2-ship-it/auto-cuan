...
/**
 * BATCH4-F8-01: has this row been evaluated under the CURRENT lifecycle rules
 * with the SAME evidence? The version stamp alone is not enough: a stamped row
 * whose phase inputs changed (a confirmed breakout, a lifted safety block) must
 * be re-derived, otherwise the lifecycle is write-once and freezes at whatever
 * phase the morning scan happened to produce.
 *
 * The fingerprint deliberately covers only the fields that feed derivePhase()
 * and scoreAdjustment(), so an unchanged row still short-circuits and stays
 * idempotent.
 */
function evidenceFingerprint(row) {
  ...
  var parts = [
    ...
    finite(input.support),
    finite(input.entryHigh),
    finite(input.volumeRatio),
    finite(input.rsi14),
    finite(input.changePct),
    input.nearAra ? 1 : 0,
    row && row.corporate_action_guard === 'BLOCKED' ? 1 : 0,
    row && row.data_quality_valid === false ? 1 : 0,
    row && row.data_quality_needs_revalidation === true ? 1 : 0
  ];
  return parts.join('|');
}
...
function applyLifecycle(row, context) {
  ...
  var blockedNow = isBlocked(row, context, deriveInputs(row));

  // BATCH4-F8-01: re-derive whenever the evidence changed, the row is currently
  // blocked (so a block is always reflected), or the version is stale. Only a
  // row that is version-current, unblocked and evidence-identical short-circuits.
  var fingerprint = evidenceFingerprint(row);
  if (row.lifecycle_version === VERSION &&
      !blockedNow &&
      row.lifecycle_evidence_fingerprint === fingerprint) {
    return row;
  }
  ...
  // BATCH4-F8-02: the score the lifecycle adjusts must be the UN-ADJUSTED base.
  // Reading `row.daytrade_score` directly stacked the previous adjustment onto
  // the new one, so a phase advance kept the old bonus and the published score
  // no longer corresponded to any derived phase. Always restore the recorded
  // base first when this row was already adjusted.
  if (applyScore && row.lifecycle_score_applied === true) {
    if (mode === 'daytrade' && finite(row.daytrade_score_before_lifecycle) != null) {
      row.daytrade_score = finite(row.daytrade_score_before_lifecycle);
    } else if (mode === 'swing' && finite(row.score_before_lifecycle) != null) {
      row.score = finite(row.score_before_lifecycle);
    }
  }
