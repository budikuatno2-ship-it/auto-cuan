...
function initTrackerState(record) {
  return {
    ...
    // Notification dedup — persisted so a process restart can never resend.
    // At most one of each per ticker/trading_date/early_watch_version.
 ...
 * anti_chase_sent, failures })` AFTER capture/tracking but BEFORE the state
 * is persisted, so notification-sent flags land in the same atomic write.
 * It is optional and defaults to a no-op — this core module never sends
 * anything by itself. A failure inside it is caught here and never prevents
 * the tracking state itself from being persisted.
 */
...
async function runEarlyWatch(options) {
  ...
  if (!opts.dryRun) {
    ...
    await (opts.writeState || writeStateAtomic)(stateFile, finalState);
