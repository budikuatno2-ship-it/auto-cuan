...
// Single authoritative runtime flag. The old shadow-era
// FAST_WATCHER_EARLY_WATCH_SHADOW_ENABLED name was never merged/activated
// as a production contract, so it is NOT honored here — setting it has no
// effect. Deliberately removed rather than kept as a second way to enable
// live Telegram sends: that shadow flag predates the at-most-once-attempt
// and stale-send safety work in this feature, so silently trusting it
// would be unsafe.
...
function flagTrue(value) {
  const s = String(value || 'false').toLowerCase().trim();
  return s === 'true' || s === '1' || s === 'yes' || s === 'on';
}

function isEnabled(env) {
  const e = env || process.env;
  return flagTrue(e[ENV_FLAG]);
}
