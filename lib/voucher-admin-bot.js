...
const { sendTransient, deletePrevious } = require('./telegram-transient-message');
...
async function processVoucherAdminUpdate(update, deps) {
  ...
  // Disposable prompts/status replace the previous admin chatter. Voucher-code
  // delivery below bypasses this helper and remains persistent.
  const reply = async (text, extra) => sendTransient({ db, sender, chatId:input.chat.id, scope:'voucher_admin', text, extra });
...
async function deliverOneChunk(input, d, reply, sender, batch) {
  ...
  // The confirmation/menu message is disposable; remove it before sending the
  // persistent voucher code payload. The code payload itself is NOT tracked as
  // transient and therefore stays visible until consumption cleanup.
  await deletePrevious(d.db, sender, input.chat.id, 'voucher_admin');

  let sent;
  try {
    if (count <= MESSAGE_CODE_LIMIT) {
      // Production uses the raw sender so voucher codes are never registered as
      // transient. The reply fallback exists only for dependency-injected tests
      // and legacy callers that provide the message transport through `reply`.
      sent = sender && typeof sender.sendMessage === 'function'
        ? await sender.sendMessage(input.chat.id, '<b>' + batch.batch_reference + '</b>\n<pre>' + formatCodes(codes, batch) + '</pre>')
        : await reply('<b>' + batch.batch_reference + '</b>\n<pre>' + formatCodes(codes, batch) + '</pre>');
    } else {
      sent = await sender.sendDocument(input.chat.id, 'vouchers-' + batch.batch_reference + '-' + chunk.chunk_index + '.txt', Buffer.from(formatCodes(codes, batch), 'utf8'));
    }
  } catch (_) {
