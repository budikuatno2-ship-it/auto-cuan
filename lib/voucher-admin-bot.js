...
async function processVoucherAdminUpdate(update,deps) {
 ...
 const data=input.callback.data; if(typeof data!=='string'||!/^v:(?:begin|list|detail|disable|audit|stats|continue:VB-[A-F0-9]{12}|type:(?:PERCENT_30|PERCENT_50|PERCENT_100|LIFETIME)|plan:[A-Z0-9_]+|confirm:[A-Fa-f0-9-]{16,64}|cancel)$/.test(data)){await sender.answerCallbackQuery(input.callback.id,{text:'Tombol kedaluwarsa.'});return {outcome:'stale'};}
...
async function deliverOneChunk(input,d,reply,sender,batch){
 ...
 const chunk=claim.data, count=chunk.expected_count, codes=[]; if(!Number.isSafeInteger(count)||count<1||count>CHUNK_SIZE||typeof chunk.attempt_id!=='string'||typeof chunk.claim_token!=='string') return {outcome:'claim_invalid',generated:0}; for(let i=0;i<count;i++) codes.push(vouchers.generateVoucherCode());
 try { const saved=await d.db.rpc('prepare_voucher_admin_batch_chunk',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_attempt_id:chunk.attempt_id,p_claim_token:chunk.claim_token,p_items:codes.map(code=>({code_hash:vouchers.voucherCodeHash(code),code_hint:vouchers.voucherCodeHint(code)}))}); if(saved.error||!saved.data||saved.data.prepared!==true||saved.data.attempt_id!==chunk.attempt_id||saved.data.expected_count!==count||saved.data.stored_count!==count) return {outcome:'prepare_failed',generated:0};
  const sent=count<=MESSAGE_CODE_LIMIT ? await reply('<b>'+batch.batch_reference+'</b>\n<pre>'+formatCodes(codes,batch)+'</pre>') : await sender.sendDocument(input.chat.id,'vouchers-'+batch.batch_reference+'-'+chunk.chunk_index+'.txt',Buffer.from(formatCodes(codes,batch),'utf8')); const messageId=Number(sent&&sent.message_id); if(!Number.isSafeInteger(messageId)||messageId<1) return {outcome:'delivery_uncertain',generated:0};
  const receipt=await d.db.rpc('record_voucher_admin_chunk_delivery',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_attempt_id:chunk.attempt_id,p_claim_token:chunk.claim_token,p_delivery_method:count<=MESSAGE_CODE_LIMIT?'message':'document',p_telegram_message_id:messageId,p_delivered_count:count});if(receipt.error){await d.db.rpc('mark_voucher_admin_chunk_delivery_uncertain',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_attempt_id:chunk.attempt_id,p_claim_token:chunk.claim_token});return {outcome:'delivery_uncertain',generated:0};}
  const fin=await d.db.rpc('finalize_voucher_admin_batch_chunk',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_attempt_id:chunk.attempt_id,p_claim_token:chunk.claim_token});if(fin.error)throw new Error('unavailable');
  const next=fin.data&&fin.data.complete?'':' Lanjutkan: /menu'; return {outcome:'delivered',generated:count,message:next};
 }catch(_){await d.db.rpc('mark_voucher_admin_chunk_delivery_uncertain',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_attempt_id:chunk.attempt_id,p_claim_token:chunk.claim_token});return {outcome:'delivery_uncertain',generated:0};}
}
async function confirm(input,d,reply,key,sender){const made=await d.db.rpc('create_voucher_admin_batch',{p_telegram_user_id:input.from.id,p_confirmation_key:key});if(made.error||!made.data)return {outcome:'failed'};if(made.data.already_exists)return {outcome:'duplicate_confirmation',progress:made.data.progress_cursor||0};const result=await deliverOneChunk(input,d,reply,sender,made.data);if(result.outcome==='delivered'&&result.message)await reply('Bagian dikirim. Gunakan tombol lanjutkan.',{reply_markup:{inline_keyboard:[[{text:'Lanjutkan batch',callback_data:'v:continue:'+made.data.batch_reference}]]}});return result;}
