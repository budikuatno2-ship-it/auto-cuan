...
async function deliverOneChunk(input,d,reply,sender,batch){
 ...
 try { const saved=await d.db.rpc('insert_voucher_admin_batch_items',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_claim_token:chunk.claim_token,p_items:codes.map(code=>({code_hash:vouchers.voucherCodeHash(code),code_hint:vouchers.voucherCodeHint(code)}))}); if(saved.error)throw new Error('unavailable');
  ...
  const fin=await d.db.rpc('finalize_voucher_admin_batch_chunk',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_claim_token:chunk.claim_token});if(fin.error)throw new Error('unavailable');
  const next=fin.data&&fin.data.complete?'':' Lanjutkan: /menu'; return {outcome:'delivered',generated:count,message:next};
 }catch(_){await d.db.rpc('cancel_voucher_admin_batch_chunk',{p_batch_reference:batch.batch_reference,p_chunk_index:chunk.chunk_index,p_claim_token:chunk.claim_token});return {outcome:'delivery_failed',generated:0};}
}
