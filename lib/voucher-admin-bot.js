...
async function processVoucherAdminUpdate(update,deps) {
 ...
 const data=input.callback.data; if(typeof data!=='string'||!/^v:(?:begin|list|detail|disable|audit|stats|continue:[A-F0-9-]{3,32}|type:(?:PERCENT_30|PERCENT_50|PERCENT_100|LIFETIME)|plan:[A-Z0-9_]+|confirm:[A-F0-9-]{16,64}|cancel)$/.test(data)){await sender.answerCallbackQuery(input.callback.id,{text:'Tombol kedaluwarsa.'});return {outcome:'stale'};}
