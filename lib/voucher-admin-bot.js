...
async function processVoucherAdminUpdate(update,deps) {
 ...
 const data=input.callback.data; if(typeof data!=='string'||!/^v:(?:begin|list|detail|disable|audit|stats|type:(?:PERCENT_30|PERCENT_50|PERCENT_100|LIFETIME)|plan:[A-Z0-9_]+|confirm:[A-F0-9-]{16,64}|cancel)$/.test(data)){await sender.answerCallbackQuery(input.callback.id,{text:'Tombol kedaluwarsa.'});return {outcome:'stale'};}
...
async function confirm(input,d,reply,key,sender){
 const made=await d.db.rpc('create_voucher_admin_batch',{p_telegram_user_id:input.from.id,p_confirmation_key:key}); if(made.error||!made.data)return {outcome:'failed'};
 const batch=made.data; let generated=0, parts=0;
 try { for(let offset=0;offset<batch.requested_quantity;offset+=CHUNK_SIZE){
   const count=Math.min(CHUNK_SIZE,batch.requested_quantity-offset), codes=[];
   for(let i=0;i<count;i++) codes.push(vouchers.generateVoucherCode());
   const saved=await d.db.rpc('insert_voucher_admin_batch_items',{p_batch_reference:batch.batch_reference,p_items:codes.map(code=>({code_hash:vouchers.voucherCodeHash(code),code_hint:vouchers.voucherCodeHint(code)}))}); if(saved.error) throw new Error('unavailable');
   if(count<=MESSAGE_CODE_LIMIT) { await reply('<b>'+batch.batch_reference+'</b>\n<pre>'+formatCodes(codes,batch)+'</pre>\nSimpan sekarang; kode lengkap tidak dapat dilihat lagi.'); parts++; }
   else { await sender.sendDocument(input.chat.id,'vouchers-'+batch.batch_reference+'-'+(offset/CHUNK_SIZE+1)+'.txt',Buffer.from(formatCodes(codes,batch),'utf8')); parts++; }
   await d.db.rpc('record_voucher_admin_batch_delivery',{p_batch_reference:batch.batch_reference,p_delivered_quantity:offset+count,p_message_part_count:parts}); generated+=count;
 } await d.db.rpc('finalize_voucher_admin_batch',{p_batch_reference:batch.batch_reference}); return {outcome:'delivered',generated};
 } catch(_){await d.db.rpc('cancel_voucher_admin_batch',{p_batch_reference:batch.batch_reference});return {outcome:'delivery_failed',generated};}
}
