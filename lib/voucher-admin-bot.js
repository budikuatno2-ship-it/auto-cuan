...
const MENU='🔐 <b>Admin Voucher</b>\nPilih menu atau gunakan /buatvoucher, /daftarvoucher, /detailvoucher, /nonaktifkanvoucher, /auditvoucher, /sisakursilifetime, /batal.';
function menuKeyboard(){return {inline_keyboard:[[{text:'Buat Voucher',callback_data:'v:begin'}],[{text:'Daftar Voucher',callback_data:'v:list'}],[{text:'Detail Voucher',callback_data:'v:detail'}],[{text:'Nonaktifkan Voucher',callback_data:'v:disable'}],[{text:'Audit Voucher',callback_data:'v:audit'}],[{text:'Sisa Slot Lifetime',callback_data:'v:seats'}]]};}
...
function parseCommand(text){const m=/^\/(start|menu|buatvoucher|daftarvoucher|detailvoucher|nonaktifkanvoucher|auditvoucher|sisakursilifetime|batal)(?:@\w+)?(?:\s+([A-Za-z0-9-]+))?\s*$/i.exec(typeof text==='string'?text:''); return m&&{name:m[1].toLowerCase(),argument:m[2]||null};}
...
async function processVoucherAdminUpdate(update,deps) {
 ...
 if(command){
  ...
  if(['daftarvoucher','detailvoucher','nonaktifkanvoucher','auditvoucher','sisakursilifetime'].includes(command.name)){await db.rpc('voucher_admin_command',{p_command:command.name,p_reference:command.argument&&validRef(command.argument)?command.argument:null}); await reply('Permintaan admin diterima. Gunakan referensi aman pada menu untuk hasil terperinci.'); return {outcome:'command'};}
 }
 if(!input.callback) {
 ...
 const data=input.callback.data; if(typeof data!=='string'||!/^v:(?:begin|list|detail|disable|audit|seats|type:(?:PERCENT_30|PERCENT_50|PERCENT_100|LIFETIME)|plan:[A-Z0-9_]+|confirm:[A-F0-9-]{16,64}|cancel)$/.test(data)){await sender.answerCallbackQuery(input.callback.id,{text:'Tombol kedaluwarsa.'});return {outcome:'stale'};}
 await sender.answerCallbackQuery(input.callback.id);
 if(data==='v:begin') return processVoucherAdminUpdate(Object.assign({},update,{message:Object.assign({},input.message,{text:'/buatvoucher'}),callback_query:null}),d);
...
async function confirm(input,d,reply,key,sender){const r=await d.db.rpc('create_voucher_admin_batch',{p_telegram_user_id:input.from.id,p_confirmation_key:key});if(r.error||!r.data)return {outcome:'failed'}; const batch=r.data; const codes=[]; for(let n=0;n<batch.requested_quantity;n+=CHUNK_SIZE) for(let i=n;i<Math.min(n+CHUNK_SIZE,batch.requested_quantity);i++) codes.push(vouchers.generateVoucherCode()); const items=codes.map(code=>({code_hash:vouchers.voucherCodeHash(code),code_hint:vouchers.voucherCodeHint(code)})); const saved=await d.db.rpc('insert_voucher_admin_batch_items',{p_batch_reference:batch.batch_reference,p_items:items});if(saved.error)return {outcome:'failed'};try {if(codes.length<=MESSAGE_CODE_LIMIT) await reply('<b>'+batch.batch_reference+'</b>\n<pre>'+formatCodes(codes,batch)+'</pre>\nSimpan sekarang; kode lengkap tidak dapat dilihat lagi.');else {for(let i=0;i<codes.length;i+=CHUNK_SIZE){const content=Buffer.from(formatCodes(codes.slice(i,i+CHUNK_SIZE),batch),'utf8'); await sender.sendDocument(input.chat.id,'vouchers-'+batch.batch_reference+'-'+(i/CHUNK_SIZE+1)+'.txt',content);}} await d.db.rpc('finalize_voucher_admin_batch',{p_batch_reference:batch.batch_reference}); return {outcome:'delivered',generated:codes.length};}catch(_){await d.db.rpc('cancel_voucher_admin_batch',{p_batch_reference:batch.batch_reference});return {outcome:'delivery_failed'};}}
module.exports={SESSION_TTL_MS,CHUNK_SIZE,MESSAGE_CODE_LIMIT,DOCUMENT_BYTES,TYPES,TERM_PLANS,MENU,menuKeyboard,safeReference,validRef,adminUpdate,parseCommand,processVoucherAdminUpdate,confirm};
