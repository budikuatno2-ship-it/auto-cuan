...
// BUG-F7-014: semua kondisi di atas dibandingkan terhadap sebuah level harga.
// Tanpa target_price, alert tersimpan tetapi tidak akan pernah bisa dievaluasi
// (dead alert). Daftar ini menegakkan target_price wajib untuk setiap kondisi.
const LEVEL_ALERT_CONDITIONS = ['PRICE_ABOVE', 'PRICE_BELOW', 'ENTRY_ZONE', 'TP_HIT', 'SL_HIT'];
  ...
  if (remaining.size > 0) {
    try {
      const q3 = await supabase
        ...
        // published_at is the real recency column.
...
// Best-effort audit log entry for the user's own "alert change history"
// view. Never allowed to fail the caller's actual create/update/delete —
// history is a read-side convenience, not a source of truth.
async function logAlertHistory(supabase, userId, entry) {
  try {
    await supabase.from('app_user_alert_history').insert({
      user_id: userId,
      alert_id: entry.alert_id || null,
      ticker: entry.ticker,
      action: entry.action,
      condition_type: entry.condition_type || null,
      target_price: entry.target_price != null ? entry.target_price : null,
      created_at: new Date().toISOString()
    });
  } catch (_) {}
}

async function getAlertHistory(supabase, userId, limit = 100) {
  if (!supabase || !userId) {
    return { success: false, error: 'Database atau User ID tidak valid.', history: [] };
  }
  const cleanLimit = Number.isFinite(Number(limit)) && Number(limit) > 0 ? Math.min(Number(limit), 200) : 100;
  try {
    const q = await supabase
      .from('app_user_alert_history')
      .select('id, alert_id, ticker, action, condition_type, target_price, created_at')
      .eq('user_id', userId)
      .order('created_at', { ascending: false })
      .limit(cleanLimit);

    if (q.error) {
      return { success: false, error: q.error.message, history: [] };
    }
    return { success: true, history: q.data || [] };
  } catch (err) {
    return { success: false, error: err.message || String(err), history: [] };
  }
}
...
async function createAlert(supabase, userId, payload = {}) {
  ...
  if (LEVEL_ALERT_CONDITIONS.indexOf(conditionType) !== -1 && !targetPrice) {
    return { success: false, error: `target_price harus berupa angka positif untuk kondisi ${conditionType}.` };
  }

  try {
    // Where this alert is delivered is a property of the ACCOUNT, never of the
    // request. `payload` is the HTTP body verbatim (api/sector-hot.js:8098), and
    // the stored value is what evaluateActiveUserAlerts() hands to the main
    // Auto-Cuan bot as `chat_id`. Honouring a caller-supplied
    // `notification_chat_id` let a signed-in user point their own alert at
    // somebody else's private chat and have the bot deliver there on a schedule.
    // It is resolved server-side only; when the account has no verified Telegram
    // binding the alert simply has no destination.
    let chatId = null;
    ...
    // `watchlist_id` is the same class of input: a foreign key arriving from the
    // request body. Store it only once it is proven to belong to this user.
    ...
    if (payload.watchlist_id) {
      try {
        const owned = await supabase
          ...
          .maybeSingle();
        if (owned && owned.data && owned.data.id) watchlistId = owned.data.id;
      } catch (_) {
        watchlistId = null;
      }
    }
    ...
    await logAlertHistory(supabase, userId, {
      alert_id: q.data && q.data.id,
      ticker: cleanTicker,
      action: 'created',
      condition_type: conditionType,
      target_price: targetPrice
...
async function updateAlert(supabase, userId, alertId, payload = {}) {
  ...
  if (payload.condition_type != null) {
    const conditionType = String(payload.condition_type).toUpperCase();
    if (VALID_ALERT_CONDITIONS.indexOf(conditionType) === -1) {
      return { success: false, error: `condition_type tidak valid. Pilihan: ${VALID_ALERT_CONDITIONS.join(', ')}` };
    }
  ...
  if (payload.target_price != null) {
    const targetPrice = normalizeNumber(payload.target_price);
    if (!targetPrice) {
      return { success: false, error: 'target_price harus berupa angka positif.' };
    }
    updates.target_price = targetPrice;
  }

  if (updates.condition_type == null && updates.target_price == null) {
    return { success: false, error: 'Tidak ada perubahan untuk disimpan.' };
  }

  // Editing an alert re-arms it: it may already have been triggered/deactivated
  // for the old target, and the user is asking to watch for a new condition.
  updates.is_triggered = false;
  ...
  try {
    const q = await supabase
      ...
      .eq('id', alertId)
      .eq('user_id', userId)
      .select('id, user_id, ticker, condition_type, target_price, is_active, is_triggered, created_at, updated_at')
      .maybeSingle();
    ...
    if (!q.data) {
      return { success: false, error: 'Alert tidak ditemukan.' };
    }
    ...
    await logAlertHistory(supabase, userId, {
      ...
      action: 'updated',
      condition_type: q.data.condition_type,
      target_price: q.data.target_price
    });
...
async function deleteAlert(supabase, userId, alertId) {
  ...
  try {
    const q = await supabase
      ...
      .eq('id', alertId)
      .eq('user_id', userId)
      .select('id, ticker, condition_type, target_price')
      .maybeSingle();
...
async function evaluateActiveUserAlerts(supabase, options = {}) {
  ...
  try {
    ...
    for (const alert of activeAlerts) {
      ...
      // Pillar 13: Alert trigger requires >= 50 match frequency and >= 1B turnover for Main Board
      const boardStr = String(px.board || px.market_board || '').toUpperCase();
      const isAccelerationBoard = boardStr.includes('AKSELERASI') || boardStr.includes('ACCELERAT');
      if (!isAccelerationBoard) {
        if (px.match_frequency != null && Number(px.match_frequency) < 50) continue;
        if (px.frequency != null && Number(px.frequency) < 50) continue;
        if (px.turnover != null && Number(px.turnover) < 1000000000) continue;
        if (px.total_turnover != null && Number(px.total_turnover) < 1000000000) continue;
      }
      ...
      if (isHit) {
        ...
        // Enforce chat_id locking ke sub akun terverifikasi
        let verifiedChatId = null;
        try {
          const tgVerify = await supabase
            .from('app_user_telegram_verifications')
            .select('telegram_private_chat_id')
            .eq('user_id', alert.user_id)
            .maybeSingle();
          if (tgVerify && tgVerify.data && tgVerify.data.telegram_private_chat_id) {
            verifiedChatId = Number(tgVerify.data.telegram_private_chat_id);
          }
        } catch (_) {
          verifiedChatId = null;
        }

        const targetChatId = verifiedChatId || (alert.notification_chat_id ? Number(alert.notification_chat_id) : null);
            ...
            try {
              await supabase
                ...
                .update({
                  is_triggered: true,
                  triggered_at: nowIso,
                  last_notified_at: targetChatId ? nowIso : null,
                  updated_at: nowIso
                })
                .eq('id', alert.id);
            } catch (_) {}
            await logAlertHistory(supabase, alert.user_id, {
              alert_id: alert.id,
              ticker: alert.ticker,
              action: 'triggered',
