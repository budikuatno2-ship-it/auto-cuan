...
const ACTIONABLE_PRICE_FIELDS = [
  ...
  'stop_loss', 'sl', 'cl', 'cut_loss', 'tp', 'tp1', 'tp2', 'tp3', 'target', 'target_price',
...
function detectPriceScaleMismatch(candidate, latestPrice, options) {
  ...
  const criticalFar = actionable.some((item) => CRITICAL_FIELDS[item.field] && (nearSplitFactor(item.value / trustedLatest) || item.value / trustedLatest >= 1.8 || item.value / trustedLatest <= 0.55));
...
function applyCorporateActionPriceScaleGuard(candidate, context) {
  ...
  const latestPrice = typeof context === 'number' ? context : (context && (context.latestPrice != null ? context.latestPrice : context.latest_price));
  const result = detectPriceScaleMismatch(row, latestPrice, context);
  row.corporate_action_guard = result.blocked ? 'BLOCKED' : (result.reason ? 'NOT_EVALUATED' : 'PASSED');
  ...
  if (result.blocked && result.stale_level_sample) row.stale_level_sample = result.stale_level_sample;
  if (!result.blocked) {
    if (result.reason) row.corporate_action_reason = result.reason;
    return row;
  }
  row.corporate_action_reason = 'price_scale_mismatch';
