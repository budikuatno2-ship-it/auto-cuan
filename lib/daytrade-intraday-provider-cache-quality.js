...
function buildProviderCacheQualityReport(samples, opts) {
  ...
  for (const s of samples) {
    ...
    for (const row of rows) {
      ...
      if (row.data_quality === 'NO_INTRADAY_DATA' || row.data_quality === 'INCOMPLETE_INTRADAY' || isUnknown || isDaily || hints.length) inc(blockerCounts, row.ticker);
  ...
  const blockReasons = [];
  if (totalProviderMissing > 0) blockReasons.push('provider_missing_count > 0');
  if (totalIncomplete > 0) blockReasons.push('INCOMPLETE_INTRADAY > 0');
  if (totalUnknown > 0) blockReasons.push('INTRADAY_UNKNOWN > 0');
  if (dailyScoreOnly.size > 0) blockReasons.push('DAILY_SCORE_ONLY fallback exists');
  const status = blockReasons.length ? 'BLOCK' : 'PASS';
  return { date: todayIso(opts && opts.nowMs), generated_at: new Date(opts && opts.nowMs || Date.now()).toISOString(), quality_status: status, recommendation: status === 'PASS' ? 'Provider/cache quality diagnostics are clean; keep production intraday scoring disabled until separately approved.' : 'Do not enable production intraday scoring; remediate provider/cache blockers and rerun validation.', sample_count: samples.length, provider_missing_count: totalProviderMissing, incomplete_intraday_count: totalIncomplete, intraday_unknown_count: totalUnknown, daily_score_only_tickers: Array.from(dailyScoreOnly).sort(), repeated_blocker_tickers: repeated(blockerCounts), top_blocker_tickers: sortedCounts(blockerCounts).slice(0, 20), sample_summaries: sampleSummaries, ticker_records: tickerRecords, block_reasons: blockReasons, read_only_confirmation: 'This provider/cache quality report only reads local validation bundle/session archive JSON and writes local report artifacts. It does not enable DAYTRADE_INTRADAY_SCORE_ENABLED, add API endpoints, add SQL or migrations, change cron, send Telegram, change PDF/share-link, add orderbook/broksum, use paid services, or use AI.' };
}
