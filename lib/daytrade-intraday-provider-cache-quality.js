...
// A provider failure is quarantined in the same validation pass in which it is
// observed.  Recurrence is useful diagnostics, but it must never be a
// prerequisite for keeping a bad row out of intraday scoring/public output.
function isProviderBadRow(row) {
  const quality = String(row && row.data_quality || '').toUpperCase();
  return quality === 'NO_INTRADAY_DATA' || quality === 'INCOMPLETE_INTRADAY';
}
...
function buildProviderCacheQualityReport(samples, opts) {
  ...
  for (const s of samples) {
    ...
    for (const row of rows) {
      ...
      if (isDaily || isProviderBadRow(row)) dailyScoreOnly.add(normalizeTicker(row.ticker));
  ...
  const visibleProviderMissing = tickerRecords.filter((rec) => rec.data_quality === 'NO_INTRADAY_DATA').length;
  const visibleIncomplete = tickerRecords.filter((rec) => rec.data_quality === 'INCOMPLETE_INTRADAY').length;
  const visibleUnknown = tickerRecords.filter((rec) => rec.intraday_priority_label === 'INTRADAY_UNKNOWN' || rec.intraday_confirmation_label === 'INTRADAY_UNKNOWN').length;
  ...
  // Quarantine every provider-bad ticker, including a one-off bad provider
  // row.  Keep the repeated list as diagnostics only.
  const providerBadTickers = tickerSet(tickerRecords.filter(isProviderBadRow));
  const quarantineTickers = new Set(providerBadTickers);
  ...
  for (const rec of tickerRecords) {
    ...
    rec.quarantine_reason = `PROVIDER_DATA_QUALITY_${rec.data_quality} DAILY_SCORE_ONLY`;
    rec.quarantine_action = 'DAILY_SCORE_ONLY';
    rec.intraday_score_adjustment = 0;
    rec.intraday_score_adjustment_preview = 0;
    // A quarantined row cannot be a confirmed or strong intraday pick.
    rec.intraday_priority_label = 'INTRADAY_UNKNOWN';
    rec.intraday_confirmation_label = 'INTRADAY_UNKNOWN';
    rec.intraday_score_adjustment_reasons = Array.from(new Set(asArray(rec.intraday_score_adjustment_reasons).concat([`PROVIDER_DATA_QUALITY_${rec.data_quality} DAILY_SCORE_ONLY 0`])));
  ...
  const nonQProviderMissing = Math.max(0, visibleProviderMissing - quarantinedProviderMissing);
  const nonQIncomplete = Math.max(0, visibleIncomplete - quarantinedIncomplete);
  const nonQUnknown = Math.max(0, visibleUnknown - quarantinedUnknown);
  const quarantineRows = Array.from(quarantineTickers).sort().map((ticker) => {
    const record = tickerRecords.find((row) => normalizeTicker(row.ticker) === ticker);
    return { ticker, count: blockerCounts[ticker] || 0, quarantine_reason: `PROVIDER_DATA_QUALITY_${record && record.data_quality || 'UNKNOWN'} DAILY_SCORE_ONLY`, quarantine_action: 'DAILY_SCORE_ONLY' };
  });
  const blockReasons = [];
