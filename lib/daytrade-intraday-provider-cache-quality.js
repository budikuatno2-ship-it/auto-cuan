...
function buildProviderCacheQualityReport(samples, opts) {
  ...
  let visibleProviderMissing = 0, visibleNonQuarantinedProviderMissing = 0;
  for (const rec of tickerRecords) {
    const ticker = normalizeTicker(rec.ticker);
    const isProviderMissingRow = rec.data_quality === 'NO_INTRADAY_DATA' || asArray(rec.remediation_hints).includes('provider missing');
    if (isProviderMissingRow) visibleProviderMissing++;
    if (!quarantineTickers.has(ticker)) {
      if (isProviderMissingRow) visibleNonQuarantinedProviderMissing++;
      continue;
    }
  ...
  const nonQProviderMissing = Math.min(totalProviderMissing, visibleNonQuarantinedProviderMissing);
  ...
  const warnReasons = [];
  if (quarantineRows.length) warnReasons.push('PASS_WITH_QUARANTINE for dry-run/reporting only; keep production intraday scoring disabled');
  const status = blockReasons.length ? 'BLOCK' : quarantineRows.length ? 'PASS_WITH_QUARANTINE' : 'PASS';
  const recommendation = status === 'PASS' ? 'Provider/cache quality diagnostics are clean; keep production intraday scoring disabled until separately approved.' : status === 'PASS_WITH_QUARANTINE' ? 'Provider/cache blockers are fully explained by explicit quarantine for dry-run/reporting only. Keep DAYTRADE_INTRADAY_SCORE_ENABLED off; quarantined tickers remain DAILY_SCORE_ONLY.' : 'Do not enable production intraday scoring; quarantined tickers are daily-only and any non-quarantined provider/cache blockers must be remediated.';
  return { date: todayIso(opts && opts.nowMs), generated_at: new Date(opts && opts.nowMs || Date.now()).toISOString(), quality_status: status, recommendation, sample_count: samples.length, provider_missing_count: totalProviderMissing, total_provider_missing_count: totalProviderMissing, incomplete_intraday_count: totalIncomplete, intraday_unknown_count: totalUnknown, visible_provider_missing_count: visibleProviderMissing, quarantined_provider_missing_count: quarantinedProviderMissing, non_quarantined_provider_missing_count: nonQProviderMissing, non_quarantined_incomplete_intraday_count: nonQIncomplete, non_quarantined_intraday_unknown_count: nonQUnknown, intraday_quarantine_count: quarantineRows.length, intraday_quarantine_tickers: quarantineRows, daily_score_only_tickers: Array.from(dailyScoreOnly).sort(), repeated_blocker_tickers: repeatedBlockers, top_blocker_tickers: sortedCounts(blockerCounts).slice(0, 20), sample_summaries: sampleSummaries, ticker_records: tickerRecords, block_reasons: blockReasons, warn_reasons: warnReasons, read_only_confirmation: 'This provider/cache quality report only reads local validation bundle/session archive JSON and writes local report artifacts. It does not enable DAYTRADE_INTRADAY_SCORE_ENABLED, add API endpoints, add SQL or migrations, change cron, send Telegram, change PDF/share-link, add orderbook/broksum, use paid services, or use AI.' };
}
