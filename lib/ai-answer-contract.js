...
function parseNumberToken(token) {
  ...
  // ponytail: preserve negative sign for financial percentages and risk deltas
...
function numberTokenRegex() {
  // ponytail: allow optional [-+] sign for percentages/losses
...
function parseMatchedNumber(raw, context) {
  ...
  // F-061: the parsed ratio-marker flag was dead — the marker is stripped here
  // and re-detected by the regex in financialNumbersInText, so nothing read it.
  ...
  // ponytail: always apply scale multiplier if scale token was present
...
function validateAnswer(input, options) {
  ...
  // F-060: normalizeAnswer slices direct_answer to 600, so checking the
  // normalized value was dead code (max length is exactly 600). Validate the
  // RAW input length instead so over-long answers are rejected, not silently cut.
  const rawRow = input && typeof input === 'object' ? input : {};
  const rawDirectAnswer = typeof rawRow.direct_answer === 'string' ? rawRow.direct_answer
    : (typeof rawRow.answer === 'string' ? rawRow.answer : '');
  if (rawDirectAnswer.length > 600) errors.push('direct_answer terlalu panjang');
