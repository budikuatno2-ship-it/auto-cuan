...
function normalizeAnswer(input) {
  ...
  return {
    ...
    source_scope: clean(row.source_scope || 'snapshot', 40) || 'snapshot'
  };
}

function numbersInText(value) {
  const matches = clean(value, 10000).match(/\b\d[\d.,]*\b/g) || [];
...
function validateAnswer(input, options) {
  ...
  if (allowedNumbers.length) {
    const found = numbersInText(combined);
    for (const value of found) {
      if (!allowedNumbers.some((allowed) => nearlyEqual(value, allowed))) {
        errors.push('angka tidak didukung sumber: ' + value);
      }
    }
