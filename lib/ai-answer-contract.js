...
function numberTokenRegex() {
  return /\b(?:rp|idr)\s*\d(?:[\d.,]*\d)?(?:\s*(?:ribu|juta|miliar|triliun))?(?:\s*rupiah)?|(?<![A-Za-z0-9_])\d(?:[\d.,]*\d)?(?:\s*(?:ribu|juta|miliar|triliun))?(?:\s*rupiah)?(?![A-Za-z0-9_])/gi;
}
...
function financialNumbersInText(value) {
  ...
  for (const match of source.matchAll(numberTokenRegex())) {
    ...
    const explicitCurrency = /^(?:rp|idr)/i.test(token) || /\brupiah$/i.test(token);
    const followedByPercent = /^\s*%/.test(source.slice(end, end + 3));
    const hasFinancialContext = explicitCurrency || followedByPercent || FINANCIAL_NUMBER_CONTEXT.test(context);
