...
function findConfirmedPivotLows(candles, lookback) {
  ...
  // Fase 3 / BUG-F3-12: `lookback` membatasi jumlah candle TERKINI yang dipindai
  // (dihitung dari ujung array), bukan batas absolut dari index 0. Pada array
  // kronologis (oldest first) versi lama hanya memeriksa candle terlama dan
  // melewatkan pivot yang baru terbentuk di ujung data.
  const maxCandleIdx = normalized.length - 3;
  const span = lookback || CONFIRMED_PIVOT_LOOKBACK;
  const minIdx = Math.max(2, normalized.length - span);
  const pivots = [];

  // Mulai dari batas lookback (2 candle kiri) hingga 2 sebelum akhir (2 candle kanan).
  for (let i = minIdx; i <= maxCandleIdx; i++) {
...
function findConfirmedPivotHighs(candles, lookback) {
  ...
  // Fase 3 / BUG-F3-12: sama seperti findConfirmedPivotLows — lookback dihitung
  // dari candle terkini, bukan dari index 0.
  const maxCandleIdx = normalized.length - 3;
  const span = lookback || CONFIRMED_PIVOT_LOOKBACK;
  const minIdx = Math.max(2, normalized.length - span);
  const pivots = [];
