...
function applySmartSetupLabels(candidate, context) {
  ...
  // (avg_tx_value_7d / avg_transaction_value / avg_value_20d). Harga murah
  // tanpa turnover tetap diblokir via gate likuiditas, bukan via nominal.
  // Nama diagnostik dipertahankan agar kompatibel dengan pengguna lama.
  ...
  if (price != null && price <= 55) {
    ...
    if (txValue == null || txValue < 500000000) { blocked = true; result.smart_setup_diagnostics.push('price_near_50_liquidity_not_confirmed'); }
  }
