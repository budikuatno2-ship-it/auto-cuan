...
function applySmartSetupLabels(candidate, context) {
  ...
  // FLOOR PRICE UPDATE 2026-09: IDX floor Rp1 (ARB -15% => Math.max(1, Math.floor(prev_close*0.85))).
  // Validasi harga valid: price >=1 (bukan >=50). SL valid hingga Rp1.
  // Saham di bawah Rp50 tetap valid selama likuiditas & akumulasi memenuhi syarat.
  // Gate likuiditas tetap berbasis turnover riil, bukan nominal harga.
  // Diagnostik lama dipertahankan untuk kompatibilitas, ditambah diagnostik baru floor Rp1.
  var txValue = n(row.avg_tx_value_7d) || n(row.avg_transaction_value) || n(row.avg_value_20d) || n(row.value_today) || n(row.valuasi);
  if (price != null && price < 1) {
    result.smart_setup_diagnostics.push('price_below_floor_1_invalid');
    result.smart_setup_diagnostics.push('price_near_50_requires_stronger_liquidity_confirmation');
    blocked = true;
    result.smart_setup_diagnostics.push('price_below_floor_1_blocked');
    result.smart_setup_diagnostics.push('price_near_50_liquidity_not_confirmed');
  } else if (price != null && price <= 50) {
    result.smart_setup_diagnostics.push('price_near_floor_1_requires_liquidity_confirmation');
    result.smart_setup_diagnostics.push('price_near_50_requires_stronger_liquidity_confirmation');
    if (txValue == null || txValue < 500000000) { blocked = true; result.smart_setup_diagnostics.push('price_near_floor_1_liquidity_not_confirmed'); result.smart_setup_diagnostics.push('price_near_50_liquidity_not_confirmed'); }
  }
  ...
  var swingCandidate = isSwingCandidate(row, context);
  applySwingPatternScore(row, context, blocked);
  lifecycle.applyLifecycle(row, {
    mode: swingCandidate ? 'swing' : 'daytrade',
    blocked: blocked,
    // Day Trade score is already adjusted during the scan before persistence.
    // Read enrichment must expose lifecycle metadata without applying the score twice.
    applyScore: swingCandidate
  });
  return row;
}
