...
const DEFAULT_FAILURE_CACHE_MS = 45 * 1000;
...
function economicalOrder(rows, source, task) {
  ...
  if (source === 'stock_analysis_followup' || task === 'heavy') {
    const normal = unique.filter((m) => !EXPENSIVE_MODELS.has(m));
    const expensive = unique.filter((m) => EXPENSIVE_MODELS.has(m));
    return normal.concat(expensive.slice(0, 1), expensive.slice(1));
  }
  return unique.filter((m) => !EXPENSIVE_MODELS.has(m)).concat(unique.filter((m) => EXPENSIVE_MODELS.has(m)));
}
function configuredModels(source, task) {
  const dedicated = envModels(source, task);
  const general = split(process.env.PORTFOLIO_AI_MODELS);
  const defaults = defaultsFor(source, task);
  return economicalOrder(dedicated.concat(defaults, general, CATALOG), source, task);
}
...
function attemptLimit(source, task) {
  ...
  let taskMax;
  if (source === 'stock_analysis_followup') taskMax = bounded(process.env.STOCK_ANALYSIS_AI_MAX_ATTEMPTS, 6, 1, 10);
  else if (task === 'heavy') taskMax = bounded(process.env.PORTFOLIO_AI_HEAVY_MAX_ATTEMPTS, 5, 1, 10);
  else if (task === 'empathy') taskMax = bounded(process.env.PORTFOLIO_AI_EMPATHY_MAX_ATTEMPTS, 4, 1, 8);
  else taskMax = bounded(process.env.PORTFOLIO_AI_FAST_MAX_ATTEMPTS, 3, 1, 6);
...
async function callModel(baseUrl, apiKey, model, messages, settings, timeoutMs) {
  ...
  try {
    ...
    return { ok: false, model, latency: nowMs() - started, status: error && error.name === 'AbortError' ? 504 : 502, retriable: true, reason: error && error.name === 'AbortError' ? 'Timeout.' : clean(error && error.message || 'Provider error.', 180) };
  } finally {
    clearTimeout(timer);
  }
}
...
function sanitizeHistory(raw) {
  return (Array.isArray(raw) ? raw : []).slice(-6).map((row) => {
    if (!row || !['user', 'assistant'].includes(row.role)) return null;
    const content = clean(row.content, 1000);
...
function compactAnalysisText(value) {
  ...
  const limit = bounded(process.env.STOCK_ANALYSIS_CONTEXT_CHARS, 7500, 3500, 12000);
...
function settingsFor(source, task) {
  if (source === 'stock_analysis_followup') return { temperature: 0.25, maxTokens: 1300 };
  if (task === 'empathy') return { temperature: 0.5, maxTokens: 900 };
  if (task === 'heavy') return { temperature: 0.25, maxTokens: 1200 };
  return { temperature: 0.35, maxTokens: 650 };
}
...
module.exports = async function handleContextAI(req, res) {
  try {
    ...
    const perModel = bounded(process.env.PORTFOLIO_AI_MODEL_TIMEOUT_MS, DEFAULT_MODEL_TIMEOUT_MS, 1500, 12000);
    const totalLimit = bounded(process.env.PORTFOLIO_AI_TOTAL_TIMEOUT_MS, DEFAULT_TOTAL_TIMEOUT_MS, 5000, 45000);
    const models = orderedModels(source, task, userId).slice(0, attemptLimit(source, task));
    ...
    const error = outcome.attempts.length ? 'Model yang dicoba sedang gangguan atau traffic ramai. Sistem menghentikan fallback agar token tidak terbuang. Coba lagi setelah beberapa saat.' : 'Waktu tunggu AI habis sebelum model sehat ditemukan.';
    ...
    return res.status(503).json({ success: false, code: 'AI_MODELS_FAILED_ECONOMY_STOP', error, attempted_models: outcome.attempts.map((x) => x.model), attempted_count: outcome.attempts.length });
  } catch (error) {
