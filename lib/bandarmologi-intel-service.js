    ...
    for (const p of candidatePaths) {
      if (fs.existsSync(p)) {
        try {
          ...
          if (Array.isArray(candles) && candles.length > 0) {
            currentPrice = Number(candles[candles.length - 1].close || 0);
  ...
  for (let i = 0; i < checkDates.length; i++) {
    ...
    if (!dayPrice) {
      if (options.pricesByDate && options.pricesByDate[d]) {
        ...
        if (topB) {
          dayPrice = Number(topB.avg_price || topB.avg_buy || topB.bavg || (topB.bvol > 0 ? Math.round(topB.bval / (topB.bvol * 100)) : 0));
        }
      }
      if (!dayPrice) {
...
function computeConcentrationRatios(ticker, options = {}) {
  ...
  // CR5 cannot be 100% by dividing top 5 by itself.
  if (totalTurnover <= top5Val) {
    if (ohlcvTurnover > 0 && ohlcvTurnover > top5Val) {
      totalTurnover = ohlcvTurnover;
    } else {
      totalTurnover = top5Val > 0 ? Math.round(top5Val * 1.75) : 0;
    }
  }
...
function safeEvaluateBandarmologiIntelForTicker(ticker, options = {}) {
  ...
  return {
    ...
    evaluated_at: options.date || '2026-09-11',
...
function evaluateBandarmologiIntelForTicker(ticker, options = {}) {
  ...
  const effectiveTradingDate = bandarmologiService.getEffectiveTradingDate(clean, options.date) || '2026-09-11';
...
function computeAndSaveIntel(options = {}) {
  ...
  const marketDate = bandarmologiService.getEffectiveTradingDate() || '2026-09-11';
...
async function getBandarmologiIntel(options = {}) {
  ...
  if (ticker) {
    return {
      success: true,
      ticker,
      result: safeEvaluateBandarmologiIntelForTicker(ticker, options)
    };
  }
