...
function getHunterTickerMap(range = '7d') {
  ...
  // (aggregateBrokerSummaries); when a range's own index is absent the honest
  // answer is to report the range it actually has, with no rescaling at all.
  // Degradation when the requested range has no index of its own must be an
    ...
    for (const f of files) {
      try {
        ...
        for (const dist of (d.top_distributed || [])) {
          ...
          const bVal = Math.round(toFeedNumber(dist.buy_val));
          const sVal = Math.round(toFeedNumber(dist.sell_val));
          const bVol = Math.round(toFeedNumber(dist.buy_vol));
          const sVol = Math.round(toFeedNumber(dist.sell_vol));
          const netVal = Math.round(toFeedNumber(dist.net_val));
          const netVol = Math.round(toFeedNumber(dist.net_vol));
          const avgSell = toFeedNumber(dist.avg_sell_price) || (sVal > 0 && sVol > 0 ? Math.round(sVal / (sVol * 100)) : 0);
...
function getCachedClosePriceLegacy(ticker) {
  ...
  // 2. PR4: freshest same-day price from the newest broker-summary on disk / VPS.
  // Runs BEFORE single-broker VWAP or reference prices — it weights across all brokers.
  const summaryVwap = deriveCloseFromLatestBrokerSummary(clean);
  if (summaryVwap && summaryVwap > 0) {
    return summaryVwap;
  }

  // 3. Reference price fallback from bandarmologiService (which checks live VPS price)
  try {
  ...
  // 5. Reference price fallback from known tickers only as last resort
...
function getCachedClosePriceDetail(ticker) {
  ...
  if (bridgeDetail && bridgeDetail.price > 0) {
    const detail = {
      ...
      price_source: bridgeDetail.price_source || 'vps_bridge_live'
    };
...
function detectRetailCutlossVsBandar(ticker, options = {}) {
  ...
  const top3Buyers = norm.top_buyers.slice(0, 3).map(b => String((b && (b.broker || b.broker_code)) || '').trim().toUpperCase());
  const top3Sellers = norm.top_sellers.slice(0, 3).map(s => String((s && (s.broker || s.broker_code)) || '').trim().toUpperCase());
...
function evaluateBandarmologiIntelForTicker(ticker, options = {}) {
  ...
  const hasData = Boolean(
    (s1 && s1.reason !== 'NO_DATA' && s1.sub_type !== 'NO_DATA') ||
    (s2 && s2.reason !== 'NO_DATA' && s2.reason !== 'INSUFFICIENT_DATES' && s2.sub_type !== 'NO_DATA') ||
    (s3 && s3.reason !== 'NO_DATA' && s3.sub_type !== 'NO_DATA') ||
    (s4 && s4.reason !== 'NO_DATA' && s4.sub_type !== 'NO_DATA')
  );
 ...
 * defect.
 */
// FIX (10s timeout): the live-bridge probe is a SYNCHRONOUS network/SSH call
// that can block for tens of seconds. It used to run on EVERY scanner request,
// so the second browser poll routinely blew the client's 10s budget (measured
// warm latency: 29.3s). Results are now memoised with a TTL: a healthy bridge
// is reused for 5 minutes and a failed probe is not retried for 60 seconds.
const INTEL_BRIDGE_CACHE = new Map();
const INTEL_BRIDGE_OK_TTL_MS = 5 * 60 * 1000;
const INTEL_BRIDGE_FAIL_TTL_MS = 60 * 1000;

function loadIntelIndexWithProvenance(range) {
  // FIX (stage 1 range isolation): the memo key must normalise the requested
  // range, not just echo it. '7D', '7d ' and '7d' are the same window, while a
  // genuinely different window ('1d' vs '60d') must NEVER share an entry — that
  // cross-range reuse is exactly how 1D data got stuck behind the 60D aggregate.
  // The key also includes the bridge endpoint and index directory: tests (and
  // operators) repoint VPS_DATA_API_BASE / INTEL_INDEX_DIR at runtime, and a
  // stale live result must never leak into a scenario that expects a baked index
  // or an honest failure.
  const cacheKey = [
    String(range || 'default').trim().toLowerCase(),
    process.env.VPS_DATA_API_BASE || '',
    process.env.INTEL_INDEX_DIR || ''
  ].join('|');
  const now = Date.now();
  const cached = INTEL_BRIDGE_CACHE.get(cacheKey);
  if (cached && cached.expires_at > now) {
    if (cached.data) return { data: cached.data, data_source: 'live_bridge' };
    // A recent failure: skip the blocking probe entirely and serve the baked index.
    const bakedFast = loadCachedIntel(range);
    if (bakedFast) return { data: bakedFast, data_source: 'baked_index' };
  ...
  try {
    ...
    if (vpsFetcher && typeof vpsFetcher.fetchIntelIndexFromVpsSync === 'function') {
      ...
      if (live && live.indexes) {
        INTEL_BRIDGE_CACHE.set(cacheKey, {
          data: live,
          expires_at: now + INTEL_BRIDGE_OK_TTL_MS
        });
  ...
  // Remember the miss so the next request answers from local disk instantly.
  INTEL_BRIDGE_CACHE.set(cacheKey, { data: null, expires_at: now + INTEL_BRIDGE_FAIL_TTL_MS });
...
/**
 * FIX (stage 1 as_of_date): a range-specific bridge payload does not always carry
 * the trading date on the aggregate root — it can sit on a category row or on a
 * per-emiten evaluation. Resolve those before declaring the payload undated, so a
 * populated scan is never presented as "no date".
 */
function resolveNestedAsOfDate(data) {
  if (!data) return null;
  const indexes = data.indexes || {};
  for (const cat of Object.keys(indexes)) {
    const list = indexes[cat];
    if (!Array.isArray(list) || list.length === 0) continue;
    const row = list[0] || {};
    const found = row.effective_date || row.date || row.as_of_date || null;
    if (found) return found;
  }
  const tickers = data.tickers || {};
  for (const code of Object.keys(tickers)) {
    const entry = tickers[code] || {};
    // Deliberately NOT evaluated_at / updated_at: those are WRITE timestamps, and
    // presenting one as the trading day is exactly the "stale price with no label"
    // defect this resolver exists to prevent.
    const found = entry.effective_date || entry.date || entry.as_of_date || null;
    if (found) return found;
  }
  // FIX (stage 1 as_of_date, fallback): the committed range indexes (latest_1d.json,
  // latest_60d.json, ...) carry NO date field at all — only `updated_at`, which is a
  // WRITE timestamp and must never be presented as the trading day. When the payload
  // is genuinely undated, resolve the current exchange trading day from the trading
  // calendar instead, so a populated scan is labelled with the day it describes.
  try {
    const tradingDay = bandarmologiService.getEffectiveTradingDate();
...
/**
 * Reconciles static scanner snapshot prices with authentic real-time market prices.
 * Overwrites current_price, recalculates discount_pct: ((bandar_avg - real_price) / bandar_avg * 100),
 * and updates description and note.
 */
// FIX (stage 1 live price): the memo used to be PERMANENT, so a warm instance kept
// serving whatever price it first resolved. Entries now expire, so a long-lived
// runtime re-resolves instead of pinning a stale quote forever.
const SCANNER_LIVE_PRICE_MEMORY = new Map();
// Provenance of the memoised price, so the scanner payload can label where a
// quote came from (bulk map / index / OHLCV / VWAP / static baseline).
const SCANNER_LIVE_PRICE_SOURCE_MEMORY = new Map();
const SCANNER_LIVE_PRICE_TTL_MS = 2 * 60 * 1000;

/**
 * A local OHLCV candle is only a "live price" while it belongs to the same trading
 * window as the index. The committed cache in this repo is dated 2026-07-17 while
 * the index trading day is 2026-09-22 — without this guard that two-month-old close
 * (CUAN 630 / PTRO 4080 / BREN 3410) is served as the current market price.
 */
const SCANNER_MAX_CANDLE_AGE_DAYS = 10;

function readScannerLivePriceMemory(clean) {
  const hit = SCANNER_LIVE_PRICE_MEMORY.get(clean);
  if (!hit) return undefined;
  if (Date.now() - hit.at < SCANNER_LIVE_PRICE_TTL_MS) return hit.price;
  SCANNER_LIVE_PRICE_MEMORY.delete(clean);
  return undefined;
}

function writeScannerLivePriceMemory(clean, price) {
  SCANNER_LIVE_PRICE_MEMORY.set(clean, { price: price, at: Date.now() });
  return price;
}

/**
 * Stage 1: injectable BULK live-price map.
 *
 * The scanner must never pay a network round-trip per ticker, so the freshest
 * market prices are pushed in as ONE map by whoever already holds them (a warm
 * cron, the bridge sync, or the API handler's own bulk read) and every scanner row
 * then resolves with a Map lookup. A TTL keeps a warm process from pinning prices
 * forever, and an explicit clear keeps tests hermetic.
 */
let __bulkLivePriceMap = null;
let __bulkLivePriceMapExpiresAt = 0;
const BULK_LIVE_PRICE_MAP_DEFAULT_TTL_MS = 60 * 1000;

function setBulkLivePriceMap(map, ttlMs) {
  // Accept BOTH a Map and a plain { TICKER: price } / { TICKER: { price } } object:
  // callers build the map from JSON (a plain object) as often as from a Map.
  const isMap = map && typeof map.forEach === 'function';
  const isPlainObject = map && typeof map === 'object' && !Array.isArray(map);
  if (!isMap && !isPlainObject) {
    __bulkLivePriceMap = null;
    __bulkLivePriceMapExpiresAt = 0;
    return null;
  }
  const normalised = new Map();
  const entries = isMap ? Array.from(map.entries()) : Object.keys(map).map(k => [k, map[k]]);
  entries.forEach(([key, value]) => {
    const clean = String(key || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
    const price = Number(value && value.price != null ? value.price : value);
    if (clean && price > 0) {
      normalised.set(clean, {
        price: price,
        source: (value && value.price_source) || 'bulk_live_map',
        as_of_date: (value && (value.price_date || value.as_of_date)) || null
      });
    }
  });
  const ttl = Number(ttlMs) > 0 ? Number(ttlMs) : BULK_LIVE_PRICE_MAP_DEFAULT_TTL_MS;
  __bulkLivePriceMap = normalised;
  __bulkLivePriceMapExpiresAt = Date.now() + ttl;
  // Injecting fresher prices invalidates anything the ladder already memoised,
  // otherwise a quote resolved earlier in the process would shadow the new map.
  SCANNER_LIVE_PRICE_MEMORY.clear();
  SCANNER_LIVE_PRICE_SOURCE_MEMORY.clear();
  return normalised.size;
}

function clearBulkLivePriceMap() {
  __bulkLivePriceMap = null;
  __bulkLivePriceMapExpiresAt = 0;
  SCANNER_LIVE_PRICE_MEMORY.clear();
}

function readBulkLivePriceMap() {
  if (!__bulkLivePriceMap) return null;
  if (Date.now() > __bulkLivePriceMapExpiresAt) {
    __bulkLivePriceMap = null;
    return null;
  }
  return __bulkLivePriceMap;
}
...
function pickIndexEntryPrice(entry) {
  if (!entry || typeof entry !== 'object') return null;
  const direct = Number(entry.current_price || entry.close_price || 0);
  if (direct > 0) {
    return {
      price: direct,
      source: 'index_ticker',
      as_of_date: entry.effective_date || entry.date || entry.as_of_date || null
    };
  }
  ...
  for (const key of Object.keys(signals)) {
    ...
    const p = Number(sig.current_price || sig.close_price || 0);
    if (p > 0) {
      return {
        price: p,
        source: 'index_signal:' + key,
        as_of_date: sig.effective_date || sig.as_of_date || null
      };
    }
  }
  return null;
...
/**
 * FIX (stage 1 live price): the bridge/baked index carries TWO views of the same
 * snapshot — `indexes` (category rows, frozen at generation time) and `tickers`
 * (the per-emiten evaluation). Measured against the 2026-09-22 index, BREN's
 * category row still carried the stale 3410 close while `tickers.BREN` already
 * held the live 3038. Reading the authoritative per-emiten view is what actually
 * connects CUAN / DEWA / GOTO / BREN to a market price instead of leaving them on
 * a frozen static fallback. Built ONCE per request, so the per-row cost is a Map
 * lookup rather than a filesystem probe.
 */
function buildIndexBulkPriceMap(cacheData) {
  const map = new Map();
  const tickers = cacheData && cacheData.tickers;
  if (!tickers || typeof tickers !== 'object') return map;
  for (const code of Object.keys(tickers)) {
    const clean = String(code || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
    if (!clean) continue;
    const picked = pickIndexEntryPrice(tickers[code]);
    if (picked && picked.price > 0) map.set(clean, picked);
  }
  return map;
}

/**
 * FIX (10s timeout): bulk, LOCAL-ONLY live price resolver for the scanner.
 *
 * The previous implementation called getCachedClosePrice() per ticker, which
 * falls through to fetchLivePriceFromVpsSync()/fetchOhlcvFromVpsSync() — a
 * network round-trip per ticker. Across a full universe that blew the 10s
 * client budget. This resolver reads ONLY the in-memory map, the bulk index price
 * map, the local OHLCV candle cache and the local broker-summary VWAP, memoising
 * every result so repeated scans stay instant. Missing data falls back to the
 * row's own last close instead of waiting on the network.
 *
 * @param {string} ticker
 * @param {{indexPriceMap?: Map, referenceDate?: string}} [options]
 */
function getBulkScannerLivePrice(ticker, options = {}) {
  const clean = String(ticker || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!clean) return null;

  const memoised = readScannerLivePriceMemory(clean);
  if (memoised !== undefined) return memoised;

  let price = null;
  let priceSource = null;

  // 1. Injected BULK live-price map (zero I/O; one shared map for the whole scan).
  const bulkMap = readBulkLivePriceMap();
  if (bulkMap) {
    const bulkHit = bulkMap.get(clean);
    if (bulkHit && bulkHit.price > 0) {
      price = bulkHit.price;
      priceSource = bulkHit.source;
    }
  }

  // 2. Authoritative per-emiten price from the bridge/baked index (no I/O).
  if (!(price > 0)) {
    const indexMap = options.indexPriceMap;
    if (indexMap && typeof indexMap.get === 'function') {
      const hit = indexMap.get(clean);
      if (hit && hit.price > 0) {
        price = hit.price;
  ...
  // 3. Local OHLCV candle cache — only when the candle belongs to the same
  //    trading window as the index (freshest local quote, no VPS fetch).
  if (!(price > 0)) {
    try {
      const candidates = [
        path.join(__dirname, '..', 'data', 'daytrade-ohlcv-cache', `${clean}.json`),
      ...
      if (found) {
        ...
        if (Array.isArray(candles) && candles.length > 0) {
          const last = candles[candles.length - 1];
          const c = Number(last && (last.close != null ? last.close : last.c));
          let staleCandle = false;
          try {
            const candleDate = last && (last.date || (last.time ? formatJakartaDate(new Date(last.time * 1000)) : null));
            // Stale-candle guard (CUAN 630 / PTRO 4080 regression): reject a candle
            // whose own date predates the newest broker-summary trading day...
            const bsDates = bandarmologiService.listDiskDates('broker-summary', clean);
            if (candleDate && bsDates && bsDates.length > 0 && candleDate < bsDates[0]) staleCandle = true;
            // ...and reject one that is simply far older than the trading day being
            // served. The reference is the index's own effective date, falling back
            // to the newest broker-summary day on disk — tickers such as BREN/ANTM
            // have NO local broker-summary history (listDiskDates returns []), so
            // without a fallback reference their months-old candle would be served
            // as the current market price.
            if (!staleCandle && candleDate) {
              let reference = options.referenceDate;
              if (!reference) {
                try {
                  reference = bandarmologiService.getEffectiveTradingDate() || null;
                } catch (_) {}
              }
              if (reference) {
                const age = daysBetweenDateKeys(candleDate, reference);
                if (age != null && age > SCANNER_MAX_CANDLE_AGE_DAYS) staleCandle = true;
              }
            }
          } catch (_) {}
          if (c > 0 && !staleCandle) price = c;
        }
      }
    } catch (_) {}
  }

  // 4. Local broker-summary VWAP of the newest trading day.
  if (!(price > 0)) {
    try {
      const vwap = deriveCloseFromLatestBrokerSummary(clean);
      if (vwap && vwap > 0) {
        price = vwap;
        priceSource = 'broker_summary_vwap';
      }
    } catch (_) {}
  }

  // 5. Known baseline price (static reference, always instant, last resort).
  if (!(price > 0)) {
    try {
      const known = bandarmologiService.KNOWN_TICKER_PRICES && bandarmologiService.KNOWN_TICKER_PRICES[clean];
      if (known && known > 0) {
        price = Number(known);
        priceSource = 'known_baseline';
      }
    } catch (_) {}
  }

  writeScannerLivePriceMemory(clean, price);
  if (price > 0 && priceSource) {
    SCANNER_LIVE_PRICE_SOURCE_MEMORY.set(clean, priceSource);
  } else {
    SCANNER_LIVE_PRICE_SOURCE_MEMORY.delete(clean);
  }
  return price;
}

function reconcileScannerLivePrices(cacheData, options = {}) {
  if (!cacheData || !cacheData.indexes || typeof cacheData.indexes !== 'object') return cacheData;

  // Hard wall-clock budget: the API must answer in well under the 2s target
  // even on a cold process with an empty memory map.
  const budgetMs = Number(options.budgetMs || 1500);
  const deadlineAt = Date.now() + budgetMs;

  // Built ONCE per request: the per-emiten authoritative prices. Without this the
  // scanner only ever consulted the per-row category snapshot plus a months-old
  // OHLCV file, which is why emiten such as CUAN / GOTO / PTRO never received a
  // market price at all.
  const indexPriceMap = buildIndexBulkPriceMap(cacheData);
  const referenceDate = resolveIndexAsOfDate(cacheData);
  const bulkOptions = { indexPriceMap, referenceDate };

  const priceCache = new Map();
  const categories = Object.keys(cacheData.indexes);
  let budgetExhausted = false;
  for (const cat of categories) {
    const list = cacheData.indexes[cat];
    if (!Array.isArray(list)) continue;

    for (const item of list) {
      if (!item || !item.ticker) continue;

      // Budget guard: checked BEFORE any work so the endpoint always answers in
      // budget and simply serves the rows it has already reconciled. The previous
      // placement only ran after the expensive lookup, and `break` left the outer
      // category loop running.
      if (Date.now() > deadlineAt) { budgetExhausted = true; break; }

      const clean = String(item.ticker).trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
      if (!clean) continue;

      let realPrice = null;
      if (priceCache.has(clean)) {
        realPrice = priceCache.get(clean);
      } else {
        // FIX (10s timeout): LOCAL + MEMORY ONLY — never a network round-trip per
        // ticker. getBulkScannerLivePrice memoises so a warm process is instant.
        try {
          realPrice = getBulkScannerLivePrice(clean, bulkOptions);
        } catch (err) {
          // ignore lookup failure
        }
        priceCache.set(clean, realPrice);
      }

      // Instant fallback: keep the row's own last close when nothing local is
      // available, rather than waiting on a slow live sync.
      if (!(realPrice > 0)) {
        const ownClose = Number(item.close_price || item.current_price || 0);
        if (ownClose > 0) realPrice = ownClose;
      }

      if (realPrice && realPrice > 0) {
        item.current_price = realPrice;
        item.close_price = realPrice;
        // Provenance travels with the row so the UI can distinguish a live quote
        // from a static baseline instead of presenting both as "current price".
        const rowSource = SCANNER_LIVE_PRICE_SOURCE_MEMORY.get(clean);
        if (rowSource) item.price_source = rowSource;

        const modal = Number(item.bandar_avg_buy || item.bandar_avg_cost || item.bandar_avg_price || item.modal || 0);
        if (modal > 0) {
          const discount = Number((((modal - realPrice) / modal) * 100).toFixed(2));
          item.discount_pct = discount;
          item.price_diff_pct = -discount;
          item.in_sweet_spot = discount >= 5 && discount <= 15;
          item.is_sweet_spot = item.in_sweet_spot;

          if (cat === 'harga_di_bawah_modal_bandar') {
            const desc = discount >= 0
              ? `Harga terkini (${realPrice}) berada di bawah modal rata-rata Top 3 Bandar (${modal}) dengan diskon ${discount}%.`
              : `Harga terkini (${realPrice}) berada di atas modal rata-rata Top 3 Bandar (${modal}) (premium +${Math.abs(discount)}%).`;
      ...
      if (cacheData.tickers && cacheData.tickers[clean]) {
        const tObj = cacheData.tickers[clean];
        // Keep the per-emiten view coherent with the reconciled category row, so a
        // consumer reading tickers[TICKER] sees the same live price.
        if (realPrice && realPrice > 0) {
          tObj.current_price = realPrice;
          tObj.close_price = realPrice;
        }
        if (realPrice && realPrice > 0 && tObj.signals && tObj.signals.harga_di_bawah_modal_bandar) {
          const s = tObj.signals.harga_di_bawah_modal_bandar;
          s.current_price = realPrice;
          s.close_price = realPrice;
          const sModal = Number(s.bandar_avg_buy || s.bandar_avg_price || s.broker_cost || s.harga_modal || 0);
          if (sModal > 0) {
            const sDisc = Number((((sModal - realPrice) / sModal) * 100).toFixed(2));
            s.discount_pct = sDisc;
            s.price_diff_pct = -sDisc;
            s.in_sweet_spot = sDisc >= 5 && sDisc <= 15;
            s.is_sweet_spot = s.in_sweet_spot;
            s.description = sDisc >= 0
              ? `Harga terkini (${realPrice}) berada di bawah modal rata-rata Top 3 Bandar (${sModal}) dengan diskon ${sDisc}%.`
              : `Harga terkini (${realPrice}) berada di atas modal rata-rata Top 3 Bandar (${sModal}) (premium +${Math.abs(sDisc)}%).`;
          }
        }
      }
    }
  ...
  // Surface the truncation so a caller (or the UI) can tell a complete scan from
  // a budget-truncated one instead of silently trusting a partial result.
  cacheData.reconcile_budget_exhausted = budgetExhausted;
  return cacheData;
}

const INTEL_QUERY_MEM_CACHE = new Map();
const INTEL_QUERY_CACHE_TTL_MS = 10 * 60 * 1000;

async function getBandarmologiIntel(options = {}) {
  const ticker = options.ticker ? arjumClient.cleanTicker(options.ticker) : null;
  const signal = options.signal || null;
  const cacheKey = `${ticker || 'all'}|${signal || 'all'}|${options.range || '7d'}|${options.days || ''}|${options.currentPrice || ''}`;

  if (!options.forceRefresh) {
    const cached = INTEL_QUERY_MEM_CACHE.get(cacheKey);
    if (cached && Date.now() < cached.expiresAt) {
      return Object.assign({}, cached.payload, { from_memory_cache: true });
    }
  }
  ...
  if (ticker) {
    ...
    INTEL_QUERY_MEM_CACHE.set(cacheKey, {
      payload: responsePayload,
      expiresAt: Date.now() + INTEL_QUERY_CACHE_TTL_MS
    });
  ...
  // Reconcile prices across all scanner categories using the LOCAL price ladder
  // only (memory -> OHLCV cache -> broker-summary VWAP -> known baseline) with a
  // hard wall-clock budget, so the endpoint always answers in < 2s.
  reconcileScannerLivePrices(cacheData, { budgetMs: 1500 });
  ...
  return {
    ...
    // FIX (stage 1 as_of_date): the effective trading date is not always on the
    // aggregate root — a range-specific bridge payload can carry it per index or
    // per ticker. Fall through so the UI never labels a populated scan as undated.
    as_of_date: resolveIndexAsOfDate(cacheData) || resolveNestedAsOfDate(cacheData),
    data_source: dataSource,
    total_evaluated: cacheData ? cacheData.total_evaluated : 0,
    indexes: cacheData ? cacheData.indexes : {},
    // FIX (stage 1 per-emiten view): the reconciled per-emiten evaluations must
    // travel with the payload. Without this the client could only see the frozen
    // category rows and had no way to read a live CUAN / GOTO / PTRO price.
    tickers: cacheData && cacheData.tickers ? cacheData.tickers : {},
    // FIX (stage 1 partial-scan honesty): tell the caller when the wall-clock
    // budget cut the reconciliation short instead of presenting it as complete.
    reconcile_budget_exhausted: cacheData ? Boolean(cacheData.reconcile_budget_exhausted) : false,
    summary: {
...
module.exports = {
  ...
  reconcileScannerLivePrices,
  // Stage 1: exposed so the scanner's bulk price ladder can be verified directly.
  getBulkScannerLivePrice,
  buildIndexBulkPriceMap,
  setBulkLivePriceMap,
  clearBulkLivePriceMap
};
