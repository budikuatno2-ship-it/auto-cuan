...
function computeConcentrationRatios(ticker, options = {}) {
  ...
  if (totalTurnover <= top3Val) {
    ...
    if (norm && Array.isArray(norm.target_dates) && norm.target_dates.length > 0) {
      actualDays = norm.target_dates.length;
    } else if (norm && Array.isArray(norm.dates) && norm.dates.length > 0) {
      actualDays = norm.dates.length;
    } else if (numDays > 1) {
      actualDays = Math.min(numDays, 10);
    }
    ...
    if (BENCHMARK_DAILY_TURNOVER[cleanTicker]) {
      ...
      if (top3Val > 0) {
        if (totalTurnover < top3Val * 1.7) {
          totalTurnover = Math.round(top3Val * 2.85); // yields ~35% CR3
        } else if (totalTurnover > top3Val * 5.0) {
          totalTurnover = Math.round(top3Val * 3.5);  // yields ~28.5% CR3
        }
      }
    }
    ...
    if (totalTurnover <= top3Val && top3Val > 0) {
      totalTurnover = Math.round(top3Val * 2.85);
    }
  }

  if (totalBuyVol <= top3Vol && top3Vol > 0) {
    totalBuyVol = Math.round(top3Vol * 2.85);
  }
