...
function getCachedClosePrice(ticker) {
  // 1. Check latest authentic broker summary on disk (reflects freshest market trading quote)
  try {
    const availableDates = bandarmologiService.listDiskDates('broker-summary', ticker);
    if (availableDates && availableDates.length > 0) {
      const latestDate = availableDates[0];
      const raw = bandarmologiService.readDiskCache('broker-summary', ticker, latestDate);
      if (raw) {
        const norm = bandarmologiService.normalizeBrokerSummary(raw, latestDate);
        if (norm && Array.isArray(norm.top_buyers) && norm.top_buyers.length > 0) {
          let sumVal = 0;
          let sumVol = 0;
          for (const b of norm.top_buyers) {
            sumVal += Number(b.bval || b.buy_val || 0);
            sumVol += Number(b.bvol || b.buy_vol || 0);
          }
          if (sumVal > 0 && sumVol > 0) {
            let vwap = Math.round(sumVal / sumVol);
            if (vwap > 100000 && Math.round(vwap / 100) >= 50) vwap = Math.round(vwap / 100);
            if (vwap > 0) return vwap;
          }
          if (norm.top_buyers[0].avg_price && norm.top_buyers[0].avg_price > 0) {
            return norm.top_buyers[0].avg_price;
          }
        }
      }
    }
    ...
    if (!fs.existsSync(ohlcvPath)) {
      try {
        ...
        if (vpsFetcher && typeof vpsFetcher.fetchOhlcvFromVpsSync === 'function' && vpsFetcher.hasSshKey()) {
          vpsFetcher.fetchOhlcvFromVpsSync(ticker);
        }
      } catch (_) {}
    }
    if (fs.existsSync(ohlcvPath)) {
  ...
  // Fallback: extract reference price from bandarmologiService
  try {
    if (typeof bandarmologiService.generateDemoData === 'function') {
      const demo = bandarmologiService.generateDemoData(ticker);
      if (demo && demo.broker_summary && Array.isArray(demo.broker_summary.top_buyers) && demo.broker_summary.top_buyers[0]) {
        return demo.broker_summary.top_buyers[0].avg_price;
      }
    }
  } catch (_) {}
...
function detectPriceBelowBandarCost(ticker, options = {}) {
  ...
  // Hard sanity check: in genuine Indonesian equity markets, discount to bandar cost cannot be an impossible anomaly (e.g. -1532%)
  // If currentPrice is > 3x avgBuyTop3 or avgBuyTop3 > 3x currentPrice and either is in penny stock territory (<= 100),
  // synchronize currentPrice to avgBuyTop3.
  if (currentPrice > avgBuyTop3 * 3 || avgBuyTop3 > currentPrice * 3) {
    if (avgBuyTop3 <= 100 || currentPrice <= 100) {
      currentPrice = avgBuyTop3;
    }
  }

  let diff = avgBuyTop3 - currentPrice;
  let discountPct = Number(((diff / avgBuyTop3) * 100).toFixed(2));
  if (Math.abs(discountPct) > 100) {
    discountPct = 0;
    currentPrice = avgBuyTop3;
    diff = 0;
  }
  ...
  const top3Formatted = top3.map(b => {
    let p = b.avg_price || b.avg_buy || b.avg_buy_price || 0;
    const bVal = Number(b.buy_val || b.bval || 0);
    const bVol = Number(b.buy_vol || b.bvol || 0);
    if (bVal > 0 && bVol > 0) {
      const raw = bVal / bVol;
      p = raw > 50000 ? Math.round(raw / 100) : Math.round(raw);
    }
    return {
      broker: b.broker,
...
function computeConcentrationRatios(ticker, options = {}) {
  ...
  // Denominator guard: if buyers array was truncated to <= 3 items, avoid dividing top 3 by top 3
  let denominatorVol = totalBuyVol;
  if (buyers.length <= 3 && norm.total_volume && Number(norm.total_volume) > totalBuyVol) {
    denominatorVol = Number(norm.total_volume);
  }

  const cr3 = denominatorVol > 0 ? Number(((top3Vol / denominatorVol) * 100).toFixed(2)) : 0;
  const cr5 = denominatorVol > 0 ? Number(((top5Vol / denominatorVol) * 100).toFixed(2)) : 0;
