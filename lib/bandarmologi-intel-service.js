...
function loadIntelIndexWithProvenance(range) {
  // The cache key MUST include the bridge endpoint and index directory: tests
  // (and operators) repoint VPS_DATA_API_BASE / INTEL_INDEX_DIR at runtime, and
  // a stale live result must never leak into a scenario that expects a baked
  // index or an honest failure.
  const cacheKey = [
 ...
 * client budget. This resolver reads ONLY the in-memory map, the local OHLCV
 * candle cache and the local broker-summary VWAP, memoising every result so
 * repeated scans stay instant. Missing data falls back to the row's own last
 * close instead of waiting on the network.
 */
function getBulkScannerLivePrice(ticker) {
  const clean = String(ticker || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!clean) return null;
  if (SCANNER_LIVE_PRICE_MEMORY.has(clean)) return SCANNER_LIVE_PRICE_MEMORY.get(clean);

  let price = null;

  // 1. Local OHLCV candle cache (freshest local quote, no VPS fetch).
  try {
    const candidates = [
      path.join(__dirname, '..', 'data', 'daytrade-ohlcv-cache', `${clean}.json`),
      path.join(process.cwd(), 'data', 'daytrade-ohlcv-cache', `${clean}.json`)
  ...
  // 2. Local broker-summary VWAP of the newest trading day.
  if (!(price > 0)) {
    try {
      const vwap = deriveCloseFromLatestBrokerSummary(clean);
      if (vwap && vwap > 0) price = vwap;
    } catch (_) {}
  }

  // 3. Known baseline price (static reference, always instant).
  if (!(price > 0)) {
    try {
      const known = bandarmologiService.KNOWN_TICKER_PRICES && bandarmologiService.KNOWN_TICKER_PRICES[clean];
      if (known && known > 0) price = Number(known);
    } catch (_) {}
  }

  SCANNER_LIVE_PRICE_MEMORY.set(clean, price);
  return price;
}
  ...
  for (const cat of categories) {
    ...
    for (const item of list) {
      ...
      if (priceCache.has(clean)) {
        ...
        try {
          realPrice = getBulkScannerLivePrice(clean);
        } catch (err) {
      ...
      // Budget guard: stop reconciling and serve what we already have.
