...
function detectSilentForeignAccumulation(ticker, options = {}) {
  ...
  const checkDates = availableDates.slice(0, Math.min(Math.max(numDays, 5), 10)); // check newest trading days
  const dailyFlows = [];
...
function evaluateBandarmologiIntelForTicker(ticker, options = {}) {
  ...
  const effectiveTradingDate = bandarmologiService.getEffectiveTradingDate(clean, options.date);
