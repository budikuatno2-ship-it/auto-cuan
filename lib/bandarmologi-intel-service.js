...
function computeConcentrationRatios(ticker, options = {}) {
  ...
  // Denominator guard: avoid division by zero or false lock to 100% or 50% when only top 3 brokers present
  if (totalTurnover <= 0) {
    totalTurnover = top3Val > 0 ? (buyers.length <= 3 ? Math.round(top3Val * 2.85) : top3Val) : 0;
  }
