 ...
 */
function reconcileScannerLivePrices(cacheData) {
  ...
  for (const cat of categories) {
    ...
    for (const item of list) {
      ...
      if (priceCache.has(clean)) {
        ...
        try {
          realPrice = getCachedClosePrice(clean);
        } catch (err) {
          // ignore lookup failure
        }
        // FIX: a stale baked index price (CUAN 630 / BUMI 150 / PTRO 4080) must never
        // survive reconciliation. When neither the live bridge nor the local candle
        // ladder can confirm a quote, derive the freshest tradable price from the
        // broker-summary VWAP of the newest trading day instead of keeping the file value.
        if (!(realPrice > 0)) {
          try {
            const summaryVwap = deriveCloseFromLatestBrokerSummary(clean);
            if (summaryVwap && summaryVwap > 0) realPrice = summaryVwap;
          } catch (err) {
            // ignore lookup failure
          }
        }
...
async function getBandarmologiIntel(options = {}) {
  ...
  // Reconcile prices across all scanner categories with real-time market prices
  reconcileScannerLivePrices(cacheData);
