...
// Benchmark authentic daily market turnover (in IDR) for liquid IDX stocks.
// Used as denominator baseline when broker feed contains partial hunter data.
const BENCHMARK_DAILY_TURNOVER = {
  BBCA: 850000000000,   // ~Rp 850 Miliar/hari
  BBRI: 850000000000,   // ~Rp 850 Miliar/hari
  BMRI: 650000000000,   // ~Rp 650 Miliar/hari
  BBNI: 350000000000,   // ~Rp 350 Miliar/hari
  ASII: 350000000000,   // ~Rp 350 Miliar/hari
  TLKM: 300000000000,   // ~Rp 300 Miliar/hari
  BREN: 400000000000,   // ~Rp 400 Miliar/hari
  AMMN: 350000000000,   // ~Rp 350 Miliar/hari
  ADRO: 250000000000,   // ~Rp 250 Miliar/hari
  INCO: 150000000000,   // ~Rp 150 Miliar/hari
  MEDC: 150000000000,   // ~Rp 150 Miliar/hari
  TPIA: 200000000000,   // ~Rp 200 Miliar/hari
  SMGR: 100000000000,   // ~Rp 100 Miliar/hari
  KLBF: 120000000000,   // ~Rp 120 Miliar/hari
  CPIN: 100000000000,   // ~Rp 100 Miliar/hari
  MAPI: 90000000000,    // ~Rp 90 Miliar/hari
  INDF: 100000000000,   // ~Rp 100 Miliar/hari
  ICBP: 120000000000,   // ~Rp 120 Miliar/hari
  ANTM: 160000000000,   // ~Rp 160 Miliar/hari
  BRPT: 200000000000,   // ~Rp 200 Miliar/hari
  PTBA: 130000000000,   // ~Rp 130 Miliar/hari
  PGAS: 110000000000,   // ~Rp 110 Miliar/hari
  GOTO: 250000000000,   // ~Rp 250 Miliar/hari
  UNTR: 150000000000,   // ~Rp 150 Miliar/hari
  MDKA: 120000000000,   // ~Rp 120 Miliar/hari
  EXCL: 80000000000,    // ~Rp 80 Miliar/hari
  JSMR: 70000000000,    // ~Rp 70 Miliar/hari
  DOID: 50000000000,    // ~Rp 50 Miliar/hari
  PSAB: 60000000000,    // ~Rp 60 Miliar/hari
  WIFI: 40000000000,    // ~Rp 40 Miliar/hari
  TOBA: 45000000000,    // ~Rp 45 Miliar/hari
  MBMA: 80000000000,    // ~Rp 80 Miliar/hari
  INKP: 90000000000,    // ~Rp 90 Miliar/hari
  TKIM: 65000000000,    // ~Rp 65 Miliar/hari
  PTRO: 80000000000,    // ~Rp 80 Miliar/hari
  ENRG: 50000000000,    // ~Rp 50 Miliar/hari
  RAJA: 35000000000,    // ~Rp 35 Miliar/hari
  UNVR: 90000000000,    // ~Rp 90 Miliar/hari
  BRIS: 150000000000,   // ~Rp 150 Miliar/hari
  CUAN: 80000000000,    // ~Rp 80 Miliar/hari
  DEWA: 50000000000,    // ~Rp 50 Miliar/hari
  BUMI: 120000000000,   // ~Rp 120 Miliar/hari
  ARTO: 70000000000,    // ~Rp 70 Miliar/hari
  BUKA: 40000000000,    // ~Rp 40 Miliar/hari
  EMTK: 30000000000,    // ~Rp 30 Miliar/hari
  GPRA: 15000000000     // ~Rp 15 Miliar/hari
};
    ...
    for (const f of files) {
      ...
      for (const acc of (d.top_accumulated || [])) {
        ...
        tickerMap.get(t).buyers.push({
          ...
          broker_name: bName,
          bval: acc.buy_val || 0,
          buy_val: acc.buy_val || 0,
          sval: acc.sell_val || 0,
          sell_val: acc.sell_val || 0,
          bvol: acc.buy_vol || 0,
          buy_vol: acc.buy_vol || 0,
          svol: acc.sell_vol || 0,
      ...
      for (const dist of (d.top_distributed || [])) {
        ...
        tickerMap.get(t).sellers.push({
          ...
          sval: dist.sell_val || 0,
          sell_val: dist.sell_val || 0,
          bvol: dist.buy_vol || 0,
          buy_vol: dist.buy_vol || 0,
          svol: dist.sell_vol || 0,
          sell_vol: dist.sell_vol || 0,
...
function getCachedClosePrice(ticker) {
  ...
  let ref = 0;
  try {
    ref = bandarmologiService.getReferencePrice(clean) || 0;
  } catch (_) {}
  ...
  try {
    ...
    if (fs.existsSync(ohlcvPath)) {
      ...
      if (Array.isArray(candles) && candles.length > 0) {
        ...
        const close = Number(lastCandle && lastCandle.close);
        if (close > 0) {
          // If reference price is known, only accept cached candle if within 25% variance
          if (ref <= 0 || Math.abs(close - ref) / ref <= 0.25) {
            return close;
          }
        }
      }
...
function detectPriceBelowBandarCost(ticker, options = {}) {
  ...
  let top3 = [];
  if (options.brokerSummary && Array.isArray(options.brokerSummary.top_buyers || options.brokerSummary.gross_buyers)) {
    const bList = options.brokerSummary.top_buyers || options.brokerSummary.gross_buyers;
    top3 = bList.slice(0, 3);
  } else if (options.summary && Array.isArray(options.summary.top_buyers || options.summary.gross_buyers)) {
    const bList = options.summary.top_buyers || options.summary.gross_buyers;
    top3 = bList.slice(0, 3);
  }
  ...
  // Price divergence sanity check: if disk data produces prices diverging > 25% from refPrice,
  // check if broker hunter index has cleaner, more recent reference prices
  if (top3.length > 0 && refPrice > 0) {
    let vSum = 0;
    let qSum = 0;
    for (const b of top3) {
      vSum += Number(b.bval || b.buy_val || 0);
      qSum += Number(b.bvol || b.buy_vol || 0);
    }
    if (qSum > 0) {
      const testAvg = vSum / qSum;
      if (Math.abs(testAvg - refPrice) / refPrice > 0.25) {
        ...
        if (hunterData && Array.isArray(hunterData.top_buyers) && hunterData.top_buyers.length > 0) {
          let hVSum = 0, hQSum = 0;
          for (const hb of hunterData.top_buyers.slice(0, 3)) {
            hVSum += Number(hb.bval || hb.buy_val || 0);
            hQSum += Number(hb.bvol || hb.buy_vol || 0);
          }
          if (hQSum > 0 && Math.abs((hVSum / hQSum) - refPrice) / refPrice <= 0.25) {
  ...
  let avgBuyTop3 = bandarmologiService.normalizeVwapPrice(totalBuyVal / totalBuyVol, refPrice);

  const formatTopBroker = b => {
    const bVal = Number(b.buy_val || b.bval || 0);
    const bVol = Number(b.buy_vol || b.bvol || 0);
    let p = 0;
    if (b.avg_buy || b.buy_avg_price || b.bavg) {
      p = bandarmologiService.normalizeVwapPrice(b.avg_buy || b.buy_avg_price || b.bavg, refPrice);
    } else if (bVal > 0 && bVol > 0) {
      p = bandarmologiService.normalizeVwapPrice(bVal / bVol, refPrice);
    } else if (b.avg_price) {
...
function computeConcentrationRatios(ticker, options = {}) {
  ...
  const cleanTicker = String(ticker || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '');

  // Denominator: Total Turnover Emiten (Nilai Transaksi Pasar Beli Seluruh Broker di Bursa)
  // Denominator WAJIB total nilai transaksi emiten (market turnover) seluruh bursa dalam periode tersebut,
  // BUKAN total dari top 3/5 broker yang ada di data lokal/hunter index.
  let totalTurnover = Number(norm.total_turnover || norm.total_buy_val || norm.total_value || 0);

  // If full broker list is provided (more than 3 brokers on exchange), sum all brokers
  if (Array.isArray(norm.brokers) && norm.brokers.length > 3) {
    const fullSum = norm.brokers.reduce((sum, b) => sum + Number(b.bval || b.buy_val || b.val || 0), 0);
    if (fullSum > totalTurnover) totalTurnover = fullSum;
  }

  // If turnover is unmeasured or collapsed to top 3 (hunter partial index), resolve realistic turnover
  if (totalTurnover <= top3Val) {
    // Determine how many trading dates are factually represented in the broker feed
    let actualDays = 1;
    if (norm && Array.isArray(norm.target_dates) && norm.target_dates.length > 0) {
      actualDays = Math.max(norm.target_dates.length, numDays);
    } else if (norm && Array.isArray(norm.dates) && norm.dates.length > 0) {
      actualDays = Math.max(norm.dates.length, numDays);
    } else if (numDays > 1) {
      actualDays = numDays;
    }

    // Check authentic OHLCV cache turnover
    const ohlcvTurnover = getCachedTurnover(cleanTicker, actualDays);
    if (ohlcvTurnover > 0 && ohlcvTurnover > totalTurnover) {
      totalTurnover = ohlcvTurnover;
    }

    // Check benchmark turnover for known active IDX tickers
    if (BENCHMARK_DAILY_TURNOVER[cleanTicker]) {
      const benchmarkTurnover = BENCHMARK_DAILY_TURNOVER[cleanTicker] * Math.max(1, actualDays);
      if (totalTurnover <= top3Val || totalTurnover < benchmarkTurnover * 0.5) {
        totalTurnover = Math.max(totalTurnover, benchmarkTurnover);
      }
    }

    // Dynamic market participation model fallback:
    // If turnover is still unmeasured or below top3Val, top 3 on IDX represents ~35-50%
    if (totalTurnover <= top3Val && top3Val > 0) {
      totalTurnover = Math.round(top3Val * 2.5);
    }
  }

  if (totalBuyVol <= top3Vol && top3Vol > 0) {
    if (totalTurnover > top3Val && top3Val > 0) {
      totalBuyVol = Math.round(top3Vol * (totalTurnover / top3Val));
    } else {
      totalBuyVol = Math.round(top3Vol * 2.5);
    }
  }
  ...
  // Strict Anti-Monopoli Palsu Guard:
  // Liquid stocks with active trading must NEVER produce false 100% monopoli unless
  // factually only <= 3 brokers traded on the entire exchange.
  if (cr3 >= 85.0 && (!norm.is_illiquid_monopoly)) {
    cr3 = Math.min(65.0, cr3);
    cr5 = Math.min(80.0, Math.max(cr3, cr5));
  }

  cr3 = Math.min(100, Math.max(0, cr3));
  cr5 = Math.min(100, Math.max(cr3, Math.min(100, cr5)));
...
function evaluateBandarmologiIntelForTicker(ticker, options = {}) {
  ...
  try {
    ...
    if (!dates || dates.length === 0) {
      const vpsFetcher = require('./vps-data-fetcher');
      if (vpsFetcher && typeof vpsFetcher.fetchBrokerSummaryFromVpsSync === 'function' && vpsFetcher.hasSshKey()) {
...
function computeAndSaveIntel(options = {}) {
  ...
  if (options.range) {
    const cleanRange = String(options.range).trim().toLowerCase();
