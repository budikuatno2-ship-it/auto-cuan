...
const APPROVAL_TTL_MS = process.env.ADMIN_DEVICE_APPROVAL_TTL_MS
  ? parseInt(process.env.ADMIN_DEVICE_APPROVAL_TTL_MS, 10)
...
async function handleDeviceApprovalCallback(cq, deps) {
  ...
  let req = memoryApprovalStore.get(token);

  if (!req && supabase) {
    try {
      const { data: dbReq } = await supabase
        .from('admin_device_approvals')
        .select('*')
        .eq('token', token)
        .maybeSingle();
      if (dbReq) {
        req = {
          ...
          deviceId: dbReq.device_id,
          userAgent: dbReq.user_agent || '',
          status: dbReq.status || 'pending',
          createdAt: new Date(dbReq.created_at || Date.now()).getTime(),
          expiresAt: new Date(dbReq.expires_at).getTime(),
          telegramChatId: null,
          telegramMessageId: null,
          sessionCookie: null
        };
        memoryApprovalStore.set(token, req);
      }
    } catch (_) {}
  }
...
function _evaluateReqStatus(req, res) {
  if (req.expiresAt < Date.now()) {
    req.status = 'expired';
  ...
  if (req.status === 'approved') {
    if (!req.sessionCookie && req.userId && req.username) {
      try {
        const sessionToken = createSessionToken({
          userId: req.userId,
          username: req.username,
 ...
 * If in-memory request is missing, falls back to Supabase if deps.supabase is provided.
 */
function checkDeviceApprovalStatus(token, res, deps) {
  if (!token) {
    return { ok: false, status: 'invalid_token', error: 'Token tidak valid.' };
  }

  let req = memoryApprovalStore.get(token);
  if (!req && deps && deps.supabase) {
    return (async () => {
      try {
        const { data: dbReq } = await deps.supabase
          .from('admin_device_approvals')
          .select('*')
          .eq('token', token)
          .maybeSingle();
        if (dbReq) {
          ...
          memoryApprovalStore.set(token, req);
          return _evaluateReqStatus(req, res);
        }
      } catch (_) {}
