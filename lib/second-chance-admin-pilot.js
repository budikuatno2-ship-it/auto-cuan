...
async function acquireLock(file) { await fsp.mkdir(path.dirname(file), { recursive: true }); try { const h = await fsp.open(file, 'wx'); await h.writeFile(JSON.stringify({ pid: process.pid }) + '\n'); return async () => { await h.close(); await fsp.unlink(file).catch(() => {}); }; } catch (e) { if (e.code === 'EEXIST') return null; throw e; } }
...
async function runPilot(opts) {
  ...
  if (!evaluation.selected || opts.dryRun) return result;
  if (mode === 'send') {
  ...
  const release = await acquireLock(lockFile); if (!release) { result.status = 'lock_busy'; result.error_code = 'lock_busy'; return result; }
  try {
    const existing = await readState(stateFile);
    if (existing) { result.status = existing.status === 'sent' ? 'already_sent' : 'already_recorded'; result.already_recorded = true; result.state_identity = existing.identity; if (existing.identity !== identity(opts.sampleDate, evaluation.selected)) result.telegram_block_reason = 'date_already_selected_different_alert'; return result; }
    if (mode === 'shadow') { await writeState(stateFile, { schema_version: 1, date: opts.sampleDate, identity: identity(opts.sampleDate, evaluation.selected), rule_version: RULE_VERSION, status: 'shadow_recorded', selected: evaluation.selected }); result.status = 'selected_shadow_recorded'; return result; }
    ...
    const sent = await adapter(message(opts.sampleDate, evaluation.selected), { chat_id: approvedAdmin(env), disable_web_page_preview: true });
    if (!sent || sent.sent !== true) { result.status = 'failed'; result.error_code = 'telegram_send_failed'; result.telegram_block_reason = 'telegram_send_failed'; return result; }
    await writeState(stateFile, { schema_version: 1, date: opts.sampleDate, identity: identity(opts.sampleDate, evaluation.selected), rule_version: RULE_VERSION, status: 'sent', selected: evaluation.selected }); result.status = 'sent'; result.telegram_sent = true; return result;
  } finally { await release(); }
}

module.exports = { RULE_VERSION, TIMEZONE, DEFAULT_MODE, DEFAULT_STATE_DIR, enabled, jakartaDate, qualifyObservation, antiChase, evaluateObservations, readCandidates, approvedAdmin, message, runPilot };
