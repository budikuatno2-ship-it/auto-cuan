    ...
    if (e && e.code === 'EPERM') {
      try {
        ...
        if (fsSync.existsSync('/proc')) {
          return fsSync.existsSync('/proc/' + pid);
        }
      } catch (_) {}
      return true;
    }
...
const _activeLockQueues = new Map();

async function acquireLock(file, options) {
  options = options || {};
  const absPath = path.resolve(file);
  const prev = _activeLockQueues.get(absPath) || Promise.resolve();
  let releaseQueue;
  const current = new Promise(resolve => { releaseQueue = resolve; });
  _activeLockQueues.set(absPath, prev.then(() => current, () => current));
  await prev.catch(() => {});

  try {
    return await _acquireLockInternal(file, options);
  } finally {
    releaseQueue();
    if (_activeLockQueues.get(absPath) === current) {
      _activeLockQueues.delete(absPath);
    }
  }
}

async function _acquireLockInternal(file, options) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      return async function release() { await fsp.unlink(file).catch(() => {}); };
    } catch (e) {
      ...
      const quarantined = await inspectLock(quarantine, nowMs, staleMs);
      if (!quarantined.stale || quarantined.alive) {
        await fsp.rename(quarantine, file).catch(() => {});
        return null;
      }
      await fsp.unlink(quarantine).catch(() => {});
    }
  }
  return null;
}
