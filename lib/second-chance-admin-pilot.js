...
async function runPilot(opts) {
  ...
  try {
    ...
    if (existing && existing.status !== 'send_failed_retryable') {
    ...
    if (mode === 'shadow') { await persist(stateFile, { schema_version: 1, date: opts.sampleDate, identity: identity(opts.sampleDate, evaluation.selected), rule_version: RULE_VERSION, status: 'shadow_recorded', selected: evaluation.selected }); result.status = 'selected_shadow_recorded'; return result; }
    const attempt = { schema_version: 1, date: opts.sampleDate, identity: identity(opts.sampleDate, evaluation.selected), rule_version: RULE_VERSION, status: 'send_in_progress', selected: evaluation.selected };
    ...
    let sent;
    try { sent = await adapter(message(opts.sampleDate, evaluation.selected), { chat_id: approvedAdmin(env), disable_web_page_preview: true }); }
    catch (_) {
...
module.exports = { RULE_VERSION, TIMEZONE, DEFAULT_MODE, DEFAULT_STATE_DIR, DEFAULT_LOCK_STALE_MS, enabled, number, validDate, jakartaDate, qualifyObservation, antiChase, evaluateObservations, readCandidates, approvedAdmin, message, acquireLock, readState, writeState, runPilot };
