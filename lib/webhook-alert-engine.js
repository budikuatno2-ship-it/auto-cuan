...
async function sendAlert(candidate, options = {}) {
  ...
  // Gate ini hanya aktif bila pemanggil menyertakan konteks waktu eksplisit
  // (`options.now`). Ini menjaga `sendAlert` tetap deterministik dan
  // backward-compatible bagi pemanggil yang jadwalnya sudah dibatasi sesi
  // bursa oleh cron, sekaligus memungkinkan sesi-aware caller/menguji memblok
  // BREAK/CLOSED secara eksplisit.
