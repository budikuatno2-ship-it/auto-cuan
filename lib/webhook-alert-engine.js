...
// ---------------------------------------------------------------------------
// BATCH 6 / F12W: short-window in-flight trigger dedup ("claim") + state machine.
//
// The 20-minute cooldown is only recorded AFTER a successful dispatch, so two
// triggers arriving within the same tick (a screener burst, or the same ticker
// fired by two pipelines) both passed checkCooldown and both dispatched.
// A claim is taken BEFORE the network call and released on failure, which makes
// the dedup window cover the in-flight period as well.
// ---------------------------------------------------------------------------
const DEFAULT_CLAIM_WINDOW_MS = 60 * 1000; // 60s
const alertClaims = new Map();

const ALERT_STATES = Object.freeze(['PENDING', 'SENT', 'FAILED']);
// PENDING is the only non-terminal state, so only these two edges are legal.
const ALERT_STATE_TRANSITIONS = Object.freeze({
  PENDING: ['SENT', 'FAILED'],
  SENT: [],
  FAILED: ['PENDING']
});

function clearAlertClaims() {
  alertClaims.clear();
}

function resolveClaimKey(ticker) {
...
/**
 * Take the in-flight dedup claim for `ticker`.
 * @returns {{ claimed: boolean, state: string|null, reason?: string, remainingMs?: number }}
 */
function claimAlert(ticker, options) {
  options = options || {};
  var key = resolveClaimKey(ticker);
  if (!key) return { claimed: false, state: null, reason: 'missing_ticker' };

  var now = resolveNow(options);
  var existing = alertClaims.get(key);
  if (existing) {
    if (now < existing.expiresAt) {
      return {
        claimed: false,
        state: existing.state,
        reason: 'duplicate_trigger_window',
        remainingMs: existing.expiresAt - now
      };
    }
    alertClaims.delete(key);
  }

  var windowMs = Number(options.claim_window_ms != null ? options.claim_window_ms : options.claimWindowMs);
  if (!Number.isFinite(windowMs) || windowMs <= 0) windowMs = DEFAULT_CLAIM_WINDOW_MS;
...
function getAlertClaim(ticker, options) {
  options = options || {};
  var key = resolveClaimKey(ticker);
  if (!key) return null;
  var entry = alertClaims.get(key);
...
function releaseAlert(ticker) {
  var key = resolveClaimKey(ticker);
  if (!key) return false;
  return alertClaims.delete(key);
}

/**
 * Move a claim through the alert state machine.
 * Illegal edges and terminal-state re-entry are rejected, so a buggy caller
 * cannot resurrect a SENT alert into PENDING and re-broadcast it.
 */
function commitAlert(ticker, nextState, options) {
  options = options || {};
  var key = resolveClaimKey(ticker);
  var target = String(nextState || '').trim().toUpperCase();
  if (!key) return { ok: false, state: null, reason: 'missing_ticker' };
  if (ALERT_STATES.indexOf(target) < 0) return { ok: false, state: null, reason: 'unknown_state' };

  var entry = alertClaims.get(key);
  if (!entry) return { ok: false, state: null, reason: 'no_active_claim' };

  var allowed = ALERT_STATE_TRANSITIONS[entry.state] || [];
  if (allowed.indexOf(target) < 0) {
    return { ok: false, state: entry.state, reason: 'illegal_transition' };
  }
  ...
  if (target === 'PENDING') {
    ...
    // Terminal states stop holding the dedup window open.
    entry.expiresAt = now;
  }
  return { ok: true, state: target, reason: null };
}

// ---------------------------------------------------------------------------
// BATCH 6 / F12W: payload signature validation.
//
// Screeners can be triggered by an external caller (cron, VPS bridge, manual
// replay). When a shared secret is configured, an unsigned or tampered payload
// must be refused before any message reaches the channel.
// ---------------------------------------------------------------------------

/**
 * Deterministic HMAC-SHA256 (hex) over the canonical JSON of `payload`.
 * Object key order is normalised so semantically-equal payloads sign equally.
 * @returns {string|null} null when the payload or secret is unusable
 */
...
/**
 * Constant-time comparison of a payload against its expected signature.
 * @returns {boolean}
 */
...
/**
 * Build the signed body for a candidate: the fields a screener alert actually
 * asserts. Kept intentionally narrow so an unrelated metadata change does not
 * invalidate an otherwise-identical signal.
 */
function buildSignaturePayload(candidate) {
  var c = candidate || {};
  return {
    ticker: String(c.ticker || '').trim().toUpperCase(),
    status: normalizeStatus(c.status || c.final_status),
    action: c.action || c.signal_action || null,
    entry_high: c.entry_high != null ? Number(c.entry_high) : null,
    stop_loss: c.stop_loss != null ? Number(c.stop_loss) : null,
    tp1: c.tp1 != null ? Number(c.tp1) : null
  };
}
...
async function sendAlert(candidate, options = {}) {
  ...
  // 0b. BATCH 6: optional payload signature gate (external trigger hardening).
  if (options.signature_secret) {
    const expected = signPayload(buildSignaturePayload(candidate), options.signature_secret);
    const provided = options.payload_signature || options.payloadSignature;
    const signatureOk = expected != null && provided != null &&
  ...
  // 1b. BATCH 6: short-window in-flight claim (opt-in via claim_dedup).
  const useClaim = options.claim_dedup === true || options.claimDedup === true;
  if (useClaim) {
    const claim = claimAlert(ticker, options);
    if (!claim.claimed) {
      return {
        success: true,
        skipped: true,
        reason: claim.reason || 'duplicate_trigger_window',
        ticker,
        channels: {}
      };
    }
  ...
  // BATCH 6: close out the in-flight claim. A delivered alert becomes SENT
  // (dedup window stays open); a failed one is released so the next trigger can
  // retry immediately instead of being stuck behind a dead claim.
  if (useClaim) {
    if (anySent) {
      commitAlert(ticker, 'SENT', options);
    } else {
      commitAlert(ticker, 'FAILED', options);
      releaseAlert(ticker);
    }
  }
...
module.exports = {
  ...
  bucketByMinute,
  buildBatchDigest,
  // BATCH 6: short-window trigger dedup + alert state machine
  ...
  commitAlert,
  clearAlertClaims,
  // BATCH 6: payload signature validation
  signPayload,
