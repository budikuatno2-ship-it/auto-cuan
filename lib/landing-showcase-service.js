...
async function refreshSnapshot(supabase) {
  try {
    ...
    const { data: sectorRows } = await supabase
      ...
      .select('group_code, group_name, avg_change_pct, top_ticker, top_change_pct, calculated_at')
    ...
    const sectors = (sectorRows || []).map(function(s) {
      return {
        ...
        score: typeof s.avg_change_pct === 'number' ? Math.round(Number(s.avg_change_pct) * 10) / 10 : null,
        avg_change_pct: typeof s.avg_change_pct === 'number'
          ? Math.round(Number(s.avg_change_pct) * 100) / 100
          : null,
        top_ticker: s.top_ticker || null,
        top_change_pct: typeof s.top_change_pct === 'number'
          ? Math.round(Number(s.top_change_pct) * 100) / 100
          : null
      };
    });
