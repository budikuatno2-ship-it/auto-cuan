...
function isAfternoonExitCutoff(scheduledTime) {
  if (scheduledTime) {
    const raw = String(scheduledTime || '').trim();
    if (/^\d{1,2}:\d{2}/.test(raw)) {
      const parts = raw.split(':').map(Number);
      const total = (parts[0] || 0) * 60 + (parts[1] || 0);
      return total > marketHoursGuard.DAYTRADE_RADAR_CUTOFF_MINUTES;
    }
  }
  return marketHoursGuard.isAfternoonExitCutoff();
}
...
async function publishRadar(options) {
  ...
  // Hard guard per-item sebelum pengiriman Telegram:
  // Blokir total jika item adalah pra-konfirmasi (EARLY WATCH 0/2 atau 1/2) atau sudah lewat batas 14:30 WIB
  for (const item of sendable) {
    if ((!allowPreConfirmation && isPreConfirmationItem(item)) || isAfternoonExitCutoff(scheduledTime)) {
      return {
        suppressed: true,
        reason: 'preconfirmation_or_after_1430_blocked',
        attempted: false,
        telegram_attempted: false,
        telegram_sent: 0,
        radar_items_sent: 0,
