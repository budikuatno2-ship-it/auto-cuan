...
function validRadarPlan(plan) {
  ...
  if (!['READY_PENDING', 'PENDING_VELOCITY', 'WAIT_PULLBACK'].includes(plan.internal_status) && plan.internal_status !== 'SPIKE_RADAR' && (plan.watch_score == null || plan.watch_score < MIN_RADAR_WATCH_SCORE)) return false;
...
async function publishRadar(options) {
  ...
  const sendable = candidates.filter(item => shouldSend(item, ledger.tickers[item.ticker], currentSession)).slice(0, MAX_RADAR_PUBLISH);
  if (!sendable.length) {
