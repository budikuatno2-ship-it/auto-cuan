...
function actionText(item) {
  if (item.internal_status === 'READY_PENDING') return 'Pantau: menunggu konfirmasi kedua.';
  if (item.current_price > item.entry_high) return `Pantau ${formatPrice(item.entry_low)}–${formatPrice(item.entry_high)}; jangan chase.`;
...
function buildRadarTelegramMessage(items, sampleDate, scheduledTime) {
  ...
  lines.push(
    ...
    'Entry hanya setelah konfirmasi Fast Watcher.'
  );
  return lines.join('\n');
}
