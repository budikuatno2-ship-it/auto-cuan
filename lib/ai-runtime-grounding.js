...
// ponytail: import budgetFacts and stockFacts for complete runtime grounding
const { planFacts, budgetFacts, stockFacts } = require('./ai-eval-derived-facts');
...
function moneyFromMessage(message) {
  ...
  // ponytail: handle multi-word phrase before amount (e.g. "modal saya sebesar 50 juta")
...
function prepareRuntimeGrounding(source, message, context) {
  ...
  } else if (source === 'stock_analysis') {
    // ponytail: provide calculation facts for single stock analysis (not follow-up)
    output.calculation_facts = { stock: stockFacts(output) };
  }
  return output;
}
