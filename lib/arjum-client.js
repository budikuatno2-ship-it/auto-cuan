...
// Default daily quota (5000 req/day). Can be overridden via ARJUM_DAILY_QUOTA.
const DEFAULT_DAILY_QUOTA = 5000;
...
async function fetchArjum(endpointPath, options = {}) {
  ...
  try {
    ...
    // Count usage the moment we get ANY response from Arjum (2xx or not) —
    // that's proof the request reached and was processed server-side. A
    // pure network/timeout failure below (no response at all) is NOT
    // counted, since there is no way to know whether Arjum ever saw it.
...
// request — always send explicit values covering the full IDX broker universe
// (100+ brokers), so neither gross mode nor net mode are artificially capped to 20+20.
...
module.exports = {
  ...
  DEFAULT_LEVEL_LIMIT,
  // Persistent cross-process usage tracking (lib/arjum-quota-tracker.js) —
  // the source of truth for "how much quota is left today", since Arjum has
  // no documented endpoint/header for it. Re-exported here so callers only
  // need to require('./arjum-client'), not both modules.
