...
function classifyBlockers({ bundle, aggregate, policy, gate }) {
  ...
  if (policy && policy.policy_status === 'BLOCK') non.push('policy_status BLOCK');
  ...
  if (bundle && bundle.validation_status === 'BLOCK') non.push('validation bundle BLOCK');
  const recurring = {
...
function buildCloseoutReport(inputs, opts) {
  ...
  const runbookStatus = runbook && runbook.runbook_status || null;
