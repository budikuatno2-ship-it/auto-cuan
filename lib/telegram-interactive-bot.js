...
function createInteractiveBot(options) {
  ...
  async function requireApproved(ctx) {
    ...
    const isZeroByokGroup = isGroup && (env.BOT_GROUP_ZERO_BYOK === 'true' || env.BOT_GROUP_ZERO_BYOK === '1' || opts.zeroByok === true);
    if (user && isActiveUser(user)) {
      // In group chats: ZERO BYOK required when BOT_GROUP_ZERO_BYOK is active.
      if (isZeroByokGroup) {
        return { ok: true, user };
      }
...
// Admin quota control: /limit <telegram_id> <amount> or inline buttons
  ...
  async function renderMarketCard(ctx, user, command, ticker, rangeDays) {
    ...
    const isGroup = isGroupChat(ctx.chat);
    const isZeroByok = isGroup && (env.BOT_GROUP_ZERO_BYOK === 'true' || env.BOT_GROUP_ZERO_BYOK === '1' || opts.zeroByok === true);
  ...
  async function handleMarket(ctx, parsed) {
    ...
    // In group chats with ZERO BYOK: quota and key session checks are bypassed
    if (!isZeroByok) {
