...
function createInteractiveBot(options) {
  ...
  async function requireApproved(ctx) {
    ...
    if (user && isActiveUser(user)) {
      ...
      // KONDISI 4: Approved tapi belum pasang Kunci AI (BYOK)
      if (isGroup) {
        const username = from.username ? '@' + from.username : (user.username || 'User');
        const dmUrl = 'https://t.me/' + (verifyUsername || 'AutoCuanVerificationBot');
        const sent = await ctx.reply(
          '⚠️ Halo ' + username + ', Kunci AI (BYOK) Anda belum dipasang.\n' +
          'Silakan buka DM bot dan ketik `/setkey <kunci_anda>`.',
          {
            reply_markup: {
...
// Accepts an optional { marketAnalysis: bool } to inject the clean trader style prompt.
  async function callByok(user, prompt, opts2) {
    ...
    if (!apiKey) {
      return 'Kunci BYOK belum tersedia. Kirim /setkey <kunci_anda> lewat chat pribadi bot.';
    }
    ...
    // Hard ceiling on answer length. The system prompt states the 3-paragraph
    // budget, but a prompt is a request, not a guarantee — the provider cap is
    // what actually stops a runaway reply. ~450 tokens is comfortably above a
    // 120-word Indonesian answer and well below the 2048 default that produced
    // multi-screen walls of text.
    const answerTokenCap = (opts2 && opts2.marketAnalysis) ? 450 : undefined;

    // Hybrid failover: the provider call goes to the primary (Vercel) endpoint
    // first with a 5s budget; on network error / timeout / 503 / 402 the same
    // request is replayed against the local VPS router. See lib/ai-evaluator.js.
    const primary = async () => {
      if (provider === 'gemini' || provider === 'google') {
        const gemini = opts.gemini || require('./ai-gemini-provider');
        const customModel = (user && user.provider_settings && (user.provider_settings.ai_model || user.provider_settings.model)) ||
          (user && user.custom_model) || env.GEMINI_MODEL;
      ...
      // OpenAI / DeepSeek / Claude / xAI / custom gateways.
      const baseUrl = (user && user.provider_settings && (user.provider_settings.ai_base_url || user.provider_settings.base_url)) ||
        (user && user.custom_base_url) ||
        (provider === 'openai' ? 'https://api.openai.com/v1' :
         provider === 'claude' ? 'https://api.anthropic.com/v1' :
         provider === 'grok' ? 'https://api.x.ai/v1' : env.CUSTOM_AI_BASE_URL);
      const model = (user && user.provider_settings && (user.provider_settings.ai_model || user.provider_settings.model)) ||
        (user && user.custom_model) ||
      ...
      if (!result.ok) return { ok: false, error: result.error, code: 'provider_error' };
      return { ok: true, text: result.text };
    };
    ...
    if (aiEvaluator && typeof aiEvaluator.evaluateWithPrimary === 'function') {
      const routed = await aiEvaluator.evaluateWithPrimary(primary, effectivePrompt, {
        ...
        model: (user && user.custom_model) || env.GEMINI_MODEL,
        // The primary closure already carries the cap directly to the provider;
        // `extra` repeats it in the fallback body so the VPS router applies the
        // same ceiling. A router that ignores the field simply behaves as before.
        ...(answerTokenCap ? { extra: { maxOutputTokens: answerTokenCap } } : {})
      });
      if (!routed.ok) return routed.error || 'Opini AI sedang tidak tersedia.';
      return routed.text;
    }
    // Evaluator unavailable (module load failure): use the provider directly.
  ...
  async function handleMarket(ctx, parsed) {
    ...
    if (command === 'scan' || command === 'screener') {
      ...
      // Market-wide AI opinion, grounded on the local screener snapshot. Unlike
      // /tanya (free-form question) this always answers the same standing prompt.
      finalText = await runProgress(ctx, async () => {
        const snapshot = opts.loadScreener ? opts.loadScreener() : loadScreener(roots.screener);
        const top = (snapshot.daytrade || []).slice(0, 5).map((row) => ({
          ticker: row.ticker || row.symbol,
          // Unified Scoring (Fase 3): same precedence as formatScanCard so the
          // AI opinion is grounded on the number the user sees on the web.
      ...
      // marketAnalysis:true so the unified-score / plan / bandarmologi rules in
      // MARKET_ANALYSIS_SYSTEM apply here too — /tanya is the command users ask
      // "skor saham ini berapa" through, so it needs the same grounding and the
      // same length ceiling as /analisa and /opini.
      finalText = await runProgress(ctx, async () => {
        const injection = marketContext.buildInjectionWithRoots(rootDir, question, { screenerPath: roots.screener });
        const prompt = injection.context
          ? injection.context + '\n\nPERTANYAAN: ' + question
