...
const GROUP_RESULT_TTL_MS = 3 * 60 * 1000;
...
const ADMIN_COMMANDS = new Set(['bersihkan']);
...
function buildMarketContext(roots, ticker, asOfDate) {
  ...
  return {
    ticker,
    asOf: asOfDate || (selected.length ? selected[selected.length - 1].date : null),
    candles30d: candles.slice(-30),
    technical: technicalSnapshot(selected),
    broker: summarizeFrames(days, [1, 3, 7]),
...
function formatBrokerCard(context) {
  ...
  const lines = ['<b>' + escapeHtml(context.ticker) + '</b> broker summary'];
  for (const frame of [1, 3, 7]) {
    const row = frames[frame];
    lines.push(
      frame + 'D (' + row.sessions + ' sesi): net ' + formatIdr(row.net) +
...
function createInteractiveBot(options) {
  ...
  async function requireApproved(ctx) {
    ...
    if (isGroupChat(ctx.chat)) {
      ...
      const url = 'https://t.me/' + verifyUsername + '?start=auth_' + from.id + '_' + token;
...
// Clean /start response: natural language, no asterisks, no dash bullets, no semicolons.
  async function handleStart(ctx) {
    ...
    await ctx.reply(
      ...
      '/analisa <TICKER> [TANGGAL]  — analisa chart + flow\n' +
      '/broksum <TICKER> [TANGGAL]  — rangkuman broker asing\n' +
      '/insider <TICKER>           — jaringan insider\n' +
      '/scan daytrade|swing|top5   — scan screener lokal\n' +
      '/tanya <pertanyaan>         — tanya soal saham\n\n' +
      'Hasil analisa di grup akan dihapus otomatis setelah 3 menit demi privasi.'
    );
  }
  ...
  async function runProgress(ctx, producer) {
...
// Accepts an optional { marketAnalysis: bool } to inject the clean trader style prompt.
  ...
  async function handleMarket(ctx, parsed) {
    ...
    if (command === 'scan') {
      ...
      await runProgress(ctx, async () => formatScanCard(snapshot, mode));
      return;
    }
    if (command === 'tanya') {
      ...
      await runProgress(ctx, async () => callByok(access.user, question));
      return;
    }
    const ticker = normalizeTicker(parsed.args[0]);
    if (!ticker) {
      await ctx.reply('Ticker tidak valid.');
      return;
    }
    const asOf = /^\d{4}-\d{2}-\d{2}$/.test(parsed.args[1] || '') ? parsed.args[1] : null;
    const context = buildMarketContext(roots, ticker, asOf);
    if (command === 'broksum' || command === 'bs') {
      await runProgress(ctx, async () => formatBrokerCard(context));
      return;
    }
    if (command === 'insider' || command === 'in') {
      await runProgress(ctx, async () => formatInsiderCard(context));
      return;
    }
    ...
    await runProgress(ctx, async () => {
      ...
      return formatBrokerCard(context) + '\n\n' + narrative + '\n\nChart sekali pakai (3 menit): ' + link;
    });
  }
  ...
  async function handleUpdate(ctx) {
    if (ctx.callbackQuery) return handleApproval(ctx);
