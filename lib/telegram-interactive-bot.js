...
const MARKET_COMMANDS = new Set(['analisa', 'a', 'broksum', 'bs', 'insider', 'in', 'scan', 'tanya', 'foreign', 'ritel']);
...
// Admin quota control: /limit <telegram_id> <amount> or inline buttons
  ...
  async function handleStart(ctx) {
    ...
    await ctx.reply(
      ...
      'Status Akun & BYOK\n' +
      'BYOK menjaga privasi data dan query personal tanpa limit platform.\n' +
      'Status: ' + status + '\n' +
      'BYOK: ' + provider + '\n' +
      ...
      '/foreign — Top 10 foreign flow\n' +
      '/ritel — Retail flow tracker\n' +
...
// Accepts an optional { marketAnalysis: bool } to inject the clean trader style prompt.
  ...
  async function handleFlow(ctx, mode) {
    const access = await requireApproved(ctx);
    if (!access.ok) return;
    const flow = aggregateFlow(roots, mode, 1, clock());
    const sorted = [...flow.rows].sort((a, b) => mode === 'foreign'
      ? b.net - a.net
      : b.net - a.net
    );
    ...
    const narrative = await callByok(access.user, JSON.stringify({
      ...
      flowSummary: { buy: buyRows.slice(0, 3), sell: sellRows.slice(0, 3), mode }
    }));
    const buyTitle = mode === 'foreign' ? 'Top 10 Net Foreign Buy' : 'Top 10 Saham Akumulasi Ritel';
    const sellTitle = mode === 'foreign' ? 'Top 10 Net Foreign Sell' : 'Top 10 Saham Distribusi Ritel';
    let text = flowTitle(mode, 1) + '\n\n';
    text += formatRank(buyRows, buyTitle) + '\n\n' + formatRank(sellRows, sellTitle) + '\n\n' + narrative;
  ...
  async function handleFlowCallback(ctx) {
    ...
    await ctx.answerCbQuery('Rentang ' + range + 'D');
    const flow = aggregateFlow(roots, mode, range, clock());
    const sorted = [...flow.rows].sort((a, b) => b.net - a.net);
    const buyRows = sorted.filter((r) => r.net > 0).slice(0, 10);
    const sellRows = sorted.filter((r) => r.net < 0).slice(-10).reverse();
    const narrative = await callByok(access.user, JSON.stringify({
      question: 'Opini objektif akumulasi ' + mode + ' untuk ' + range + 'D. Support, resisten, manajemen risiko.',
      flowSummary: { buy: buyRows.slice(0, 3), sell: sellRows.slice(0, 3), mode, range }
    }));
    const buyTitle = mode === 'foreign' ? 'Top 10 Net Foreign Buy' : 'Top 10 Saham Akumulasi Ritel';
    const sellTitle = mode === 'foreign' ? 'Top 10 Net Foreign Sell' : 'Top 10 Saham Distribusi Ritel';
    let text = flowTitle(mode, range) + '\n\n';
    text += formatRank(buyRows, buyTitle) + '\n\n' + formatRank(sellRows, sellTitle) + '\n\n' + narrative;
