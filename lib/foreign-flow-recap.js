...
async function sendForeignFlowRecap(options = {}) {
  ...
  // F-077: the notifier exports sendTelegramMessage, not sendMessage — the old
  // name threw a latent TypeError on the send path. sendTelegramMessage accepts
  // parse_mode, so HTML tags in the message still render.
  const result = await telegramNotifier.sendTelegramMessage(message, {
    parse_mode: 'HTML',
