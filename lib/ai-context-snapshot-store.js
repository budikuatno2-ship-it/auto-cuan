...
function tickerOf(value) {
  ...
  // ponytail: support index symbols like ^JKSE in addition to 3-5 letter tickers
  return /^(\^[A-Z]{4}|IHSG|[A-Z]{3,5})$/.test(ticker) ? ticker : '';
}
...
function sanitizePortfolioContext(raw) {
  ...
  // Per-ticker capture metadata. Without it the model is handed a bare number
  // and cannot tell a quote taken a minute ago from one taken last week, so it
  // cannot honour "say so when the stored price is stale or missing".
  const priceMeta = {};
  const sourceMeta = input.price_meta && typeof input.price_meta === 'object' && !Array.isArray(input.price_meta)
    ? input.price_meta
    : {};
  Object.keys(sourceMeta).slice(0, 80).forEach((key) => {
    const ticker = tickerOf(key);
    const row = sourceMeta[key];
    if (!ticker || !row || typeof row !== 'object') return;
    const stale = typeof row.provider_marked_stale === 'boolean'
      ? row.provider_marked_stale
      : (typeof row.stale === 'boolean' ? row.stale : null);
    priceMeta[ticker] = {
      at: clean(row.at || row.iso, 40) || null,
      age_minutes: finite(row.age_minutes),
      provider_age_hours: finite(row.provider_age_hours),
      provider_marked_stale: stale,
      price_date: clean(row.price_date, 20) || null
    };
  });
  ...
  // `summary` must count toward "has content" too: it is already sanitized
  // above (capped to 30 keys, values clamped) by this point, but a request
  // carrying only `summary` (no plans/journal/changes/budget) used to fail
  // this check and fall through to hydrateContext()'s raw-`incoming` fallback
  // below, which bypasses this function's size/length caps entirely and lets
  // an arbitrary-size, unsanitized `summary` object reach the prompt.
  // ponytail: preserve empty plans array to prevent db resurrection
  if (!Array.isArray(input.plans) && !plans.length && !journal.length && !changes.length && !budget && !Object.keys(summary).length) return null;
  return {
    plans,
    prices,
...
// The read path runs precisely when sanitizeContext() returned null, so the
// sanitized context that contextKey() expects does not exist yet. Derive the
// same key from the raw request instead, applying the identical ticker rules.
// Returns '' when the request carries no usable key.
function incomingContextKey(source, raw) {
  if (source !== 'stock_analysis_followup') return 'portfolio';
  const input = raw && typeof raw === 'object' ? raw : {};
  return tickerOf(input.ticker);
}
...
async function hydrateContext(req, source, incoming) {
  ...
  // Rows are written keyed on (user_id, source, context_key). Reading on
  // (user_id, source) alone and taking the newest row hydrated whichever ticker
  // the user analysed most recently, not the one this request is about. When the
  // key cannot be derived we hydrate nothing rather than guess a stock.
  const readKey = incomingContextKey(source, incoming);
  if (!readKey) return incoming || {};
