...
function recordCacheHit() {
  _stats.cacheHits++;
  // ponytail: ensure totalRequests >= cacheHits so cacheHitRate stays <= 1.0
  if (_stats.totalRequests < _stats.cacheHits) {
    _stats.totalRequests = _stats.cacheHits;
  }
}
...
function getAiTelemetryStats() {
  ...
  const cacheHitRate = _stats.totalRequests > 0
    ? Math.min(1, Math.round((_stats.cacheHits / _stats.totalRequests) * 10000) / 10000)
    : 0;
