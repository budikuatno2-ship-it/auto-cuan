...
var _geminiProvider = require('../lib/ai-gemini-provider');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const GEMINI_API_KEY = (process.env.API_KEY_ANALISA_SAHAM_PORTOFOLIO || process.env.GEMINI_API_KEY || '').trim();
    ...
    if (source === 'chat_mode' && chatMessage) {
      ...
      if (detectedIHSG && intent !== 'follow_up_question' && intent !== 'casual_chat' && intent !== 'contextual_data_followup') {
        ...
        if (!ihsgData || !ihsgData.last) {
          // Only a level the user actually STATED — see extractStatedPrice. A year
          // in "IHSG proyeksi 2026" is not one, and treating it as one also
          // skipped the authoritative server quote below.
          var ihsgStatedLevel = extractStatedPrice(chatMessage, 'IHSG');
          if (ihsgStatedLevel !== null) {
            ihsgData = { last: ihsgStatedLevel, priceChange1D: null, volumeVsAvg20: null, rsi14: null, ma20: null, ma50: null, ma100: null, ma200: null, high: null, low: null, resistance1: null, resistance2: null, support1: null, support2: null, pivotPoint: null, fib382: null, fib500: null, fib618: null, fib786: null };
            ihsgQuoteSource = 'message_stated_price';
          }
        }
      ...
      if (detectedStockTicker) {
        ...
        if (!stockData || !stockData.last) {
          // Only a price the user actually STATED. An unlabelled number is not
          // one — see extractStatedPrice. When nothing qualifies this stays null
          // so the authoritative server quote below is fetched.
          var extractedPrice = extractStatedPrice(chatMessage, detectedStockTicker);
          if (extractedPrice && extractedPrice > 0) {
            ...
            stockQuoteSource = 'message_stated_price';
          }
        }
        ...
        // Final fallback: the price carried over from the PREVIOUS analysis.
        // It is kept last on purpose — it belongs to whatever was analysed
        // before, so it is only better than having no price at all. It used to
        // be consulted before the server quote, which meant a stale price could
        // pre-empt a live one.
        if (!stockData || !stockData.last) {
          var carriedPrice = body.context && body.context.currentPrice != null
            ? parseFloat(String(body.context.currentPrice).replace(/[^0-9.]/g, '')) : NaN;
          if (isFinite(carriedPrice) && carriedPrice > 0) {
            stockData = { last: carriedPrice, priceChange1D: null, volumeVsAvg20: null, rsi14: null, ma20: null, ma50: null, ma100: null, ma200: null, high: null, low: null, resistance1: null, resistance2: null, support1: null, support2: null, pivotPoint: null };
            stockQuoteSource = 'context_carried_price';
          }
        }
        if (stockData && stockData.last) {
      ...
      if (intent === 'ticker_only') {
        // F-064: routeIntent reaches ticker_only only for a bare ticker, but it
        // strips [Info: ...] blocks internally while this response used the RAW
        // chatMessage (unescaped) — reflecting user HTML. Strip the same blocks
        // and escape before interpolation so no markup is ever rendered.
        var tickerOnlyDisplay = escapeHtmlText(stripEnrichmentBlocks(chatMessage).trim().toUpperCase());
        return res.status(200).json({
          html: '<p class="text-sm text-gray-300"><strong class="text-emerald-400">' + tickerOnlyDisplay + '</strong> terdeteksi. Harga sekarang berapa? Contoh: "WMUU 58"</p>',
          intent: intent
        });
      }

      if (intent === 'follow_up_question') {
        prompt = 'Kamu Auto-Cuan AI. User tanya follow-up singkat tentang saham/indeks yang sudah dianalisis. Jawab LANGSUNG dan SELESAI dalam 2-5 kalimat saja. Gunakan data dari konteks. WAJIB selesaikan kalimat terakhir, jangan terpotong. Format: HTML <p class="text-sm text-gray-300"> saja. Boleh <strong> untuk angka penting. JANGAN ulangi full analysis. JANGAN tampilkan Kesimpulan Cepat, decision-card, metric-grid, atau template apapun. Bahasa Indonesia santai, natural, to the point.';
        if (body.context && body.context.ticker) {
          prompt += '\n\nKonteks: ' + body.context.ticker;
          ...
          if (body.context.support) prompt += ' | Support: ' + (Array.isArray(body.context.support) ? body.context.support.join(', ') : body.context.support);
          if (body.context.resistance) prompt += ' | Resistance: ' + (Array.isArray(body.context.resistance) ? body.context.resistance.join(', ') : body.context.resistance);
        }
        ...
        if (body.context) {
          var c = body.context;
          if (c.ticker) techContext += c.ticker;
          if (c.currentPrice) techContext += ' Rp' + c.currentPrice;
          if (c.finalDecision) techContext += ' | ' + c.finalDecision;
          if (c.entryArea) techContext += ' | Entry: ' + c.entryArea;
          if (c.stopLoss) techContext += ' | SL: ' + c.stopLoss;
          if (c.takeProfits) techContext += ' | TP: ' + c.takeProfits;
          if (c.support) techContext += ' | S: ' + (Array.isArray(c.support) ? c.support.join(', ') : c.support);
          if (c.resistance) techContext += ' | R: ' + (Array.isArray(c.resistance) ? c.resistance.join(', ') : c.resistance);
    ...
    if (ticker && currentPrice) {
      ...
      return res.status(200).json({ html: sanitizeOutput(tHtml, fcaConfirmed, 'ticker_price_basic'), intent: 'ticker_price_basic', provider: tProvider });
    }
    ...
    return res.status(500).json({ success: false, error: 'Terjadi kesalahan saat memproses analisis.', html: '<p class="text-sm text-red-400">Terjadi kesalahan. Coba kirim ulang beberapa saat lagi.</p>' });
  }
};

// Bounded upstream call. callGemini/callDeepSeek/fetchYahooChartOnce already
// capped themselves; the image and casual-chat handlers below did not, so a
// stalled provider held this request open until the platform's own 60s cap for
// this route (see vercel.json "api/analyze.js".maxDuration) killed it. When that
// happens the browser receives the platform's HTML error page instead of JSON,
// and the analysis surfaces as a generic failure with no reason attached.
//
// The chart path issues TWO of these back to back (DeepSeek image, then Gemini
// Vision fallback), so their individual cap has to leave room for both inside
// the same 60s budget.
...
// Single provider call (one upstream round trip in the request).
var UPSTREAM_TIMEOUT_MS = 20000;
// Per-call cap where two calls run sequentially in one request (22s + 22s plus
// I/O overhead stays inside the route's 60s ceiling).
var UPSTREAM_TIMEOUT_CHAINED_MS = 22000;

async function callGemini(apiKey, systemPrompt, userMessage, maxTokens) {
  var geminiModel = _geminiProvider.sanitizeGeminiModel(process.env.GEMINI_MODEL, _geminiProvider.DEFAULT_GEMINI_MODEL);
...
async function handleChartVision(apiKey, images, singleImage, mimeType, userMessage) {
  ...
  try {
    var response = await fetchWithTimeout(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }, UPSTREAM_TIMEOUT_CHAINED_MS);
    // F-062: return null on failure so the caller does not mislabel a failed
    // Gemini Vision call as a successful 'gemini-vision' response (mirrors
    // handleChartDeepSeek). The caller owns the user-facing failure copy.
    if (!response.ok) return null;
    var result = await response.json();
    var candidates = result.candidates || [];
    if (candidates.length > 0 && candidates[0].content && candidates[0].content.parts && candidates[0].content.parts[0]) {
...
// Minimal HTML escaper for the one place we render user text (F-064). Dynamic
// AI answers already pass through sanitizeOutput; this is only for echoed input.
function escapeHtmlText(str) {
  // Entities are built from char codes so the source stays literal-safe.
  var AMP = '&' + 'amp;';
  var LT = '&' + 'lt;';
  var GT = '&' + 'gt;';
  var QUOT = '&' + 'quot;';
...
// === INTENT ROUTER ===
// stripEnrichmentBlocks() below (the shared helper) removes the [Info: ...] and
// [Auto-Cuan ...] blocks the frontend appends BEFORE keyword analysis OR echo.
function routeIntent(message, context) {
  var msg = stripEnrichmentBlocks(message).trim();
...
// === STATED-PRICE EXTRACTION =================================================
// Which number in a chat message is the stock's price.
//
// This replaces a single rule that was used for both stocks and IHSG:
//     chatMessage.match(/\b(\d{2,6})\b/)          // stocks
//     chatMessage.match(/\b(\d{4,5}(?:\.\d+)?)\b/) // IHSG
// The FIRST 2-6 digit number anywhere in the message became the price, so
// "beli 100 lot BBCA harga sekarang 9250" priced BBCA at 100, and
// "IHSG proyeksi 2026" put the index at 2026.
//
// The damage was not only the printed number. The server fetches its own
// authoritative quote ONLY when it still has no price, so a scraped lot count
// satisfied that check and skipped the one source that would have returned a
// correct price together with MA/RSI/support/resistance. A recoverable missing
// quote turned into a wrong, half-empty card.
//
// The browser already solves this in detectPrice() (public/index.html:5199):
// take a price that is LABELLED as one, or a ticker immediately followed by a
// number. This mirrors those rules. When nothing qualifies it returns null —
// which is the useful answer, because null is what makes the caller go and
// fetch the real quote.
function stripEnrichmentBlocks(message) {
  // [Info: ...] and [Auto-Cuan ...] are appended by the browser. Numbers inside
  // them (a company founding year, a volume figure) are never a stated price.
  return String(message == null ? '' : message)
    .replace(/\[Info:[^\]]*\]/gi, ' ')
    .replace(/\[Auto-Cuan[\s\S]*$/i, ' ');
}

function cleanStatedPrice(raw) {
  var text = String(raw == null ? '' : raw).trim();
  // A group of exactly three digits after a separator is a thousands group
  // ("9.250", "9,250"); anything else after a separator is a decimal part
  // ("7850.25", Indonesian "7.850,25"). Dropping that distinction turned
  // "7.850,25" into 785025.
  var cleaned = text.replace(/[.,](\d{3})(?![0-9])/g, '$1');
  cleaned = cleaned.replace(/,/g, '.').replace(/[^0-9.]/g, '');
  // Keep only the first decimal point if several survived.
  var firstDot = cleaned.indexOf('.');
  if (firstDot >= 0) {
    cleaned = cleaned.slice(0, firstDot + 1) + cleaned.slice(firstDot + 1).replace(/\./g, '');
  }
  var price = parseFloat(cleaned);
  if (!isFinite(price) || price <= 0 || price > 999999) return null;
  return price;
}

function extractStatedPrice(message, ticker) {
  var msg = stripEnrichmentBlocks(message);
  if (!msg.trim()) return null;
  var lower = msg.toLowerCase();
  var m;

  // 1. Explicitly labelled price, most specific label first.
  var labelled = [
    /harga\s*sekarang\s*(?:tuh|itu|nya|adalah|di|=|:)?\s*(?:rp\.?\s*)?(\d[\d.,]*)/i,
    /harganya\s*(?:tuh|itu|adalah|di|=|:)?\s*(?:rp\.?\s*)?(\d[\d.,]*)/i,
    /di\s*harga\s*(?:rp\.?\s*)?(\d[\d.,]*)/i,
    /(?:^|[^a-z])harga\s*(?:tuh|itu|nya|adalah|=|:)?\s*(?:rp\.?\s*)?(\d[\d.,]*)/i,
    /sekarang\s*(?:tuh|itu|di|=|:)?\s*(?:rp\.?\s*)?(\d[\d.,]*)/i,
    /current\s*price\s*(?:is|=|:)?\s*(?:rp\.?\s*)?(\d[\d.,]*)/i
  ];
  for (var i = 0; i < labelled.length; i++) {
    m = lower.match(labelled[i]);
    if (m) {
      var byLabel = cleanStatedPrice(m[1]);
      if (byLabel !== null) return byLabel;
    }
  }

  // 2. The ticker immediately followed by a number ("BBCA 9250", "IHSG 7850").
  var symbol = String(ticker == null ? '' : ticker).trim().toUpperCase();
  if (/^[A-Z]{3,5}$/.test(symbol)) {
    m = msg.match(new RegExp('\\b' + symbol + '\\s+(?:rp\\.?\\s*)?(\\d[\\d.,]*)\\b', 'i'));
    if (m) {
      var byTicker = cleanStatedPrice(m[1]);
      if (byTicker !== null) return byTicker;
    }
  }

  // 3. Last resort: the message states exactly ONE number and nothing else that
  //    could be mistaken for it. Two or more candidates with no label is an
  //    ambiguity the server must not resolve by guessing — it returns null so
  //    the authoritative quote is fetched instead.
  var numbers = msg.match(/\d[\d.,]*/g) || [];
  if (numbers.length === 1 && /^[\s\d.,]*$/.test(msg.replace(new RegExp(symbol, 'gi'), '').replace(/[^\w\s.,]/g, ''))) {
    return cleanStatedPrice(numbers[0]);
  }
  return null;
}
...
function buildIHSGFixedTemplate(d, rawMsg) {
  ...
  var last = d.last;
  var changePct = d.priceChange1D != null ? d.priceChange1D : null;
  var volRatio = d.volumeVsAvg20 != null ? d.volumeVsAvg20 : null;
  var rsi14 = d.rsi14 != null ? d.rsi14 : null;
  var ma20 = d.ma20 || '\u2014';
  var ma50 = d.ma50 || '\u2014';
  var ma100 = d.ma100 || '\u2014';
  var ma200 = d.ma200 || '\u2014';
  ...
  var isDataIncomplete = (changePct == null || rsi14 == null);

  // Determine status/bias/confidence/sikap from data
  var statusMarket, biasIHSG, confidence, sikap;
  if (isDataIncomplete) {
    statusMarket = 'Data Belum Lengkap';
    biasIHSG = 'Neutral';
    confidence = 'Low';
    sikap = 'Wait';
  } else if (changePct <= -1.5 && rsi14 < 40) { statusMarket = 'Risk-Off'; biasIHSG = 'Bearish'; confidence = 'High'; sikap = 'Defensive'; }
  ...
  // RSI condition
  var rsiCondition = rsi14 == null ? 'data tidak tersedia' : (rsi14 > 70 ? 'overbought' : rsi14 > 55 ? 'bullish momentum' : rsi14 > 45 ? 'netral' : rsi14 > 30 ? 'bearish' : 'oversold');
  ...
  // Market pressure
  var marketPressure = changePct == null ? 'data perubahan harga tidak tersedia' : (changePct < -1 ? 'tekanan jual dominan' : changePct < 0 ? 'tekanan jual ringan' : changePct > 1 ? 'tekanan beli dominan' : changePct > 0 ? 'tekanan beli ringan' : 'keseimbangan sementara');

  // Direction
  var arahUtama = isDataIncomplete
    ? 'Arah belum dapat dianalisis karena data teknikal belum lengkap'
    : (changePct >= 0.5 ? 'Potensi lanjut naik jika momentum terjaga' : changePct <= -0.5 ? 'Potensi tekanan lanjut jika support tidak bertahan' : 'Sideways, menunggu katalis arah');
  ...
  // Risk Guard
  var riskGuard = isDataIncomplete
    ? 'Data teknikal belum lengkap untuk evaluasi risiko otomatis. Pantau level support/resistance dan gunakan konfirmasi live.'
    : (rsi14 < 30 ? 'RSI14 sudah oversold (' + rsi14 + '), potensi technical rebound ada tapi butuh konfirmasi volume.' : changePct <= -2 ? 'Penurunan signifikan (' + changePct + '%), waspada tekanan lanjutan jika support ' + s1 + ' tidak bertahan.' : 'Kondisi teknikal ' + rsiCondition + ', pantau level ' + s1 + ' sebagai acuan risiko.');
  ...
  var changeSummary = changePct != null
    ? (changePct >= 0 ? 'naik' : (changePct <= -3 ? 'anjlok' : 'turun')) + ' ' + String(Math.abs(changePct).toFixed(2)).replace('.', ',') + '%'
    : 'perubahan harga tidak tersedia';
  var volSummary = volRatio != null
    ? 'dengan volume ' + String(volRatio).replace('.', ',') + 'x rata-rata'
    : 'volume vs rata-rata tidak tersedia';
  var rsiSummary = rsi14 != null
    ? 'RSI 14 menyentuh ' + String(rsi14).replace('.', ',') + ' (' + rsiCondition + ')'
    : 'RSI 14 data tidak tersedia';

  html += 'IHSG ditutup ' + last + ', ' + changeSummary + ' ' + volSummary + '. ';
  html += 'Harga ' + maPosition + ' dan ' + rsiSummary + ', menunjukkan ' + marketPressure + '.</p>';
  ...
  html += '<div>RSI14: ' + (rsi14 != null ? rsi14 : '\u2014') + '</div>';
  html += '<div>Volume: ' + (volRatio != null ? volRatio + 'x avg20' : '\u2014') + '</div>';
...
function buildStockFixedTemplate(d, ticker, rawMsg) {
  ...
  var ma100 = d.ma100;
  var ma200 = d.ma200;
  var high = d.high != null ? d.high : '\u2014';
  var low = d.low != null ? d.low : '\u2014';
  ...
  if (isDataIncomplete) {
    ...
    bias = 'Neutral';
    confidence = 'Low';
    action = 'Pantau data teknikal live';
  } else if (changePct <= -5 && ma20 != null && ma50 != null && last < ma20 && last < ma50) {
  ...
  var rsiLabel = rsi14 == null ? 'data tidak tersedia' : (rsi14 > 70 ? 'overbought' : rsi14 > 55 ? 'bullish momentum' : rsi14 > 45 ? 'netral' : rsi14 > 30 ? 'bearish/lemah' : 'oversold');
  var maPosition = '\u2014';
  ...
  var changeText = changePct == null
    ? 'perubahan harga belum tersedia'
    : (changePct >= 0 ? 'naik ' + pct(changePct) : (changePct <= -5 ? 'anjlok ' : 'turun ') + pct(changePct));
  var pressureSummary = changePct == null
    ? 'Data teknikal belum lengkap untuk membaca tekanan harga'
    : (changePct <= -3 ? 'Tekanan jual masih dominan, belum ada sinyal pemulihan' : changePct <= -1 ? 'Tekanan jual terlihat, perlu konfirmasi support' : changePct >= 2 ? 'Tekanan beli dominan, momentum positif' : 'Belum ada tekanan dominan yang jelas');
  ...
  if (isDataIncomplete) {
    setupAction = 'Data Belum Lengkap';
    setupReason = 'Data teknikal belum lengkap (RSI/volume/perubahan harga absen)';
    setupValid = 'Data teknikal live terkonfirmasi';
    setupCancel = 'Data tetap tidak tersedia atau konfirmasi breakdown';
  } else if (bias === 'Bearish') {
    setupAction = 'Avoid Dulu'; setupReason = 'Momentum negatif, harga ' + maPosition.charAt(0).toLowerCase() + maPosition.slice(1) + ', RSI ' + rsiLabel; setupValid = 'Harga reclaim ' + v(r1) + ' dengan volume di atas rata-rata'; setupCancel = 'Breakdown di bawah ' + v(s1) + ' menuju ' + v(s2);
  } else if (bias === 'Bullish') {
    setupAction = 'Entry saat konfirmasi'; setupReason = 'Momentum positif, harga ' + maPosition.charAt(0).toLowerCase() + maPosition.slice(1) + ', RSI ' + rsiLabel; setupValid = 'Harga bertahan di atas ' + v(ma20) + ' dan volume konsisten'; setupCancel = 'Harga breakdown ' + v(s1) + ' atau volume menurun';
  } else {
    setupAction = 'Watchlist'; setupReason = 'Kondisi netral, menunggu konfirmasi arah'; setupValid = 'Breakout ' + v(r1) + ' atau reclaim MA20 dengan volume'; setupCancel = 'Breakdown ' + v(s1) + ' dengan volume jual';
  }
  ...
  if (isDataIncomplete) {
    entryPlan = 'Tunggu kelengkapan data teknikal sebelum menyusun entry.';
    stopLoss = s1 ? 'Di bawah ' + v(s1) : '\u2014';
    tp1 = v(r1);
    tp2 = v(r2);
    catatan = 'Data teknikal belum lengkap. Keputusan trading tidak diambil dari data fiktif.';
  } else if (bias === 'Bearish') {
    entryPlan = 'Tunggu konfirmasi rebound di atas ' + v(reclaimLevel) + ' dengan volume.'; stopLoss = s1 ? 'Di bawah ' + v(s1) : '\u2014'; tp1 = v(r1); tp2 = v(r2); catatan = 'Hindari beli saat tekanan jual masih dominan dan RSI ' + rsiLabel + ' tanpa konfirmasi.';
  } else if (bias === 'Bullish') {
    entryPlan = 'Area ' + v(s1) + '\u2013' + v(ma20) + ' jika ada konfirmasi volume.'; stopLoss = s1 ? 'Di bawah ' + v(s1) : '\u2014'; tp1 = v(r1); tp2 = v(r2); catatan = 'Entry valid jika volume meningkat dan harga bertahan di atas support.';
  } else {
    entryPlan = 'Tunggu konfirmasi breakout ' + v(r1) + ' atau rebound dari ' + v(s1) + '.'; stopLoss = s1 ? 'Di bawah ' + v(s1) : '\u2014'; tp1 = v(r1); tp2 = v(r2); catatan = 'Kondisi netral, perlu trigger arah sebelum ambil posisi.';
  }
  // setupValid uses reclaimLevel for consistency with Entry/Konfirmasi
  var setupValidFinal = isDataIncomplete ? setupValid : (bias === 'Bearish' ? 'Harga reclaim ' + v(reclaimLevel) + ' dengan volume di atas rata-rata' : setupValid);
  var riskGuard = isDataIncomplete
    ? 'Data teknikal belum lengkap. Hindari spekulasi dan prioritaskan konfirmasi data live.'
    : (rsi14 < 30 ? 'RSI oversold (' + idn(rsi14) + ') tapi bukan otomatis sinyal beli. Tunggu konfirmasi volume.' : changePct <= -5 ? 'Penurunan sangat besar (' + pct(changePct) + '). Hindari averaging tanpa konfirmasi.' : changePct <= -2 ? 'Tekanan jual kuat. Cut loss jika breakdown ' + v(s1) + '.' : 'Pantau level ' + v(s1) + ' sebagai acuan risiko. Disiplin stop loss.');
  var trendSummary = isDataIncomplete ? 'Belum terkonfirmasi' : (bias === 'Bullish' ? 'Bullish' : bias === 'Bearish' ? 'Bearish' : 'Neutral');
  var momentumSummary = rsi14 == null ? '\u2014' : (rsi14 > 55 ? 'Kuat' : rsi14 > 45 ? 'Netral' : rsi14 > 30 ? 'Lemah' : 'Sangat lemah (oversold)');
  var volumeSummary = volRatio == null ? '\u2014' : (volRatio > 2 ? 'Spike (' + ratio(volRatio) + ' avg)' : volRatio > 1.3 ? 'Di atas rata-rata' : volRatio > 0.7 ? 'Normal' : 'Di bawah rata-rata');
  var fibSummary = hasFib ? fibNearest : '\u2014';
  var riskSummary = changePct == null ? 'Sedang (Data belum lengkap)' : (changePct <= -3 ? 'Tinggi' : changePct <= -1 ? 'Sedang' : 'Rendah-Sedang');
  ...
  var invalidation = isDataIncomplete
    ? 'Analisis tidak valid jika data teknikal tetap tidak tersedia.'
    : (bias === 'Bearish' ? 'Breakdown di bawah ' + v(s1) + ' menuju ' + v(s2) + ' dengan volume jual meningkat.' : 'Setup batal jika harga breakdown ' + v(s1) + ' dengan volume jual signifikan.');
  var finalReason = isDataIncomplete
    ? '<div>Data teknikal belum lengkap. Tidak mengambil keputusan dari data rekaan.</div>'
    : ('<div>1. Trend: harga ' + maPosition.charAt(0).toLowerCase() + maPosition.slice(1) + '</div><div>2. Momentum: RSI ' + idn(rsi14) + ' (' + rsiLabel + ')</div><div>3. Volume: ' + ratio(volRatio) + ' rata-rata</div>' + (hasFib ? '<div>4. Fibonacci: ' + fibNearest + '</div><div>5. Perubahan harga: ' + changeText + '</div>' : '<div>4. Perubahan harga: ' + changeText + '</div>'));
  ...
  // 2. Ringkasan Cepat
  var volText = volRatio != null ? ' dengan volume ' + ratio(volRatio) + ' rata-rata' : ' dengan volume \u2014';
  html += '<p><strong>Ringkasan Cepat</strong><br>' + ticker + ' ' + changeText + volText + '. Harga ' + maPosition.charAt(0).toLowerCase() + maPosition.slice(1) + ' dan RSI ' + rsiLabel + '. ' + pressureSummary + '.</p>';
  // 2b. Potensi Besok
  var arahBesok = isDataIncomplete
    ? 'Arah belum dapat dianalisis karena data teknikal belum lengkap'
    : (bias === 'Bearish' ? 'Potensi tekanan berlanjut jika support ' + v(s1) + ' tidak bertahan' : bias === 'Bullish' ? 'Potensi lanjut naik jika momentum dan volume terjaga' : 'Arah belum jelas, menunggu konfirmasi di level kunci');
  var entryAmanBesok = isDataIncomplete
    ? 'Tunggu konfirmasi data teknikal.'
    : (bias === 'Bearish' ? 'Belum ideal. Tunggu reclaim ' + v(reclaimLevel) + ' dengan volume.' : 'Area ' + v(s1) + '\u2013' + v(ma20 || r1) + ' jika ada konfirmasi volume.');
  ...
  var syaratReboundStock = isDataIncomplete
    ? 'Tunggu kelengkapan data teknikal'
    : (bias === 'Bearish' ? 'Bertahan di atas ' + v(s1) + ' + volume beli meningkat + RSI berbalik naik dari ' + idn(rsi14) : 'Konfirmasi breakout ' + v(r1) + ' atau bertahan di atas ' + v(ma20 || s1));
  html += '<div class="scenario-list"><div><strong>Potensi Besok</strong><br><br>';
  ...
  html += '<div>RSI14: ' + (rsi14 != null ? idn(rsi14) : '\u2014') + ' (' + rsiLabel + ')</div>';
  html += '<div>Volume: ' + (volRatio != null ? ratio(volRatio) + ' avg 20D' : '\u2014') + '</div>';
...
async function handleOrderbook(apiKey, images, singleImage, mimeType, userMessage) {
  ...
  try {
    var response = await fetchWithTimeout(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }, UPSTREAM_TIMEOUT_MS);
...
// Exposed for focused unit tests only (same convention as api/register-user.js
// and api/analyze.js). Nothing in the request path reads these.
module.exports.__test = {
  fetchWithTimeout: fetchWithTimeout,
  UPSTREAM_TIMEOUT_MS: UPSTREAM_TIMEOUT_MS,
  UPSTREAM_TIMEOUT_CHAINED_MS: UPSTREAM_TIMEOUT_CHAINED_MS,
  extractStatedPrice,
  stripEnrichmentBlocks,
  escapeHtmlText,
  cleanStatedPrice,
