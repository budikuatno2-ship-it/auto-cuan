...
function safeReason(error) {
  const text = [error && error.message,error && error.details,error && error.hint].filter(Boolean).join(' ');
  for (const reason of SAFE_REASONS) if (text.includes(reason)) return reason;
  return 'membership_operation_failed';
}
async function rpc(name, args) {
  const { data, error } = await client().rpc(name, args || {});
  if (error) { const e = new Error('membership_operation_failed'); e.code = error.code; e.reason = safeReason(error); throw e; }
  return data;
}
...
async function packages() {
  const { data, error } = await client().from('membership_packages')
    ...
    .eq('active', true).eq('product_type','membership').order('sort_order');
  if (error) { const e=new Error('membership_operation_failed'); e.reason=safeReason(error); throw e; }
  return data;
}
async function packageBySlug(slug) {
  const { data, error } = await client().from('membership_packages')
    .select('id,slug,name,description,duration_days,lifetime,price_idr,product_type')
    .eq('slug', String(slug || '').toLowerCase()).eq('active', true).single();
  if (error) { const e=new Error('package_unavailable'); e.reason='package_unavailable'; throw e; }
  return data;
}
function bankInstructions() {
  const instructions = env.optional('MEMBERSHIP_BANK_INSTRUCTIONS');
  if (!instructions) { const e=new Error('bank_instructions_missing'); e.reason='bank_instructions_missing'; throw e; }
  return instructions;
}
async function createPurchase(telegramId, packageId, voucherCode) {
  ...
  return rpc('membership_create_purchase', {
    ...
    p_voucher_hash: voucherCode ? core.voucherHash(voucherCode, voucherPepper) : null,
    p_bank_instructions: bankInstructions()
  });
}
async function beginCheckout(telegramId, packageSlug) {
  return rpc('membership_begin_checkout', { p_telegram_user_id:String(telegramId), p_package_slug:packageSlug });
}
...
async function checkoutPurchase(telegramId, voucherCode) {
  ...
  return rpc('membership_checkout_purchase', {
    p_telegram_user_id:String(telegramId),
    p_voucher_hash:voucherCode ? core.voucherHash(voucherCode,pepper) : null,
    p_bank_instructions:bankInstructions()
  });
}
async function cancelPurchase(telegramId, purchaseId) {
  return rpc('membership_cancel_purchase', { p_telegram_user_id:String(telegramId), p_purchase_id:purchaseId });
}
async function submitProof(telegramId, purchaseId, metadata) {
  return rpc('membership_admin_bot_submit_payment_proof', { p_telegram_user_id: String(telegramId), p_purchase_id: purchaseId, p_file_id: metadata.fileId, p_file_unique_id: metadata.uniqueId, p_mime_type: metadata.mimeType, p_file_size: metadata.size });
}
...
async function review(purchaseId, approve, reason, adminId, idempotencyKey) {
  return rpc('membership_admin_bot_review_purchase', { p_purchase_id: purchaseId, p_approve: approve, p_reason: reason || null, p_admin_user_id: adminId, p_idempotency_key: idempotencyKey });
}
...
module.exports = { client, rpc, claimUpdate, releaseUpdate, bindAdmin, account, packages, packageBySlug, createPurchase, beginCheckout, checkoutSession, cancelCheckout, checkoutPurchase, cancelPurchase, submitProof, notifyAdminsOfProof, channelGrant, recordChannelInvite, claimChannelJoin, finalizeChannelJoin, releaseChannelJoin, review, pending, proof, audit, enforceExpiry, expiryCandidates, revokeChannelGrant, runChannelExpiryEnforcement };
