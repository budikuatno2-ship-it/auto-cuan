...
function buildLegacyTradePlan(candidate) {
  ...
  var legacyEntryLow = pick(['entry_low', 'entry2', 'entry_2']);
  var legacyEntryHigh = pick(['entry_high', 'entry1', 'entry_1']);
  if (legacyEntryLow != null && legacyEntryHigh != null && legacyEntryLow > legacyEntryHigh) {
    var _tmpL = legacyEntryLow; legacyEntryLow = legacyEntryHigh; legacyEntryHigh = _tmpL;
  }
  return {
    source: 'legacy',
    plan_version: 'legacy',
    ticker: candidate.ticker ? String(candidate.ticker).toUpperCase() : null,
    entry_low: legacyEntryLow,
    entry_high: legacyEntryHigh,
    tp1: pick(['tp1', 'tp1n']),
    tp2: pick(['tp2', 'tp2n']),
...
function isPlanV2Usable(plan) {
  ...
  // Fase 3 / BUG-F3-13: `support` adalah field legacy. Plan V2 yang sah boleh
  // tidak membawa scalar `support` selama anchor struktural (stop_anchor_price
  // atau structural_invalidation) tersedia — tanpa ini plan valid ditolak dan
  // transparansi V2 hilang karena dipaksa fallback ke legacy.
  const hasStructuralAnchor = num(plan.stop_anchor_price) !== null || num(plan.structural_invalidation) !== null;
  for (const f of MANDATORY_PUBLIC_FIELDS) {
    if (f === 'support' && hasStructuralAnchor) continue;
...
// Canonical selection contexts. `public` drives the website + Telegram display
// (governed by TRADE_PLAN_V2_PUBLIC_ENABLED). `intraday_shadow` drives ONLY the
// internal intraday sample collector's plan-lock (governed by the dedicated
// TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED + shadow flags) and is fully decoupled from
// the public rollout — the public flag never affects it.
const SELECTION_MODES = Object.freeze(['public', 'intraday_shadow']);

/** Normalise the requested selection mode (default 'public'). */
function resolveSelectionMode(options) {
  const raw = options && (options.selection_mode || options.selectionMode);
  return raw === 'intraday_shadow' ? 'intraday_shadow' : 'public';
}

/**
 * THE single canonical plan selection every consumer must use. Plan-selection
 * logic lives ONLY here — callers pick the appropriate selection context instead
 * of duplicating it.
 *
 * Two selection contexts (options.selection_mode):
 *
 *   'public' (default) — website + Telegram presentation:
 *     - TRADE_PLAN_V2_PUBLIC_ENABLED false (default) => source 'legacy'
 *       (the unchanged legacy numbers; public output stays byte-identical).
 *     - public flag true + usable V2 plan            => source 'trade_plan_v2'.
 *     - public flag true + missing/rejected/stale/unsafe/below-min-RR V2 =>
 *       source 'legacy_fallback' (a COMPLETE legacy plan — never null SL/TP).
 *     The intraday-lock flag is IGNORED here.
 *
 *   'intraday_shadow' — internal intraday sample collector plan-lock ONLY:
 *     - TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED true AND shadow flag true AND usable
 *       V2 => source 'trade_plan_v2'.
 *     - lock+shadow true + rejected/stale/unsafe/missing/below-min-RR V2 =>
 *       source 'legacy_fallback' (a COMPLETE legacy plan — never null SL/TP).
 *     - intraday-lock flag false (default) => source 'legacy'.
 *     The public flag is IGNORED here, so intraday locking can never leak V2 onto
 *     the website or Telegram, and public V2 can never auto-enable intraday V2.
 *
 * The canonical field contract (CANONICAL_PLAN_FIELDS) and the downstream
 * plan_lock_id are identical across both contexts. `public_v2_enabled` reflects
 * ONLY the true public presentation state (isPublicEnabled) so the intraday lock
 * never signals public V2.
 *
 * @param {object} candidate screener candidate row
 * @param {object} options { mode|screener_type, env, selection_mode, planV2,
 *                           generated_at, ... }
 */
function selectCanonicalTradePlan(candidate, options) {
  options = options || {};
  const env = options.env;
  const selectionMode = resolveSelectionMode(options);
  const screenerType = resolveScreenerType(
  ...
  // Public presentation state is ALWAYS governed solely by the public flag, so
  // the intraday lock can never leak V2 onto the website or Telegram.
  const publicEnabled = flags.isPublicEnabled(env);

  // Decide whether THIS selection context may lock V2 at all.
  let v2SelectionEnabled;
  if (selectionMode === 'intraday_shadow') {
    // Internal intraday plan-lock is decoupled from public rollout: it locks V2
    // only when BOTH the dedicated intraday-lock flag AND the shadow flag are on.
    v2SelectionEnabled = flags.isIntradayLockEnabled(env) && flags.isShadowEnabled(env);
  } else {
    v2SelectionEnabled = publicEnabled;
  }

  if (!v2SelectionEnabled) {
    return canonicalFromLegacy(legacy, 'legacy', publicEnabled, screenerType, null, generatedAt);
  }
  ...
  if (!isPlanV2Usable(planV2)) {
    return canonicalFromLegacy(legacy, 'legacy_fallback', publicEnabled, screenerType, planV2, generatedAt);
  }
  const c = canonicalFromV2(planV2, 'trade_plan_v2', publicEnabled);
 ...
 *
 * The `selection_mode` option is passed straight through to the canonical
 * selector. The intraday sample collector passes 'intraday_shadow' so its lock is
 * governed by TRADE_PLAN_V2_INTRADAY_LOCK_ENABLED + the shadow flag, independent
 * of the public rollout. Callers that omit it get the default 'public' context.
 *
