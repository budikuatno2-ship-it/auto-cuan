...
const { getUserApiKey, resolveAiCredentials } = require('./user-ai-credentials');
const { fetchChartOhlc, buildChartPng, normalizeForeignTicker } = require('./chart-image-renderer');
const { sanitizeGeminiModel, DEFAULT_GEMINI_MODEL, FALLBACK_GEMINI_MODEL } = require('./ai-gemini-provider');

const GEMINI_BASE_URL = 'https://generativelanguage.googleapis.com/v1beta/models';
const PRIMARY_MODEL = sanitizeGeminiModel(process.env.GEMINI_MODEL, DEFAULT_GEMINI_MODEL);
const FALLBACK_MODEL = sanitizeGeminiModel(process.env.GEMINI_FALLBACK_MODEL, FALLBACK_GEMINI_MODEL);
...
async function getAnalysisStatus(req, db, ticker, options = {}) {
  ...
  if ((!access || !access.ok) && auth && auth.session) {
    ...
    access = {
      ...
      user: {
        id: auth.session.uid,
        username: auth.session.un,
        isAdmin: auth.session.adm === true
      },
      premium: isPrem,
      entitlement: {
        premium: isPrem,
        current_plan: isAdm ? 'lifetime' : (isPrem ? 'premium' : 'free'),
        lifetime_state: isAdm ? 'active' : null
      }
    };
  }
  ...
  const [credentials, usage] = await Promise.all([
    resolveAiCredentials(db, userId, access),
  ...
  return {
    ok: true,
    status: 200,
    hasKey: credentials.ok,
    hasPersonalKey: credentials.hasPersonalKey,
    hasAppKey: credentials.hasAppKey,
    effectiveKeySource: credentials.source,
    hasFallbackKey: Boolean(credentials.fallbackKey),
    maskedKey: credentials.maskedPersonalKey,
    needsKey: !credentials.ok,
...
async function runChartAnalysis(req, db, ticker, options = {}) {
  ...
  if (!credentials.ok || !credentials.primaryKey) {
    return {
      ok: false,
      status: 400,
      code: 'API_KEY_REQUIRED',
      error: credentials.isSubscribed
        ? 'API key Google Gemini belum tersedia di sistem. Hubungi admin atau masukkan API key cadangan Anda di pengaturan.'
        : 'API key Google Gemini belum diisi. Masukkan API key Gemini Anda terlebih dahulu di pengaturan.'
    };
  }
  ...
  // 5. Call Gemini Vision API (with automatic failover to personal key if subscribed primary key fails)
  let visionRes = await callGeminiVision(
    credentials.primaryKey,
    systemPrompt,
    safeTicker,
    base64Png,
    PRIMARY_MODEL,
  ...
  if (!visionRes.ok && credentials.fallbackKey && (visionRes.status === 429 || visionRes.status >= 500 || visionRes.code === 'GEMINI_KEY_REJECTED')) {
    const fallbackRes = await callGeminiVision(
      credentials.fallbackKey,
      systemPrompt,
      safeTicker,
      base64Png,
      PRIMARY_MODEL,
...
async function checkUnifiedAiQuota(db, access) {
  const tierInfo = resolveUserTier(access);
  const user = access && access.user;
  const userId = user && (user.id || user.uid || user.username);
  if (!userId) {
    return { ok: false, status: 401, code: 'UNAUTHORIZED', error: 'Sesi login diperlukan.' };
  }
  const wibDate = getWibDateString();
  const usage = await getUserUsage(db, userId, wibDate);
  const isUnlimited = tierInfo.maxDaily === Infinity;
  const remaining = isUnlimited ? Infinity : Math.max(0, tierInfo.maxDaily - usage.count);

  if (!isUnlimited && usage.count >= tierInfo.maxDaily) {
    return {
      ok: false,
      status: 429,
      code: 'QUOTA_EXCEEDED',
      error: `Kuota harian AI Anda (${usage.count}/${tierInfo.maxDaily}) telah habis untuk hari ini (reset 00:00 WIB). Jatah kuota ini dibagi bersama antara Analisis Chart AI, Analisis Saham, dan Portofolio.`,
      quota: {
        tier: tierInfo.tier,
        label: tierInfo.label,
        usedToday: usage.count,
        maxDaily: tierInfo.maxDaily,
        remaining: 0
      }
    };
  }
