...
function buildDbRow(item, sampleDate, scheduledTime) {
  ...
  // F-066: no fabricated score — null when no real score exists; no clamp floor.
  const rawScore = publishScore ?? watchScore ?? engineScore;
  const finalScore = rawScore == null ? null : Math.min(100, Math.round(rawScore));
  const row = {
...
function buildTelegramMessage(item, sampleDate, scheduledTime) {
  ...
  const lines = [
    '🚀 AUTO-CUAN DAY TRADE — SIGNAL TERKONFIRMASI',
    '',
    `Ticker: ${safeText(item.ticker, 5)}`,
    ...
    `Konfirmasi: ${item.ready_streak || 2}/2 minimum — aturan 2 dari 3 snapshot`,
...
async function registerConfirmedPicksForMonitoring(client, items, sampleDate, scheduledTime) {
  if (!client || !Array.isArray(items) || !items.length) {
    return { inserted: 0, skipped: 0 };
  }
  const date = sampleDate || new Date().toISOString().slice(0, 10);
  const nowIso = new Date().toISOString();

  try {
    const { data: existing } = await client
      .from('telegram_daily_picks')
      .select('ticker,monitor_source,plan_lock_id,raw_payload')
      .eq('date', date);

    const existingKeys = new Set();
    for (const r of existing || []) {
      const src = String(r.monitor_source || '').toLowerCase();
      const planId = r.plan_lock_id || '';
      if (r.ticker) {
        const t = String(r.ticker).toUpperCase();
        existingKeys.add(`${t}|${src}|${planId}`);
        const rawTime = r.raw_payload && r.raw_payload.scheduled_time ? String(r.raw_payload.scheduled_time).replace(/[^a-zA-Z0-9]/g, '') : '';
        if (rawTime) {
          existingKeys.add(`${t}|${src}|${rawTime}`);
        } else {
          existingKeys.add(`${t}|${src}`);
        }
      }
    }
    ...
    for (const item of items) {
      ...
      const tp1 = numberOrNull(obs.tp1 ?? metrics.tp1);
      const tp2 = numberOrNull(obs.tp2);
      const lastPrice = numberOrNull(obs.current_price ?? obs.last_price ?? metrics.current_price);
      // F-066: no fabricated score — skip items with no real score at all.
      const rawScore = numberOrNull(item.publish_score) ?? numberOrNull(item.watch_score) ?? numberOrNull(obs.score);
      if (rawScore == null) continue;
      const score = Math.min(100, Math.round(rawScore));

      const sessionTag = scheduledTime ? String(scheduledTime).replace(/[^a-zA-Z0-9]/g, '') : '';
      const basePlanLockId = `fw-${date}-${ticker}-${entryLow || 0}-${entryHigh || 0}-${tp1 || 0}-${stopLoss || 0}`;
      const planLockId = sessionTag
        ? `fw-${date}-${ticker}-${sessionTag}-${entryLow || 0}-${entryHigh || 0}-${tp1 || 0}-${stopLoss || 0}`
        : basePlanLockId;
      const dedupKey = `${ticker}|daytrade_signal|${planLockId}`;
      const baseDedupKey = `${ticker}|daytrade_signal|${basePlanLockId}`;
      const sourceKey = sessionTag ? `${ticker}|daytrade_signal|${sessionTag}` : `${ticker}|daytrade_signal`;

      if (existingKeys.has(dedupKey) || existingKeys.has(baseDedupKey) || existingKeys.has(sourceKey)) {
        continue;
      }
      existingKeys.add(dedupKey);
      existingKeys.add(baseDedupKey);
      existingKeys.add(sourceKey);

      rowsToInsert.push({
        date: date,
        ticker: ticker,
        stock_name: safeText(obs.stock_name || ticker, 120),
        status: 'WAITING',
        is_final: false,
        entry1: entryHigh != null ? entryHigh : entryLow,
        entry2: entryLow != null ? entryLow : entryHigh,
        tp1: tp1,
        tp2: tp2,
        sl: stopLoss,
        score: score,
        ...
        plan_lock_id: planLockId,
        raw_payload: {
          ticker: ticker,
          monitor_source: 'daytrade_signal',
          plan_lock_id: planLockId,
          trade_plan_source: 'fast_watcher_live',
    ...
    if (insErr) {
      return { inserted: 0, skipped: 0, error: insErr.message };
    }

    return { inserted: rowsToInsert.length, skipped: items.length - rowsToInsert.length };
  } catch (e) {
...
async function publishConfirmed(options) {
  ...
  const requestedPublishable = Array.isArray(opts.publishable)
    ? opts.publishable.slice(0, MAX_LIVE_PUBLISH)
    : [];

  if (!requestedPublishable.length) {
    return {
      attempted: false,
      system_published: 0,
      telegram_sent: 0,
      reason: 'nothing_confirmed'
    };
  }

  const productionEligibilityBlocked = [];

  const publishable = requestedPublishable.filter(item => {
    const observation = item && item.observation && typeof item.observation === 'object'
      ? item.observation
      : {};

    const eligibility = productionEligibility.classifyProductionEligibility(observation);

    if (eligibility.eligible) return true;

    productionEligibilityBlocked.push({
      ticker: safeText(item && item.ticker, 5) || null,
      setup_id: safeText(item && item.setup_id, 128) || null,
      reason: eligibility.reason,
      risk_status: eligibility.risk_status
    });
  ...
  if (!publishable.length) {
    return {
      ...
      system_published: 0,
      telegram_sent: 0,
      reason: 'nothing_production_eligible',
      production_eligibility_blocked: productionEligibilityBlocked
    };
  }
  ...
  for (const item of publishable) {
    ...
    if (!setupId) continue;
    const sessionKey = `${setupId}|${currentSession}`;
    const prior = ledger.setups[sessionKey] || ledger.setups[setupId];
    if (prior && prior.system_published_at) {
      const priorSession = prior.session || getMarketSession(prior.scheduled_time);
      // Jika setup sudah pernah dikirim di sesi sebelumnya (misal: Sesi 1) dan sekarang Sesi 2,
      // breakout baru di Sesi 2 tetap diizinkan untuk dipublish & dikirim alert-nya
      if (priorSession !== currentSession) {
        newItems.push(item);
        continue;
      }
  ...
  // F-066: items without a real score must not enter the public ranking table.
  const scoredItems = newItems.filter(item => {
    const obs = item.observation || {};
    const s = numberOrNull(item.publish_score) ?? numberOrNull(item.watch_score) ?? numberOrNull(obs.score ?? obs.daytrade_score);
    return s != null;
  });
  const rows = scoredItems.map(item => buildDbRow(item, sampleDate, scheduledTime));
  const dbResult = rows.length ? await upsertSystemRows(client, rows) : { ok: true, rows: [] };
  if (!dbResult.ok) {
  ...
  for (const item of newItems) {
    ...
    const sessionKey = `${item.setup_id}|${currentSession}`;
    const entry = {
      ticker: item.ticker,
      setup_id: item.setup_id,
    ...
    ledger.setups[item.setup_id] = entry;
    ledger.setups[sessionKey] = entry;
  }
  await writeJsonAtomic(ledgerFile, ledger);

  const telegramEnabled = String(env.FAST_WATCHER_TELEGRAM_ENABLED || '') === '1';
  const seenTelegramTickers = new Set();
  const telegramItems = [];
  for (const it of [...newItems, ...retryTelegram]) {
    const t = String(it && it.ticker || '').toUpperCase();
    if (t && !seenTelegramTickers.has(t)) {
      seenTelegramTickers.add(t);
      telegramItems.push(it);
      if (telegramItems.length >= MAX_LIVE_PUBLISH) break;
    }
  }
  let telegramSent = 0;
  ...
  if (telegramEnabled) {
    ...
    for (const entry of sendResults) {
      ...
      if (result && result.sent) {
        ...
        const sessionKey = `${item.setup_id}|${currentSession}`;
        if (ledger.setups[item.setup_id]) ledger.setups[item.setup_id].telegram_sent_at = sentAt;
        if (ledger.setups[sessionKey]) ledger.setups[sessionKey].telegram_sent_at = sentAt;
      } else {
    ...
    // Register successfully sent signals to telegram_daily_picks for follow-up TP/SL monitoring
    let monitorRegistered = 0;
    if (client && deliveredItems.length > 0) {
      const monitorRes = await registerConfirmedPicksForMonitoring(client, deliveredItems, sampleDate, scheduledTime);
      monitorRegistered = monitorRes.inserted || 0;
    }
    ...
    return {
      ...
      monitor_registered: monitorRegistered,
...
module.exports = {
  ...
  publishConfirmed,
  registerConfirmedPicksForMonitoring,
  getMarketSession,
  readJson,
  writeJsonAtomic
};
