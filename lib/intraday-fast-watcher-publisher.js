...
async function registerConfirmedPicksForMonitoring(client, items, sampleDate, scheduledTime) {
  ...
  try {
    const { data: existing } = await client
      ...
      .select('ticker,monitor_source,plan_lock_id')
      .eq('date', date);
    ...
    for (const r of existing || []) {
      ...
      if (r.ticker) {
        existingKeys.add(`${String(r.ticker).toUpperCase()}|${src}|${planId}`);
        existingKeys.add(`${String(r.ticker).toUpperCase()}|${src}`);
      }
    }
    ...
    for (const item of items) {
      ...
      const planLockId = `fw-${date}-${ticker}-${entryLow || 0}-${entryHigh || 0}-${tp1 || 0}-${stopLoss || 0}`;
      const dedupKey = `${ticker}|daytrade_signal|${planLockId}`;
      const sourceKey = `${ticker}|daytrade_signal`;
