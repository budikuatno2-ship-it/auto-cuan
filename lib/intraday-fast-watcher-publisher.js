...
function buildDbRow(item, sampleDate, scheduledTime) {
  ...
  const row = {
    ...
    setup: safeText(obs.candidate_type || obs.setup || 'FAST_WATCHER_MOMENTUM', 120),
    ...
    time_plan: 'Fast Watcher guarded-live: dua konfirmasi momentum intraday.',
    notes: `FAST_WATCHER_CONFIRMED | watch=${watchScore ?? '-'} | publish=${publishScore ?? '-'} | ${sampleDate} ${scheduledTime} WIB`,
...
function buildTelegramMessage(item, sampleDate, scheduledTime) {
  ...
  const lines = [
    ...
    `Momentum harga: ${priceChange == null ? '-' : `${priceChange >= 0 ? '+' : ''}${priceChange.toFixed(2)}%`}`,
    ...
    '',
    'Status: momentum terkonfirmasi dan entry masih dalam batas aman. Bukan auto-order.'
  ];
  return lines.join('\n');
}
