...
function createLocalEvaluationLogger(options = {}) {
  ...
  let count = 0; let firstTimestamp = null; let lastTimestamp = null; let finalized = false;
  return {
    append(record) {
...
function observeEvaluation(productionValue, options, records) {
  ...
  try {
    const logger = createLocalEvaluationLogger(options);
    for (const record of records || []) logger.append(record);
    logger.finalize();
  } catch (error) {
