...
async function getVoucherAdminCapability(db, env) {
  ...
  try {
    // The version RPC exists only after every required Phase 5C correction is
    // applied. A table-only probe could enable an incomplete database contract.
    const result = await db.rpc('voucher_admin_schema_version');
    if (result && !result.error && result.data === 'phase5c-lifecycle-v3') return { enabled: true, ready: true };
  } catch (_) { /* fail closed */ }
  return { enabled: true, ready: false, reason: 'unavailable' };
}
