...
function evaluateIntradayReadiness(input) {
  ...
  const staleCache = countValue(m.data_quality, 'STALE_CACHE');
  if (staleCache > 0) block.push(`STALE_CACHE count is non-zero: ${staleCache}`);
  const incomplete = countValue(m.data_quality, 'INCOMPLETE_INTRADAY');
  if (incomplete > 0) block.push(`INCOMPLETE_INTRADAY count is non-zero: ${incomplete}`);
...
function reportDate(report, file, prefix) {
  const explicit = asObject(report).date;
  if (explicit) return String(explicit);
  return extractDate(file, prefix);
}
async function loadInputs(opts) {
  ...
  const intraday = await readJsonFile(intradayFile);
  const compare = await readJsonFile(compareFile);
  if (opts.latest) {
    const intradayDate = reportDate(intraday, intradayFile, INTRADAY_PREFIX);
    const compareDate = reportDate(compare, compareFile, COMPARE_PREFIX);
    if (intradayDate !== compareDate) throw new Error(`Latest report date mismatch: intraday=${intradayDate} compare=${compareDate}`);
  }
  return { intraday, compare, sources: { intraday: intradayFile, compare: compareFile } };
}
...
module.exports = { DEFAULT_REPORTS_DIR, INTRADAY_PREFIX, COMPARE_PREFIX, READINESS_PREFIX, evaluateIntradayReadiness, buildIntradayReadinessReport, markdownReport, readJsonFile, latestFile, reportDate, loadInputs, writeReports };
