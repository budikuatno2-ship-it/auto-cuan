...
async function getBrokerHunterData(brokerCode, options = {}) {
  ...
  if (!isCustom) {
    const gitTrackedPath = path.join(GIT_TRACKED_INDEX_DIR, `${code}_${range}.json`);
    const localCachePath = path.join(HUNTER_CACHE_DIR, `${code}_${range}.json`);
    const targetFile = fs.existsSync(gitTrackedPath) ? gitTrackedPath : (fs.existsSync(localCachePath) ? localCachePath : null);
