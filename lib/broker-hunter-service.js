...
/**
 * Resolve the git-tracked index directory PER CALL.
 *
 * This used to be an import-time constant, which meant nothing could redirect it:
 * a test (or a dry run) invoking generateBrokerHunterIndex() silently overwrote the
 * COMMITTED production index files under data/broker-hunter-indexes — verified by
 * mtime bisection, broker-hunter.test.js was rewriting AK-1d.json and friends on
 * every run. An explicit override keeps the generator's production behaviour while
 * letting a caller isolate its writes.
 */
function getGitTrackedIndexDir() {
  return process.env.BROKER_HUNTER_INDEX_DIR || path.join(__dirname, '..', 'data', 'broker-hunter-indexes');
}
const GIT_TRACKED_INDEX_DIR = getGitTrackedIndexDir();
const ARJUM_BASE_DIR = process.env.ARJUM_DATA_DIR || path.join(__dirname, '..', 'data', 'arjum-data');

/**
 * Resolve the hunter cache dir PER CALL for the same reason as the git-tracked
 * index dir: it is derived from ARJUM_DATA_DIR, and an import-time constant meant a
 * caller that repointed ARJUM_DATA_DIR after load kept writing to the real repo path.
 */
function getHunterCacheDir() {
  const base = process.env.ARJUM_DATA_DIR || ARJUM_BASE_DIR;
  return path.join(base, 'broker-hunter');
}
const HUNTER_CACHE_DIR = getHunterCacheDir();
...
function loadUniverseTickers() {
  ...
  try {
    ...
    if (fs.existsSync(sumDir)) {
      return fs.readdirSync(sumDir).filter(isValidIdxTicker);
    }
  } catch (_) {}
...
function extractBrokerTx(summary, brokerCode, ticker) {
  ...
  const buyers = firstNonEmptyArray(summary.gross_buyers, summary.top_buyers, summary.buyers);
  ...
  const sellers = firstNonEmptyArray(summary.gross_sellers, summary.top_sellers, summary.sellers);
...
async function getBrokerHunterData(brokerCode, options = {}) {
  ...
  if (!isCustom && !options.force) {
    // Resolve per call so an ARJUM_DATA_DIR / BROKER_HUNTER_INDEX_DIR override
    // applied after module load is honoured instead of the import-time paths.
    const gitDir = getGitTrackedIndexDir();
    const cacheDir = getHunterCacheDir();
    const candidatePaths = [
      path.join(gitDir, `${code}_${range}.json`),
      path.join(gitDir, `${code}-${range}.json`),
      path.join(cacheDir, `${code}_${range}.json`),
      path.join(cacheDir, `${code}-${range}.json`)
    ];
  ...
  if (targetDates.length === 0) {
    // No static fallback date: keep targetDates empty so the downstream
    // "no data available" branch renders an honest empty result instead of
    // pretending a stale hardcoded session has broker activity.
    targetDates = [];
  }
...
async function generateBrokerHunterIndex(options = {}) {
  ...
  // Resolve per call so an override applied after module load still takes effect.
  const gitTrackedIndexDir = getGitTrackedIndexDir();
  const hunterCacheDir = getHunterCacheDir();
  fs.mkdirSync(hunterCacheDir, { recursive: true });
  fs.mkdirSync(gitTrackedIndexDir, { recursive: true });
  ...
  for (const range of ranges) {
    for (const code of brokerCodes) {
      ...
      const outPath = path.join(hunterCacheDir, `${code}_${range}.json`);
      const gitPath = path.join(gitTrackedIndexDir, `${code}_${range}.json`);
      ...
      const outHyphen = path.join(hunterCacheDir, `${code}-${range}.json`);
      const gitHyphen = path.join(gitTrackedIndexDir, `${code}-${range}.json`);
  ...
  const catalogPath = path.join(gitTrackedIndexDir, 'catalog.json');
  ...
  fs.writeFileSync(path.join(hunterCacheDir, 'catalog.json'), JSON.stringify(catalog, null, 2), 'utf8');
  if (brokerCodes.length >= Object.keys(BROKER_NAMES).length || options.updateGitCatalog) {
    fs.writeFileSync(path.join(gitTrackedIndexDir, 'catalog.json'), JSON.stringify(catalog, null, 2), 'utf8');
  }
...
module.exports = {
  ...
  extractBrokerTx,
  // Live getters: these are derived from env vars that a caller may repoint after
  // module load (see getHunterCacheDir / getGitTrackedIndexDir), so exporting a
  // frozen import-time string would report a path the service no longer uses.
  get HUNTER_CACHE_DIR() { return getHunterCacheDir(); },
  get GIT_TRACKED_INDEX_DIR() { return getGitTrackedIndexDir(); },
  getHunterCacheDir,
  getGitTrackedIndexDir
};
