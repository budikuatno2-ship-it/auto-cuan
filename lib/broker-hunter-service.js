...
async function getBrokerHunterData(brokerCode, options = {}) {
    ...
    try {
      ...
      if (vpsFetcher && typeof vpsFetcher.fetchBrokerHunterFromVpsSync === 'function' && vpsFetcher.hasSshKey()) {
        ...
        if (fetched && Array.isArray(fetched.top_accumulated)) {
          return Object.assign({}, fetched, { success: true, from_cache: true });
        }
      }
    } catch (_) {}
  }
  ...
  for (const ticker of tickers) {
    ...
    if (validTxCount > 0 && (totBval > 0 || totSval > 0)) {
      ...
      let avgBuy = totBvol > 0 ? Math.round(totBval / totBvol) : 0;
      if (avgBuy > 100000 && Math.round(avgBuy / 100) >= 50) {
        avgBuy = Math.round(avgBuy / 100);
      }
      let avgSell = totSvol > 0 ? Math.round(totSval / totSvol) : 0;
      if (avgSell > 100000 && Math.round(avgSell / 100) >= 50) {
        avgSell = Math.round(avgSell / 100);
      }
