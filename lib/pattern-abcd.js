...
function discoverPivots(candles) {
  ...
  for (var i = PIVOT_WINDOW; i < candles.length - PIVOT_WINDOW; i++) {
    ...
    if (isHigh) pivots.push({ index: i, type: 'high', value: high, time: candles[i].time });
    if (isLow) pivots.push({ index: i, type: 'low', value: low, time: candles[i].time });
  }
...
function stableId(ticker, direction, pivots) {
  return ['abcd', ticker, direction].concat(pivots.map(function(p) { return p.time.replace(/-/g, ''); })).join('-');
}
...
function evaluateGroup(candles, pivots, options) {
  ...
  var candidate = {
    id: stableId(options.ticker, direction, pivots), ruleVersion: RULE_VERSION,
