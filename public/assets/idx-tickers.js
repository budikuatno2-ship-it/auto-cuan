// IDX/BEI Ticker to Company Name Mapping
// Source: Bursa Efek Indonesia official listing
window.IDX_TICKERS = {
...
function normalizeTicker(input) {
  return String(input || "").trim().toUpperCase().replace(/^IDX:/, "").replace(/[^A-Z0-9]/g, "");
}

function getCompanyName(ticker) {
  var key = normalizeTicker(ticker);
  return window.IDX_TICKERS && window.IDX_TICKERS[key] ? window.IDX_TICKERS[key] : null;
}

function formatTickerLabel(ticker) {
  var key = normalizeTicker(ticker);
  var company = getCompanyName(key);
