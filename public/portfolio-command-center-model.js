(function (root, factory) {
  ...
  var VERSION = 'portfolio-command-center-model-v1';
  var LOT_SIZE = 100;
  ...
  function budgetCapacity(input) {
    ...
    return {
      ...
      maxPriceOneLotIdr: maxPriceOneLot,
  ...
  function affordability(price, capacity) {
    ...
    var label = lots >= 4 ? 'Leluasa' : lots >= 2 ? 'Cukup' : lots === 1 ? 'Ketat' : 'Di luar batas';
    var tone = lots >= 4 ? 'good' : lots >= 2 ? 'info' : lots === 1 ? 'warn' : 'danger';
    return {
      lots: lots,
  ...
  function summarize(plans, prices) {
    ...
    normalized.forEach(function (plan) {
      ...
      if (status.attention) {
        actions.push({
          ...
          entryPriceIdr: plan.entryPriceIdr,
          stopLossIdr: plan.stopLossIdr,
          tp1Idr: plan.tp1Idr,
          tp2Idr: plan.tp2Idr
        });
      }
    ...
    return {
      ...
      pricedCount: priced,
      missingPriceCount: Math.max(0, normalized.length - priced),
      totalRiskIdr: totalRisk,
      totalExposureIdr: totalExposure,
      totalPnlIdr: priced ? totalPnl : null,
  ...
  function buildSnapshot(plans, prices) {
    ...
    summary.plans.forEach(function (plan) {
      ...
      rows[plan.ticker] = {
        ...
        status: status.code,
        lots: plan.lots,
        entry: plan.entryPriceIdr,
        stop: plan.stopLossIdr,
        tp1: plan.tp1Idr,
        tp2: plan.tp2Idr,
        risk: plan.estimatedMaxLossIdr
      };
    });
  ...
  function compareSnapshots(previous, current) {
    var events = [];
    var before = previous && previous.rows ? previous.rows : {};
    var after = current && current.rows ? current.rows : {};
    Object.keys(after).forEach(function (ticker) {
      var now = after[ticker];
      var old = before[ticker];
      if (!old) {
        events.push({ type: 'PLAN_ADDED', ticker: ticker, tone: 'info', text: ticker + ' ditambahkan ke rencana atau pantauan.' });
        return;
      }
      if (old.status !== now.status) {
        events.push({ type: 'STATUS_CHANGED', ticker: ticker, tone: now.status === 'STOP_TOUCHED' || now.status === 'NEAR_STOP' ? 'danger' : 'info', text: ticker + ' berubah dari ' + old.status + ' menjadi ' + now.status + '.' });
      }
      if (old.price && now.price && old.price !== now.price) {
        var pct = (now.price - old.price) / old.price * 100;
        if (Math.abs(pct) >= 0.5) {
          events.push({ type: 'PRICE_CHANGED', ticker: ticker, tone: pct >= 0 ? 'good' : 'warn', text: ticker + ' bergerak ' + (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% sejak pembaruan sebelumnya.' });
        }
      }
      if (old.risk !== now.risk) {
        events.push({ type: 'RISK_CHANGED', ticker: ticker, tone: 'warn', text: 'Estimasi risiko ' + ticker + ' berubah.' });
      }
    });
    Object.keys(before).forEach(function (ticker) {
      if (!after[ticker]) events.push({ type: 'PLAN_REMOVED', ticker: ticker, tone: 'neutral', text: ticker + ' tidak lagi ada di pantauan.' });
    });
    return events;
  }
  ...
  function journalSummary(entries) {
    ...
    entries.forEach(function (row) {
      ...
      if (typeof row.followedPlan === 'boolean') {
        summary.assessed += 1;
        if (row.followedPlan) summary.followed += 1;
      }
    });
