      ...
      .then(function (statusData) {
        ...
        return fetch(API_ENDPOINT + '&action=analyze', {
          ...
          .then(function (result) {
            ...
            // Never invent a model name: if the server did not report one, say so
            // rather than showing a hardcoded label that may be wrong.
            var modelLabel = analysisData.model ? escapeHtml(String(analysisData.model)) : 'tidak dilaporkan';
            cardHtml += '<div><span style="font-size:13px;font-weight:800;color:#fff;display:flex;align-items:center;gap:6px"><span>🤖</span> Analisis Chart (AI) — ' + escapeHtml(ticker) + '</span><span style="font-size:10px;color:#94a3b8">Model: ' + modelLabel + (result.cached ? ' · (Cache Hari Ini)' : '') + '</span></div>';
