...
| Provider not serving inference (see below) | 503 | `AI_PROVIDER_TEMPORARILY_UNAVAILABLE` | "Provider AI sedang mengalami gangguan sementara" + labelled local summary |
...
## Provider inference unavailable

`AI_MODELS_FAILED_SAFE_STOP` says *"every model refused the request or is full"*.
When the provider is not serving inference at all that is the wrong description,
so the condition is classified and reported as itself.

**What this claims, and what it does not.** The evidence proves inference is
temporarily unavailable **to this account/key** across several catalog-valid
model families. It does **not** prove that every WeizeRouter customer is
affected. The diagnostic field is named `provider_inference_unavailable` rather
than anything global for exactly that reason; the user-facing wording stays
`"Provider AI sedang mengalami gangguan sementara…"`, which is true either way.
...
- the model directory state for this request is `CATALOG_WITH_OVERLAP` — without
  a healthy catalog, unavailable inference cannot be told apart from model ids
  the provider no longer serves, so the router refuses to guess;
...
the request disqualifies it. Mixed evidence is not proof, and one surviving
vendor disproves it outright.
...
*before* the negative failure cache, so a repeated question reports the provider
