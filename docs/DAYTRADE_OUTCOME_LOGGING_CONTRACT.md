...
Identity has two separate canonical hashes. The result-independent `finalization_key` binds the initial digest, evaluator version, canonical full-policy hash, complete versioned horizon semantics (including evidence boundary), and coverage model version plus interval. It drives duplicate rejection, the storage filename, and one-to-one manifest mapping. The `result_identity` binds that key plus terminal evidence, levels, entry, exact touch/ambiguity evidence, excursions, performance, completion, and rejection codes. Thus changed evidence changes the result identity but cannot authorize a second finalization for the same evaluation key. For complete coverage, terminal evidence independently stores the horizon-final close, bar end, source-as-of, and bounded provenance. `market_date` is derived with the repository's Asia/Jakarta market-date function and is cross-checked against the verified initial manifest directory.
...
Whole-tree preflight rejects symlinks at every level, special/unexpected files, `.open` and temporary files, either quarantine tree, orphan raw or manifest artifacts, checksum/count failures, path escape, missing linkage, duplicate mappings, and duplicate finalization keys. It builds a digest-to-canonical-initial-metadata map and uses one full-link comparison for both finalized outcomes and a newly submitted record. The new record is rejected before any outcome directory or file creation if any `initial_link` field differs or its result-independent finalization key already exists. It cross-validates both manifest hashes against the normalized record and reproduces the B0.3 shared run ID, code/config identity, run mode, scheduler source, scheduled slot, common observation time, unique ticker, date, and first/last timestamp invariants.
