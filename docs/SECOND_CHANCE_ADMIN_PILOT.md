...
Required numbers must be actual finite JSON numbers. Numeric strings, whitespace, booleans, arrays, objects, `NaN`, and infinities are rejected. Before Telegram invocation the evaluator durably records `send_in_progress`. A `sent:false` result is retryable only when delivery definitely did not occur: Telegram was disabled, token/chat/message was missing, or the API definitely rejected the request with `chunks_sent === 0`. Timeout, fetch error, any partial multi-chunk delivery (`chunks_sent > 0`), missing/unknown reason, and all other ambiguous results become `delivery_uncertain` and block automatic resend. A retry additionally requires explicit send mode, exact identity equality, and uses the persisted selected payload. Identity changes fail closed, and shadow cannot overwrite retryable send state. If Telegram confirms success but final persistence fails, the durable `send_in_progress` record remains and also blocks resend. Locks contain PID and timestamp: live locks are preserved, while sufficiently old dead-owner locks may be recovered without bypassing durable delivery uncertainty.
...
Stable statuses include `no_qualifying_alert`, `selected_dry_run`, `selected_shadow_recorded`, `already_recorded`, `already_sent`, `sent`, `blocked_feature_disabled`, `blocked_missing_admin`, `blocked_historical_send`, `blocked_invalid_mode`, `blocked_delivery_uncertain`, `blocked_retry_requires_send`, `blocked_retry_identity_mismatch`, `source_missing`, `source_invalid`, `invalid_input`, `lock_busy`, and `failed`. Explicitly blocked operations exit non-zero while still emitting complete JSON with `--json`.
