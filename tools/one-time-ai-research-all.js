...
async function callAIStream(modelName, payload, opts) {
  ...
  try {
    ...
    const result = parseStreamChunks(text);
    return result;
  } finally {
    clearTimeout(timer);
  }
...
function createFailFastTracker() {
  // Map<model, { consecutive: number, stopped: boolean, reason: string|null }>
  const state = {};
  return {
    /** Record an error. Returns true if model should be stopped. */
...
function extractHttpStatus(error) {
  ...
  // Check error.statusCode (set by callAIStream)
  if (error.statusCode && typeof error.statusCode === 'number') return error.statusCode;
  // Parse from error message: "AI API HTTP 402: ..."
