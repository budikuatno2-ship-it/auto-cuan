...
const DEVICE_COOKIE_MAX_AGE_SECONDS = 180 * 24 * 60 * 60; // 180 days; still useless without a fresh Telegram grant.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'device-poll') return await pollDevice(req, res, db);
