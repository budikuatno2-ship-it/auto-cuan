...
-- Phase 5B: vouchers are additive and unapplied. Only a HMAC digest is stored;
-- VOUCHER_CODE_PEPPER is application configuration and is intentionally absent here.
CREATE TABLE IF NOT EXISTS public.vouchers (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), code_hash text NOT NULL UNIQUE CHECK (length(code_hash)=64), code_hint text NOT NULL CHECK (length(code_hint)<=32),
 voucher_type text NOT NULL CHECK (voucher_type IN ('PERCENT_30','PERCENT_50','PERCENT_100','LIFETIME')),
 allowed_plan_codes text[] NOT NULL CHECK (cardinality(allowed_plan_codes)>0 AND allowed_plan_codes <@ ARRAY['PREMIUM_1_MONTH','PREMIUM_2_MONTHS','PREMIUM_3_MONTHS','LIFETIME']::text[]),
 starts_at timestamptz NOT NULL, ends_at timestamptz, max_redemptions integer, per_user_limit integer, redemption_count integer NOT NULL DEFAULT 0,
 enabled boolean NOT NULL DEFAULT true, created_by_user_id uuid NOT NULL REFERENCES public.app_users(id), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
 disabled_at timestamptz, disabled_by_user_id uuid REFERENCES public.app_users(id), disable_reason text,
 CHECK (redemption_count>=0), CHECK (max_redemptions IS NULL OR max_redemptions>0), CHECK (per_user_limit IS NULL OR per_user_limit>0), CHECK (ends_at IS NULL OR starts_at<ends_at),
 CHECK ((voucher_type='LIFETIME') = (allowed_plan_codes=ARRAY['LIFETIME']::text[])), CHECK (voucher_type='LIFETIME' OR NOT ('LIFETIME'=ANY(allowed_plan_codes)))
);
CREATE TABLE IF NOT EXISTS public.voucher_redemptions (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), voucher_id uuid NOT NULL REFERENCES public.vouchers(id), user_id uuid NOT NULL REFERENCES public.app_users(id), selected_plan_code text NOT NULL REFERENCES public.subscription_plans(code),
 voucher_type_snapshot text NOT NULL CHECK (voucher_type_snapshot IN ('PERCENT_30','PERCENT_50','PERCENT_100','LIFETIME')), discount_percent_snapshot integer NOT NULL CHECK (discount_percent_snapshot IN (0,30,50,100)), normal_price_idr_snapshot bigint NOT NULL CHECK(normal_price_idr_snapshot>=0), final_price_idr_snapshot bigint NOT NULL CHECK(final_price_idr_snapshot>=0),
 status text NOT NULL CHECK(status IN ('quoted','pending_payment','activated','rejected','cancelled','expired')), idempotency_key uuid NOT NULL, entitlement_id uuid REFERENCES public.user_entitlements(id), payment_order_id uuid, redeemed_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
 UNIQUE(user_id,idempotency_key)
);
CREATE INDEX IF NOT EXISTS idx_vouchers_usable ON public.vouchers(code_hash,enabled,starts_at,ends_at); CREATE INDEX IF NOT EXISTS idx_voucher_redemptions_user ON public.voucher_redemptions(user_id,voucher_id,status,created_at DESC); CREATE INDEX IF NOT EXISTS idx_voucher_redemptions_status ON public.voucher_redemptions(status,created_at DESC);
ALTER TABLE public.vouchers ENABLE ROW LEVEL SECURITY; ALTER TABLE public.voucher_redemptions ENABLE ROW LEVEL SECURITY; REVOKE ALL ON public.vouchers,public.voucher_redemptions FROM PUBLIC,anon,authenticated;

-- Both redemptions lock account, voucher and existing history. The idempotency
-- row is checked first, so replay returns the same safe state without consuming again.
CREATE OR REPLACE FUNCTION public.redeem_percent_100_voucher(p_user_id uuid,p_code_hash text,p_plan_code text,p_idempotency_key uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE u public.app_users%ROWTYPE; v public.vouchers%ROWTYPE; p public.subscription_plans%ROWTYPE; e public.user_entitlements%ROWTYPE; r public.voucher_redemptions%ROWTYPE; l public.telegram_subscription_links%ROWTYPE; base timestamptz; expiry timestamptz;
BEGIN
 SELECT * INTO r FROM public.voucher_redemptions WHERE user_id=p_user_id AND idempotency_key=p_idempotency_key FOR UPDATE; IF FOUND THEN RETURN jsonb_build_object('activated',r.status='activated','plan_code',r.selected_plan_code,'expires_at',(SELECT expires_at FROM public.user_entitlements WHERE id=r.entitlement_id)); END IF;
 SELECT * INTO u FROM public.app_users WHERE id=p_user_id FOR UPDATE; IF NOT FOUND OR u.is_blocked THEN RAISE EXCEPTION 'rejected'; END IF; SELECT * INTO l FROM public.telegram_subscription_links WHERE user_id=u.id AND link_state='linked' FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'rejected'; END IF;
 SELECT * INTO v FROM public.vouchers WHERE code_hash=p_code_hash FOR UPDATE; IF NOT FOUND OR v.voucher_type<>'PERCENT_100' OR NOT v.enabled OR v.starts_at>now() OR (v.ends_at IS NOT NULL AND v.ends_at<=now()) OR NOT(p_plan_code=ANY(v.allowed_plan_codes)) OR (v.max_redemptions IS NOT NULL AND v.redemption_count>=v.max_redemptions) OR EXISTS(SELECT 1 FROM public.voucher_redemptions x WHERE x.voucher_id=v.id AND x.user_id=u.id AND x.status IN('pending_payment','activated') GROUP BY x.voucher_id HAVING count(*)>=COALESCE(v.per_user_limit,2147483647)) THEN RAISE EXCEPTION 'rejected'; END IF;
 SELECT * INTO p FROM public.subscription_plans WHERE code=p_plan_code AND active FOR UPDATE; IF NOT FOUND OR p.kind<>'term' THEN RAISE EXCEPTION 'rejected'; END IF; SELECT * INTO e FROM public.user_entitlements WHERE user_id=u.id AND lifetime=true AND status='active' FOR UPDATE; IF FOUND THEN RAISE EXCEPTION 'rejected'; END IF;
 SELECT * INTO e FROM public.user_entitlements WHERE user_id=u.id AND lifetime=false AND status='active' AND source<>'trial' AND expires_at>now() ORDER BY expires_at DESC LIMIT 1 FOR UPDATE; base:=CASE WHEN FOUND THEN e.expires_at ELSE now() END; expiry:=base+(p.duration_months::text||' months')::interval;
 INSERT INTO public.user_entitlements(user_id,plan_code,source,status,starts_at,expires_at,lifetime,activation_idempotency_key) VALUES(u.id,p.code,'voucher','active',base,expiry,false,p_idempotency_key) RETURNING * INTO e; INSERT INTO public.voucher_redemptions(voucher_id,user_id,selected_plan_code,voucher_type_snapshot,discount_percent_snapshot,normal_price_idr_snapshot,final_price_idr_snapshot,status,idempotency_key,entitlement_id,redeemed_at) VALUES(v.id,u.id,p.code,'PERCENT_100',100,0,0,'activated',p_idempotency_key,e.id,now()) RETURNING * INTO r; UPDATE public.vouchers SET redemption_count=redemption_count+1,updated_at=now() WHERE id=v.id; IF u.is_approved=false THEN UPDATE public.app_users SET is_approved=true WHERE id=u.id; INSERT INTO public.subscription_events(user_id,entitlement_id,event_type,metadata) VALUES(u.id,e.id,'account_auto_approved_by_voucher',jsonb_build_object('previous_approval_state','pending','new_approval_state','approved')); END IF; INSERT INTO public.subscription_events(user_id,entitlement_id,event_type,metadata) VALUES(u.id,e.id,'subscription_voucher_redeemed',jsonb_build_object('voucher_type','PERCENT_100','plan_code',p.code,'discount_percent',100,'final_price_idr',0,'starts_at',base,'expires_at',expiry)); RETURN jsonb_build_object('activated',true,'plan_code',p.code,'expires_at',expiry);
END $$;
CREATE OR REPLACE FUNCTION public.redeem_lifetime_voucher(p_user_id uuid,p_code_hash text,p_plan_code text,p_idempotency_key uuid) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$ DECLARE v public.vouchers%ROWTYPE; e public.user_entitlements%ROWTYPE; seat integer; r public.voucher_redemptions%ROWTYPE; BEGIN
 SELECT * INTO r FROM public.voucher_redemptions WHERE user_id=p_user_id AND idempotency_key=p_idempotency_key FOR UPDATE; IF FOUND THEN RETURN jsonb_build_object('activated',r.status='activated','plan_code','LIFETIME'); END IF; SELECT * INTO v FROM public.vouchers WHERE code_hash=p_code_hash FOR UPDATE; IF NOT FOUND OR v.voucher_type<>'LIFETIME' OR p_plan_code<>'LIFETIME' OR NOT v.enabled OR v.starts_at>now() OR(v.ends_at IS NOT NULL AND v.ends_at<=now()) OR(v.max_redemptions IS NOT NULL AND v.redemption_count>=v.max_redemptions) THEN RAISE EXCEPTION 'rejected'; END IF; INSERT INTO public.user_entitlements(user_id,plan_code,source,status,starts_at,expires_at,lifetime,activation_idempotency_key) VALUES(p_user_id,'LIFETIME','voucher','active',now(),NULL,true,p_idempotency_key) RETURNING * INTO e; seat:=public.allocate_lifetime_seat(e.id,'voucher'); INSERT INTO public.voucher_redemptions(voucher_id,user_id,selected_plan_code,voucher_type_snapshot,discount_percent_snapshot,normal_price_idr_snapshot,final_price_idr_snapshot,status,idempotency_key,entitlement_id,redeemed_at) VALUES(v.id,p_user_id,'LIFETIME','LIFETIME',100,0,0,'activated',p_idempotency_key,e.id,now()); UPDATE public.vouchers SET redemption_count=redemption_count+1 WHERE id=v.id; UPDATE public.app_users SET is_approved=true WHERE id=p_user_id AND is_approved=false; INSERT INTO public.subscription_events(user_id,entitlement_id,event_type,metadata) VALUES(p_user_id,e.id,'subscription_lifetime_voucher_redeemed',jsonb_build_object('voucher_type','LIFETIME','plan_code','LIFETIME','lifetime_seat_number',seat)); RETURN jsonb_build_object('activated',true,'plan_code','LIFETIME','lifetime_seat_number',seat); END $$;
REVOKE ALL ON FUNCTION public.redeem_percent_100_voucher(uuid,text,text,uuid),public.redeem_lifetime_voucher(uuid,text,text,uuid) FROM PUBLIC,anon,authenticated; GRANT EXECUTE ON FUNCTION public.redeem_percent_100_voucher(uuid,text,text,uuid),public.redeem_lifetime_voucher(uuid,text,text,uuid) TO service_role;
-- Dedicated durable idempotency domain for the voucher administrator bot.
CREATE TABLE IF NOT EXISTS public.voucher_admin_telegram_webhook_updates (update_id bigint PRIMARY KEY, created_at timestamptz NOT NULL DEFAULT now());
ALTER TABLE public.voucher_admin_telegram_webhook_updates ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.voucher_admin_telegram_webhook_updates FROM PUBLIC,anon,authenticated;
