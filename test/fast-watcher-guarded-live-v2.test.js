...
function obs(ticker,time,extra){return {ticker,scheduled_time:time,current_price:100,entry_low:98,entry_high:103,tp1:119,stop_loss:95,current_status:'EARLY_RADAR',volume:1000,average_volume:700,relative_volume:1.1,momentum_component:14,liquidity_component:16,risk_reward:2,high:103,low:98,freshness:{is_stale:false},...(extra||{})};}
test('volatility-relative momentum confirms a near-miss twice',()=>{const first=pool.process({sampleDate:'2026-07-31',scheduledTime:'09:10',shortlistRows:[{ticker:'PADA',source_rank:1}],observations:[obs('PADA','09:10')],priorState:null}); assert.equal(first.state.tickers.PADA.status,'WATCHING'); const second=pool.process({sampleDate:'2026-07-31',scheduledTime:'09:12',shortlistRows:[{ticker:'PADA',source_rank:1}],observations:[obs('PADA','09:12',{current_price:101,volume:1700,relative_volume:1.5})],priorState:first.state}); assert.equal(second.state.tickers.PADA.status,'READY_PENDING'); const third=pool.process({sampleDate:'2026-07-31',scheduledTime:'09:14',shortlistRows:[{ticker:'PADA',source_rank:1}],observations:[obs('PADA','09:14',{current_price:102,volume:2800,relative_volume:1.9})],priorState:second.state}); assert.equal(third.state.tickers.PADA.status,'READY_PENDING'); const fourth=pool.process({sampleDate:'2026-07-31',scheduledTime:'09:15',shortlistRows:[{ticker:'PADA',source_rank:1}],observations:[obs('PADA','09:15',{current_price:102.8,volume:4200,relative_volume:2.4})],priorState:third.state}); assert.equal(fourth.state.tickers.PADA.status,'READY_CONFIRMED'); assert.equal(fourth.publishable.length,1);});
