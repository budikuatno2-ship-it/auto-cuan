...
function inputs(overrides) {
  ...
  return {
    ...
    runbook: Object.assign({ date: '2026-07-08', runbook_status: 'READY_FOR_MANUAL_APPROVAL' }, overrides.runbook || {})
  };
}

test('force done for day + gate BLOCK => DONE_FOR_DAY_NOT_READY and no more archives', () => {
  const report = lib.buildCloseoutReport(inputs({ gate: { gate_status: 'BLOCK' }, runbook: { runbook_status: 'NOT_READY' } }), { forceDoneForDay: true, nowMs: Date.parse('2026-07-08T07:00:00Z') });
...
test('after market close + gate BLOCK => DONE_FOR_DAY_NOT_READY', () => {
  const report = lib.buildCloseoutReport(inputs({ gate: { gate_status: 'BLOCK' }, runbook: { runbook_status: 'NOT_READY' } }), { timezone: 'Asia/Jakarta', marketCloseLocal: '16:00', nowMs: Date.parse('2026-07-08T09:30:00Z') });
...
test('spacing-only blocker before market close => NEED_MORE_SPACED_SAMPLES', () => {
  const report = lib.buildCloseoutReport(inputs({
    ...
    runbook: { runbook_status: 'NOT_READY' },
...
test('non-spacing blocker before market close => NOT_READY_NON_SPACING_BLOCKERS and no more archives', () => {
  const report = lib.buildCloseoutReport(inputs({
    gate: { gate_status: 'BLOCK' }, runbook: { runbook_status: 'NOT_READY' },
...
test('output includes read-only confirmation', () => {
  const report = lib.buildCloseoutReport(inputs({ gate: { gate_status: 'BLOCK' }, runbook: { runbook_status: 'NOT_READY' } }), { forceDoneForDay: true });
  const md = lib.markdownReport(report);
...
test('CLI reads latest reports and writes markdown and json', async () => {
  ...
  await fs.writeFile(path.join(dir, 'daytrade-intraday-staged-enable-runbook-2026-07-08.json'), JSON.stringify(Object.assign(inputs().runbook, { runbook_status: 'NOT_READY' })));
