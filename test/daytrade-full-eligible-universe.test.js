...
test('full eligible Day Trade universe uses official boards and deterministic eligibility exclusions', () => {
  ...
  // BATCH 4: harga murah (49) TIDAK lagi hard-reject via nominal; CHEAP kini lolos.
  assert.equal(result.tickers.length, 773);
  assert.equal(result.diagnostics.excluded_by_reason.invalid_or_unknown_board, 3);
  assert.equal(result.diagnostics.excluded_by_reason.price_below_50, undefined);
