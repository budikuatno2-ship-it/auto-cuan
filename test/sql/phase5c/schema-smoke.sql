...
DO $$
BEGIN
  IF to_regprocedure('public.voucher_admin_schema_version()') IS NULL
     OR public.voucher_admin_schema_version() IS DISTINCT FROM 'phase5c-complete-v4'
  THEN
    RAISE EXCEPTION 'incomplete Phase 5C schema marker';
  END IF;
END $$;
