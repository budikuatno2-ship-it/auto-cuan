...
test('Multi-Day Aggregation: 7D range scales values and attaches range_label', async () => {
  ...
  const hasMultiDayEvidence = Array.isArray(data7d.broker_summary.date_headers)
    ? data7d.broker_summary.date_headers.length > 1
    : false;
  if (turnover1d > 0 && hasMultiDayEvidence && !data7d.is_empty && (!data7d.broker_summary || !data7d.broker_summary.is_empty)) {
