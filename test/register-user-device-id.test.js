...
test('the database insert never receives a null device_id', async () => {
  await withEnv(async function() {
    var inputs = [
      { username: 'c1', passwordHash: 'h', deviceId: 'dev_ok' },
      { username: 'c2', passwordHash: 'h' },
      { username: 'c3', passwordHash: 'h', deviceId: '' },
      { username: 'c4', passwordHash: 'h', deviceId: null }
    ];
...
test('username length validation remains intact', async () => {
  await withEnv(async function() {
    ...
    var handler = loadHandlerWithMock(mock);
    var res = makeRes();
    await handler({ method: 'POST', body: { username: 'a', passwordHash: 'h', deviceId: 'dev_x' } }, res);
    assert.equal(res.statusCode, 400);
